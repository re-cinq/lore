...
- A terminal Agent CR becomes its kubernetes event. ([validated by reports a terminal Agent CR as its kubernetes event](apps/cluster-agent/src/listeners/agent-reporting.test.ts#L120))
- A CR that has not reached a terminal phase reports nothing, so the repeated
  ...
  ([validated by reports nothing for a CR that has not reached a terminal phase](apps/cluster-agent/src/listeners/agent-reporting.test.ts#L145))
- A failed report is swallowed here, unlike everywhere else this repo reports
  ...
  re-emits what was missed. ([validated by swallows a failed report so one bad CR cannot end the watch](apps/cluster-agent/src/listeners/agent-reporting.test.ts#L163))
- A report retries before it is given up on, because it now crosses a network
  ...
  `dedupeKey`. ([validated by retries a failed insert, since the report now crosses a network](apps/cluster-agent/src/listeners/agent-reporting.test.ts#L248), [`agent-reporting.test.ts:269`](apps/cluster-agent/src/listeners/agent-reporting.test.ts#L269))
- The catch-up pass walks the namespace one page at a time. 180 accumulated CRs
  ...
  2026-07-24. ([validated by walks every page rather than holding the namespace at once](apps/cluster-agent/src/listeners/agent-reporting.test.ts#L183), [`agent-reporting.test.ts:203`](apps/cluster-agent/src/listeners/agent-reporting.test.ts#L203), [`agent-reporting.test.ts:228`](apps/cluster-agent/src/listeners/agent-reporting.test.ts#L228))
