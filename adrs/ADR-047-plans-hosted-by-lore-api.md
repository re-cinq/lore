...
- **A Refine's context lives on the run**, not in the agent's answer: lore-api records slot, hash and uses when the Refine is asked for, and proposes only that section's ops when the file comes back ([validated by](../apps/lore-api/src/work/plans/planning-line.test.ts#L118), [validated by](../apps/lore-api/src/work/plans/plan-file.test.ts#L89)).
