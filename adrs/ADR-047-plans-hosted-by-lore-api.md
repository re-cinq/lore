...
- **The page reads one state.** `planPageState` folds the plan's status and its run into one of ten states; the editor's read-only flag, the outline's actions (approve, reopen, retry, the spec PR link) and the run line all read it. Regenerate is never offered on an approved plan.
- **A revision after merge is a new spec PR.** Re-approving a plan whose specs merged starts a spec pass briefed as an amendment of what is on main, on a new branch; decomposition runs again for it. The alternative — forking the merged line onto its delivered branch — would push onto a branch GitHub may already have deleted and inherit the old PR number, so the fork was not used.

planning-station's editor learned to say who approved the plan in its outline and to delete a comment behind a confirmation (planning-editor 0.6.0).
