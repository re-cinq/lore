...
The layering is written down. `layers.yaml` at the repo root maps each folder to the folders it may import, and `re-lint/no-cross-layer-import` checks every import against it, so the architecture is a file a person can read and disagree with rather than a shape inferred by tracing imports.
...
to the folders they may import; `re-lint/no-cross-layer-import` reports an import
