...
describe("parseAdrRefs", () => {
  it("extracts distinct ADR numbers, normalizing zero-padding", () => {
    expect(parseAdrRefs("Builds on ADR-016 and see ADR-7; per ADR-016 again.")).toEqual([16, 7]);
  });
  it("returns nothing when no ADR is cited", () => {
    expect(parseAdrRefs("A plain statement with no decision reference.")).toEqual([]);
  });
