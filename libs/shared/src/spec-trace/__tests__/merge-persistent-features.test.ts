...
const baseGraph: SpecGraph = {
  nodes: [
    { id: "u1", type: "Feature", label: "7-feature-planning", path: "specs/7-feature-planning" },
    { id: "u2", type: "Spec", label: "spec.md", path: "specs/7-feature-planning/spec.md" },
  ],
...
describe("mergePersistentFeatures", () => {
  it("enriches a computed Feature node sharing the path (persistent wins)", () => {
    const merged = mergePersistentFeatures(baseGraph, [
      { id: "f1", title: "Smart Feature Planning", path: "specs/7-feature-planning", status: "pr-open" },
    ]);
  ...
  it("injects a standalone node for a draft with no computed Feature", () => {
    const merged = mergePersistentFeatures(baseGraph, [
      { id: "f2", title: "Draft Idea", path: "specs/draft-idea", status: "draft" },
    ]);
  ...
  it("leaves links untouched and does not duplicate matched features", () => {
    const merged = mergePersistentFeatures(baseGraph, [
      { id: "f1", title: "Smart Feature Planning", path: "specs/7-feature-planning", status: "implemented" },
    ]);
