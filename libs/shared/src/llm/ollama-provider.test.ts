...
describe("OllamaProvider", () => {
  it("posts a combined system+user prompt to the generate endpoint and extracts the response", async () => {
    ...
    const fetchFn = async (url: any, init: any) => {
      ...
      return new Response(JSON.stringify({ response: "ollama-answer" }), { status: 200 });
    };
    const provider = new OllamaProvider({ model: "llama3", baseUrl: "http://localhost:11434", fetchFn: fetchFn as typeof fetch });

    const result = await provider.complete({ prompt: "u", systemPrompt: "s" });

    expect(captured!.url).toBe("http://localhost:11434/api/generate");
    expect(captured!.body).toMatchObject({ model: "llama3", prompt: "s\n\nu", stream: false });
