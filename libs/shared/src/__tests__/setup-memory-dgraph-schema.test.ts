...
const APPLIER = join(REPO_ROOT, "scripts", "infra", "setup-memory-dgraph-schema.sh");
...
describe.skipIf(!reachable)("setup-memory-dgraph-schema applier (live Dgraph)", () => {
  ...
  it("declares the HNSW vector index and the xid upsert index", async () => {
    ...
    expect(byPred["Memory.embedding"]).toMatchObject({ type: "float32vector" });
    expect(String((byPred["Memory.embedding"].tokenizer as string[])?.[0])).toMatch(/hnsw/);
