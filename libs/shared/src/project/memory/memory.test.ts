...
function fakeMemory(): MemoryPort {
  ...
  const keyOf = (repo: string, agentId: string, key: string) => `${repo}|${agentId}|${key}`;
  return {
    ...
    read: async (repo, key, agentId) => rows.get(keyOf(repo, agentId, key)) ?? null,
    list: async (repo, agentId) =>
      [...rows.entries()].filter(([k]) => k.startsWith(`${repo}|${agentId}|`)).map(([, v]) => v),
  };
}
...
describe("Memory", () => {
  it("writes then reads back the value for the repo", async () => {
    ...
    const written = await facade.write("deploy-gotcha", "use --set-string", "agent-1");
    const read = await facade.read("deploy-gotcha", "agent-1");

    expect(written).toEqual({ key: "deploy-gotcha", version: 1 });
    expect(read).toEqual({ key: "deploy-gotcha", value: "use --set-string", version: 1 });
  });
