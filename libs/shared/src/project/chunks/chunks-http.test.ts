...
describe("ChunksHttp", () => {
  it("reads spec chunks, sending the bearer token to the repo-scoped endpoint", async () => {
    const { fetchImpl, calls } = fakeFetch({
      "/api/repos/o/r/chunks/spec": { specs: [{ id: "1", repo: "o/r", filePath: "s.md", content: "x" }] },
    });
    const http = new ChunksHttp("https://api", "o/r", "tok", fetchImpl);

    expect(await http.specChunks("o/r")).toEqual([{ id: "1", repo: "o/r", filePath: "s.md", content: "x" }]);
  ...
  it("maps hasChunk and staleChunkCount to their query endpoints", async () => {
    const { fetchImpl, calls } = fakeFetch({
      "/api/repos/o/r/chunks/has?content_type=doc&file_suffix=CLAUDE.md": { has: true },
  ...
  it("reads backfill chunks (with embeddings) via their kinds", async () => {
    const { fetchImpl } = fakeFetch({
      "/api/repos/o/r/chunks/spec-backfill": { specs: [{ repo: "o/r", filePath: "s.md", content: "x", ingestedAt: "t", embedding: [0.1] }] },
      "/api/repos/o/r/chunks/code-backfill": { chunks: [{ filePath: "a.test.ts", content: "c", metadata: {}, embedding: [0.2] }] },
    });
    const http = new ChunksHttp("https://api", "o/r", undefined, fetchImpl);

    expect((await http.specChunksForBackfill("o/r"))[0].embedding).toEqual([0.1]);
    expect((await http.codeChunksForBackfill("o/r"))[0].filePath).toBe("a.test.ts");
  });
  ...
  it("throws on a non-ok response and on the Floor-only write surface", async () => {
    ...
    await expect(http.specChunks("o/r")).rejects.toThrow(/chunks.spec failed: 404/);
    await expect(http.insertChunk("s", {} as never)).rejects.toThrow(/Floor-only/);
  });
});
