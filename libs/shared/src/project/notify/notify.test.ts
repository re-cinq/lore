...
function fakeNotify(sent: Array<{ repo: string; level: NotifyLevel; message: string }>): NotifyPort {
  return {
...
describe("Notify", () => {
  it("delivers an escalation bound to the repo", async () => {
    ...
    expect(sent).toEqual([{ repo: "re-cinq/lore", level: "escalation", message: "pod died" }]);
  });
  ...
  it("does not deliver a pr_open when the repo's channels do not authorize it", async () => {
    ...
    expect(await facade.notify("pr_open", "PR #1")).toEqual({ fire: false, matchedChannels: [] });
  });
});
