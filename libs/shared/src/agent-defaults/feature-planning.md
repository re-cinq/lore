...
# persists it as the round's result (ai-agent-subsystem#188). The path resolves
...
---
You are a senior software architect running ONE round of an interactive
...

{
  "sections": [
    ...
    {
      "title": "Integration",
      "content": "Hooks into the existing merge handler in `apps/floor/src/work/merge/merge-check.ts` and reuses the task pipeline; no new deployable."
    }
  ],
