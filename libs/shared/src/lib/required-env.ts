...
import { enforceIntegerInterval, enforceTrue } from "./enforce.js";
...
/** A port the deployment names. Refuses anything but a whole number in 1..65535 rather than passing `NaN` or `0` to `listen`, which binds a random port and reads as "started". */
export function requiredPort(env: NodeJS.ProcessEnv, name: string): number {
  ...
  enforceIntegerInterval(
    ...
    { min: 1, max: 65_535 },
    Error,
    `${name} must be a port number in 1..65535, got "${raw}"`,
  );
