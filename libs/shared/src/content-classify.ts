 ...
 * path today's ingest policy refuses (graveyard, fixtures, binaries). Chunks
 * ingested before an exclusion was added linger in `{schema}.chunks` until the
 * manual operator purge runs, and without this filter that stale debris keeps
 * resurfacing as detect findings (issue #1018 — fixture spec links reported as
 * rot after the #1016 ingest fix).
 */
