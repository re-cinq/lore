...
export interface LoreTaskSpec {
  ...
  /** Files the pod downloads into its workspace before the agent starts — references only, so an input of any size never rides the Agent object. */
  files?: Array<{ path: string; url: string; headersSecret?: string }>;
