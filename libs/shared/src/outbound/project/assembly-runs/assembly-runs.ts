...
/** Repo-scoped facade over {@link AssemblyRunsPort}: fills `repo` from the Project's fullName so callers write `project.assemblyRuns.start("implementation", { taskId })`. */
export class AssemblyRuns {
  ...
  /** The open run working this subject, or null — "is something already doing this?", which a caller answers a duplicate request with. */
  findOpenBySubject(subjectKey: string): Promise<OpenRunSummary | null> {
    return this.port.findOpenBySubject(this.repo, subjectKey);
  }

  /** Every run for this subject, newest first — "what has worked on this", which is how a reader finds the run to show without knowing which task or which blueprint produced it. */
