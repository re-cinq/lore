...
describe("parseReviewVerdict", () => {
  it("reads the REVIEW_RESULT line", () => {
    expect(
      parseReviewVerdict("blah\nREVIEW_RESULT: CHANGES_REQUESTED fix x"),
...
describe("agentNodeOutcome", () => {
  it("maps Failed to failed with the reason (or a default)", () => {
    expect(
      agentNodeOutcome({ phase: "Failed", failureReason: "boom" }),
    ).toMatchObject({
      outcome: "failed",
      extras: { "Lore-Validation-Summary": "boom" },
    });
    expect(
      agentNodeOutcome({ phase: "Failed" }).extras?.["Lore-Validation-Summary"],
  ...
  it("maps a review verdict, else success", () => {
    ...
    expect(
      agentNodeOutcome({ phase: "Succeeded", output: "REVIEW_RESULT:APPROVED" })
        .outcome,
    ).toBe("success");
...
describe("createAgentNodeHandler", () => {
  ...
  it("times out to failed after maxPolls without a terminal phase", async () => {
    const { deps, calls } = fakeDeps([{ phase: "Running" }], {
      maxPolls: 2,
      pollIntervalMs: 1,
    });
