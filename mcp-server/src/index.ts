...
// --- Spec-traceability test-interface tools (project-test-interface) ---
...
server.tool(
  ...
  async () => {
    try {
      const { listTestsTool, loadTestCommandManifest } = await import("./spec-trace-tools.js");
      const { getRepoRoot } = await import("./local-runner.js");
...
server.tool(
  ...
  async ({ selector }) => {
    try {
      const { runTestTool, loadTestCommandManifest } = await import("./spec-trace-tools.js");
      const { getRepoRoot } = await import("./local-runner.js");
      const root = getRepoRoot() || process.cwd();
