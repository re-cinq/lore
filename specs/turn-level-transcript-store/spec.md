...
- The table stores one row per stream-json line with the **untruncated** envelope in a JSONB column, alongside the same correlation columns `agent_run_events` carries: `task_id`, `agent_cr_name`, `assembly_line_id`, `node_id`, `iteration`, plus the raw line kind in `event_type`. ([validated by `agent-run-turns.test.ts:137`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L137), [`agent-run-turns.test.ts:372`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L372))
- `id` is a `BIGINT GENERATED ALWAYS AS IDENTITY` primary key that doubles as the read cursor, so it is carried as a string-encoded bigint across every boundary and never narrowed to a JS number. ([validated by `agent-run-turns.test.ts:161`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L161), [`agent-run-turns.test.ts:372`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L372))
...
- `insertBatch` resolves `agentCrName` to (`assemblyLineId`, `nodeId`, `iteration`) against `pipeline.assembly_line_nodes` at write time, taking the newest matching node row when two lines collide on a CR name. ([validated by `agent-run-turns.test.ts:38`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L38), [`agent-run-turns.test.ts:304`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L304))
- A row whose `agentCrName` matches no node row is still inserted, with `agentCrName` retained and the three correlated fields left null — skip-not-fail, because ingest must never lose a batch. ([validated by `agent-run-turns.test.ts:92`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L92))
- One uncorrelated row never suppresses the rest of its batch: the remaining rows insert normally. ([validated by `agent-run-turns.test.ts:108`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L108))
- `insertBatch` returns the persisted rows ascending by id, comparing ids numerically rather than lexicographically so a bigint cursor cannot page backwards past 10 digits. Both adapters share one comparator, and it is a total order — equal ids compare equal, never "greater". ([validated by `agent-run-turns.test.ts:239`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L239), [`agent-run-turns.test.ts:372`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L372), [`agent-run-turns.test.ts:466`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L466), [`agent-run-turns.test.ts:470`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L470), [`agent-run-turns.test.ts:476`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L476), [`agent-run-turns.test.ts:482`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L482))
- The batch crosses to Postgres as a **single bound `jsonb` parameter** expanded by `jsonb_to_recordset`, never a string-built `VALUES` list: turn envelopes carry agent-controlled text that must never reach statement text. ([validated by `agent-run-turns.test.ts:339`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L339))
- The envelope crosses the port as JSON **text** and is cast to `jsonb` inside the statement, so the ingest path never re-parses a payload it is already holding as a string; the one serialization left is the adapter's single `JSON.stringify` of the whole batch at the Postgres boundary. ([validated by `agent-run-turns.test.ts:363`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L363))
- An empty batch issues no query at all and returns an empty array. ([validated by `agent-run-turns.test.ts:173`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L173), [`agent-run-turns.test.ts:297`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L297))
- `listByLine` returns one assembly line's turns ascending by id, above a cursor and capped by a limit, so a reader pages a finished run without gaps or duplicates. ([validated by `agent-run-turns.test.ts:206`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L206), [`agent-run-turns.test.ts:214`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L214), [`agent-run-turns.test.ts:226`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L226), [`agent-run-turns.test.ts:419`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L419))
- `listByTask` reads the same way scoped to a task id. It has no production caller today and is kept deliberately: it is the ONLY path that can reach the uncorrelated rows FR1 requires the table to preserve, and a store that keeps rows nothing can ever read would contradict its own reason for keeping them. Delete it only together with the decision to stop storing uncorrelated turns. ([validated by `agent-run-turns.test.ts:257`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L257), [`agent-run-turns.test.ts:431`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L431))
- `pruneOld` deletes rows older than a day horizon and returns the count deleted. ([validated by `agent-run-turns.test.ts:272`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L272), [`agent-run-turns.test.ts:442`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L442))
- *(Amended 2026-08-19, #1389:)* `AgentRunTurnInsert` carries an optional `dedupKey`. A non-null key already stored skips its row — silently, never a batch failure — and `insertBatch` returns only the rows the call actually inserted; a null key never dedups, and pruning a row frees its key. The Pg adapter uses a BARE `ON CONFLICT DO NOTHING`, deliberately naming no arbiter target: naming the partial index would `42P10`-fail every insert against a database missing it, degrading "duplicates" into total turn loss. ([validated by `agent-run-turns.test.ts:488`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L488), [`agent-run-turns.test.ts:500`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L500), [`agent-run-turns.test.ts:512`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L512), [`agent-run-turns.test.ts:521`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L521), [`agent-run-turns.test.ts:536`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L536), [`agent-run-turns.test.ts:549`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L549))
