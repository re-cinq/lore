...
`apps/floor/src/transport/http/routes/agent-events.ts` maps each stream-json
line through `apps/floor/src/work/agent/agent-run-events.ts`, whose
...
(`apps/floor/src/transport/http/routes/agent-logs.ts`) serves an agent's
stdout live from the pod (`KubePodLogs`) and falls back to the Cloud
Logging archive (`CloudLoggingPodLogs` in
`apps/floor/src/work/station/agent-pod-logs.ts`) after the pod is
...
- The table stores one row per stream-json line with the **untruncated** envelope in a JSONB column, alongside the same correlation columns `agent_run_events` carries: `task_id`, `agent_cr_name`, `assembly_line_id`, `node_id`, `iteration`, plus the raw line kind in `event_type`. ([validated by `agent-run-turns.test.ts:137`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L137), [`agent-run-turns.test.ts:372`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L372))
- `id` is a `BIGINT GENERATED ALWAYS AS IDENTITY` primary key that doubles as the read cursor, so it is carried as a string-encoded bigint across every boundary and never narrowed to a JS number. ([validated by `agent-run-turns.test.ts:161`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L161), [`agent-run-turns.test.ts:372`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L372))
...
- `insertBatch` resolves `agentCrName` to (`assemblyLineId`, `nodeId`, `iteration`) against `pipeline.assembly_line_nodes` at write time, taking the newest matching node row when two lines collide on a CR name. ([validated by `agent-run-turns.test.ts:38`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L38), [`agent-run-turns.test.ts:304`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L304))
- A row whose `agentCrName` matches no node row is still inserted, with `agentCrName` retained and the three correlated fields left null — skip-not-fail, because ingest must never lose a batch. ([validated by `agent-run-turns.test.ts:92`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L92))
- One uncorrelated row never suppresses the rest of its batch: the remaining rows insert normally. ([validated by `agent-run-turns.test.ts:108`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L108))
- `insertBatch` returns the persisted rows ascending by id, comparing ids numerically rather than lexicographically so a bigint cursor cannot page backwards past 10 digits. Both adapters share one comparator, and it is a total order — equal ids compare equal, never "greater". ([validated by `agent-run-turns.test.ts:239`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L239), [`agent-run-turns.test.ts:372`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L372), [`agent-run-turns.test.ts:466`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L466), [`agent-run-turns.test.ts:470`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L470), [`agent-run-turns.test.ts:476`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L476), [`agent-run-turns.test.ts:482`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L482))
- The batch crosses to Postgres as a **single bound `jsonb` parameter** expanded by `jsonb_to_recordset`, never a string-built `VALUES` list: turn envelopes carry agent-controlled text that must never reach statement text. ([validated by `agent-run-turns.test.ts:339`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L339))
- The envelope crosses the port as JSON **text** and is cast to `jsonb` inside the statement, so the ingest path never re-parses a payload it is already holding as a string; the one serialization left is the adapter's single `JSON.stringify` of the whole batch at the Postgres boundary. ([validated by `agent-run-turns.test.ts:363`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L363))
- An empty batch issues no query at all and returns an empty array. ([validated by `agent-run-turns.test.ts:173`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L173), [`agent-run-turns.test.ts:297`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L297))
- `listByLine` returns one assembly line's turns ascending by id, above a cursor and capped by a limit, so a reader pages a finished run without gaps or duplicates. ([validated by `agent-run-turns.test.ts:206`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L206), [`agent-run-turns.test.ts:214`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L214), [`agent-run-turns.test.ts:226`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L226), [`agent-run-turns.test.ts:419`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L419))
- `listByTask` reads the same way scoped to a task id. It has no production caller today and is kept deliberately: it is the ONLY path that can reach the uncorrelated rows FR1 requires the table to preserve, and a store that keeps rows nothing can ever read would contradict its own reason for keeping them. Delete it only together with the decision to stop storing uncorrelated turns. ([validated by `agent-run-turns.test.ts:257`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L257), [`agent-run-turns.test.ts:431`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L431))
- `pruneOld` deletes rows older than a day horizon and returns the count deleted. ([validated by `agent-run-turns.test.ts:272`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L272), [`agent-run-turns.test.ts:442`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L442))
- *(Amended 2026-08-19, #1389:)* `AgentRunTurnInsert` carries an optional `dedupKey`. A non-null key already stored skips its row — silently, never a batch failure — and `insertBatch` returns only the rows the call actually inserted; a null key never dedups, and pruning a row frees its key. The Pg adapter uses a BARE `ON CONFLICT DO NOTHING`, deliberately naming no arbiter target: naming the partial index would `42P10`-fail every insert against a database missing it, degrading "duplicates" into total turn loss. ([validated by `agent-run-turns.test.ts:488`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L488), [`agent-run-turns.test.ts:500`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L500), [`agent-run-turns.test.ts:512`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L512), [`agent-run-turns.test.ts:521`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L521), [`agent-run-turns.test.ts:536`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L536), [`agent-run-turns.test.ts:549`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L549))
...
- Turn collection is unconditional: every `/api/agent-events` POST collects turns, with no feature flag and nothing for an operator to switch on. ([validated by `agent-sink-turns.test.ts:17`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L21), [`agent-events-turns.test.ts:53`](apps/floor/src/transport/http/routes/agent-events-turns.test.ts#L53))
- Collecting turns perturbs nothing else. The cost rows and the run-visualization rows are byte-for-byte what they would be without the turn store, so the projection and the SSE view are unaffected. With no off switch in production this property is the only thing standing between the store and a regression in the outputs that were already there. ([validated by `agent-sink-turns.test.ts:25`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L29), [`agent-sink-turns.test.ts:21`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L25), [`agent-events-turns.test.ts:69`](apps/floor/src/transport/http/routes/agent-events-turns.test.ts#L69))
- A turn is built from the envelope the single pass **already parsed** — the collector re-parses nothing and re-serializes nothing, taking the raw line the scanner already yielded as the stored envelope. ([validated by `agent-run-turns.test.ts:16`](apps/floor/src/work/agent/agent-run-turns.test.ts#L16), [`agent-run-turns.test.ts:61`](apps/floor/src/work/agent/agent-run-turns.test.ts#L61), [`agent-run-turns.test.ts:68`](apps/floor/src/work/agent/agent-run-turns.test.ts#L68), [`agent-sink-turns.test.ts:33`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L37), [`agent-sink-turns.test.ts:43`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L47))
- The stored envelope is redacted with the same `redactSecrets` the GCS archive used, before it ever reaches the database, because a queryable store raises a redaction miss from "buried in GCS" to "searchable". ([validated by `agent-run-turns.test.ts:76`](apps/floor/src/work/agent/agent-run-turns.test.ts#L76))
- Redaction is verified not to have broken the line's JSON: an unchanged line is kept as-is, and a redacted line that no longer parses is dropped rather than risking a batch-wide insert failure. ([validated by `agent-run-turns.test.ts:61`](apps/floor/src/work/agent/agent-run-turns.test.ts#L61), [`agent-run-turns.test.ts:88`](apps/floor/src/work/agent/agent-run-turns.test.ts#L88))
- A dropped turn is counted and warned about, never silent. The private-key pattern is not anchored inside one JSON string, so an agent can emit a `BEGIN`/`END` pair straddling JSON structure and thereby keep its own line out of the transcript; a store justified by fidelity has to make that loss visible. ([validated by `agent-sink-turns.test.ts:77`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L81), [`agent-sink-turns.test.ts:84`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L88), [`agent-sink-turns.test.ts:93`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L97), [`agent-sink-turns.test.ts:97`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L101), [`agent-events-turns.test.ts:109`](apps/floor/src/transport/http/routes/agent-events-turns.test.ts#L109), [`agent-events-turns.test.ts:125`](apps/floor/src/transport/http/routes/agent-events-turns.test.ts#L125))
- Turns left out by the per-batch cap are counted and warned about too, separately from the redaction drops. With both paths counted, every way the sink can lose a turn is visible, which is what lets "this transcript is complete" be read off the metrics instead of assumed. ([validated by `agent-sink-turns.test.ts:110`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L115), [`agent-sink-turns.test.ts:117`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L122), [`agent-sink-turns.test.ts:121`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L126), [`agent-sink-turns.test.ts:125`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L130), [`agent-events-turns.test.ts:137`](apps/floor/src/transport/http/routes/agent-events-turns.test.ts#L137))
- A line the subsystem attributed to no task, or of a kind this Floor has never seen, is still collected — carrying a null task id or a null kind, matching the table's nullable columns. ([validated by `agent-run-turns.test.ts:96`](apps/floor/src/work/agent/agent-run-turns.test.ts#L96), [`agent-run-turns.test.ts:105`](apps/floor/src/work/agent/agent-run-turns.test.ts#L105), [`agent-sink-turns.test.ts:49`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L53))
- Collection is capped per POST at the same order as the projection's cap, so a pathological multi-megabyte report cannot materialize an unbounded row set on the single Floor replica. ([validated by `agent-run-turns.test.ts:113`](apps/floor/src/work/agent/agent-run-turns.test.ts#L113), [`agent-sink-turns.test.ts:56`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L60))
- Turn collection is skipped entirely for an oversized body, reusing the projection's existing `MAX_VIZ_BODY_BYTES` gate rather than adding a second size rule. ([validated by `agent-events-turns.test.ts:88`](apps/floor/src/transport/http/routes/agent-events-turns.test.ts#L88))
- A failure to persist turns is counted and logged, never propagated: cost accounting is the sink's contract and a non-authoritative store must not be able to fail it. ([validated by `agent-events-turns.test.ts:78`](apps/floor/src/transport/http/routes/agent-events-turns.test.ts#L78), [`agent-events-turns.test.ts:53`](apps/floor/src/transport/http/routes/agent-events-turns.test.ts#L53))
- *(Amended 2026-08-19, #1389:)* a turn skipped as an already-stored duplicate (its `dedupKey` matched an existing row) is counted (`turn_deduped`) and warned like the two dropped-turn paths: expected on a relay retry, but the only path that could ever swallow a legitimate line, so it must be visible. The relay's collector reads the key from the envelope's `source.turn_key`; pod envelopes carry none. ([validated by `agent-run-turns.test.ts:119`](apps/floor/src/work/agent/agent-run-turns.test.ts#L119), [`agent-run-turns.test.ts:130`](apps/floor/src/work/agent/agent-run-turns.test.ts#L130), [`agent-run-turns.test.ts:138`](apps/floor/src/work/agent/agent-run-turns.test.ts#L138), [`agent-events-turns.test.ts:159`](apps/floor/src/transport/http/routes/agent-events-turns.test.ts#L159), [`agent-events-turns.test.ts:173`](apps/floor/src/transport/http/routes/agent-events-turns.test.ts#L173))
...
- The route hands back one assembly line's turns with their envelopes untruncated, behind the same `ingest-token` bearer auth every other Floor read carries. ([validated by `agent-turns-history.test.ts:55`](apps/floor/src/transport/http/routes/agent-turns-history.test.ts#L55), [`agent-turns-history.test.ts:100`](apps/floor/src/transport/http/routes/agent-turns-history.test.ts#L100))
- The route reads through `listByLine`, scoping rows by line **and** cursor rather than by cursor alone. ([validated by `agent-turns-history.test.ts:65`](apps/floor/src/transport/http/routes/agent-turns-history.test.ts#L65))
- The limit is clamped to a maximum and falls back to a default for a missing, non-numeric, zero or negative value, because the token is shared with the web-ui and an unbounded limit would be an unbounded read; the route reads one lookahead row past the clamped limit to answer `hasMore`. ([validated by `agent-turns-history.test.ts:78`](apps/floor/src/transport/http/routes/agent-turns-history.test.ts#L78))
- The response carries an explicit `hasMore` flag — true only when a row exists past the returned page, with the lookahead row withheld, and always present including on an empty page — so the web-ui walks end on the server's answer instead of comparing page length against a client-side copy of the clamp (#1310). ([validated by `agent-turns-history.test.ts:115`](apps/floor/src/transport/http/routes/agent-turns-history.test.ts#L115), [`agent-turns-history.test.ts:127`](apps/floor/src/transport/http/routes/agent-turns-history.test.ts#L127), [`agent-turns-history.test.ts:139`](apps/floor/src/transport/http/routes/agent-turns-history.test.ts#L139))
- The cursor falls back to `0` for anything that is not a run of digits, so a malformed `after` reads the run from its start instead of erroring. ([validated by `agent-turns-history.test.ts:91`](apps/floor/src/transport/http/routes/agent-turns-history.test.ts#L91))
...
- The task route hands back one task's turns — including rows correlated to no assembly line — behind the same `ingest-token` bearer auth. ([validated by `agent-turns-by-task.test.ts:56`](apps/floor/src/transport/http/routes/agent-turns-by-task.test.ts#L56), [`agent-turns-by-task.test.ts:104`](apps/floor/src/transport/http/routes/agent-turns-by-task.test.ts#L104))
- It reads through `listByTask`, scoping rows by task **and** cursor, and its literal `task` path segment never collides with the registered line route sharing the `/api/agent-turns` prefix. ([validated by `agent-turns-by-task.test.ts:69`](apps/floor/src/transport/http/routes/agent-turns-by-task.test.ts#L69), [`agent-turns-by-task.test.ts:117`](apps/floor/src/transport/http/routes/agent-turns-by-task.test.ts#L117))
- The limit clamp, default, lookahead and `hasMore` flag are byte-identical to the line route's, for the same shared-token reason. ([validated by `agent-turns-by-task.test.ts:82`](apps/floor/src/transport/http/routes/agent-turns-by-task.test.ts#L82), [`agent-turns-by-task.test.ts:136`](apps/floor/src/transport/http/routes/agent-turns-by-task.test.ts#L136), [`agent-turns-by-task.test.ts:155`](apps/floor/src/transport/http/routes/agent-turns-by-task.test.ts#L155))
- The cursor falls back to `0` for a malformed `after`, same as the line route. ([validated by `agent-turns-by-task.test.ts:95`](apps/floor/src/transport/http/routes/agent-turns-by-task.test.ts#L95))
...
- The panel renders collapsed and fetches nothing until first opened, so the default view is unaffected; reopening it while the first walk is still in flight never starts a second one, and the mount is keyed on the run id so run navigation cannot leak a stale transcript. The page URL and cursor arithmetic are pure presenter functions. ([validated by `FullTranscriptPanel.test.tsx:95`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L95), [`FullTranscriptPanel.test.tsx:204`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L204), [`turn-transcript-presenter.test.ts:42`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L52), [`turn-transcript-presenter.test.ts:48`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L58), [`turn-transcript-presenter.test.ts:54`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L64))
- Once opened it pages the proxy with the cursor, ending the walk on the response's explicit `hasMore` flag when it is a boolean and falling back to the pre-#1310 short-page rule when the flag is absent (an older Floor during deploy skew), reading the cursor off the raw page so one unparseable row cannot end the paging early — and a `hasMore: true` page with no usable cursor still ends the walk — showing the cap notice, since the one-shot walk never retries — rather than refetching itself forever. ([validated by `FullTranscriptPanel.test.tsx:128`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L128), [`FullTranscriptPanel.test.tsx:301`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L301), [`FullTranscriptPanel.test.tsx:320`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L320), [`FullTranscriptPanel.test.tsx:333`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L333), [`FullTranscriptPanel.test.tsx:368`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L368), [`turn-transcript-presenter.test.ts:70`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L80), [`turn-transcript-presenter.test.ts:74`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L84), [`turn-transcript-presenter.test.ts:78`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L88), [`turn-transcript-presenter.test.ts:82`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L92), [`turn-transcript-presenter.test.ts:124`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L134), [`turn-transcript-presenter.test.ts:128`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L140), [`turn-transcript-presenter.test.ts:132`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L144), [`turn-transcript-presenter.test.ts:136`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L148), [`turn-transcript-presenter.test.ts:149`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L159), [`turn-transcript-presenter.test.ts:154`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L164), [`turn-transcript-presenter.test.ts:158`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L168), [`turn-transcript-presenter.test.ts:166`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L176), [`turn-transcript-presenter.test.ts:171`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L181))
- The walk is bounded, never silent about it: it requests the Floor route's maximum page size rather than its default, stops at a hard turn cap or at a per-walk page bound (so a Floor clamp far below the requested page size costs bounded requests, not a storm), and shows a notice carrying the actual loaded count instead of materializing an unbounded run in the tab. ([validated by `FullTranscriptPanel.test.tsx:145`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L145), [`FullTranscriptPanel.test.tsx:349`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L349), [`turn-transcript-presenter.test.ts:62`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L72), [`turn-transcript-presenter.test.ts:66`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L76), [`turn-transcript-presenter.test.ts:143`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L153))
- It renders the selected node's turns as a formatted conversation by default — one segment per node visit (specs/job-log-streaming FR, `@/lib/turn-segments`), each classified turn shown with its own timestamp via `conversationEntries`/`clockTime`, so a lifecycle marker with no `type` reads as a sentence instead of an unlabeled `unknown` row and an assistant/user turn shows its message text rather than only its kind; switching the selected node refilters the one line-scoped fetch instead of refetching. A Raw toggle (hidden until turns arrive, formatted by default) restores the untruncated per-turn envelope labeled by the raw stream-json kind and the iteration — the full-fidelity escape hatch this panel exists for. ([validated by `FullTranscriptPanel.test.tsx:104`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L104), [`FullTranscriptPanel.test.tsx:115`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L115), [`FullTranscriptPanel.test.tsx:164`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L164), [`FullTranscriptPanel.test.tsx:178`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L178), [`FullTranscriptPanel.test.tsx:189`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L189), [`FullTranscriptPanel.test.tsx:382`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L382), [`FullTranscriptPanel.test.tsx:409`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L409), [`FullTranscriptPanel.test.tsx:443`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L443), [`turn-transcript-presenter.test.ts:92`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L102), [`turn-transcript-presenter.test.ts:104`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L114), [`turn-transcript-presenter.test.ts:108`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L118), [`turn-transcript-presenter.test.ts:116`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L126), [`turn-transcript-presenter.test.ts:197`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L207), [`turn-transcript-presenter.test.ts:220`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L230), [`turn-transcript-presenter.test.ts:237`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L247), [`turn-transcript-presenter.test.ts:248`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L258), [`turn-transcript-presenter.test.ts:270`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L280), [`turn-transcript-presenter.test.ts:276`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L286), [`turn-transcript-presenter.test.ts:280`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L290))
- The full transcript folds a hook's cumulative turns the same way the blob parser does, keeping the last turn's timestamp, so a run's interleaved SessionStart hooks read as one line each instead of one raw envelope per progress turn. ([validated by [`turn-transcript-presenter.test.ts:299`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L299), [`turn-transcript-presenter.test.ts:316`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L316))
- A tool call that runs long reports itself with cumulative `tool_progress` heartbeats — nineteen for one Skill call in the run that reported this — so a call's beats fold on its `parent_tool_use_id` (each beat's own `tool_use_id` is a fresh `<parent>-heartbeat-<n>`, which would fold nothing) and read as one `· <tool> still running… (<elapsed>)` line carrying the newest total, in the blob viewers and the full transcript alike, rather than one raw envelope per beat, and a beat that reports no clock at all drops the parenthetical rather than claiming the call has run for zero seconds. ([validated by [`agent-log-entries.test.ts:569`](apps/web-ui/src/lib/agent-log-entries.test.ts#L582), [`LogEntriesView.test.tsx:278`](apps/web-ui/src/components/LogEntriesView.test.tsx#L282), [`agent-log-entries.test.ts:543`](apps/web-ui/src/lib/agent-log-entries.test.ts#L556), [`agent-log-entries.test.ts:554`](apps/web-ui/src/lib/agent-log-entries.test.ts#L567), [`agent-log-entries.test.ts:585`](apps/web-ui/src/lib/agent-log-entries.test.ts#L598), [`LogEntriesView.test.tsx:264`](apps/web-ui/src/components/LogEntriesView.test.tsx#L268), [`turn-transcript-presenter.test.ts:339`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L339))
- A node with no stored turns and a failed fetch each get an explicit message, so an empty panel is never ambiguous between "pruned", "never stored", and "broken"; a failed walk retries when the panel is reopened instead of pinning the error until a page reload, showing the loading state rather than the stale error while the retry is in flight. ([validated by `FullTranscriptPanel.test.tsx:219`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L219), [`FullTranscriptPanel.test.tsx:232`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L232), [`FullTranscriptPanel.test.tsx:243`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L243), [`FullTranscriptPanel.test.tsx:262`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L262))
- The panel shows only node-correlated turns by design: correlation is all-or-nothing at write time, so the line-keyed read can never return a node-less row — the rows correlated to nothing are reachable only through FR4's task-keyed route, which the UI does not surface yet. ([validated by `turn-transcript-presenter.test.ts:92`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L102), [`agent-run-turns.test.ts:63`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L63))
