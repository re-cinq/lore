...
- The envelope crosses the port as JSON **text** and is cast to `jsonb` inside the statement, so the ingest path never re-parses a payload it is already holding as a string; the one serialization left is the adapter's single `JSON.stringify` of the whole batch at the Postgres boundary. ([validated by `agent-run-turns.test.ts:300`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L300))
...
- `listByTask` reads the same way scoped to a task id. It has no production caller today and is kept deliberately: it is the ONLY path that can reach the uncorrelated rows FR1 requires the table to preserve, and a store that keeps rows nothing can ever read would contradict its own reason for keeping them. Delete it only together with the decision to stop storing uncorrelated turns. ([validated by `agent-run-turns.test.ts:222`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L222), [`agent-run-turns.test.ts:368`](libs/shared/src/project/agent-run-turns/agent-run-turns.test.ts#L368))
