...
- The walk survives React Strict Mode's development double mount: the panel marks itself live again on every mount, so the walk started before the simulated unmount still delivers its turns instead of leaving the panel on Loading ([validated by](../../apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L555)).
- The panel shows only node-correlated turns by design: correlation is all-or-nothing at write time, so the line-keyed read can never return a node-less row — the rows correlated to nothing are reachable only through FR4's task-keyed route, which the UI does not surface yet. ([validated by `turn-transcript-presenter.test.ts:92`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L100), [`agent-run-turns.test.ts:63`](libs/shared/src/outbound/project/agent-run-turns/agent-run-turns.test.ts#L63))
