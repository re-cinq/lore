...
- Turn collection is unconditional: every `/api/agent-events` POST collects turns, with no feature flag and nothing for an operator to switch on. ([validated by `agent-sink-turns.test.ts:17`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L17), [`agent-events-turns.test.ts:53`](apps/floor/src/transport/http/routes/agent-events-turns.test.ts#L53))
- Collecting turns perturbs nothing else. The cost rows and the run-visualization rows are byte-for-byte what they would be without the turn store, so the projection and the SSE view are unaffected. With no off switch in production this property is the only thing standing between the store and a regression in the outputs that were already there. ([validated by `agent-sink-turns.test.ts:25`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L25), [`agent-sink-turns.test.ts:21`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L21), [`agent-events-turns.test.ts:69`](apps/floor/src/transport/http/routes/agent-events-turns.test.ts#L69))
- A turn is built from the envelope the single pass **already parsed** — the collector re-parses nothing and re-serializes nothing, taking the raw line the scanner already yielded as the stored envelope. ([validated by `agent-run-turns.test.ts:16`](apps/floor/src/work/agent/agent-run-turns.test.ts#L16), [`agent-run-turns.test.ts:61`](apps/floor/src/work/agent/agent-run-turns.test.ts#L61), [`agent-run-turns.test.ts:68`](apps/floor/src/work/agent/agent-run-turns.test.ts#L68), [`agent-sink-turns.test.ts:33`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L33), [`agent-sink-turns.test.ts:43`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L43))
...
- A dropped turn is counted and warned about, never silent. The private-key pattern is not anchored inside one JSON string, so an agent can emit a `BEGIN`/`END` pair straddling JSON structure and thereby keep its own line out of the transcript; a store justified by fidelity has to make that loss visible. ([validated by `agent-sink-turns.test.ts:77`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L77), [`agent-sink-turns.test.ts:84`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L84), [`agent-sink-turns.test.ts:93`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L93), [`agent-sink-turns.test.ts:97`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L97), [`agent-events-turns.test.ts:109`](apps/floor/src/transport/http/routes/agent-events-turns.test.ts#L109), [`agent-events-turns.test.ts:125`](apps/floor/src/transport/http/routes/agent-events-turns.test.ts#L125))
- Turns left out by the per-batch cap are counted and warned about too, separately from the redaction drops. With both paths counted, every way the sink can lose a turn is visible, which is what lets "this transcript is complete" be read off the metrics instead of assumed. ([validated by `agent-sink-turns.test.ts:110`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L110), [`agent-sink-turns.test.ts:117`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L117), [`agent-sink-turns.test.ts:121`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L121), [`agent-sink-turns.test.ts:125`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L125), [`agent-events-turns.test.ts:137`](apps/floor/src/transport/http/routes/agent-events-turns.test.ts#L137))
- A line the subsystem attributed to no task, or of a kind this Floor has never seen, is still collected — carrying a null task id or a null kind, matching the table's nullable columns. ([validated by `agent-run-turns.test.ts:96`](apps/floor/src/work/agent/agent-run-turns.test.ts#L96), [`agent-run-turns.test.ts:105`](apps/floor/src/work/agent/agent-run-turns.test.ts#L105), [`agent-sink-turns.test.ts:49`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L49))
- Collection is capped per POST at the same order as the projection's cap, so a pathological multi-megabyte report cannot materialize an unbounded row set on the single Floor replica. ([validated by `agent-run-turns.test.ts:113`](apps/floor/src/work/agent/agent-run-turns.test.ts#L113), [`agent-sink-turns.test.ts:56`](apps/floor/src/work/agent/agent-sink-turns.test.ts#L56))
...
- A node with no stored turns and a failed fetch each get an explicit message, so an empty panel is never ambiguous between "pruned", "never stored", and "broken"; a failed walk retries when the panel is reopened instead of pinning the error until a page reload, showing the loading state rather than the stale error while the retry is in flight. ([validated by `FullTranscriptPanel.test.tsx:205`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L205), [`FullTranscriptPanel.test.tsx:218`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L218), [`FullTranscriptPanel.test.tsx:229`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L229), [`FullTranscriptPanel.test.tsx:248`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L248))
