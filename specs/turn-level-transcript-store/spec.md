...
- Turn collection is unconditional: every `/api/agent-events` POST collects turns, with no feature flag and nothing for an operator to switch on. ([validated by `agent-sink-turns.test.ts:17`](apps/floor/src/jobs/agent/agent-sink-turns.test.ts#L17), [`agent-events-turns.test.ts:53`](apps/floor/src/delivery/http/routes/agent-events-turns.test.ts#L53))
- Collecting turns perturbs nothing else. The cost rows and the run-visualization rows are byte-for-byte what they would be without the turn store, so the projection and the SSE view are unaffected. With no off switch in production this property is the only thing standing between the store and a regression in the outputs that were already there. ([validated by `agent-sink-turns.test.ts:25`](apps/floor/src/jobs/agent/agent-sink-turns.test.ts#L25), [`agent-sink-turns.test.ts:21`](apps/floor/src/jobs/agent/agent-sink-turns.test.ts#L21), [`agent-events-turns.test.ts:69`](apps/floor/src/delivery/http/routes/agent-events-turns.test.ts#L69))
...
- The stored envelope is redacted with the same `redactSecrets` the GCS archive used, before it ever reaches the database, because a queryable store raises a redaction miss from "buried in GCS" to "searchable". ([validated by `agent-run-turns.test.ts:76`](apps/floor/src/jobs/agent/agent-run-turns.test.ts#L76))
- Redaction is verified not to have broken the line's JSON: an unchanged line is kept as-is, and a redacted line that no longer parses is dropped rather than risking a batch-wide insert failure. ([validated by `agent-run-turns.test.ts:61`](apps/floor/src/jobs/agent/agent-run-turns.test.ts#L61), [`agent-run-turns.test.ts:88`](apps/floor/src/jobs/agent/agent-run-turns.test.ts#L88))
- A dropped turn is counted and warned about, never silent. The private-key pattern is not anchored inside one JSON string, so an agent can emit a `BEGIN`/`END` pair straddling JSON structure and thereby keep its own line out of the transcript; a store justified by fidelity has to make that loss visible. ([validated by `agent-sink-turns.test.ts:77`](apps/floor/src/jobs/agent/agent-sink-turns.test.ts#L77), [`agent-sink-turns.test.ts:84`](apps/floor/src/jobs/agent/agent-sink-turns.test.ts#L84), [`agent-sink-turns.test.ts:93`](apps/floor/src/jobs/agent/agent-sink-turns.test.ts#L93), [`agent-sink-turns.test.ts:97`](apps/floor/src/jobs/agent/agent-sink-turns.test.ts#L97), [`agent-events-turns.test.ts:109`](apps/floor/src/delivery/http/routes/agent-events-turns.test.ts#L109), [`agent-events-turns.test.ts:125`](apps/floor/src/delivery/http/routes/agent-events-turns.test.ts#L125))
- Turns left out by the per-batch cap are counted and warned about too, separately from the redaction drops. With both paths counted, every way the sink can lose a turn is visible, which is what lets "this transcript is complete" be read off the metrics instead of assumed. ([validated by `agent-sink-turns.test.ts:110`](apps/floor/src/jobs/agent/agent-sink-turns.test.ts#L110), [`agent-sink-turns.test.ts:117`](apps/floor/src/jobs/agent/agent-sink-turns.test.ts#L117), [`agent-sink-turns.test.ts:121`](apps/floor/src/jobs/agent/agent-sink-turns.test.ts#L121), [`agent-sink-turns.test.ts:125`](apps/floor/src/jobs/agent/agent-sink-turns.test.ts#L125), [`agent-events-turns.test.ts:137`](apps/floor/src/delivery/http/routes/agent-events-turns.test.ts#L137))
- A line the subsystem attributed to no task, or of a kind this Floor has never seen, is still collected — carrying a null task id or a null kind, matching the table's nullable columns. ([validated by `agent-run-turns.test.ts:96`](apps/floor/src/jobs/agent/agent-run-turns.test.ts#L96), [`agent-run-turns.test.ts:105`](apps/floor/src/jobs/agent/agent-run-turns.test.ts#L105), [`agent-sink-turns.test.ts:49`](apps/floor/src/jobs/agent/agent-sink-turns.test.ts#L49))
- Collection is capped per POST at the same order as the projection's cap, so a pathological multi-megabyte report cannot materialize an unbounded row set on the single Floor replica. ([validated by `agent-run-turns.test.ts:113`](apps/floor/src/jobs/agent/agent-run-turns.test.ts#L113), [`agent-sink-turns.test.ts:56`](apps/floor/src/jobs/agent/agent-sink-turns.test.ts#L56))
- Turn collection is skipped entirely for an oversized body, reusing the projection's existing `MAX_VIZ_BODY_BYTES` gate rather than adding a second size rule. ([validated by `agent-events-turns.test.ts:88`](apps/floor/src/delivery/http/routes/agent-events-turns.test.ts#L88))
- A failure to persist turns is counted and logged, never propagated: cost accounting is the sink's contract and a non-authoritative store must not be able to fail it. ([validated by `agent-events-turns.test.ts:78`](apps/floor/src/delivery/http/routes/agent-events-turns.test.ts#L78), [`agent-events-turns.test.ts:53`](apps/floor/src/delivery/http/routes/agent-events-turns.test.ts#L53))
- *(Amended 2026-08-19, #1389:)* a turn skipped as an already-stored duplicate (its `dedupKey` matched an existing row) is counted (`turn_deduped`) and warned like the two dropped-turn paths: expected on a relay retry, but the only path that could ever swallow a legitimate line, so it must be visible. The relay's collector reads the key from the envelope's `source.turn_key`; pod envelopes carry none. ([validated by `agent-run-turns.test.ts:119`](apps/floor/src/jobs/agent/agent-run-turns.test.ts#L119), [`agent-run-turns.test.ts:130`](apps/floor/src/jobs/agent/agent-run-turns.test.ts#L130), [`agent-run-turns.test.ts:138`](apps/floor/src/jobs/agent/agent-run-turns.test.ts#L138), [`agent-events-turns.test.ts:159`](apps/floor/src/delivery/http/routes/agent-events-turns.test.ts#L159), [`agent-events-turns.test.ts:173`](apps/floor/src/delivery/http/routes/agent-events-turns.test.ts#L173))
