...
- A tool call that runs long reports itself with cumulative `tool_progress` heartbeats — nineteen for one Skill call in the run that reported this — so a call's beats fold on its `parent_tool_use_id` (each beat's own `tool_use_id` is a fresh `<parent>-heartbeat-<n>`, which would fold nothing) and read as one `· <tool> still running… (<elapsed>)` line carrying the newest total, in the blob viewers and the full transcript alike, rather than one raw envelope per beat. ([validated by [`agent-log-entries.test.ts:543`](apps/web-ui/src/lib/agent-log-entries.test.ts#L543), [`agent-log-entries.test.ts:554`](apps/web-ui/src/lib/agent-log-entries.test.ts#L554), [`agent-log-entries.test.ts:569`](apps/web-ui/src/lib/agent-log-entries.test.ts#L569), [`LogEntriesView.test.tsx:264`](apps/web-ui/src/components/LogEntriesView.test.tsx#L264), [`turn-transcript-presenter.test.ts:342`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L342))
