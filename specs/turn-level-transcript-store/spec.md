...
- A turn is built from the envelope the single pass **already parsed** — the collector re-parses nothing and re-serializes nothing, taking the raw line the scanner already yielded as the stored envelope. ([validated by `agent-run-turns.test.ts:16`](apps/floor/src/jobs/agent/agent-run-turns.test.ts#L16), [`agent-run-turns.test.ts:29`](apps/floor/src/jobs/agent/agent-run-turns.test.ts#L59), [`agent-run-turns.test.ts:36`](apps/floor/src/jobs/agent/agent-run-turns.test.ts#L66), [`agent-sink-turns.test.ts:41`](apps/floor/src/jobs/agent/agent-sink-turns.test.ts#L41), [`agent-sink-turns.test.ts:51`](apps/floor/src/jobs/agent/agent-sink-turns.test.ts#L51))
- The stored envelope is redacted with the same `redactSecrets` the GCS archive used, before it ever reaches the database, because a queryable store raises a redaction miss from "buried in GCS" to "searchable". ([validated by `agent-run-turns.test.ts:46`](apps/floor/src/jobs/agent/agent-run-turns.test.ts#L76))
- Redaction is verified not to have broken the line's JSON: an unchanged line is kept as-is, and a redacted line that no longer parses is dropped rather than risking a batch-wide insert failure. ([validated by `agent-run-turns.test.ts:29`](apps/floor/src/jobs/agent/agent-run-turns.test.ts#L59), [`agent-run-turns.test.ts:58`](apps/floor/src/jobs/agent/agent-run-turns.test.ts#L88))
...
- A line the subsystem attributed to no task, or of a kind this Floor has never seen, is still collected — carrying a null task id or a null kind, matching the table's nullable columns. ([validated by `agent-run-turns.test.ts:66`](apps/floor/src/jobs/agent/agent-run-turns.test.ts#L96), [`agent-run-turns.test.ts:75`](apps/floor/src/jobs/agent/agent-run-turns.test.ts#L105), [`agent-sink-turns.test.ts:57`](apps/floor/src/jobs/agent/agent-sink-turns.test.ts#L57))
- Collection is capped per POST at the same order as the projection's cap, so a pathological multi-megabyte report cannot materialize an unbounded row set on the single Floor replica. ([validated by `agent-run-turns.test.ts:83`](apps/floor/src/jobs/agent/agent-run-turns.test.ts#L113), [`agent-sink-turns.test.ts:64`](apps/floor/src/jobs/agent/agent-sink-turns.test.ts#L64))
