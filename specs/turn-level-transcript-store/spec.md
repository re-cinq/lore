...
- A node with no stored turns and a failed fetch each get an explicit message, so an empty panel is never ambiguous between "pruned", "never stored", and "broken"; a failed walk retries when the panel is reopened instead of pinning the error until a page reload. ([validated by `FullTranscriptPanel.test.tsx:187`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L187), [`FullTranscriptPanel.test.tsx:200`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L200), [`FullTranscriptPanel.test.tsx:211`](apps/web-ui/src/app/assembly-runs/[id]/FullTranscriptPanel.test.tsx#L211))
