...
- [ ] T058 [P] Delete legacy local-runner code paths — **deferred** until graph executor is wired in production. The supervisor skeleton currently doesn't call `executeAssemblyLine()`; the legacy local-runner is still the only path to local execution. T058 runs after pilot proves the new path
