...
- [ ] T019 [US1] Modify `mcp-server/src/pipeline.ts` `createTask()`: before creating GitHub Issue, consult repo settings — skip Issue when `create_issue: never` or (`create_issue: on_gate` and `approval_required: false`)
- [ ] T020 [P] [US1] Implement path-allowlist matcher helper in `agent/src/lib/path-match.ts` using `minimatch`; function `allPathsMatch(changedPaths: string[], allowlist: string[]): boolean` returning true only when every changed path matches at least one glob
- [ ] T021 [US1] Implement auto-merge engine in `agent/src/jobs/auto-merge.ts` triggered after `[stage:retrospective]` lands and PR is opened: evaluates `auto_merge.paths` (T020), `min_trust ≤ repo.trust.level`, green CI, bot review APPROVED, no human CHANGES_REQUESTED; if all pass, merges via `gh api -X PUT /repos/:owner/:repo/pulls/:n/merge`; writes `auto_merge_decision` audit entry with rule trace
- [ ] T022 [P] [US1] Implement notification gate in `agent/src/lib/notify.ts`: `notify({channel, level, repo, ...})` consults `settings.dark_factory.notify`; `escalation` always fires, `watched` fires only when task creator opted in
- [ ] T023 [US1] Add OTEL span `lore.auto_merge.decision` in `auto-merge.ts` with attributes `pr_number`, `decision`, `rule`, `trust_level`, `path_match_count`, `ci_status`, `bot_review_state`
- [ ] T024 [US1] End-to-end smoke: enable dark mode on `re-cinq/test-darkmode` via the two-key ceremony, trigger `gap-fill`, verify quickstart Scenario A passes (PR merged, no Issue, all stages present, audit entry written)
