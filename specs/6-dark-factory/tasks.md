...
- [X] T014 [US1] Implement graph executor in `agent/src/supervisor/assembly-line-executor.ts`: walks from `entry`, dispatches each node by type (agent/validate/gate/retrospective), commits a stage commit at end of each node with `formatTrailers()`, refreshes lease before each node
- [X] T015 [US1] Implement resume logic in `assembly-line-executor.ts`: at start, call `lastStageOnBranch(branchName)` and skip nodes already passed (the resume path is shared with US2)
...
- [X] T025 [US2] Lease TTL refresh hook before each node in `assembly-line-executor.ts` (delivered in T014: `await leaseBackend.refresh(branchName, holder, undefined, currentId)` per node); ensures the 10-minute TTL is renewed on each phase
...
- [X] T040 [US5] Wired into `assembly-line-executor.ts` via `IterationMaxExceededError` (typed) + `onIterationMaxExceeded` hook on `ExecuteOptions`. The hook fires before the throw, so callers (the supervisor, when graph integration lands) can call `escalate()` with full context. Hook errors are caught + logged so escalation failures don't mask the original iteration_max problem
...
- [ ] T058 [P] Delete legacy local-runner code paths — **still deferred**, but the rationale moved on: the graph executor IS wired in production (the Floor's event-driven walk replaced the in-process `executeAssemblyLine`, which was itself retired — spec FR6.7–FR6.9); the legacy local-runner remains the only LOCAL execution path because the local runner never adopted the assembly-line library (spec FR2.3)
