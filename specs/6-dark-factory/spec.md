...
- **FR6.7** The `assembly_line.start` handler MUST be the sole executor entry: it validates the definition (an unknown definition closes the row as failed and resolves — configuration errors never retry), marks the row running, routes by definition family (gap-fill/runbook in-process; everything else to the Floor AssemblyLine), and fire-and-backgrounds the run so the event loop is never blocked by a minutes-long walk; the background continuation closes the row and, on the in-process path, applies the terminal task status. ([validated by `start-event-handler.test.ts:66`](apps/floor/src/jobs/assembly-line/start-event-handler.test.ts#L66), [`start-event-handler.test.ts:88`](apps/floor/src/jobs/assembly-line/start-event-handler.test.ts#L88), [`start-event-handler.test.ts:111`](apps/floor/src/jobs/assembly-line/start-event-handler.test.ts#L111), [`start-event-handler.test.ts:124`](apps/floor/src/jobs/assembly-line/start-event-handler.test.ts#L124); implemented by [`start-event-handler.ts:42`](apps/floor/src/jobs/assembly-line/start-event-handler.ts#L42))
