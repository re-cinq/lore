...
- **FR6.11** An Agent's `status.output` is an NDJSON event stream whose terminal line carries the agent text inside a JSON string field (`{"type":"result","result":"…"}`), so newlines and any embedded JSON arrive escaped. The Floor MUST unwrap that envelope once, at the read boundary, before any text parser runs — a parser scanning for a single-line marker survives the escaping while one needing a real newline (the ```REVIEW_FINDINGS block) silently matches nothing, which yields a node that records `changes_requested` while the PR receives no review at all. Unwrapping MUST be idempotent so already-plain and legacy output pass through unchanged. The duties a terminal node owes — post the review, record the outcome and advance, publish the PR check — MUST be shared by the event path and the reaper (FR6.10), and the review MUST be posted BEFORE the state transition, since both paths ignore a non-running row and a post attempted after it can never be repaired by a retry. A review that reaches a verdict but posts nothing, and a check that fails to publish, MUST be audited (`review_findings_unparsed` / `review_post_failed` / `pr_check_publish_failed`) rather than warned away — each is indistinguishable from a clean review at the PR. ([validated by `agent-output.test.ts:9`](libs/assembly-lines/src/agent-output.test.ts#L9), [`agent-output.test.ts:15`](libs/assembly-lines/src/agent-output.test.ts#L15), [`agent-output.test.ts:22`](libs/assembly-lines/src/agent-output.test.ts#L22), [`agent-output.test.ts:34`](libs/assembly-lines/src/agent-output.test.ts#L34), [`agent-output.test.ts:40`](libs/assembly-lines/src/agent-output.test.ts#L40), [`agent-output.test.ts:46`](libs/assembly-lines/src/agent-output.test.ts#L46), [`agent-output.test.ts:52`](libs/assembly-lines/src/agent-output.test.ts#L52), [`agent-output.test.ts:58`](libs/assembly-lines/src/agent-output.test.ts#L58), [`agent-output.test.ts:66`](libs/assembly-lines/src/agent-output.test.ts#L66), [`agent-output.test.ts:70`](libs/assembly-lines/src/agent-output.test.ts#L70), [`node-terminal.test.ts:69`](apps/floor/src/jobs/assembly-line/node-terminal.test.ts#L69), [`node-terminal.test.ts:82`](apps/floor/src/jobs/assembly-line/node-terminal.test.ts#L82), [`node-terminal.test.ts:88`](apps/floor/src/jobs/assembly-line/node-terminal.test.ts#L88), [`node-event-handler.test.ts:125`](apps/floor/src/jobs/assembly-line/node-event-handler.test.ts#L125); implemented by [`agent-output.ts:44`](libs/assembly-lines/src/agent-output.ts#L44), [`node-terminal.ts:51`](apps/floor/src/jobs/assembly-line/node-terminal.ts#L51))
