...
- **FR6.6** The executor MUST report every visited node — including back-edge re-iterations — to an injectable trace sink whose failures never fail the walk; the Floor adapts the shared AssemblyLinesPort as that sink, recording CR name and stage-commit sha per node. ([validated by `assembly-line-executor.test.ts:198`](libs/assembly-lines/src/assembly-line-executor.test.ts#L198), [`assembly-line-executor.test.ts:245`](libs/assembly-lines/src/assembly-line-executor.test.ts#L245), [`floor-assembly-line-run.test.ts:123`](apps/floor/src/jobs/assembly-line/floor-assembly-line-run.test.ts#L123); implemented by [`assembly-line-executor.ts:118`](libs/assembly-lines/src/assembly-line-executor.ts#L118))

- **FR6.7** The `assembly_line.start` handler MUST be the sole executor entry: it validates the definition (a task-less row with an unknown definition closes as failed and resolves — configuration errors never retry), marks the row running, routes by definition shape (detect-shaped definitions to the repo-less detect runner; single-CR rows — task-backed, no builtin definition — mark running and return, per FR6.8; every other definition to the Floor AssemblyLine, one Agent CR per node), and fire-and-backgrounds the run so the event loop is never blocked by a minutes-long walk; the background continuation closes the row (terminal task status on the station path is the agent-watcher's, not the handler's). ([validated by `start-event-handler.test.ts:115`](apps/floor/src/jobs/assembly-line/start-event-handler.test.ts#L115), [`start-event-handler.test.ts:217`](apps/floor/src/jobs/assembly-line/start-event-handler.test.ts#L217), [`start-event-handler.test.ts:233`](apps/floor/src/jobs/assembly-line/start-event-handler.test.ts#L233), [`start-event-handler.test.ts:272`](apps/floor/src/jobs/assembly-line/start-event-handler.test.ts#L272); implemented by [`start-event-handler.ts:39`](apps/floor/src/jobs/assembly-line/start-event-handler.ts#L39))

- **FR6.8** Every Agent-CR task execution MUST have a run row — single-CR task types (onboard / review / runbook: no builtin assembly line) also create a `pipeline.assembly_lines` row at launch (`task_id` set, zero node rows), so the run table is the total execution history. The start handler marks such rows running without walking; the agent-watcher closes them from the task's post-handler status (`pr-created`/`review` → `pr_created`, `failed`/`needs-human-help` → `failed`, else `completed`) when the task's one CR goes terminal. Token reclaim keys on the task type's routing, not on row existence. ([validated by `agent-cr-station-backend.test.ts:90`](apps/floor/src/jobs/station/agent-cr-station-backend.test.ts#L90), [`start-event-handler.test.ts:293`](apps/floor/src/jobs/assembly-line/start-event-handler.test.ts#L293), [`agent-watcher-logic.test.ts:87`](apps/floor/src/jobs/watcher/agent-watcher-logic.test.ts#L87); implemented by [`agent-cr-station-backend.ts:35`](apps/floor/src/jobs/station/agent-cr-station-backend.ts#L35), [`agent-watcher.ts:340`](apps/floor/src/jobs/watcher/agent-watcher.ts#L340))
