...
`(name, project_id)`. ([validated by `migration-0015.test.ts:18`](apps/lore-api/src/work/agents/migration-0015.test.ts#L14))
...
no-ops on a fresh DB that already built the new name from 0015. ([validated by `migration-0016.test.ts:20`](apps/lore-api/src/work/agents/migration-0016.test.ts#L14), [validated by `migration-0016.test.ts:32`](apps/lore-api/src/work/agents/migration-0016.test.ts#L26))
...
- **FR1 — Port-only access.** Agent-definition config is reached only through
  ...
  reads `lore.agent_definitions` directly. ([validated by `agent-defs.test.ts:49`](libs/shared/src/outbound/project/agents/agent-defs.test.ts#L44))
- **FR2 — Precedence.** Resolution field-merges project → org → yaml; a null
  nullable field on a layer inherits the next layer down. ([validated by `agent-defs-port.test.ts:37`](libs/shared/src/outbound/project/agents/agent-defs-port.test.ts#L37))
- **FR3 — DB adapter merge.** `PgAgentDefs` field-merges the repo's project row
  over the org row over the yaml base; its writes target the repo's own project row, so a delete removes only that repo's override. ([validated by `agent-defs-pg.test.ts:66`](libs/shared/src/outbound/project/agents/agent-defs-pg.test.ts#L66), [validated by deletes the repo's project row for a name, scoped to the repo](libs/shared/src/outbound/project/agents/agent-defs-pg.test.ts#L162))
- **FR4 — Yaml prompt inheritance.** A seeded org row that leaves `prompt` null
  inherits the prompt from the yaml base. ([validated by `agent-defs-pg.test.ts:66`](libs/shared/src/outbound/project/agents/agent-defs-pg.test.ts#L66))
- **FR5 — Offline fallback.** `AgentDefsYaml` resolves task types from
  `task-types.yaml` and refuses writes (read-only without a DB). ([validated by `agent-defs-yaml.test.ts:133`](libs/shared/src/outbound/project/agents/agent-defs-yaml.test.ts#L130))
- **FR5a — Base task-types reader.** The `pipeline-config` reader that
  ...
  base `prompt_template` unless the override sets its own. ([validated by `loads task types from the project's YAML file`](libs/server-core/src/work/pipeline/pipeline-config.test.ts#L17), [validated by `handles missing YAML gracefully`](libs/server-core/src/work/pipeline/pipeline-config.test.ts#L42), [validated by `returns config for a known task type`](libs/server-core/src/work/pipeline/pipeline-config.test.ts#L79), [validated by `returns null for unknown task type`](libs/server-core/src/work/pipeline/pipeline-config.test.ts#L88), [validated by `implementation type has claude-code execution mode`](libs/server-core/src/work/pipeline/pipeline-config.test.ts#L92), [validated by `review type has timeout configured`](libs/server-core/src/work/pipeline/pipeline-config.test.ts#L99), [validated by `each claude-code task type has a prompt_template`](libs/server-core/src/work/pipeline/pipeline-config.test.ts#L106), [validated by `substitutes {description} in the template`](libs/server-core/src/work/pipeline/pipeline-config.test.ts#L140), [validated by `falls back to default template for unknown type`](libs/server-core/src/work/pipeline/pipeline-config.test.ts#L147), [validated by `preserves template structure around the description`](libs/server-core/src/work/pipeline/pipeline-config.test.ts#L154), [validated by `handles empty description`](libs/server-core/src/work/pipeline/pipeline-config.test.ts#L161), [validated by `handles description with special characters`](libs/server-core/src/work/pipeline/pipeline-config.test.ts#L167), [validated by `returns configured default repo`](libs/server-core/src/work/pipeline/pipeline-config.test.ts#L192), [validated by `falls back to re-cinq/lore for unknown types`](libs/server-core/src/work/pipeline/pipeline-config.test.ts#L198), [validated by `returns base config unchanged when repoSettings is null`](libs/server-core/src/work/pipeline/pipeline-config.test.ts#L220), [validated by `returns default base config for an unknown type with no repoSettings`](libs/server-core/src/work/pipeline/pipeline-config.test.ts#L228), [validated by `applies a per-type override field on top of the base config`](libs/server-core/src/work/pipeline/pipeline-config.test.ts#L238), [validated by `uses the override prompt_template when the repo sets one`](libs/server-core/src/work/pipeline/pipeline-config.test.ts#L246), [validated by `keeps the base prompt_template when overrides omit it`](libs/server-core/src/work/pipeline/pipeline-config.test.ts#L256); implemented by [`pipeline-config.ts:1`](libs/server-core/src/work/pipeline/pipeline-config.ts#L1))
- **FR6 — Runner fetches over the API.** `AgentDefsHttp` resolves an agent by
  fetching the agent-definitions endpoint with the bearer token and is read-only. ([validated by `agent-defs-http.test.ts:79`](libs/shared/src/outbound/project/agents/agent-defs-http.test.ts#L79))
- **FR7 — Facade delegation.** `project.agentDefs` delegates the definition methods
  to the wired port, bound to the facade's repo. ([validated by delegates create/delete to the defs port with the bound repo](libs/shared/src/outbound/project/agents/agent-defs.test.ts#L63))
- **FR8 — Request validation + image gate.** The schema normalizes the body and
  ...
  rejects an invalid body with `400 invalid_agent`. ([validated by flags a write that sets a non-empty image](apps/lore-api/src/work/agents/agents-schema.test.ts#L170), [validated by `agents-schema.test.ts:29`](apps/lore-api/src/work/agents/agents-schema.test.ts#L29), [validated by `agents-schema.test.ts:10`](apps/lore-api/src/work/agents/agents-schema.test.ts#L10), [validated by `agents-schema.test.ts:33`](apps/lore-api/src/work/agents/agents-schema.test.ts#L33), [validated by `agents-schema.test.ts:62`](apps/lore-api/src/work/agents/agents-schema.test.ts#L74), [validated by `agents-schema.test.ts:126`](apps/lore-api/src/work/agents/agents-schema.test.ts#L174), [validated by `agents-route.test.ts:148`](apps/lore-api/src/transport/routes/agent-definitions/agents-route.test.ts#L148), [validated by rejects an invalid update body with 400](apps/lore-api/src/transport/routes/agent-definitions/agents-route.test.ts#L272), [validated by carries every scalar field present in the body](apps/lore-api/src/work/agents/agents-schema.test.ts#L102), [validated by clears nullable fields when explicitly set to null](apps/lore-api/src/work/agents/agents-schema.test.ts#L122))
- **FR9 — Authorization scope.** GET (resolve/list) is read-scoped; agent writes
  ...
  `404` for an unknown name. ([validated by resolves one agent by name (GET, read scope)](apps/lore-api/src/transport/routes/agent-definitions/agents-route.test.ts#L108), [validated by creates an agent (admin tier) and audits it](apps/lore-api/src/transport/routes/agent-definitions/agents-route.test.ts#L129), [validated by `agents-route.test.ts:101`](apps/lore-api/src/transport/routes/agent-definitions/agents-route.test.ts#L101), [validated by `agents-route.test.ts:116`](apps/lore-api/src/transport/routes/agent-definitions/agents-route.test.ts#L116))
- **FR10 — API delegates + audits + two-key.** The route delegates CRUD to
  ...
  failure — updates an agent by name, and returns `503` without a DB pool. ([validated by creates an agent (admin tier) and audits it](apps/lore-api/src/transport/routes/agent-definitions/agents-route.test.ts#L129), [validated by two-key gates a create that sets an image (no approval header)](apps/lore-api/src/transport/routes/agent-definitions/agents-route.test.ts#L155), [validated by `agents-route.test.ts:270`](apps/lore-api/src/transport/routes/agent-definitions/agents-route.test.ts#L328), [validated by `agents-route.test.ts:166`](apps/lore-api/src/transport/routes/agent-definitions/agents-route.test.ts#L166), [validated by `agents-route.test.ts:191`](apps/lore-api/src/transport/routes/agent-definitions/agents-route.test.ts#L191), [validated by `agents-route.test.ts:209`](apps/lore-api/src/transport/routes/agent-definitions/agents-route.test.ts#L209), [validated by two-key gates an update that sets an image (no approval header)](apps/lore-api/src/transport/routes/agent-definitions/agents-route.test.ts#L223), [validated by applies an image update after CODEOWNERS approval](apps/lore-api/src/transport/routes/agent-definitions/agents-route.test.ts#L233), [validated by returns 403 on a CODEOWNERS failure for an image update](apps/lore-api/src/transport/routes/agent-definitions/agents-route.test.ts#L254), [validated by `agents-route.test.ts:90`](apps/lore-api/src/transport/routes/agent-definitions/agents-route.test.ts#L90))
- **FR11 — Floor prompt resolution.** The worker/handler build the prompt from
  the resolved definition, falling back to the yaml template. ([validated by substitutes {description} into the resolved agent's prompt](apps/floor/src/outbound/agent-invocation.test.ts#L5), [validated by falls back to the yaml task-type template when the definition has no prompt](apps/floor/src/outbound/agent-invocation.test.ts#L11))
- **FR12 — Migration seed + backfill.** Migration 0015 seeds an org row per task
  ...
  granting `lore_ui` SELECT only inside a role-exists guard. ([validated by seeds an org row for every task-types.yaml task type, idempotently](apps/lore-api/src/work/agents/migration-0015.test.ts#L24), [validated by `migration-0015.test.ts:48`](apps/lore-api/src/work/agents/migration-0015.test.ts#L44), [validated by `migration-0015.test.ts:58`](apps/lore-api/src/work/agents/migration-0015.test.ts#L54))
...
- **FR23 — Pod resources as catalog data.** A definition's pod resources
  ...
  - The API keeps the block beside a null config on create — the route merges it over the config the new row inherits — accepts quantities up to the exa suffixes, and rejects a value that is not a Kubernetes quantity string at the edge. ([validated by keeps pod_resources beside a null config on create so the route can merge it over the inherited layer](apps/lore-api/src/work/agents/agents-schema.test.ts#L39), [validated by accepts exa-scale quantities 1E and 2Ei](apps/lore-api/src/work/agents/agents-schema.test.ts#L52), [validated by rejects a pod_resources quantity that is not a Kubernetes quantity string](apps/lore-api/src/work/agents/agents-schema.test.ts#L63))
  - A patch carries `pod_resources` separately when present — null meaning clear — and omits it when absent. ([validated by carries pod_resources when present and omits it when absent](apps/lore-api/src/work/agents/agents-schema.test.ts#L80), [validated by a null pod_resources means clear — carried as null](apps/lore-api/src/work/agents/agents-schema.test.ts#L96))
  - Writes merge the block over the config the row inherits so a project fork keeps the keys it inherits — config is whole-object across layers. A create merges in the route over the resolved org/yaml config (null removes the block, an empty result collapses to null); an update hands the adapter the block plus the inherited config and the merge happens inside the upsert. ([validated by keeps the resolved config's other keys while replacing pod_resources](apps/lore-api/src/work/agents/agents-schema.test.ts#L140), [validated by null removes pod_resources and an empty result collapses to null](apps/lore-api/src/work/agents/agents-schema.test.ts#L155), [validated by null existing config plus pod_resources yields just the block](apps/lore-api/src/work/agents/agents-schema.test.ts#L162), [validated by a create with pod_resources writes the block merged over the inherited config, never the block alone](apps/lore-api/src/transport/routes/agent-definitions/agents-route.test.ts#L281), [validated by an update with pod_resources hands the adapter the block plus the inherited config to merge inside the upsert](apps/lore-api/src/transport/routes/agent-definitions/agents-route.test.ts#L307))
  - The sync render honours the stored block on the rendered Station. *(amended 2026-09-02)* A stored override merges PER KEY onto the platform defaults instead of replacing the whole object: a memory-only override used to silently drop the ephemeral-storage defaults, Autopilot backfilled a bare 1Gi, and every tdd-round pod was evicted mid-install for a setting that only meant to raise memory. The price — a default cannot be unset, only overridden — is deliberate. ([validated by a full pod_resources override takes every key of both maps](libs/shared/src/outbound/project/agents/agent-crd.test.ts#L163), [a memory-only pod_resources override keeps the cpu and ephemeral-storage defaults](libs/shared/src/outbound/project/agents/agent-crd.test.ts#L193))
...
- **FR24 — Org-default definitions editable from the global /agents page.** The
  ...
  - `updateOrgDefinition` upserts the org row against the `project_id IS NULL` partial unique index and appends the `lore.catalog_events` row the cluster-agents' sync loops tail, in one statement. ([validated by upserts the org row (project_id NULL conflict target) and appends a catalog event in one statement](libs/shared/src/outbound/project/agents/agent-defs-pg.test.ts#L204))
  - `PUT /api/agent-definitions/{name}` (admin scope) writes the org default and returns it. ([validated by upserts the org-default row and returns the written definition](apps/lore-api/src/transport/routes/agent-definitions/org-update.test.ts#L69))
  - The org write merges `pod_resources` inside the upsert, over the org row's own config — atomic under the row lock, so a concurrent edit is never read in one statement and lost in the next — with the yaml layer as the fallback for a row that has no config of its own; null removes the block and an emptied config collapses to NULL. A body that never mentions `pod_resources` keeps the row's config exactly as it was. The per-repo project row upsert merges the same way. ([validated by merges pod_resources inside the upsert: binds touched=true and the block, and merges over the row's own config in SQL](apps/lore-api/src/transport/routes/agent-definitions/org-update.test.ts#L94), [validated by a PUT that never mentions pod_resources binds touched=false so the row's config is kept as is](apps/lore-api/src/transport/routes/agent-definitions/org-update.test.ts#L122), [validated by without a pod_resources write binds touched=false and keeps the row's config in the conflict branch](libs/shared/src/outbound/project/agents/agent-defs-pg.test.ts#L231), [validated by with a pod_resources write merges the block over the row's own config in SQL, the inherited layer as fallback](libs/shared/src/outbound/project/agents/agent-defs-pg.test.ts#L254), [validated by a null pod_resources write removes the block: binds touched=true with no block so an emptied config collapses to NULL](libs/shared/src/outbound/project/agents/agent-defs-pg.test.ts#L285), [validated by binds touched, inherited config and block after the repo and merges in the project row's conflict branch](libs/shared/src/outbound/project/agents/agent-defs-pg.test.ts#L309))
  - A non-empty `image` is refused with `400 image_org_gated`: the two-key image ceremony is CODEOWNERS-scoped to a repo, and no repo is in hand on the org surface. ([validated by rejects a non-empty image with 400 — org image changes go through the per-repo two-key flow](apps/lore-api/src/transport/routes/agent-definitions/org-update.test.ts#L140))
  - An invalid body is rejected `400 invalid_agent`. ([validated by rejects an invalid body with 400 invalid_agent](apps/lore-api/src/transport/routes/agent-definitions/org-update.test.ts#L154))
