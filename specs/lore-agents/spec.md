...
- **FR3 — DB adapter merge.** `PgAgentDefs` field-merges the repo's project row
  over the org row over the yaml base; its writes target the repo's own project row, so a delete removes only that repo's override. ([validated by `agent-defs-pg.test.ts:82`](libs/shared/src/project/agents/agent-defs-pg.test.ts#L84), [validated by deletes the repo's project row for a name, scoped to the repo](libs/shared/src/project/agents/agent-defs-pg.test.ts#L172))
...
- **FR23 — Pod resources as catalog data.** A definition's pod resources
  ...
  - The sync render honours the stored block on the rendered Station. ([validated by config pod_resources replace the default pod resources on the Station](libs/shared/src/project/agents/agent-crd.test.ts#L172))
