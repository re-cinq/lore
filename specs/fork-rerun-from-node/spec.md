...
web-ui → lore-api `POST /api/assembly-runs` → the FR1 `resumeFrom` start. ([offers retry in the node card's header on a finished run, posting the implement fork source](apps/web-ui/src/app/assembly-runs/[id]/RunVisualizationPanel.test.tsx#L812), [`start-run.test.ts:163`](apps/lore-api/src/transport/routes/assembly-lines/start-run.test.ts#L163)) The
...
- The inspector offers the retry in the node CARD'S HEADER row — beside the outcome pill, where the failure it answers is announced — only on a terminal run with a resolvable fork source, and posts the run id plus the SOURCE visit's node id and iteration as the request body. ([offers retry in the node card's header on a finished run, posting the implement fork source](apps/web-ui/src/app/assembly-runs/[id]/RunVisualizationPanel.test.tsx#L812), [offers retry on a looping run's validate node, posting implement@2 as the fork source](apps/web-ui/src/app/assembly-runs/[id]/RunVisualizationPanel.test.tsx#L850), [offers no retry while the run is still running](apps/web-ui/src/app/assembly-runs/[id]/RunVisualizationPanel.test.tsx#L883), [offers no retry on the entry node — there is no prefix to fork from](apps/web-ui/src/app/assembly-runs/[id]/RunVisualizationPanel.test.tsx#L900))
