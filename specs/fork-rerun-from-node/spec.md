...
- A `resumeFrom` start requires the caller to supply the current definition's hash, and rejects a mismatch against the source line's stored hash rather than replaying node rows against a graph that has since changed. ([validated by `resume.test.ts:153`](libs/shared/src/project/assembly-lines/resume.test.ts#L153), [`resume.test.ts:220`](libs/shared/src/project/assembly-lines/resume.test.ts#L220))
- A source line whose stored hash is NULL — every row predating the column — is rejected with a message naming the backfill, an honest limitation preferable to forking across silent definition drift. ([validated by `resume.test.ts:210`](libs/shared/src/project/assembly-lines/resume.test.ts#L210))
