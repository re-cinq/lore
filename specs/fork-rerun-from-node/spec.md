...
- `AssemblyLinesPort.start` accepts an optional `resumeFrom: { lineId, nodeId }`, mints a fresh per-attempt assembly-line id exactly as a plain start does, and returns it. ([validated by `assembly-lines.test.ts:925`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L925))
- A `resumeFrom` start leaves the source line's own row and node rows untouched — the fork is a new attempt, never an edit of the recorded one. ([validated by `assembly-lines.test.ts:992`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L992))
- `branch` is inherited from the source line, and passing `branch` alongside `resumeFrom` is a validation error rather than a silent override. ([validated by `resume.test.ts:136`](libs/shared/src/project/assembly-lines/resume.test.ts#L136), [`assembly-lines.test.ts:1212`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1212))
- `taskId` is inherited from the source line, and passing `taskId` alongside `resumeFrom` is a validation error on the same grounds. ([validated by `resume.test.ts:146`](libs/shared/src/project/assembly-lines/resume.test.ts#L146), [`assembly-lines.test.ts:1212`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1212))
- `args` are inherited from the source line when omitted and replaced wholesale when supplied, so an operator can inject corrected inputs for the replayed remainder; a legacy source row whose `args` column is NULL forks with `{}`, exactly as a plain start would store. ([validated by `assembly-lines.test.ts:1011`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1011), [`assembly-lines.test.ts:1235`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1235), [`assembly-lines.test.ts:1287`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1287))
...
- The repo-scoped `AssemblyLines` facade passes `resumeFrom` through unchanged, so callers write `project.assemblyLines.start(name, { resumeFrom, definitionHash })`. ([validated by `assembly-lines.test.ts:1316`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1316))

## FR2 — Copy semantics

- The copy runs through the chosen node's **latest completed row inclusive**, so earlier iterations of that node — everything a back-edge produced before it — ride along in visit order. ([validated by `resume.test.ts:75`](libs/shared/src/project/assembly-lines/resume.test.ts#L75), [`resume.test.ts:80`](libs/shared/src/project/assembly-lines/resume.test.ts#L80), [`resume.test.ts:84`](libs/shared/src/project/assembly-lines/resume.test.ts#L84), [`resume.test.ts:90`](libs/shared/src/project/assembly-lines/resume.test.ts#L90), [`resume.test.ts:101`](libs/shared/src/project/assembly-lines/resume.test.ts#L101), [`resume.test.ts:128`](libs/shared/src/project/assembly-lines/resume.test.ts#L128), [`assembly-lines.test.ts:948`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L948), [`assembly-lines.test.ts:1195`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1195))
- Copied rows carry the source row's `outcome`, `commit_sha`, `started_at` and `finished_at` — the inherited prefix keeps the provenance of the run that actually produced it rather than masquerading as fresh work — but never its `agent_cr_name`, which is nulled on copy: the run-viz and cost correlation joins resolve a CR name to the newest matching node row, and an echoed name would steal the source's late-arriving agent-event and cost rows. ([validated by `resume.test.ts:111`](libs/shared/src/project/assembly-lines/resume.test.ts#L111), [`assembly-lines.test.ts:948`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L948), [`assembly-lines.test.ts:1251`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1251))
...
- The Postgres adapter writes the line row, the `assembly_line.start` event and every copied node row in ONE data-modifying CTE, exactly as a plain start writes its two, so a fork is never half-created. ([validated by `assembly-lines.test.ts:1179`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1179), [`assembly-lines.test.ts:1195`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1195), [`assembly-lines.test.ts:1251`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1251))
- Validation completes before anything is written; the properties it reads — a terminal line's status, definition name, repo and stamped hash — are immutable once observed, so the read-then-write split introduces no window in which a validated fork becomes invalid. ([validated by `assembly-lines.test.ts:1064`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1064), [`assembly-lines.test.ts:1164`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1164), [`assembly-lines.test.ts:1266`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1266), [`assembly-lines.test.ts:1275`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1275))
- The plain (non-fork) start path is unchanged: same statement, same parameters, no copied-rows CTE, and the double agrees with the adapter on it exactly — both state an explicit null parentage in the `assembly_line.start` event, and neither stores a caller-supplied `definitionHash`, which is a resume input the Floor re-stamps. ([validated by `assembly-lines.test.ts:1299`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1299), [`assembly-lines.test.ts:1347`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1347), [`assembly-lines.test.ts:1361`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1361))

## FR3 — Only terminal lines fork

- A `resumeFrom` whose source line is still `queued` or `running` is refused, naming the status. ([validated by `resume.test.ts:200`](libs/shared/src/project/assembly-lines/resume.test.ts#L200), [`assembly-lines.test.ts:1084`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1084), [`assembly-lines.test.ts:1266`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1266))
...
- The stamp records the hash only when the row carries none, in the Postgres adapter and the in-memory double alike, so a redelivered start that loads a since-edited definition cannot re-point a run at a graph it never ran. *(Amended 2026-08-14: the same call now stores the cloned graph beside the hash — see FR6.38 in `specs/6-dark-factory/spec.md`. The write-once rule is unchanged and covers both, for exactly this reason; the guard's left-hand side becomes the run's OWN stored graph rather than a hash of a file that may no longer exist.)* ([validated by `assembly-lines.test.ts:846`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L846), [`assembly-lines.test.ts:856`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L856), [`assembly-lines.test.ts:865`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L865), [`assembly-lines.test.ts:875`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L875))
...
- A `resumeFrom` start requires the caller to supply the current definition's hash, and rejects a mismatch against the source line's stored hash rather than replaying node rows against a graph that has since changed. ([validated by `resume.test.ts:156`](libs/shared/src/project/assembly-lines/resume.test.ts#L156), [`resume.test.ts:223`](libs/shared/src/project/assembly-lines/resume.test.ts#L223), [`assembly-lines.test.ts:1275`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1275))
...
- The `assembly_line.start` event params carry the fork parentage, so the audit record of *why* a line exists rides with the trigger. ([validated by `assembly-lines.test.ts:1028`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1028), [`assembly-lines.test.ts:1052`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1052), [`assembly-lines.test.ts:1179`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1179))
- The line row itself records `resumed_from_line_id` and `resumed_from_node_id`, because `pipeline.events` rows are pruned once handled and an event alone is not a durable audit substrate. ([validated by `assembly-lines.test.ts:925`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L925), [`assembly-lines.test.ts:1052`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1052), [`assembly-lines.test.ts:1212`](libs/shared/src/project/assembly-lines/assembly-lines.test.ts#L1212))
- The walk needs no change: the Floor's ordinary start handler marks the forked row running and `advanceLine` replays the inherited rows through `nextTransition`, which returns a launch for the successor of the cutoff node. ([validated by `advance.test.ts:831`](apps/floor/src/jobs/assembly-line/advance.test.ts#L831), [`advance.test.ts:850`](apps/floor/src/jobs/assembly-line/advance.test.ts#L850))
- The branch-overlap guard measures against the inherited prefix rather than an empty node list, so a fork that lands on a branch another open line already holds still defers as `lease_held` instead of racing it. ([validated by `advance.test.ts:863`](apps/floor/src/jobs/assembly-line/advance.test.ts#L863), [`advance.test.ts:886`](apps/floor/src/jobs/assembly-line/advance.test.ts#L886), [`advance.test.ts:898`](apps/floor/src/jobs/assembly-line/advance.test.ts#L898))
- The prefix size the guard reads is the count fixed at fork time, never one recomputed from the line's current rows. A fork's own walk may revisit the node it resumed from — `implementation.yaml` loops `validate` back to `implement` — which would make a recomputed count grow back to the row count and re-arm the guard mid-walk, closing a RUNNING fork as `lease_held`. The stored count cannot move, so the test is false forever after the fork's first launch. ([validated by `advance.test.ts:996`](apps/floor/src/jobs/assembly-line/advance.test.ts#L996), [`advance.test.ts:898`](apps/floor/src/jobs/assembly-line/advance.test.ts#L898))
