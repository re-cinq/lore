...
- The repo-scoped `AssemblyLines` facade passes `resumeFrom` through unchanged, so callers write `project.assemblyLines.start(name, { resumeFrom, definitionHash })`. ([validated by `assembly-lines.test.ts:1316`](libs/shared/src/project/assembly-runs/assembly-runs.test.ts#L1579))
...
- A `resumeFrom` start requires the caller to supply the current definition's hash, and rejects a mismatch against the source line's stored hash rather than replaying node rows against a graph that has since changed. ([validated by `resume.test.ts:156`](libs/shared/src/project/assembly-runs/resume.test.ts#L159), [`resume.test.ts:223`](libs/shared/src/project/assembly-runs/resume.test.ts#L222), [`assembly-lines.test.ts:1275`](libs/shared/src/project/assembly-runs/assembly-runs.test.ts#L1540))
...
- The walk needs no change: the Floor's ordinary start handler marks the forked row running and `advanceLine` replays the inherited rows through `nextTransition`, which returns a launch for the successor of the cutoff node. ([validated by `advance.test.ts:861`](apps/floor/src/jobs/assembly-run/advance.test.ts#L861), [`advance.test.ts:880`](apps/floor/src/jobs/assembly-run/advance.test.ts#L880))
