...
- The walk needs no change: the Floor's ordinary start handler marks the forked row running and `advanceLine` replays the inherited rows through `nextTransition`, which returns a launch for the successor of the cutoff node. ([validated by `advance.test.ts:923`](apps/floor/src/jobs/assembly-run/advance.test.ts#L923), [`advance.test.ts:942`](apps/floor/src/jobs/assembly-run/advance.test.ts#L942))
