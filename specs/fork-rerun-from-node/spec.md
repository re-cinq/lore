...
- A `resumeFrom` start leaves the source line's own row and node rows untouched — the fork is a new attempt, never an edit of the recorded one. ([validated by `assembly-lines.test.ts:1262`](libs/shared/src/project/assembly-runs/assembly-runs.test.ts#L1274))
...
- The repo-scoped `AssemblyLines` facade passes `resumeFrom` through unchanged, so callers write `project.assemblyLines.start(name, { resumeFrom, definitionHash })`. ([validated by `assembly-lines.test.ts:1599`](libs/shared/src/project/assembly-runs/assembly-runs.test.ts#L1611))
