...
- The walk needs no change: the Floor's ordinary start handler marks the forked row running and `advanceLine` replays the inherited rows through `nextTransition`, which returns a launch for the successor of the cutoff node. ([validated by `advance.test.ts:885`](apps/floor/src/jobs/assembly-run/advance.test.ts#L885), [`advance.test.ts:904`](apps/floor/src/jobs/assembly-run/advance.test.ts#L904))
