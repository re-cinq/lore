...
web-ui → lore-api `POST /api/assembly-runs` → the FR1 `resumeFrom` start. ([validated by offers retry on a finished run's validate node](apps/web-ui/src/app/assembly-runs/[id]/RunVisualizationPanel.test.tsx#L960), [`start-run.test.ts:127`](apps/lore-api/src/api/routes/assembly-lines/start-run.test.ts#L163)) The
...
successor, so the HTTP surface adds no second routing rule. ([validated by `retry-resume.test.ts:11`](apps/web-ui/src/app/assembly-runs/[id]/retry-resume.test.ts#L11), [`RunVisualizationPanel.test.tsx:960`](apps/web-ui/src/app/assembly-runs/[id]/RunVisualizationPanel.test.tsx#L960))
...
- The inspector offers the retry in the node CARD'S HEADER row — beside the outcome pill, where the failure it answers is announced — only on a terminal run with a resolvable fork source, and posts the run id plus the SOURCE visit's node id and iteration as the request body. ([validated by `RunVisualizationPanel.test.tsx:960`](apps/web-ui/src/app/assembly-runs/[id]/RunVisualizationPanel.test.tsx#L960), [`RunVisualizationPanel.test.tsx:998`](apps/web-ui/src/app/assembly-runs/[id]/RunVisualizationPanel.test.tsx#L998), [`RunVisualizationPanel.test.tsx:1031`](apps/web-ui/src/app/assembly-runs/[id]/RunVisualizationPanel.test.tsx#L1031), [`RunVisualizationPanel.test.tsx:1048`](apps/web-ui/src/app/assembly-runs/[id]/RunVisualizationPanel.test.tsx#L1048))
...
- On a forked run, the history is read through the `resumed_from_run_id` chain — each source run's rows keep their details, and a fork of a fork contributes each ancestor's attempts, oldest ancestor first, bounded at 5 hops. The fork's FIRST launch is precisely the retried node, so the source's failure reaches exactly the dispatch the retry exists for. ([validated by a forked run's first launch carries the source run's failure detail](apps/floor/src/jobs/assembly-run/advance.test.ts#L606), [follows a fork-of-fork chain oldest first and stops at the hop bound](apps/floor/src/jobs/assembly-run/advance.test.ts#L671))
- A run that is no fork reads nothing beyond the visit rows the walk already holds — the source-run reads are gated on `resumed_from_run_id`, so the plain path pays zero extra queries. ([validated by a plain run reads no source runs while dispatching](apps/floor/src/jobs/assembly-run/advance.test.ts#L728))
