...
- `AssemblyLinesPort.start` accepts an optional `resumeFrom: { lineId, nodeId }`, mints a fresh per-attempt assembly-line id exactly as a plain start does, and returns it. ([validated by `assembly-lines.test.ts:925`](libs/shared/src/project/assembly-runs/assembly-runs.test.ts#L1162))
- A `resumeFrom` start leaves the source line's own row and node rows untouched — the fork is a new attempt, never an edit of the recorded one. ([validated by `assembly-lines.test.ts:992`](libs/shared/src/project/assembly-runs/assembly-runs.test.ts#L1229))
...
- The `assembly_line.start` event params carry the fork parentage, so the audit record of *why* a line exists rides with the trigger. ([validated by `assembly-lines.test.ts:1028`](libs/shared/src/project/assembly-runs/assembly-runs.test.ts#L1265), [`assembly-lines.test.ts:1052`](libs/shared/src/project/assembly-runs/assembly-runs.test.ts#L1289), [`assembly-lines.test.ts:1179`](libs/shared/src/project/assembly-runs/assembly-runs.test.ts#L1416))
- The line row itself records `resumed_from_line_id` and `resumed_from_node_id`, because `pipeline.events` rows are pruned once handled and an event alone is not a durable audit substrate. ([validated by `assembly-lines.test.ts:925`](libs/shared/src/project/assembly-runs/assembly-runs.test.ts#L1162), [`assembly-lines.test.ts:1052`](libs/shared/src/project/assembly-runs/assembly-runs.test.ts#L1289), [`assembly-lines.test.ts:1212`](libs/shared/src/project/assembly-runs/assembly-runs.test.ts#L1449))
- The walk needs no change: the Floor's ordinary start handler marks the forked row running and `advanceLine` replays the inherited rows through `nextTransition`, which returns a launch for the successor of the cutoff node. ([validated by `advance.test.ts:831`](apps/floor/src/jobs/assembly-line/advance.test.ts#L919), [`advance.test.ts:850`](apps/floor/src/jobs/assembly-line/advance.test.ts#L938))
- The branch-overlap guard measures against the inherited prefix rather than an empty node list, so a fork that lands on a branch another open line already holds still defers as `lease_held` instead of racing it. ([validated by `advance.test.ts:863`](apps/floor/src/jobs/assembly-line/advance.test.ts#L951), [`advance.test.ts:886`](apps/floor/src/jobs/assembly-line/advance.test.ts#L974), [`advance.test.ts:898`](apps/floor/src/jobs/assembly-line/advance.test.ts#L986))
- The prefix size the guard reads is the count fixed at fork time, never one recomputed from the line's current rows. A fork's own walk may revisit the node it resumed from — `implementation.yaml` loops `validate` back to `implement` — which would make a recomputed count grow back to the row count and re-arm the guard mid-walk, closing a RUNNING fork as `lease_held`. The stored count cannot move, so the test is false forever after the fork's first launch. ([validated by `advance.test.ts:996`](apps/floor/src/jobs/assembly-line/advance.test.ts#L1084), [`advance.test.ts:898`](apps/floor/src/jobs/assembly-line/advance.test.ts#L986))
