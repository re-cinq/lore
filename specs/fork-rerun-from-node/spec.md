...
- The walk needs no change: the Floor's ordinary start handler marks the forked row running and `advanceLine` replays the inherited rows through `nextTransition`, which returns a launch for the successor of the cutoff node. ([validated by `advance.test.ts:953`](apps/floor/src/jobs/assembly-run/advance.test.ts#L953), [`advance.test.ts:972`](apps/floor/src/jobs/assembly-run/advance.test.ts#L972))
- The branch-overlap guard measures against the inherited prefix rather than an empty node list, so a fork that lands on a branch another open line already holds still defers as `lease_held` instead of racing it. ([validated by `advance.test.ts:985`](apps/floor/src/jobs/assembly-run/advance.test.ts#L985), [`advance.test.ts:1008`](apps/floor/src/jobs/assembly-run/advance.test.ts#L1008), [`advance.test.ts:1020`](apps/floor/src/jobs/assembly-run/advance.test.ts#L1020))
- The prefix size the guard reads is the count fixed at fork time, never one recomputed from the line's current rows. A fork's own walk may revisit the node it resumed from — `implementation.yaml` loops `validate` back to `implement` — which would make a recomputed count grow back to the row count and re-arm the guard mid-walk, closing a RUNNING fork as `lease_held`. The stored count cannot move, so the test is false forever after the fork's first launch. ([validated by `advance.test.ts:1118`](apps/floor/src/jobs/assembly-run/advance.test.ts#L1118), [`advance.test.ts:1020`](apps/floor/src/jobs/assembly-run/advance.test.ts#L1020))
