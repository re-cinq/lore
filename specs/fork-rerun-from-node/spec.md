...
web-ui → lore-api `POST /api/assembly-runs` → the FR1 `resumeFrom` start. ([validated by offers retry on a finished run's validate node](apps/web-ui/src/app/assembly-runs/[id]/RunVisualizationPanel.test.tsx#L962), [`start-run.test.ts:163`](apps/lore-api/src/transport/routes/assembly-lines/start-run.test.ts#L163)) The
