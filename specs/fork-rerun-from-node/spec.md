...
- `AssemblyLinesPort.start` accepts an optional `resumeFrom: { lineId, nodeId }`, mints a fresh per-attempt assembly-line id exactly as a plain start does, and returns it. ([validated by `assembly-lines.test.ts:1195`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1180))
- A `resumeFrom` start leaves the source line's own row and node rows untouched — the fork is a new attempt, never an edit of the recorded one. ([validated by `assembly-lines.test.ts:1262`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1265))
- `branch` is inherited from the source line, and passing `branch` alongside `resumeFrom` is a validation error rather than a silent override. ([validated by `resume.test.ts:180`](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L180), [`assembly-lines.test.ts:1482`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1484))
- `taskId` is inherited from the source line, and passing `taskId` alongside `resumeFrom` is a validation error on the same grounds. ([validated by `resume.test.ts:190`](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L190), [`assembly-lines.test.ts:1482`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1484))
- `args` are inherited from the source line when omitted and replaced wholesale when supplied, so an operator can inject corrected inputs for the replayed remainder; a legacy source row whose `args` column is NULL forks with `{}`, exactly as a plain start would store. ([validated by `assembly-lines.test.ts:1281`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1284), [`assembly-lines.test.ts:1511`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1510), [`assembly-lines.test.ts:1572`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1563))
- A `resumeFrom` start whose `repo` differs from the source line's is rejected: a fork inherits the source's branch, and a branch means nothing in another repository. ([validated by `resume.test.ts:216`](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L216))
- A `resumeFrom` start whose `definitionName` differs from the source line's is rejected — replaying one definition's node rows against another definition's graph is not a fork. ([validated by `resume.test.ts:226`](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L226))
- The repo-scoped `AssemblyLines` facade passes `resumeFrom` through unchanged, so callers write `project.assemblyLines.start(name, { resumeFrom, definitionHash })`. ([validated by `assembly-lines.test.ts:1599`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1590))

## FR2 — Copy semantics

- The copy runs through the chosen node's **latest completed row inclusive**, so earlier iterations of that node — everything a back-edge produced before it — ride along in visit order. ([validated by `resume.test.ts:75`](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L87), [`resume.test.ts:92`](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L92), [`resume.test.ts:96`](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L96), [`resume.test.ts:102`](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L102), [`resume.test.ts:129`](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L129), [`resume.test.ts:156`](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L156), [`assembly-lines.test.ts:1218`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1203), [`assembly-lines.test.ts:1465`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1468))
- Copied rows carry the source row's `outcome`, `commit_sha`, `started_at` and `finished_at` — the inherited prefix keeps the provenance of the run that actually produced it rather than masquerading as fresh work — but never its `agent_cr_name`, which is nulled on copy: the run-viz and cost correlation joins resolve a CR name to the newest matching node row, and an echoed name would steal the source's late-arriving agent-event and cost rows. ([validated by `resume.test.ts:111`](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L139), [`assembly-lines.test.ts:1218`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1203), [`assembly-lines.test.ts:1527`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1526))
- A `resumeFrom` naming a node the source line never completed — never visited, or visited and still open — is rejected, because there is no replayable prefix to copy. ([validated by `resume.test.ts:305`](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L305))
- *(amended 2026-09-02)* `resumeFrom` MAY name an `iteration`, and the copy then runs through exactly that completed visit instead of the node's latest row. This is what makes a LOOP retry expressible: on a line with back-edges, the node before the retry target can have run again later — or BE the target, on a self-edge, where cutting at the latest row would copy the failed visit itself and the fork's replay would re-derive the spent `iteration_max` as a fork dead on arrival. A named iteration the source never completed, or one still open, is rejected on the same no-replayable-prefix grounds. ([validated by names the exact visit when an iteration is given](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L111), [`resume.test.ts:117`](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L117), [`resume.test.ts:121`](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L121), [`resume.test.ts:271`](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L271), [`resume.test.ts:289`](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L289), [`assembly-runs.test.ts:1247`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1247))
- A prefix containing an unfinished row before the cutoff is rejected: an outcome-less row replays as `await`, so copying it would mint a line that can never advance. ([validated by `resume.test.ts:319`](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L319))
- The Postgres adapter writes the line row, the `assembly_run.start` event and every copied node row in ONE data-modifying CTE, exactly as a plain start writes its two, so a fork is never half-created. ([validated by `assembly-lines.test.ts:1449`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1452), [`assembly-lines.test.ts:1465`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1468), [`assembly-lines.test.ts:1527`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1526))
- Validation completes before anything is written; the properties it reads — a terminal line's status, definition name, repo and stamped hash — are immutable once observed, so the read-then-write split introduces no window in which a validated fork becomes invalid. ([validated by `assembly-lines.test.ts:1334`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1337), [`assembly-lines.test.ts:1434`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1437), [`assembly-lines.test.ts:1551`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1542), [`assembly-lines.test.ts:1560`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1551))
- The plain (non-fork) start path is unchanged: same statement, same parameters, no copied-rows CTE, and the double agrees with the adapter on it exactly — both state an explicit null parentage in the `assembly_run.start` event, and neither stores a caller-supplied `definitionHash`, which is a resume input the Floor re-stamps. ([validated by `assembly-lines.test.ts:1560`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1551), [`assembly-lines.test.ts:1630`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1621), [`assembly-lines.test.ts:1644`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1635))

## FR3 — Only terminal lines fork

- A `resumeFrom` whose source line is still `queued` or `running` is refused, naming the status. ([validated by `resume.test.ts:240`](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L240), [`assembly-lines.test.ts:1354`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1357), [`assembly-lines.test.ts:1551`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1542))
- A `resumeFrom` whose source line does not exist is refused. ([validated by `resume.test.ts:210`](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L210))
...
- The stamp records the hash only when the row carries none, in the Postgres adapter and the in-memory double alike, so a redelivered start that loads a since-edited definition cannot re-point a run at a graph it never ran. *(Amended 2026-08-14: the same call now stores the cloned graph beside the hash — see FR6.38 in `specs/6-dark-factory/spec.md`. The write-once rule is unchanged and covers both, for exactly this reason; the guard's left-hand side becomes the run's OWN stored graph rather than a hash of a file that may no longer exist.)* ([validated by `assembly-lines.test.ts:1115`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1103), [`assembly-lines.test.ts:1126`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1113), [`assembly-lines.test.ts:1135`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1122), [`assembly-lines.test.ts:1145`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1132))
- The Floor's `assembly_run.start` handler stamps the hash once, at the moment the definition resolves, and never overwrites an already-stamped value. ([validated by `start-event-handler.test.ts:240`](apps/floor/src/work/assembly-run/start-event-handler.test.ts#L240), [`start-event-handler.test.ts:271`](apps/floor/src/work/assembly-run/start-event-handler.test.ts#L271), [`start-event-handler.test.ts:293`](apps/floor/src/work/assembly-run/start-event-handler.test.ts#L335))
- A `resumeFrom` start requires the caller to supply the current definition's hash, and rejects a mismatch against the source line's stored hash rather than replaying node rows against a graph that has since changed. ([validated by `resume.test.ts:200`](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L200), [`resume.test.ts:263`](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L263), [`assembly-lines.test.ts:1560`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1551))
- A source line whose stored hash is NULL — every row predating the column — is rejected with a message naming the backfill, an honest limitation preferable to forking across silent definition drift. ([validated by `resume.test.ts:253`](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L253))

## FR5 — Audit trail and walk integration

- The `assembly_run.start` event params carry the fork parentage, so the audit record of *why* a line exists rides with the trigger. ([validated by `assembly-lines.test.ts:1298`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1301), [`assembly-lines.test.ts:1322`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1325), [`assembly-lines.test.ts:1449`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1452))
- The line row itself records `resumed_from_run_id` and `resumed_from_node_id`, because `pipeline.events` rows are pruned once handled and an event alone is not a durable audit substrate. ([validated by `assembly-lines.test.ts:1195`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1180), [`assembly-lines.test.ts:1322`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1325), [`assembly-lines.test.ts:1482`](libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts#L1484))
- The walk needs no change: the Floor's ordinary start handler marks the forked row running and `advanceLine` replays the inherited rows through `nextTransition`, which returns a launch for the successor of the cutoff node. ([validated by `walk-state.test.ts:6`](apps/floor/src/work/assembly-run/walk-state.test.ts#L6), [`advance-line.test.ts:804`](apps/floor/src/work/assembly-run/advance-line.test.ts#L806))
- *(added 2026-09-02)* A fork's start REOPENS the settled task it inherits. The source's terminal walk settled the task (`settleTaskForLine`, usually `failed`), and nothing wrote the resumption back: task-keyed surfaces (the implementation-loop page's "current" ticket) kept reporting the source's verdict while the fork ran, and a failed loop task stops guarding its issue — the backlog driver could pick the same ticket into a second task while the fork works the first. The start handler calls settle-task's start-side twin when the event carries a non-null `resumedFrom`: a `failed`/`cancelled`/`completed`/`needs-human-help` task flips back to `running` under CAS — a human retrying from the run page IS the help a parked task waited for — with the transition recorded and the source attempt's `failure_reason` cleared, so a running task never wears a stale failure; open states no-op (a duplicate delivery), and `merged` stays merged — that work shipped, and a fork over it is a deliberate rerun, not the task coming back. ([validated by reopens a failed, cancelled, completed or needs-human-help task as running](apps/floor/src/work/assembly-run/reopen-task.test.ts#L44), [`reopen-task.test.ts:51`](apps/floor/src/work/assembly-run/reopen-task.test.ts#L51), [`reopen-task.test.ts:67`](apps/floor/src/work/assembly-run/reopen-task.test.ts#L67), [`reopen-task.test.ts:78`](apps/floor/src/work/assembly-run/reopen-task.test.ts#L78), [`reopen-task.test.ts:93`](apps/floor/src/work/assembly-run/reopen-task.test.ts#L93), [a fork's start reopens the settled task before the walk launches](apps/floor/src/work/assembly-run/start-event-handler.test.ts#L90), [`start-event-handler.test.ts:108`](apps/floor/src/work/assembly-run/start-event-handler.test.ts#L108))
...
web-ui → lore-api `POST /api/assembly-runs` → the FR1 `resumeFrom` start. ([offers retry in the node card's header on a finished run, posting the implement fork source](apps/web-ui/src/app/assembly-runs/[id]/RunVisualizationPanel.test.tsx#L827), [`start-run.test.ts:163`](apps/lore-api/src/transport/routes/assembly-lines/start-run.test.ts#L163)) The
...
successor, so the HTTP surface adds no second routing rule. ([validated by `retry-resume.test.ts:11`](apps/web-ui/src/app/assembly-runs/[id]/retry-resume.test.ts#L11), [offers retry in the node card's header on a finished run, posting the implement fork source](apps/web-ui/src/app/assembly-runs/[id]/RunVisualizationPanel.test.tsx#L827))

- `POST /api/assembly-runs` accepts an optional `resume_from: { run_id, node_id, iteration? }` and forwards it to `start` as the FR1 `resumeFrom` input, on the same wire-naming grounds as `definition`; a zero or negative iteration is a schema 400, because visits count from 1. ([validated by `start-run.test.ts:127`](apps/lore-api/src/transport/routes/assembly-lines/start-run.test.ts#L163), [passes resume_from.iteration through](apps/lore-api/src/transport/routes/assembly-lines/start-run.test.ts#L267), [`start-run.test.ts:288`](apps/lore-api/src/transport/routes/assembly-lines/start-run.test.ts#L288), [`start-run.test.ts:304`](apps/lore-api/src/transport/routes/assembly-lines/start-run.test.ts#L304))
- `branch` alongside `resume_from` is rejected at the route edge — the FR1 port refuses it too, but a schema 400 names the field instead of surfacing a thrown error as a 500. ([validated by `start-run.test.ts:320`](apps/lore-api/src/transport/routes/assembly-lines/start-run.test.ts#L320))
- *(amended 2026-09-02)* The route fills `blueprintHash` from the CURRENT definition on a `resume_from` start — the drift guard's left-hand side loads where the definitions live, since `libs/shared` cannot derive it. Without this, every HTTP fork died as an opaque 500 ("resume-from start requires definitionHash") while the route's own tests, which stub `start`, stayed green. A `resume_from` naming a definition the loader does not know is a 400. ([validated by fills blueprintHash from the current definition](apps/lore-api/src/transport/routes/assembly-lines/start-run.test.ts#L189), [`start-run.test.ts:205`](apps/lore-api/src/transport/routes/assembly-lines/start-run.test.ts#L205))
- *(amended 2026-09-02)* A fork the port REFUSES — definition drift, a non-terminal source, a visit the source never completed — comes back as a 409 carrying the refusal's own message, because the port validates before writing anything and the person clicking retry needs the reason, not "Internal Server Error". The refusals are their own type (`ResumeRefusedError`), so the mapping discriminates by TYPE, never by message prose: anything else the start throws — a dropped DB connection — surfaces as the 500 it is, because an outage dressed as "the fork was refused" sends the operator chasing definition drift. The web-ui proxy hands the refusal to the button verbatim (4xx pass through; only a reasonless upstream answer degrades to a 502), and the button renders it inline. ([validated by returns 409 carrying the port's refusal](apps/lore-api/src/transport/routes/assembly-lines/start-run.test.ts#L229), [lets an unexpected start failure surface as the 500 it is](apps/lore-api/src/transport/routes/assembly-lines/start-run.test.ts#L252), [throws ResumeRefusedError for every refusal](libs/shared/src/outbound/project/assembly-runs/resume.test.ts#L164))
- *(amended 2026-09-10)* A fork is also refused while another open run already holds the subject key it would take over, and the refusal names that run. The source being over frees its key only from the source: the backlog loop's repo-wide `backlog` key is shared by every ticket, so retrying one ticket's run while another ticket's is in flight used to trip the `(repo, subject_key)` in-flight index and surface as a bare 500 (a 502 at the button). Both adapters check before writing, and the Pg one maps a unique violation from a start that won the race in between to the same refusal. A fork that names its own subject key is a fork all the same: it is refused, never joined onto the run already working that subject. ([validated by a fork is refused, naming the run in flight, while another run holds the subject it would take over](libs/shared/src/outbound/project/assembly-runs/assembly-runs.contract.test.ts#L903), [a fork naming its own subject is refused, not joined, while another run holds that subject](libs/shared/src/outbound/project/assembly-runs/assembly-runs.contract.test.ts#L1324))
- The retry target maps to its fork source purely from the visit rows: the visit immediately before the target's latest row, named by `(node_id, iteration)`. *(amended 2026-09-02)* Naming the ITERATION is what keeps the mapping exact on looping lines — the predecessor node may have run again after the retry target (a `validate → implement` back-edge), and on a self-edge the predecessor row belongs to the retried node itself, whose earlier iteration is the cut. The original node-only form refused both shapes; loops are exactly where retries matter most, so the refusal was the defect. ([validated by `retry-resume.test.ts:11`](apps/web-ui/src/app/assembly-runs/[id]/retry-resume.test.ts#L11), [names the predecessor row by iteration when that node ran again later](apps/web-ui/src/app/assembly-runs/[id]/retry-resume.test.ts#L42), [retrying a self-looped node names its own earlier iteration](apps/web-ui/src/app/assembly-runs/[id]/retry-resume.test.ts#L56), [`retry-resume.test.ts:68`](apps/web-ui/src/app/assembly-runs/[id]/retry-resume.test.ts#L68), [offers retry on a looping run's validate node, posting implement@2 as the fork source](apps/web-ui/src/app/assembly-runs/[id]/RunVisualizationPanel.test.tsx#L865))
...
- The inspector offers the retry in the node CARD'S HEADER row — beside the outcome pill, where the failure it answers is announced — only on a terminal run with a resolvable fork source, and posts the run id plus the SOURCE visit's node id and iteration as the request body. ([offers retry in the node card's header on a finished run, posting the implement fork source](apps/web-ui/src/app/assembly-runs/[id]/RunVisualizationPanel.test.tsx#L827), [offers retry on a looping run's validate node, posting implement@2 as the fork source](apps/web-ui/src/app/assembly-runs/[id]/RunVisualizationPanel.test.tsx#L865), [offers no retry while the run is still running](apps/web-ui/src/app/assembly-runs/[id]/RunVisualizationPanel.test.tsx#L898), [offers no retry on the entry node — there is no prefix to fork from](apps/web-ui/src/app/assembly-runs/[id]/RunVisualizationPanel.test.tsx#L915))
...
- An agent node's dispatch prompt carries the node's OWN earlier failed attempts — every recorded failed visit of that node with a failure detail, oldest first; detail-less visits and non-failure outcomes (`success`, `changes_requested`) contribute nothing. ([validated by collects every failed attempt of the launched node](apps/floor/src/work/assembly-run/launch-spec.test.ts#L95), [`launch-spec.test.ts:107`](apps/floor/src/work/assembly-run/launch-spec.test.ts#L110))
- The attempts render as their own prompt section, capped at the last 3 with each detail truncated at the same bound as the incoming-failure block; no failures, no section. It is appended IN ADDITION to the incoming-failure block — they answer different questions ("what just routed here" vs "what did I break on before") — and an attempt identical to the incoming failure is dropped rather than shown twice. ([validated by returns the prompt untouched with no prior failures](apps/floor/src/work/assembly-run/launch-spec.test.ts#L121), [`launch-spec.test.ts:122`](apps/floor/src/work/assembly-run/launch-spec.test.ts#L125), [`launch-spec.test.ts:136`](apps/floor/src/work/assembly-run/launch-spec.test.ts#L139))
- On a forked run, the history is read through the `resumed_from_run_id` chain — each source run's rows keep their details, and a fork of a fork contributes each ancestor's attempts, oldest ancestor first, bounded at 5 hops. The fork's FIRST launch is precisely the retried node, so the source's failure reaches exactly the dispatch the retry exists for. ([validated by a forked run's first launch carries the source run's failure detail](apps/floor/src/work/assembly-run/finish-node.test.ts#L284), [follows a fork-of-fork chain oldest first and stops at the hop bound](apps/floor/src/work/assembly-run/finish-node.test.ts#L349))
- A run that is no fork reads nothing beyond the visit rows the walk already holds — the source-run reads are gated on `resumed_from_run_id`, so the plain path pays zero extra queries. ([validated by a plain run reads no source runs while dispatching](apps/floor/src/work/assembly-run/finish-node.test.ts#L406))

## FR8 — Run any station, at any time *(added 2026-09-23)*

- `POST /api/assembly-runs/{id}/run-station` with `{ node_id, actor }` starts a FRESH line on the same work — the source's repo, branch, task, subject key and args — whose blueprint clone enters at the named node (`entry_node`, 6-dark-factory FR6.38), and answers 201 with its id. A node the source's graph does not have is a 400; a run that does not exist a 404 ([validated by](apps/lore-api/src/work/assembly-runs/run-station.test.ts#L47), [validated by](apps/lore-api/src/work/assembly-runs/run-station.test.ts#L102), [validated by](apps/lore-api/src/work/assembly-runs/run-station.test.ts#L142), [validated by](apps/lore-api/src/transport/routes/assembly-lines/run-station.test.ts#L44), [validated by](apps/lore-api/src/transport/routes/assembly-lines/run-station.test.ts#L70)).
- An OPEN source line is retired first, finished `cancelled` with a reason naming the person and the station, because the engine keeps one open line per subject and the person asked for THIS station to run, not for the parked one to keep waiting; a line that already ended is left as it is ([validated by](apps/lore-api/src/work/assembly-runs/run-station.test.ts#L47), [validated by](apps/lore-api/src/work/assembly-runs/run-station.test.ts#L85)).
- Running a HUMAN station parks the fresh line on it, and the park is announced like any other (6-dark-factory FR6.40b), so its worker can act: running the planning line's `author` opens an approved plan for writing, where it used to leave the line parked on a read-only plan nobody could answer ([validated by tells the parked-node seam that the line waits on its feature_review author, once its row is recorded](apps/floor/src/work/assembly-run/advance-line.test.ts#L1174), [validated by asks lore-api to open plan p1 of re-cinq/lore when its line parks on the feature_review author](apps/floor/src/work/agent/plan-author-waiting.test.ts#L23)).
- When some OTHER open line already works the subject, the start is refused with a 409 naming it rather than silently joining it, which is what a plain start would do ([validated by](apps/lore-api/src/work/assembly-runs/run-station.test.ts#L119)).
- The web-ui proxies the click the way the retry proxy does: it resolves the source run's repo server-side, authorizes the person against it, sends their login as the actor, and passes a refusal through with its reason ([validated by](apps/web-ui/src/app/api/assembly-runs/run-station/route.test.ts#L50), [validated by](apps/web-ui/src/app/api/assembly-runs/run-station/route.test.ts#L75), [validated by](apps/web-ui/src/app/api/assembly-runs/run-station/route.test.ts#L94)).
- The node inspector offers **Run this station** on every node, visited or not, live or ended, beside the retry where one exists. It asks first — a live run's popup says the run is retired — posts only on confirm, navigates to the fresh run, and shows a refusal inline with the control re-enabled ([validated by](apps/web-ui/src/app/assembly-runs/[id]/RunVisualizationPanel.test.tsx#L812), [validated by](apps/web-ui/src/app/assembly-runs/[id]/RunStationButton.test.tsx#L31), [validated by](apps/web-ui/src/app/assembly-runs/[id]/RunStationButton.test.tsx#L55), [validated by](apps/web-ui/src/app/assembly-runs/[id]/RunStationButton.test.tsx#L74)).

### Rationale — why a fresh line, not a resume

The retry (FR6) only offers a node with an exact fork source on a terminal
run. A person watching a line parked on a human station, or one whose agent
node is mid-flight, could not make any station run: the plan page's author
saw "changes requested" on the run page and had no way to force the spec
work on. Every node of the inspector now offers **Run this station**, on any
run, in any state.
