...
- [ ] T002 [P] FR4 hash — `libs/assembly-lines/src/definition-hash.ts` + `definition-hash.test.ts`, exported from `libs/assembly-lines/src/index.ts`. Tests first: stable across key ordering, changes with any node/edge edit, 64-hex output.
...
- [ ] T005 FR4 stamping surface — extend `assembly-lines-port.ts` (`resumeFrom`, `definitionHash`, `resumedFrom*` on the record) and add `stampDefinitionHash` to both adapters. Tests appended to `libs/shared/src/project/assembly-lines/assembly-lines.test.ts`.
