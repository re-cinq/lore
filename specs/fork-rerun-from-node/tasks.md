...
- [x] T004 [P] FR2 cutoff + validation — `libs/shared/src/outbound/project/assembly-runs/resume.ts` + `resume.test.ts`: `resumeCutoffIndex()` and `resolveResumePrefix()`, the pure rules both adapters share. Tests first, one per rejection reason. Carries the `assembly-lines-port.ts` surface those rules read (`resumeFrom`, `definitionHash`, `resumedFrom*`) plus the adapter changes that surface type-forces, since a port widening and its implementations cannot land apart.

## Phase 3 — The start API

- [x] T005 FR4 stamping — `stampDefinitionHash` in both adapters, write-once. Tests appended to `libs/shared/src/outbound/project/assembly-runs/assembly-runs.test.ts`.
...
- [x] T008 FR4 stamping — `apps/floor/src/work/assembly-run/start-event-handler.ts` stamps `definitionHash(definition)` when the definition resolves. Tests appended to `start-event-handler.test.ts`.
- [x] T009 FR5 walk — `apps/floor/src/work/assembly-run/advance-line.ts` counts the inherited prefix in the branch-overlap guard, and a forked line advances to the cutoff node's successor. Tests appended to `advance-line.test.ts`.

## Phase 5 — Close out

- [x] T010 Delete `adrs/ADR-041-fork-rerun-from-node.md`, superseded by the spec. (The `| Status |` flip rode T009 — the ladder linter is an ERROR, so the commit that links the last testable statement is the commit that must carry it. `Implemented` and `Shipped` are the same ladder tier; the honest label for unmerged, undeployed code with no caller is `Implemented`.)
