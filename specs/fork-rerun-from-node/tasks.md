...
- [x] T004 [P] FR2 cutoff + validation — `libs/shared/src/project/assembly-runs/resume.ts` + `resume.test.ts`: `resumeCutoffIndex()` and `resolveResumePrefix()`, the pure rules both adapters share. Tests first, one per rejection reason. Carries the `assembly-lines-port.ts` surface those rules read (`resumeFrom`, `definitionHash`, `resumedFrom*`) plus the adapter changes that surface type-forces, since a port widening and its implementations cannot land apart.

## Phase 3 — The start API

- [x] T005 FR4 stamping — `stampDefinitionHash` in both adapters, write-once. Tests appended to `libs/shared/src/project/assembly-runs/assembly-runs.test.ts`.
...
- [x] T008 FR4 stamping — `apps/floor/src/jobs/assembly-run/start-event-handler.ts` stamps `definitionHash(definition)` when the definition resolves. Tests appended to `start-event-handler.test.ts`.
- [x] T009 FR5 walk — `apps/floor/src/jobs/assembly-run/advance-line.ts` counts the inherited prefix in the branch-overlap guard, and a forked line advances to the cutoff node's successor. Tests appended to `advance-line.test.ts`.
