...
[`ingestTestReport`](../../libs/shared/src/work/spec-trace/ingest-test-report.ts).
...
[`projectSpecFile`](../../libs/shared/src/work/spec-trace/project-spec-file.ts) turns into
...
[`linksForStatements`](../../libs/shared/src/domain/spec-link-parser.ts) to read every
...
([validated by `resolve-test-lines.test.ts:29`](libs/shared/src/work/spec-trace/resolve-test-lines.test.ts#L29))

`it.skip` / `it.only` and other modifiers resolve, and the final test's span
runs to end of file.
([validated by `resolve-test-lines.test.ts:39`](libs/shared/src/work/spec-trace/resolve-test-lines.test.ts#L39))

A descriptor whose leaf name matches no declaration is returned unchanged
(line-blind), so the binder skips it.
([validated by `leaves a descriptor whose leaf name matches no declaration unchanged`](libs/shared/src/work/spec-trace/resolve-test-lines.test.ts#L39), [validated by `resolve-test-lines:47`](libs/shared/src/work/spec-trace/resolve-test-lines.test.ts#L46))
...
([validated by `bind-descriptors-to-spec-links.test.ts:36`](libs/shared/src/work/spec-trace/bind-descriptors-to-spec-links.test.ts#L36))

A descriptor that matches **no** inline test link is returned unchanged, with
no `spec` anchor.
([validated by `bind-descriptors-to-spec-links.test.ts:42`](libs/shared/src/work/spec-trace/bind-descriptors-to-spec-links.test.ts#L42))

A descriptor that already carries a `spec` anchor is left untouched — a
hand-authored anchor wins over a derived one.
([validated by `bind-descriptors-to-spec-links.test.ts:51`](libs/shared/src/work/spec-trace/bind-descriptors-to-spec-links.test.ts#L51))

A descriptor with no `startLine`/`endLine` is returned unchanged, since line
containment cannot be evaluated.
([validated by `bind-descriptors-to-spec-links.test.ts:60`](libs/shared/src/work/spec-trace/bind-descriptors-to-spec-links.test.ts#L60))
...
([validated by `bind-descriptors-to-spec-links.test.ts:69`](libs/shared/src/work/spec-trace/bind-descriptors-to-spec-links.test.ts#L69))
...
parse.
([validated by `spec-anchor.test.ts:28`](libs/shared/src/work/spec-trace/spec-anchor.test.ts#L28), [validated by `spec-anchor:5`](libs/shared/src/work/spec-trace/spec-anchor.test.ts#L5), [validated by `spec-anchor:12`](libs/shared/src/work/spec-trace/spec-anchor.test.ts#L12), [validated by `spec-anchor:16`](libs/shared/src/work/spec-trace/spec-anchor.test.ts#L16), [validated by `spec-anchor:22`](libs/shared/src/work/spec-trace/spec-anchor.test.ts#L22), [validated by `spec-anchor:37`](libs/shared/src/work/spec-trace/spec-anchor.test.ts#L37))
...
([validated by `bind-descriptors-to-spec-links.test.ts:81`](libs/shared/src/work/spec-trace/bind-descriptors-to-spec-links.test.ts#L81), [validated by `bind-descriptors-to-spec-links:107`](libs/shared/src/work/spec-trace/bind-descriptors-to-spec-links.test.ts#L105))

A link with no `#Lline` anchor seeds no `(path, line)` index entry and binds
nothing.
([validated by `bind-descriptors-to-spec-links.test.ts:90`](libs/shared/src/work/spec-trace/bind-descriptors-to-spec-links.test.ts#L90), [validated by `bind-descriptors-to-spec-links:122`](libs/shared/src/work/spec-trace/bind-descriptors-to-spec-links.test.ts#L120))
...
## Amendment (2026-09-10): the report's branch is consumed

The `TestReport` carried `branch` from the start and nothing downstream read it.
It is read now. The ingress that receives a report — the Floor's ci-tests hook,
or lore-api's incremental ingest route — compares that branch with the repo's
default branch, and a report for any other branch is ingested into that
branch's overlay instead of the repo's `main` graph, with its head commit
stamped on the overlay anchor as the coordinate its line numbers are expressed
in. The `lore-code-trace` binary needs no flag for this: the branch it already
reads from git is the whole routing key.

A report for the default branch, for a detached `HEAD`, or naming no branch at
all is unchanged in every respect: it writes `main`'s chunks and stamps `main`'s
baseline exactly as before. See
