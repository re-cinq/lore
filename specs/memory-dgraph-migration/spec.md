...
3. `DgraphMemoryStore` passes the same behavioral test fixtures as `PostgresMemoryStore` against a real local Dgraph container (no mocks): write→read version increment, soft-delete + TTL exclusion, paginated list, RRF hybrid search, contradiction detection at cosine ≥ 0.92, episode `content_hash` dedup, entity/edge temporal invalidation. ([validated by `returns the stored value at version 1 after writeMemory then readMemory of a new key`](libs/shared/src/dgraph-memory-store.test.ts#L206), [validated by `returns version 2 and the latest value after writing the same key twice`](libs/shared/src/dgraph-memory-store.test.ts#L228), [validated by `soft-deletes so readMemory returns nothing`](libs/shared/src/dgraph-memory-store.test.ts#L283), [validated by `excludes a memory whose expires_at is in the past`](libs/shared/src/dgraph-memory-store.test.ts#L248), [validated by `returns total 2 and the two live keys, excluding the soft-deleted one`](libs/shared/src/dgraph-memory-store.test.ts#L298), [validated by `searchMemories returns the memory whose value matches the keyword query`](libs/shared/src/dgraph-memory-store.test.ts#L320), [validated by `searchMemories returns the vector-nearest memory when the keyword query matches nothing`](libs/shared/src/dgraph-memory-store.test.ts#L347), [validated by `invalidates the prior fact and records a FactConflict for a near-duplicate embedding`](libs/shared/src/dgraph-memory-store.test.ts#L142), [validated by `writeEpisode of identical content twice creates exactly one Episode node`](libs/shared/src/dgraph-memory-store.test.ts#L414), [validated by `upsertEdge creates an active GraphRel of the given relation_type from source to target`](libs/shared/src/dgraph-memory-store.test.ts#L501), [validated by `invalidates the prior edge when a same-source same-relation edge points at a different target`](libs/shared/src/dgraph-memory-store.test.ts#L546), [validated by `persistFact writes an active Fact node retrievable for the agent`](libs/shared/src/dgraph-memory-store.test.ts#L74))

4. `lore_query_graph` performs multi-hop traversal via `@recurse` to a caller-supplied `depth`, returns only `active` edges by default, and is cycle-safe (`loop:false`). ([validated by `queryGraph returns the 1-hop outgoing neighbour as a hop at depth 1`](libs/shared/src/dgraph-memory-store.test.ts#L593), [validated by `traverses two hops so A--uses-->B--hosts-->C yields the depth-2 hop B--hosts-->C`](libs/shared/src/dgraph-memory-store.test.ts#L617), [validated by `excludes an invalidated (active=false) edge from traversal by default`](libs/shared/src/dgraph-memory-store.test.ts#L643), [validated by `terminates on a cycle A--links-->B--links-->A without infinite recursion`](libs/shared/src/dgraph-memory-store.test.ts#L664))
