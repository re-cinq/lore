...
11. Rollback is a single Helm value (`LORE_MEMORY_BACKEND=postgres`) + rollout; Postgres `memory.*` remains read-only and queryable for the rollback window. *(The single-value flip is `selectMemoryStore`; the read-only Postgres window is an infra/DB-grant concern.)* ([validated by `rolls back to postgres on the single value LORE_MEMORY_BACKEND=postgres`](libs/shared/src/memory-store.test.ts#L71), [validated by `flips the served backend with only the LORE_MEMORY_BACKEND value (cutover and rollback)`](libs/shared/src/memory-store.test.ts#L78), [validated by `throws on an unrecognized LORE_MEMORY_BACKEND value instead of silently serving postgres`](libs/shared/src/memory-store.test.ts#L92))
