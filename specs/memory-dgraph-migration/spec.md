...
[`shared/src/project/knowledge/live-graph.ts:19`](libs/shared/src/outbound/project/knowledge/live-graph.ts#L19)).
...
1. A `MemoryStore` interface exists in `shared/`; `PostgresMemoryStore` implements every method by delegating to the current SQL, and all existing memory/facts/graph/context-assembly tests stay green with `LORE_MEMORY_BACKEND` unset (default `postgres`). ([validated by `returns version 1 for a brand-new key`](libs/shared/src/outbound/postgres-memory-store.test.ts#L38), [validated by `returns the latest stored value with version 1 for a single write`](libs/shared/src/outbound/postgres-memory-store.test.ts#L160), [validated by `soft-deletes so readMemory returns nothing`](libs/shared/src/outbound/postgres-memory-store.test.ts#L81), [validated by `returns total 2 and the two live keys, excluding the soft-deleted one`](libs/shared/src/outbound/postgres-memory-store.test.ts#L117))

2. `selectMemoryStore()` returns `DgraphMemoryStore` when `LORE_MEMORY_BACKEND=dgraph`, `PostgresMemoryStore` otherwise, and throws (not returns null) when the selected backend's client is absent. ([validated by `returns a postgres store when LORE_MEMORY_BACKEND is unset`](libs/shared/src/outbound/memory-store.test.ts#L39), [validated by `throws when postgres backend is selected without a pgPool`](libs/shared/src/outbound/memory-store.test.ts#L45), [validated by `throws when dgraph backend is selected without a dgraph client`](libs/shared/src/outbound/memory-store.test.ts#L49), [validated by `returns a dgraph store when dgraph backend is selected with a client`](libs/shared/src/outbound/memory-store.test.ts#L54))

3. `DgraphMemoryStore` passes the same behavioral test fixtures as `PostgresMemoryStore` against a real local Dgraph container (no mocks): write→read version increment, soft-delete + TTL exclusion, paginated list, RRF hybrid search, contradiction detection at cosine ≥ 0.92, episode `content_hash` dedup, entity/edge temporal invalidation. ([validated by `returns the stored value at version 1 after writeMemory then readMemory of a new key`](libs/shared/src/outbound/dgraph-memory-store.test.ts#L201), [validated by `returns version 2 and the latest value after writing the same key twice`](libs/shared/src/outbound/dgraph-memory-store.test.ts#L223), [validated by `soft-deletes so readMemory returns nothing`](libs/shared/src/outbound/dgraph-memory-store.test.ts#L278), [validated by `excludes a memory whose expires_at is in the past`](libs/shared/src/outbound/dgraph-memory-store.test.ts#L243), [validated by `returns total 2 and the two live keys, excluding the soft-deleted one`](libs/shared/src/outbound/dgraph-memory-store.test.ts#L293), [validated by `searchMemories returns the memory whose value matches the keyword query`](libs/shared/src/outbound/dgraph-memory-store.test.ts#L315), [validated by `searchMemories returns the vector-nearest memory when the keyword query matches nothing`](libs/shared/src/outbound/dgraph-memory-store.test.ts#L342), [validated by `invalidates the prior fact and records a FactConflict for a near-duplicate embedding`](libs/shared/src/outbound/dgraph-memory-store.test.ts#L137), [validated by `writeEpisode of identical content twice creates exactly one Episode node`](libs/shared/src/outbound/dgraph-memory-store.test.ts#L409), [validated by `upsertEdge creates an active GraphRel of the given relation_type from source to target`](libs/shared/src/outbound/dgraph-memory-store.test.ts#L496), [validated by `invalidates the prior edge when a same-source same-relation edge points at a different target`](libs/shared/src/outbound/dgraph-memory-store.test.ts#L541), [validated by `persistFact writes an active Fact node retrievable for the agent`](libs/shared/src/outbound/dgraph-memory-store.test.ts#L65))

4. `lore_query_graph` performs multi-hop traversal via `@recurse` to a caller-supplied `depth`, returns only `active` edges by default, and is cycle-safe (`loop:false`). ([validated by `queryGraph returns the 1-hop outgoing neighbour as a hop at depth 1`](libs/shared/src/outbound/dgraph-memory-store.test.ts#L588), [validated by `traverses two hops so A--uses-->B--hosts-->C yields the depth-2 hop B--hosts-->C`](libs/shared/src/outbound/dgraph-memory-store.test.ts#L612), [validated by `excludes an invalidated (active=false) edge from traversal by default`](libs/shared/src/outbound/dgraph-memory-store.test.ts#L638), [validated by `terminates on a cycle A--links-->B--links-->A without infinite recursion`](libs/shared/src/outbound/dgraph-memory-store.test.ts#L659))

5. RRF, session diversification, transfer scoring, and importance scoring are imported by both stores from `shared/src/memory-ranking.ts` — one definition, identical results. ([validated by `carries confidence from the candidate onto the fused result`](libs/shared/src/domain/memory-ranking.test.ts#L13), [validated by `returns 0.5 for neutral text with no portable or local keywords`](libs/shared/src/domain/memory-ranking.test.ts#L31), [validated by `keeps only the 3 highest-scoring from one agent_id::source over the cap`](libs/shared/src/domain/memory-ranking.test.ts#L116), [validated by `returns 10 for a fresh memory with no score adjustments`](libs/shared/src/domain/memory-ranking.test.ts#L176))

6. The backfill exporter preserves every Postgres UUID as the node `xid`, preserves 768-dim embeddings (dim == 768, cosine(self) == 1.0 on a sample), resolves all relationships, and is idempotent (a second run leaves node counts unchanged). ([validated by `creates a Memory node whose Memory.xid equals the Postgres memories row id`](libs/shared/src/work/backfill-memory.test.ts#L98), [validated by `preserves the 768-dim embedding so cosine(original, stored) is 1.0`](libs/shared/src/work/backfill-memory.test.ts#L226), [validated by `points Fact.memory at the Memory node carrying the facts.memory_id xid`](libs/shared/src/work/backfill-memory.test.ts#L140), [validated by `creates exactly one Memory node per xid after running twice`](libs/shared/src/work/backfill-memory.test.ts#L187))

7. The backfill Job exits non-zero unless row-count parity holds per table and retrieval top-K Jaccard ≥ 0.8 on a sampled query set; the Jaccard gate is inclusive (passes at exactly the threshold) and the threshold is configurable (a stricter custom threshold fails an otherwise-passing sample). ([validated by `passes with exit 0 when every table count matches and mean jaccard is 0.87`](libs/shared/src/work/backfill-parity.test.ts#L41), [validated by `fails with non-zero exit naming the table when facts count mismatches`](libs/shared/src/work/backfill-parity.test.ts#L57), [validated by `fails with non-zero exit naming the jaccard gate when tables match but mean jaccard is 0.5`](libs/shared/src/work/backfill-parity.test.ts#L73), [validated by `accumulates both a table-mismatch and a jaccard failure when both gates fail`](libs/shared/src/work/backfill-parity.test.ts#L101), [validated by `passes when mean jaccard is exactly the threshold 0.8 (gate is >=)`](libs/shared/src/work/backfill-parity.test.ts#L88), [validated by `fails a jaccard of 0.9 against a stricter custom threshold of 0.95`](libs/shared/src/work/backfill-parity.test.ts#L119))

8. `ShadowMemoryStore` serves reads from the primary (never the shadow) even when the shadow throws, emits a `lore.memory.shadow_divergence` metric only on genuine divergence — never when the two agree, and a shadow throw is not counted as a divergence — and logs (never swallows) shadow errors. ([validated by `serves the primary result when the shadow read throws`](libs/shared/src/outbound/shadow-memory-store.test.ts#L78), [validated by `emits lore.memory.shadow_divergence when primary and shadow differ`](libs/shared/src/outbound/shadow-memory-store.test.ts#L60), [validated by `logs the shadow error through the injected sink when the shadow read throws`](libs/shared/src/outbound/shadow-memory-store.test.ts#L90), [validated by `serves readMemory from the primary, not the shadow`](libs/shared/src/outbound/shadow-memory-store.test.ts#L48), [validated by `emits no divergence metric when primary and shadow agree`](libs/shared/src/outbound/shadow-memory-store.test.ts#L111), [validated by `emits no divergence metric when the shadow read throws (a throw is not a divergence)`](libs/shared/src/outbound/shadow-memory-store.test.ts#L129))

9. Dgraph runs under ACL with a scoped `lore-memory-app` runtime user; the guardian credential is used only by a `pre-install` bootstrap Job; runtime creds come from ESO + Secret Manager + Workload Identity with no value hardcoded in any chart. The guard returns no violations for an empty document set, never mistakes a `secretKeyRef` env (no literal value) for a hardcoded credential, and never flags the `pre-install` bootstrap Job for using the guardian credential. *(Enforced by the `auditDgraphAcl` policy guard — the deployment manifests/charts that satisfy it are the remaining infra build.)* ([validated by `returns no violations for a fully compliant Dgraph deployment set`](libs/shared/src/outbound/dgraph-acl-policy.test.ts#L119), [validated by `flags a dgraph alpha workload whose args do not enable --acl`](libs/shared/src/outbound/dgraph-acl-policy.test.ts#L41), [validated by `flags a runtime StatefulSet that references the guardian credential`](libs/shared/src/outbound/dgraph-acl-policy.test.ts#L67), [validated by `flags a ServiceAccount missing the Workload Identity annotation`](libs/shared/src/outbound/dgraph-acl-policy.test.ts#L104), [validated by `flags a container env that hardcodes a credential value`](libs/shared/src/outbound/dgraph-acl-policy.test.ts#L9), [validated by `returns no violations for no documents`](libs/shared/src/outbound/dgraph-acl-policy.test.ts#L5), [validated by `does not flag a secretKeyRef env (no literal value) as a hardcoded credential`](libs/shared/src/outbound/dgraph-acl-policy.test.ts#L198), [validated by `does not flag the pre-install bootstrap Job for using the guardian credential`](libs/shared/src/outbound/dgraph-acl-policy.test.ts#L229))

10. `scripts/dev-local.sh` brings up a local Dgraph container and applies the schema; `LORE_MEMORY_BACKEND=dgraph npm start` exercises the memory MCP tools end-to-end with no Postgres `memory.*` access. ([validated by `declares the HNSW vector index and the xid upsert index`](libs/shared/src/outbound/setup-memory-dgraph-schema.test.ts#L42), [validated by `is idempotent — a second apply leaves the predicate schema unchanged`](libs/shared/src/outbound/setup-memory-dgraph-schema.test.ts#L57))

11. Rollback is a single Helm value (`LORE_MEMORY_BACKEND=postgres`) + rollout; Postgres `memory.*` remains read-only and queryable for the rollback window. *(The single-value flip is `selectMemoryStore`; the read-only Postgres window is an infra/DB-grant concern.)* ([validated by `rolls back to postgres on the single value LORE_MEMORY_BACKEND=postgres`](libs/shared/src/outbound/memory-store.test.ts#L67), [validated by `flips the served backend with only the LORE_MEMORY_BACKEND value (cutover and rollback)`](libs/shared/src/outbound/memory-store.test.ts#L74), [validated by `throws on an unrecognized LORE_MEMORY_BACKEND value instead of silently serving postgres`](libs/shared/src/outbound/memory-store.test.ts#L88))

The active store is held in a module-global registry: `setMemoryStore(store)` registers it and `memoryStore()` returns exactly that instance, throwing when no store has been set. ([validated by `throws when no store has been set`](libs/shared/src/outbound/memory-store.test.ts#L10), [validated by `returns the store registered via setMemoryStore`](libs/shared/src/outbound/memory-store.test.ts#L14))
