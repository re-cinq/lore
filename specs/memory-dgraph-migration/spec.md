...
3. `DgraphMemoryStore` passes the same behavioral test fixtures as `PostgresMemoryStore` against a real local Dgraph container (no mocks): write→read version increment, soft-delete + TTL exclusion, paginated list, RRF hybrid search, contradiction detection at cosine ≥ 0.92, episode `content_hash` dedup, entity/edge temporal invalidation. ([validated by `returns the stored value at version 1 after writeMemory then readMemory of a new key`](libs/shared/src/dgraph-memory-store.test.ts#L201), [validated by `returns version 2 and the latest value after writing the same key twice`](libs/shared/src/dgraph-memory-store.test.ts#L223), [validated by `soft-deletes so readMemory returns nothing`](libs/shared/src/dgraph-memory-store.test.ts#L278), [validated by `excludes a memory whose expires_at is in the past`](libs/shared/src/dgraph-memory-store.test.ts#L243), [validated by `returns total 2 and the two live keys, excluding the soft-deleted one`](libs/shared/src/dgraph-memory-store.test.ts#L293), [validated by `searchMemories returns the memory whose value matches the keyword query`](libs/shared/src/dgraph-memory-store.test.ts#L315), [validated by `searchMemories returns the vector-nearest memory when the keyword query matches nothing`](libs/shared/src/dgraph-memory-store.test.ts#L342), [validated by `invalidates the prior fact and records a FactConflict for a near-duplicate embedding`](libs/shared/src/dgraph-memory-store.test.ts#L137), [validated by `writeEpisode of identical content twice creates exactly one Episode node`](libs/shared/src/dgraph-memory-store.test.ts#L409), [validated by `upsertEdge creates an active GraphRel of the given relation_type from source to target`](libs/shared/src/dgraph-memory-store.test.ts#L496), [validated by `invalidates the prior edge when a same-source same-relation edge points at a different target`](libs/shared/src/dgraph-memory-store.test.ts#L541), [validated by `persistFact writes an active Fact node retrievable for the agent`](libs/shared/src/dgraph-memory-store.test.ts#L65))

4. `lore_query_graph` performs multi-hop traversal via `@recurse` to a caller-supplied `depth`, returns only `active` edges by default, and is cycle-safe (`loop:false`). ([validated by `queryGraph returns the 1-hop outgoing neighbour as a hop at depth 1`](libs/shared/src/dgraph-memory-store.test.ts#L588), [validated by `traverses two hops so A--uses-->B--hosts-->C yields the depth-2 hop B--hosts-->C`](libs/shared/src/dgraph-memory-store.test.ts#L612), [validated by `excludes an invalidated (active=false) edge from traversal by default`](libs/shared/src/dgraph-memory-store.test.ts#L638), [validated by `terminates on a cycle A--links-->B--links-->A without infinite recursion`](libs/shared/src/dgraph-memory-store.test.ts#L659))
...
6. The backfill exporter preserves every Postgres UUID as the node `xid`, preserves 768-dim embeddings (dim == 768, cosine(self) == 1.0 on a sample), resolves all relationships, and is idempotent (a second run leaves node counts unchanged). ([validated by `creates a Memory node whose Memory.xid equals the Postgres memories row id`](libs/shared/src/backfill-memory.test.ts#L98), [validated by `preserves the 768-dim embedding so cosine(original, stored) is 1.0`](libs/shared/src/backfill-memory.test.ts#L226), [validated by `points Fact.memory at the Memory node carrying the facts.memory_id xid`](libs/shared/src/backfill-memory.test.ts#L140), [validated by `creates exactly one Memory node per xid after running twice`](libs/shared/src/backfill-memory.test.ts#L187))
