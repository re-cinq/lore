...
5. RRF, session diversification, transfer scoring, and importance scoring are imported by both stores from `shared/src/memory-ranking.ts` — one definition, identical results. ([validated by `carries confidence from the candidate onto the fused result`](libs/shared/src/work/memory-ranking.test.ts#L11), [validated by `returns 0.5 for neutral text with no portable or local keywords`](libs/shared/src/work/memory-ranking.test.ts#L29), [validated by `keeps only the 3 highest-scoring from one agent_id::source over the cap`](libs/shared/src/work/memory-ranking.test.ts#L53), [validated by `returns 10 for a fresh memory with no score adjustments`](libs/shared/src/work/memory-ranking.test.ts#L113))
