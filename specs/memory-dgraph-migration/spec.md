...
The retrieval-parity number is a set-based top-K Jaccard: `jaccard(a, b)` treats each input as a set (deduping repeats), returns |a∩b| / |a∪b|, scores disjoint sets 0 and two empty sets 1; `meanTopkJaccard` averages the per-query Jaccards and returns 0 (never NaN) on an empty sample so the retrieval gate fails loudly rather than passing vacuously. ([validated by `returns 1 for two identical result sets`](libs/shared/src/backfill-parity.test.ts#L9), [validated by `returns 0.5 for {a,b,c} vs {a,b,d} (2 shared of 4 total)`](libs/shared/src/backfill-parity.test.ts#L13), [validated by `returns 0 for disjoint result sets`](libs/shared/src/backfill-parity.test.ts#L17), [validated by `returns 1 for two empty sets (vacuously identical)`](libs/shared/src/backfill-parity.test.ts#L21), [validated by `treats inputs as sets, deduping repeats`](libs/shared/src/backfill-parity.test.ts#L25), [validated by `returns 0.7 for [1, 0.5, 0.6] (sum 2.1 over 3)`](libs/shared/src/backfill-parity.test.ts#L31), [validated by `returns 0 (not NaN) for an empty sample so the retrieval gate fails loudly`](libs/shared/src/backfill-parity.test.ts#L35))
...
1. A `MemoryStore` interface exists in `shared/`; `PostgresMemoryStore` implements every method by delegating to the current SQL, and all existing memory/facts/graph/context-assembly tests stay green with `LORE_MEMORY_BACKEND` unset (default `postgres`). ([validated by `returns version 1 for a brand-new key`](libs/shared/src/postgres-memory-store.test.ts#L46), [validated by `returns the latest stored value with version 1 for a single write`](libs/shared/src/postgres-memory-store.test.ts#L168), [validated by `soft-deletes so readMemory returns nothing`](libs/shared/src/postgres-memory-store.test.ts#L89), [validated by `returns total 2 and the two live keys, excluding the soft-deleted one`](libs/shared/src/postgres-memory-store.test.ts#L125))

2. `selectMemoryStore()` returns `DgraphMemoryStore` when `LORE_MEMORY_BACKEND=dgraph`, `PostgresMemoryStore` otherwise, and throws (not returns null) when the selected backend's client is absent. ([validated by `returns a postgres store when LORE_MEMORY_BACKEND is unset`](libs/shared/src/memory-store.test.ts#L42), [validated by `throws when postgres backend is selected without a pgPool`](libs/shared/src/memory-store.test.ts#L48), [validated by `throws when dgraph backend is selected without a dgraph client`](libs/shared/src/memory-store.test.ts#L52), [validated by `returns a dgraph store when dgraph backend is selected with a client`](libs/shared/src/memory-store.test.ts#L57))

3. `DgraphMemoryStore` passes the same behavioral test fixtures as `PostgresMemoryStore` against a real local Dgraph container (no mocks): write→read version increment, soft-delete + TTL exclusion, paginated list, RRF hybrid search, contradiction detection at cosine ≥ 0.92, episode `content_hash` dedup, entity/edge temporal invalidation. ([validated by `returns the stored value at version 1 after writeMemory then readMemory of a new key`](libs/shared/src/dgraph-memory-store.test.ts#L217), [validated by `returns version 2 and the latest value after writing the same key twice`](libs/shared/src/dgraph-memory-store.test.ts#L239), [validated by `soft-deletes so readMemory returns nothing`](libs/shared/src/dgraph-memory-store.test.ts#L294), [validated by `excludes a memory whose expires_at is in the past`](libs/shared/src/dgraph-memory-store.test.ts#L259), [validated by `returns total 2 and the two live keys, excluding the soft-deleted one`](libs/shared/src/dgraph-memory-store.test.ts#L309), [validated by `searchMemories returns the memory whose value matches the keyword query`](libs/shared/src/dgraph-memory-store.test.ts#L331), [validated by `searchMemories returns the vector-nearest memory when the keyword query matches nothing`](libs/shared/src/dgraph-memory-store.test.ts#L358), [validated by `invalidates the prior fact and records a FactConflict for a near-duplicate embedding`](libs/shared/src/dgraph-memory-store.test.ts#L153), [validated by `writeEpisode of identical content twice creates exactly one Episode node`](libs/shared/src/dgraph-memory-store.test.ts#L425), [validated by `upsertEdge creates an active GraphRel of the given relation_type from source to target`](libs/shared/src/dgraph-memory-store.test.ts#L508), [validated by `invalidates the prior edge when a same-source same-relation edge points at a different target`](libs/shared/src/dgraph-memory-store.test.ts#L553), [validated by `persistFact writes an active Fact node retrievable for the agent`](libs/shared/src/dgraph-memory-store.test.ts#L85))

4. `lore_query_graph` performs multi-hop traversal via `@recurse` to a caller-supplied `depth`, returns only `active` edges by default, and is cycle-safe (`loop:false`). ([validated by `queryGraph returns the 1-hop outgoing neighbour as a hop at depth 1`](libs/shared/src/dgraph-memory-store.test.ts#L600), [validated by `traverses two hops so A--uses-->B--hosts-->C yields the depth-2 hop B--hosts-->C`](libs/shared/src/dgraph-memory-store.test.ts#L624), [validated by `excludes an invalidated (active=false) edge from traversal by default`](libs/shared/src/dgraph-memory-store.test.ts#L650), [validated by `terminates on a cycle A--links-->B--links-->A without infinite recursion`](libs/shared/src/dgraph-memory-store.test.ts#L671))
...
6. The backfill exporter preserves every Postgres UUID as the node `xid`, preserves 768-dim embeddings (dim == 768, cosine(self) == 1.0 on a sample), resolves all relationships, and is idempotent (a second run leaves node counts unchanged). ([validated by `creates a Memory node whose Memory.xid equals the Postgres memories row id`](libs/shared/src/backfill-memory.test.ts#L119), [validated by `preserves the 768-dim embedding so cosine(original, stored) is 1.0`](libs/shared/src/backfill-memory.test.ts#L247), [validated by `points Fact.memory at the Memory node carrying the facts.memory_id xid`](libs/shared/src/backfill-memory.test.ts#L161), [validated by `creates exactly one Memory node per xid after running twice`](libs/shared/src/backfill-memory.test.ts#L208))
...
8. `ShadowMemoryStore` serves reads from the primary (never the shadow) even when the shadow throws, emits a `lore.memory.shadow_divergence` metric only on genuine divergence — never when the two agree, and a shadow throw is not counted as a divergence — and logs (never swallows) shadow errors. ([validated by `serves the primary result when the shadow read throws`](libs/shared/src/shadow-memory-store.test.ts#L82), [validated by `emits lore.memory.shadow_divergence when primary and shadow differ`](libs/shared/src/shadow-memory-store.test.ts#L64), [validated by `logs the shadow error through the injected sink when the shadow read throws`](libs/shared/src/shadow-memory-store.test.ts#L94), [validated by `serves readMemory from the primary, not the shadow`](libs/shared/src/shadow-memory-store.test.ts#L52), [validated by `emits no divergence metric when primary and shadow agree`](libs/shared/src/shadow-memory-store.test.ts#L115), [validated by `emits no divergence metric when the shadow read throws (a throw is not a divergence)`](libs/shared/src/shadow-memory-store.test.ts#L133))
...
10. `scripts/dev-local.sh` brings up a local Dgraph container and applies the schema; `LORE_MEMORY_BACKEND=dgraph npm start` exercises the memory MCP tools end-to-end with no Postgres `memory.*` access. ([validated by `declares the HNSW vector index and the xid upsert index`](libs/shared/src/setup-memory-dgraph-schema.test.ts#L60), [validated by `is idempotent — a second apply leaves the predicate schema unchanged`](libs/shared/src/setup-memory-dgraph-schema.test.ts#L75))

11. Rollback is a single Helm value (`LORE_MEMORY_BACKEND=postgres`) + rollout; Postgres `memory.*` remains read-only and queryable for the rollback window. *(The single-value flip is `selectMemoryStore`; the read-only Postgres window is an infra/DB-grant concern.)* ([validated by `rolls back to postgres on the single value LORE_MEMORY_BACKEND=postgres`](libs/shared/src/memory-store.test.ts#L70), [validated by `flips the served backend with only the LORE_MEMORY_BACKEND value (cutover and rollback)`](libs/shared/src/memory-store.test.ts#L77), [validated by `throws on an unrecognized LORE_MEMORY_BACKEND value instead of silently serving postgres`](libs/shared/src/memory-store.test.ts#L91))

The active store is held in a module-global registry: `setMemoryStore(store)` registers it and `memoryStore()` returns exactly that instance, throwing when no store has been set. ([validated by `throws when no store has been set`](libs/shared/src/memory-store.test.ts#L12), [validated by `returns the store registered via setMemoryStore`](libs/shared/src/memory-store.test.ts#L16))
