...
> All tasks shipped. Implementation verified against codebase on 2026-04-20.
> See [plan.md](plan.md) and [spec.md](spec.md) for design rationale.

## Phase 1: Schema Migrations

- [x] T001 Add `retrieval_count`, `last_retrieved_at`, `half_life_days` columns to `memory.facts` in `setup-memory-schema.sh`
- [x] T002 Add `retrieval_count`, `last_retrieved_at`, `half_life_days` columns to `memory.memories` in `setup-memory-schema.sh`
- [x] T003 Add `confidence` column with CHECK constraint to `memory.facts`
- [x] T004 Create `memory.fact_conflicts` table with indexes
- [x] T005 Add `context_refs JSONB` column to `pipeline.tasks`
...
- [x] T018 Query recent conflicts (7-day window) in `context-assembly.ts` and prefix with `[CONFLICT]`
...
- [x] T032 Schema migration idempotency test (run setup-memory-schema.sh twice)
