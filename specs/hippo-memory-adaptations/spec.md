...
| FR | File(s) | Notes |
|----|---------|-------|
| FR-1 (schema) | `scripts/infra/setup-memory-schema.sh` | All hippo-memory columns added idempotently (lines 78–111). File exists and is safe to re-run. |
...
| FR-6 (outcome feedback) | `agent/src/jobs/merge-check.ts`, `mcp-server/src/pipeline.ts`, `mcp-server/src/context-assembly.ts` | `assembleContext()` accepts `includeIds: boolean` and returns `context_refs: { fact_ids, memory_ids }`. `merge-check` reads `context_refs` and adjusts `half_life_days` (+5 merge, −3 rejection). **Partially wired — see known gap below.** |
...
### Known gap: FR-6 outcome feedback wiring

`assembleContext()` and `merge-check.ts` are fully implemented, but
`routes.ts:299` calls `assembleContext()` without `includeIds: true`:

```typescript
// mcp-server/src/routes.ts ~line 299 (current)
const result = await assembleContext(pool, query, template, 8000, repo || undefined);
//                                                                    ↑ missing: true
```

As a result, `result.context_refs` is always undefined, so
`pipeline.tasks.context_refs` is never populated and
`merge-check.ts` silently no-ops on feedback.

To close the gap, two one-line changes are needed:

1. Pass `includeIds: true` in `routes.ts`:
   ```typescript
   const result = await assembleContext(pool, query, template, 8000, repo || undefined, undefined, undefined, true);
   ```

2. Forward `result.context_refs` to `createTask()` in the
   `/api/task` POST handler (or wherever the task creation
   POST is assembled after `assembleContext`).

All the machinery (schema column, `assembleContext` `includeIds`
param, `createTask` `contextRefs` param, `merge-check` feedback
reads) is in place.
