...
A lifecycle marker's `phase` is carried on the entry, so a phase like `init` reads as `· init started` instead of falling back to the unclassified default; a marker with no phase keeps the bare `· agent <status>` line. ([validated by `agent-log-entries.test.ts:332`](apps/web-ui/src/lib/agent-log-entries.test.ts#L353), [`agent-log-entries.test.ts:338`](apps/web-ui/src/lib/agent-log-entries.test.ts#L359), [`LogEntriesView.test.tsx:31`](apps/web-ui/src/components/LogEntriesView.test.tsx#L35))
...
A `rate_limit_event` line becomes a rate-limit entry naming every window's whole-percent utilization, reset time and status, reading `unifiedWindows` when present and falling back to the single top-level window otherwise; a shape carrying no readable window is kept raw rather than rendering a confidently empty sentence. `utilization` is the fraction the event sends, never treated as an already-computed percent. ([validated by `agent-log-entries.test.ts:346`](apps/web-ui/src/lib/agent-log-entries.test.ts#L367), [`agent-log-entries.test.ts:359`](apps/web-ui/src/lib/agent-log-entries.test.ts#L380), [`agent-log-entries.test.ts:381`](apps/web-ui/src/lib/agent-log-entries.test.ts#L402), [`agent-log-entries.test.ts:387`](apps/web-ui/src/lib/agent-log-entries.test.ts#L408), [`agent-log-entries.test.ts:410`](apps/web-ui/src/lib/agent-log-entries.test.ts#L431), [`agent-log-entries.test.ts:418`](apps/web-ui/src/lib/agent-log-entries.test.ts#L439), [`agent-log-entries.test.ts:433`](apps/web-ui/src/lib/agent-log-entries.test.ts#L454), [`LogEntriesView.test.tsx:41`](apps/web-ui/src/components/LogEntriesView.test.tsx#L45))
...
A declared-artifact delivery — the subsystem raising a recipe-declared `output.watch` file as a `kind:"file"` event once the agent exits (e.g. `pr.description`) — becomes a file entry carrying the event name, path and content; a declared artifact the agent never produced carries its `reason` instead of content, and a `kind:"file"` line with no event name stays raw, since nothing could route it. ([validated by `agent-log-entries.test.ts:754`](apps/web-ui/src/lib/agent-log-entries.test.ts#L756), [`agent-log-entries.test.ts:766`](apps/web-ui/src/lib/agent-log-entries.test.ts#L768), [`agent-log-entries.test.ts:778`](apps/web-ui/src/lib/agent-log-entries.test.ts#L780))

A file entry renders as a collapsed details summarized `⇢ <event> — <path>` over the delivered content, and a never-produced artifact as an error line naming the event and reason. ([validated by `LogEntriesView.test.tsx:338`](apps/web-ui/src/components/LogEntriesView.test.tsx#L340), [`LogEntriesView.test.tsx:351`](apps/web-ui/src/components/LogEntriesView.test.tsx#L353))
