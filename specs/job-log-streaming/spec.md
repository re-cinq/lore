...
A `system` line of any subtype the parser does not recognize is summarized as `· system: <subtype>` over its pretty-printed payload rather than dumped raw, while `init` and `thinking_tokens` keep their own richer entries. ([validated by [`agent-log-entries.test.ts:601`](apps/web-ui/src/lib/agent-log-entries.test.ts#L601), [`agent-log-entries.test.ts:615`](apps/web-ui/src/lib/agent-log-entries.test.ts#L615))
