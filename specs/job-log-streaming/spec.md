...
The run page's log viewers — the per-node assembly-line pod-log panels and the full transcript panel — parse an NDJSON stream line by line and render a human-readable transcript instead of the raw JSON blob, with a line that cannot be parsed as JSON shown raw, and a Raw/Formatted toggle (formatted by default) preserving access to the underlying stream (the pod's verbatim bytes for the pod-log panels; the stored turn envelopes for the transcript panel); the parser is grounded in real production lines captured from a live review node (`agent-log-entries.fixtures.ts`). ([validated by `agent-log-entries.test.ts:239`](apps/web-ui/src/lib/agent-log-entries.test.ts#L258))

#### Parsing (`agent-log-entries.ts`)

Each NDJSON line of an agent pod's stream parses into a typed entry; empty lines are skipped and an empty blob yields no entries. ([validated by `agent-log-entries.test.ts:40`](apps/web-ui/src/lib/agent-log-entries.test.ts#L61))

`parseAgentLog` is built on two smaller exports the run page's timestamped conversation reuses — `parseAgentLogLine` classifies one line, and `logEntriesFromValue` classifies an already-decoded envelope without re-serializing it, since the transcript store hands out parsed JSONB — sharing the same ticker-collapse rule, `supersedesPrevious`, so a run of `thinking_tokens` entries collapses identically whether it crosses lines in one blob or turns in the timed view. ([validated by `agent-log-entries.test.ts:286`](apps/web-ui/src/lib/agent-log-entries.test.ts#L305), [`agent-log-entries.test.ts:290`](apps/web-ui/src/lib/agent-log-entries.test.ts#L309), [`agent-log-entries.test.ts:296`](apps/web-ui/src/lib/agent-log-entries.test.ts#L315), [`agent-log-entries.test.ts:304`](apps/web-ui/src/lib/agent-log-entries.test.ts#L323), [`agent-log-entries.test.ts:312`](apps/web-ui/src/lib/agent-log-entries.test.ts#L331), [`agent-log-entries.test.ts:320`](apps/web-ui/src/lib/agent-log-entries.test.ts#L339))

A line that is not JSON — a runner/supervisor marker or an unterminated fragment from an in-flight poll — passes through verbatim as a raw entry, as does valid JSON of an unrecognized shape. ([validated by `agent-log-entries.test.ts:47`](apps/web-ui/src/lib/agent-log-entries.test.ts#L66), [validated by `agent-log-entries.test.ts:53`](apps/web-ui/src/lib/agent-log-entries.test.ts#L72), [validated by `agent-log-entries.test.ts:212`](apps/web-ui/src/lib/agent-log-entries.test.ts#L231))

Lifecycle markers become status entries, carrying the exit code when present. ([validated by `agent-log-entries.test.ts:59`](apps/web-ui/src/lib/agent-log-entries.test.ts#L78))

A lifecycle marker's `phase` is carried on the entry, so a phase like `init` reads as `· init started` instead of falling back to the unclassified default; a marker with no phase keeps the bare `· agent <status>` line. ([validated by `agent-log-entries.test.ts:332`](apps/web-ui/src/lib/agent-log-entries.test.ts#L351), [`agent-log-entries.test.ts:338`](apps/web-ui/src/lib/agent-log-entries.test.ts#L357), [`LogEntriesView.test.tsx:31`](apps/web-ui/src/components/LogEntriesView.test.tsx#L33))

The system init line becomes a session-init entry exposing the model, the Claude Code version, and the pretty-printed full payload. ([validated by `agent-log-entries.test.ts:68`](apps/web-ui/src/lib/agent-log-entries.test.ts#L87))

Consecutive thinking_tokens ticker lines coalesce into a single counter entry holding the run's latest count; a run broken by any other entry starts a new counter. ([validated by `agent-log-entries.test.ts:81`](apps/web-ui/src/lib/agent-log-entries.test.ts#L100), [validated by `agent-log-entries.test.ts:93`](apps/web-ui/src/lib/agent-log-entries.test.ts#L112))

A hook's `hook_*` system lines become one hook entry naming the hook, its phase, and — once it finishes — its outcome and exit code, so a hook reads as a sentence rather than a JSON blob. ([validated by [`agent-log-entries.test.ts:453`](apps/web-ui/src/lib/agent-log-entries.test.ts#L464), [`agent-log-entries.test.ts:465`](apps/web-ui/src/lib/agent-log-entries.test.ts#L476), [`agent-log-entries.test.ts:479`](apps/web-ui/src/lib/agent-log-entries.test.ts#L490))

`hook_progress` restates the whole output so far rather than an increment, so consecutive entries sharing a `hook_id` collapse to the newest one; the fold is adjacent-only and keyed on identity, so two hooks running concurrently keep their interleaved order instead of collapsing across each other. ([validated by [`agent-log-entries.test.ts:485`](apps/web-ui/src/lib/agent-log-entries.test.ts#L496), [`agent-log-entries.test.ts:506`](apps/web-ui/src/lib/agent-log-entries.test.ts#L517), [`agent-log-entries.test.ts:526`](apps/web-ui/src/lib/agent-log-entries.test.ts#L537))

A `system` line of any subtype the parser does not recognize is summarized as `· system: <subtype>` over its pretty-printed payload rather than dumped raw, while `init` and `thinking_tokens` keep their own richer entries. ([validated by [`agent-log-entries.test.ts:601`](apps/web-ui/src/lib/agent-log-entries.test.ts#L612), [`agent-log-entries.test.ts:615`](apps/web-ui/src/lib/agent-log-entries.test.ts#L626))

A hook renders as one dim line carrying its verdict — `✓ (exit 0)`, `✗ (exit 2)`, or `running…` while it is still going; a finished hook that reported no exit code is judged on its outcome alone, so `success` reads as `✓` rather than being marked failed by the absent code — with any output kept behind the summary and a non-zero exit styled as an error. ([validated by [`LogEntriesView.test.tsx:211`](apps/web-ui/src/components/LogEntriesView.test.tsx#L213), [`LogEntriesView.test.tsx:219`](apps/web-ui/src/components/LogEntriesView.test.tsx#L221), [`LogEntriesView.test.tsx:227`](apps/web-ui/src/components/LogEntriesView.test.tsx#L229), [`LogEntriesView.test.tsx:235`](apps/web-ui/src/components/LogEntriesView.test.tsx#L237), [`LogEntriesView.test.tsx:243`](apps/web-ui/src/components/LogEntriesView.test.tsx#L296), [`LogEntriesView.test.tsx:243`](apps/web-ui/src/components/LogEntriesView.test.tsx#L245))

Assistant and user messages emit one entry per content block — thinking, text, and tool_use — in order. ([validated by `agent-log-entries.test.ts:108`](apps/web-ui/src/lib/agent-log-entries.test.ts#L127), [validated by `agent-log-entries.test.ts:117`](apps/web-ui/src/lib/agent-log-entries.test.ts#L136), [validated by `agent-log-entries.test.ts:218`](apps/web-ui/src/lib/agent-log-entries.test.ts#L237))

A tool_use block is summarized as `→ <tool>: <most relevant arg>`, falling back to the bare tool name when no recognized text argument is present. ([validated by `agent-log-entries.test.ts:126`](apps/web-ui/src/lib/agent-log-entries.test.ts#L145), [validated by `agent-log-entries.test.ts:135`](apps/web-ui/src/lib/agent-log-entries.test.ts#L154))

A tool_result block yields its text — string or array form, tool references bracketed — with is_error preserved and real newlines kept. ([validated by `agent-log-entries.test.ts:141`](apps/web-ui/src/lib/agent-log-entries.test.ts#L160), [validated by `agent-log-entries.test.ts:147`](apps/web-ui/src/lib/agent-log-entries.test.ts#L166), [validated by `agent-log-entries.test.ts:156`](apps/web-ui/src/lib/agent-log-entries.test.ts#L175))

User text blocks (the injected task prompt) become user-text entries. ([validated by `agent-log-entries.test.ts:162`](apps/web-ui/src/lib/agent-log-entries.test.ts#L181))

The terminal result line yields a result entry with the agent text, error flag, duration, cost and turn count; a station's LORE_NODE_RESULT text is kept verbatim. ([validated by `agent-log-entries.test.ts:171`](apps/web-ui/src/lib/agent-log-entries.test.ts#L190), [validated by `agent-log-entries.test.ts:190`](apps/web-ui/src/lib/agent-log-entries.test.ts#L209))

Station eventLine progress lines become station-log entries. ([validated by `agent-log-entries.test.ts:184`](apps/web-ui/src/lib/agent-log-entries.test.ts#L203))

A `rate_limit_event` line becomes a rate-limit entry naming every window's whole-percent utilization, reset time and status, reading `unifiedWindows` when present and falling back to the single top-level window otherwise; a shape carrying no readable window is kept raw rather than rendering a confidently empty sentence. `utilization` is the fraction the event sends, never treated as an already-computed percent. ([validated by `agent-log-entries.test.ts:346`](apps/web-ui/src/lib/agent-log-entries.test.ts#L365), [`agent-log-entries.test.ts:359`](apps/web-ui/src/lib/agent-log-entries.test.ts#L378), [`agent-log-entries.test.ts:381`](apps/web-ui/src/lib/agent-log-entries.test.ts#L400), [`agent-log-entries.test.ts:387`](apps/web-ui/src/lib/agent-log-entries.test.ts#L406), [`agent-log-entries.test.ts:410`](apps/web-ui/src/lib/agent-log-entries.test.ts#L429), [`agent-log-entries.test.ts:418`](apps/web-ui/src/lib/agent-log-entries.test.ts#L437), [`agent-log-entries.test.ts:433`](apps/web-ui/src/lib/agent-log-entries.test.ts#L452), [`LogEntriesView.test.tsx:41`](apps/web-ui/src/components/LogEntriesView.test.tsx#L43))

Lines wrapped in the ai-agent-subsystem's attribution envelope — single or the transitional double wrap — classify identically to their bare payload. ([validated by `agent-log-entries.test.ts:200`](apps/web-ui/src/lib/agent-log-entries.test.ts#L219), [validated by `agent-log-entries.test.ts:206`](apps/web-ui/src/lib/agent-log-entries.test.ts#L225))
...
A gemini `init` event becomes a session-init entry exposing the model and the pretty-printed payload, with no Claude Code version claimed. ([validated by `agent-log-entries.test.ts:633`](apps/web-ui/src/lib/agent-log-entries.test.ts#L633))

A gemini `message` with string content classifies by role — user-text for the injected prompt, assistant-text otherwise. ([validated by `agent-log-entries.test.ts:643`](apps/web-ui/src/lib/agent-log-entries.test.ts#L643))

Assistant prose arrives ONLY as `delta: true` chunks — gemini-cli emits no final complete message — so adjacent chunks merge into one assistant-text entry, and the fold is adjacent-only: a tool call between chunks starts a new paragraph rather than gluing across it. `mergedDelta` is the one home for the rule, shared by the blob parser and the run page's timed conversation, which keeps the merged utterance on its FIRST chunk's timestamp. ([validated by `agent-log-entries.test.ts:682`](apps/web-ui/src/lib/agent-log-entries.test.ts#L682), [`agent-log-entries.test.ts:697`](apps/web-ui/src/lib/agent-log-entries.test.ts#L697), [`agent-log-entries.test.ts:733`](apps/web-ui/src/lib/agent-log-entries.test.ts#L733), [`turn-transcript-presenter.test.ts:371`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L371))

A top-level gemini `tool_use` summarizes as `→ <tool>: <most relevant arg>` from `tool_name` and `parameters`, through the same summary rule the claude blocks use. ([validated by `agent-log-entries.test.ts:652`](apps/web-ui/src/lib/agent-log-entries.test.ts#L652))

A top-level gemini `tool_result` yields its `output` — or its `error.message` when there is no output — with `status: "error"` mapped to the error flag; `status` is the CLI's verdict on the dispatch, so a failed command under `status: "success"` stays non-error, exactly as the stream reports it. ([validated by `agent-log-entries.test.ts:662`](apps/web-ui/src/lib/agent-log-entries.test.ts#L662), [`agent-log-entries.test.ts:672`](apps/web-ui/src/lib/agent-log-entries.test.ts#L672))

Gemini's standalone `error` event — a shape claude does not have; claude carries errors inside its result line — becomes an agent-error entry styled by severity: an `error` renders with the error style, a `warning` dimmed. ([validated by `agent-log-entries.test.ts:711`](apps/web-ui/src/lib/agent-log-entries.test.ts#L711), [`LogEntriesView.test.tsx:304`](apps/web-ui/src/components/LogEntriesView.test.tsx#L304), [`LogEntriesView.test.tsx:316`](apps/web-ui/src/components/LogEntriesView.test.tsx#L316))

A gemini `result` keyed by `status` folds into the existing result entry: `status: "error"` sets the error flag and the entry text falls back to `error.message`, since gemini's terminal line carries no `result` string. ([validated by `agent-log-entries.test.ts:721`](apps/web-ui/src/lib/agent-log-entries.test.ts#L721), [`agent-log-entries.test.ts:727`](apps/web-ui/src/lib/agent-log-entries.test.ts#L727))

The captured production sample parses into the expected ordered kind sequence with one counter per ticker run. ([validated by `agent-log-entries.test.ts:239`](apps/web-ui/src/lib/agent-log-entries.test.ts#L258))

formatTokens abbreviates counts of a thousand or more with a `k` suffix, formatDuration renders the two largest time units, and clip flattens whitespace and truncates with an ellipsis. ([validated by `agent-log-entries.test.ts:263`](apps/web-ui/src/lib/agent-log-entries.test.ts#L282), [validated by `agent-log-entries.test.ts:271`](apps/web-ui/src/lib/agent-log-entries.test.ts#L290), [validated by `agent-log-entries.test.ts:279`](apps/web-ui/src/lib/agent-log-entries.test.ts#L298))

#### Rendering (`LogEntriesView`)

Lifecycle and station-log entries render as dimmed `·` lines, with the exit code appended when present. ([validated by `LogEntriesView.test.tsx:17`](apps/web-ui/src/components/LogEntriesView.test.tsx#L23), [validated by `LogEntriesView.test.tsx:58`](apps/web-ui/src/components/LogEntriesView.test.tsx#L64), [validated by `LogEntriesView.test.tsx:146`](apps/web-ui/src/components/LogEntriesView.test.tsx#L152))

The session-init entry renders as a collapsed details block whose summary names the model and Claude Code version. ([validated by `LogEntriesView.test.tsx:68`](apps/web-ui/src/components/LogEntriesView.test.tsx#L74))

The thinking-tokens counter renders italic-dimmed as `thinking… ~N tokens`, and tool summaries render with the accent class. ([validated by `LogEntriesView.test.tsx:82`](apps/web-ui/src/components/LogEntriesView.test.tsx#L88), [validated by `LogEntriesView.test.tsx:92`](apps/web-ui/src/components/LogEntriesView.test.tsx#L98))

A short single-line tool result renders inline; a long or multiline one collapses into an expandable details block carrying the error class when is_error. ([validated by `LogEntriesView.test.tsx:100`](apps/web-ui/src/components/LogEntriesView.test.tsx#L106), [validated by `LogEntriesView.test.tsx:117`](apps/web-ui/src/components/LogEntriesView.test.tsx#L123))

The terminal result renders a summary footer with duration, cost and turn count — or `✗ failed` carrying the error class — with the result text expandable. ([validated by `LogEntriesView.test.tsx:127`](apps/web-ui/src/components/LogEntriesView.test.tsx#L133), [validated by `LogEntriesView.test.tsx:136`](apps/web-ui/src/components/LogEntriesView.test.tsx#L142))

The user prompt renders as collapsed details prefixed `user:`. ([validated by `LogEntriesView.test.tsx:158`](apps/web-ui/src/components/LogEntriesView.test.tsx#L164))

Raw entries render verbatim; assistant text and thinking render with their own classes. ([validated by `LogEntriesView.test.tsx:175`](apps/web-ui/src/components/LogEntriesView.test.tsx#L181), [validated by `LogEntriesView.test.tsx:187`](apps/web-ui/src/components/LogEntriesView.test.tsx#L193))
