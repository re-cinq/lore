...
1. Running task shows live logs in pipeline detail page. ([validated by `TaskLogs.test.tsx:166`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L166))

2. Logs update on the task page's coordinated refresh (10s poll, or stream-driven when the task has a live assembly-line run) while the task is running. ([validated by `TaskLogs.test.tsx:524`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L524))

3. Completed/failed tasks show final output. ([validated by `TaskLogs.test.tsx:564`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L564))
...
The viewer reads the task's stored agent turns (`pipeline.agent_run_turns`) through the logs proxy — the GCS byte log it replaced had no cluster-side writer left (#1292) — and its initial fetch requests the page-limit URL with no `after` cursor. ([validated by `TaskLogs.test.tsx:149`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L149))

The mount fetch fires exactly once — a response that settles new state must not re-fire it, so incremental refreshes happen only on the page coordinator's ticks. ([validated by `TaskLogs.test.tsx:738`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L738))

Before any turns resolve — or while a task that has not finished has no stored turns yet — the viewer shows the placeholder "Logs will appear when the agent starts.", and the "Agent Output" heading always renders. ([validated by `TaskLogs.test.tsx:131`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L131), [validated by `TaskLogs.test.tsx:645`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L645), [validated by `TaskLogs.test.tsx:584`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L584))

A task past its queued/running phases (including badge-less settled statuses like needs-human-help) with zero stored turns shows the no-stored-turns explainer — naming locally-run tasks (no ingest yet), tasks that ran before the transcript store, and the 30-day retention — instead of the placeholder. ([validated by `TaskLogs.test.tsx:629`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L629), [validated by `TaskLogs.test.tsx:832`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L832))

Status maps to a badge: "Completed" for succeeded/pr-created/merged, "In Review" for review, "Failed" for failed/cancelled; a running task that later reports completion transitions from the polling note to the Completed badge. ([validated by `TaskLogs.test.tsx:184`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L184), [validated by `TaskLogs.test.tsx:198`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L198), [validated by `TaskLogs.test.tsx:212`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L212), [validated by `TaskLogs.test.tsx:224`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L224), [validated by `TaskLogs.test.tsx:236`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L236), [validated by `TaskLogs.test.tsx:248`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L248), [validated by `TaskLogs.test.tsx:596`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L596))

While running, the viewer renders a pulse indicator and a refresh note — "Live" when the run event stream is attached, "Auto-refreshing" otherwise — suffixed with "— N turns received" once turns have arrived and bare at zero turns. ([validated by `TaskLogs.test.tsx:262`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L262), [validated by `TaskLogs.test.tsx:286`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L286))

Once the first page landed, the coordinator's polls request `after=<last row id>` and append only the returned turns; a poll returning nothing new appends nothing and duplicates nothing, and an in-flight latch keeps overlapping walks from double-appending. ([validated by `TaskLogs.test.tsx:299`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L299), [validated by `TaskLogs.test.tsx:344`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L344))

A page of exactly the page limit continues the walk within the same fetch cycle; the walk stops on a short page, at the run page's turn load cap, or when a full page fails to advance the cursor (which would otherwise refetch itself forever). ([validated by `TaskLogs.test.tsx:371`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L371), [validated by `TaskLogs.test.tsx:795`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L795), [validated by `task-logs-presenter.test.ts:71`](apps/web-ui/src/app/tasks/[id]/task-logs-presenter.test.ts#L71), [validated by `task-logs-presenter.test.ts:75`](apps/web-ui/src/app/tasks/[id]/task-logs-presenter.test.ts#L75), [validated by `task-logs-presenter.test.ts:81`](apps/web-ui/src/app/tasks/[id]/task-logs-presenter.test.ts#L81), [validated by `task-logs-presenter.test.ts:85`](apps/web-ui/src/app/tasks/[id]/task-logs-presenter.test.ts#L85))

The cap bounds the tab's memory across the task's whole lifetime, not one walk: once the cap is loaded, later poll ticks fetch nothing more and the capped notice is shown. ([validated by `TaskLogs.test.tsx:760`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L760))

The X-Task-Status header is read before the ok-check, so a failing upstream still settles the badge and ends the poll loop of a completed task. ([validated by `TaskLogs.test.tsx:811`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L811))

The poll cursor survives hostile pages — it advances to the last row carrying a string id and stays put over an empty or id-less page — and the logs URL carries the page limit, the encoded task id, and the cursor. ([validated by `task-logs-presenter.test.ts:53`](apps/web-ui/src/app/tasks/[id]/task-logs-presenter.test.ts#L53), [validated by `task-logs-presenter.test.ts:57`](apps/web-ui/src/app/tasks/[id]/task-logs-presenter.test.ts#L57), [validated by `task-logs-presenter.test.ts:61`](apps/web-ui/src/app/tasks/[id]/task-logs-presenter.test.ts#L61), [validated by `task-logs-presenter.test.ts:65`](apps/web-ui/src/app/tasks/[id]/task-logs-presenter.test.ts#L65), [validated by `task-logs-presenter.test.ts:33`](apps/web-ui/src/app/tasks/[id]/task-logs-presenter.test.ts#L33), [validated by `task-logs-presenter.test.ts:39`](apps/web-ui/src/app/tasks/[id]/task-logs-presenter.test.ts#L39), [validated by `task-logs-presenter.test.ts:45`](apps/web-ui/src/app/tasks/[id]/task-logs-presenter.test.ts#L45))

Fetch failures surface distinct messages: a 403 renders the access-denied message and stops polling, a 401 renders the sign-in message, any other non-ok response renders an "HTTP <code>" message, a thrown fetch renders the rejection message, and a later successful fetch clears the error. ([validated by `TaskLogs.test.tsx:401`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L401), [validated by `TaskLogs.test.tsx:433`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L433), [validated by `TaskLogs.test.tsx:452`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L452), [validated by `TaskLogs.test.tsx:469`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L469), [validated by `TaskLogs.test.tsx:486`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L486))
...
The task log viewer renders the parsed transcript by default — tool lines, one counter per ticker run, the result footer — with no raw JSON visible. ([validated by `TaskLogs.test.tsx:661`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L661))

Clicking Raw shows the stored stream — each turn's envelope re-serialized as one NDJSON line — and clicking Formatted restores the transcript; the toggle is hidden until turns arrive. ([validated by `TaskLogs.test.tsx:705`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L705), [validated by `TaskLogs.test.tsx:724`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L724))

A task's turns span every node visit of its assembly line, so the formatted view renders one segment per consecutive (node, iteration) run — labeled `node · iteration N`, bare node name when the iteration is unknown, no label for uncorrelated turns — and the parser runs per segment so each visit keeps its own transcript. ([validated by `TaskLogs.test.tsx:680`](apps/web-ui/src/app/tasks/[id]/TaskLogs.test.tsx#L680), [validated by `task-logs-presenter.test.ts:93`](apps/web-ui/src/app/tasks/[id]/task-logs-presenter.test.ts#L93), [validated by `task-logs-presenter.test.ts:108`](apps/web-ui/src/app/tasks/[id]/task-logs-presenter.test.ts#L108), [validated by `task-logs-presenter.test.ts:122`](apps/web-ui/src/app/tasks/[id]/task-logs-presenter.test.ts#L122), [validated by `task-logs-presenter.test.ts:131`](apps/web-ui/src/app/tasks/[id]/task-logs-presenter.test.ts#L131), [validated by `task-logs-presenter.test.ts:137`](apps/web-ui/src/app/tasks/[id]/task-logs-presenter.test.ts#L137), [validated by `task-logs-presenter.test.ts:143`](apps/web-ui/src/app/tasks/[id]/task-logs-presenter.test.ts#L143), [validated by `task-logs-presenter.test.ts:149`](apps/web-ui/src/app/tasks/[id]/task-logs-presenter.test.ts#L149))
