...
The run page's log viewers — the per-node assembly-line pod-log panels and the full transcript panel — parse an NDJSON stream line by line and render a human-readable transcript instead of the raw JSON blob, with a line that cannot be parsed as JSON shown raw, and a Raw/Formatted toggle (formatted by default) on the pod-log panels preserving access to the pod's verbatim bytes (_amended 2026-09-09:_ the transcript panel no longer carries that toggle — it renders the formatted conversation alone, and the stored envelopes are read through `GET /api/agent-turns/{assemblyRunId}`); the parser is grounded in real production lines captured from a live review node (`agent-log-entries.fixtures.ts`). ([validated by `agent-log-entries.test.ts:239`](apps/web-ui/src/lib/agent-log-entries.test.ts#L260))
...
`parseAgentLog` is built on two smaller exports the run page's timestamped conversation reuses — `parseAgentLogLine` classifies one line, and `logEntriesFromValue` classifies an already-decoded envelope without re-serializing it, since the transcript store hands out parsed JSONB — sharing the same ticker-collapse rule, `supersedesPrevious`, so a run of `thinking_tokens` entries collapses identically whether it crosses lines in one blob or turns in the timed view. ([validated by `agent-log-entries.test.ts:286`](apps/web-ui/src/lib/agent-log-entries.test.ts#L327), [`agent-log-entries.test.ts:290`](apps/web-ui/src/lib/agent-log-entries.test.ts#L331), [`agent-log-entries.test.ts:296`](apps/web-ui/src/lib/agent-log-entries.test.ts#L337), [`agent-log-entries.test.ts:304`](apps/web-ui/src/lib/agent-log-entries.test.ts#L345), [`agent-log-entries.test.ts:312`](apps/web-ui/src/lib/agent-log-entries.test.ts#L353), [`agent-log-entries.test.ts:320`](apps/web-ui/src/lib/agent-log-entries.test.ts#L361))
...
A lifecycle marker's `phase` is carried on the entry, so a phase like `init` reads as `· init started` instead of falling back to the unclassified default; a marker with no phase keeps the bare `· agent <status>` line. ([validated by `agent-log-entries.test.ts:332`](apps/web-ui/src/lib/agent-log-entries.test.ts#L373), [`agent-log-entries.test.ts:338`](apps/web-ui/src/lib/agent-log-entries.test.ts#L379), [`LogEntriesView.test.tsx:33`](apps/web-ui/src/components/LogEntriesView.test.tsx#L33))
...
A hook's `hook_*` system lines become one hook entry naming the hook, its phase, and — once it finishes — its outcome and exit code, so a hook reads as a sentence rather than a JSON blob. ([validated by [`agent-log-entries.test.ts:453`](apps/web-ui/src/lib/agent-log-entries.test.ts#L486), [`agent-log-entries.test.ts:465`](apps/web-ui/src/lib/agent-log-entries.test.ts#L498), [`agent-log-entries.test.ts:479`](apps/web-ui/src/lib/agent-log-entries.test.ts#L512))

`hook_progress` restates the whole output so far rather than an increment, so consecutive entries sharing a `hook_id` collapse to the newest one; the fold is adjacent-only and keyed on identity, so two hooks running concurrently keep their interleaved order instead of collapsing across each other. ([validated by [`agent-log-entries.test.ts:485`](apps/web-ui/src/lib/agent-log-entries.test.ts#L518), [`agent-log-entries.test.ts:506`](apps/web-ui/src/lib/agent-log-entries.test.ts#L539), [`agent-log-entries.test.ts:526`](apps/web-ui/src/lib/agent-log-entries.test.ts#L559))

A `system` line of any subtype the parser does not recognize is summarized as `· system: <subtype>` over its pretty-printed payload rather than dumped raw, while `init` and `thinking_tokens` keep their own richer entries. ([validated by [`agent-log-entries.test.ts:601`](apps/web-ui/src/lib/agent-log-entries.test.ts#L634), [`agent-log-entries.test.ts:615`](apps/web-ui/src/lib/agent-log-entries.test.ts#L648))

A hook renders as one dim line carrying its verdict — `✓ (exit 0)`, `✗ (exit 2)`, or `running…` while it is still going; a finished hook that reported no exit code is judged on its outcome alone, so `success` reads as `✓` rather than being marked failed by the absent code — with any output kept behind the summary and a non-zero exit styled as an error. ([validated by [`LogEntriesView.test.tsx:211`](apps/web-ui/src/components/LogEntriesView.test.tsx#L215), [`LogEntriesView.test.tsx:219`](apps/web-ui/src/components/LogEntriesView.test.tsx#L223), [`LogEntriesView.test.tsx:227`](apps/web-ui/src/components/LogEntriesView.test.tsx#L231), [`LogEntriesView.test.tsx:235`](apps/web-ui/src/components/LogEntriesView.test.tsx#L239), [`LogEntriesView.test.tsx:243`](apps/web-ui/src/components/LogEntriesView.test.tsx#L298), [`LogEntriesView.test.tsx:243`](apps/web-ui/src/components/LogEntriesView.test.tsx#L247), [`LogEntriesView.test.tsx:118`](../../apps/web-ui/src/components/LogEntriesView.test.tsx#L118), [`LogEntriesView.test.tsx:172`](../../apps/web-ui/src/components/LogEntriesView.test.tsx#L172), [`LogEntriesView.test.tsx:201`](../../apps/web-ui/src/components/LogEntriesView.test.tsx#L201), [`LogEntriesView.test.tsx:249`](../../apps/web-ui/src/components/LogEntriesView.test.tsx#L249), [`LogEntriesView.test.tsx:257`](../../apps/web-ui/src/components/LogEntriesView.test.tsx#L257), [`LogEntriesView.test.tsx:324`](../../apps/web-ui/src/components/LogEntriesView.test.tsx#L324), [`LogEntriesView.test.tsx:356`](../../apps/web-ui/src/components/LogEntriesView.test.tsx#L356), [`LogEntriesView.test.tsx:369`](../../apps/web-ui/src/components/LogEntriesView.test.tsx#L369))
...
A `rate_limit_event` line becomes a rate-limit entry naming every window's whole-percent utilization, reset time and status, reading `unifiedWindows` when present and falling back to the single top-level window otherwise; a shape carrying no readable window is kept raw rather than rendering a confidently empty sentence. `utilization` is the fraction the event sends, never treated as an already-computed percent. ([validated by `agent-log-entries.test.ts:346`](apps/web-ui/src/lib/agent-log-entries.test.ts#L387), [`agent-log-entries.test.ts:359`](apps/web-ui/src/lib/agent-log-entries.test.ts#L400), [`agent-log-entries.test.ts:381`](apps/web-ui/src/lib/agent-log-entries.test.ts#L422), [`agent-log-entries.test.ts:387`](apps/web-ui/src/lib/agent-log-entries.test.ts#L428), [`agent-log-entries.test.ts:410`](apps/web-ui/src/lib/agent-log-entries.test.ts#L451), [`agent-log-entries.test.ts:418`](apps/web-ui/src/lib/agent-log-entries.test.ts#L459), [`agent-log-entries.test.ts:433`](apps/web-ui/src/lib/agent-log-entries.test.ts#L474), [`LogEntriesView.test.tsx:43`](apps/web-ui/src/components/LogEntriesView.test.tsx#L43), [`agent-log-entries.test.ts:139`](../../apps/web-ui/src/lib/agent-log-entries.test.ts#L139), [`agent-log-entries.test.ts:148`](../../apps/web-ui/src/lib/agent-log-entries.test.ts#L148), [`agent-log-entries.test.ts:172`](../../apps/web-ui/src/lib/agent-log-entries.test.ts#L172), [`agent-log-entries.test.ts:187`](../../apps/web-ui/src/lib/agent-log-entries.test.ts#L187), [`agent-log-entries.test.ts:193`](../../apps/web-ui/src/lib/agent-log-entries.test.ts#L193), [`agent-log-entries.test.ts:215`](../../apps/web-ui/src/lib/agent-log-entries.test.ts#L215), [`agent-log-entries.test.ts:243`](../../apps/web-ui/src/lib/agent-log-entries.test.ts#L243), [`agent-log-entries.test.ts:318`](../../apps/web-ui/src/lib/agent-log-entries.test.ts#L338), [`agent-log-entries.test.ts:326`](../../apps/web-ui/src/lib/agent-log-entries.test.ts#L346), [`agent-log-entries.test.ts:343`](../../apps/web-ui/src/lib/agent-log-entries.test.ts#L363), [`agent-log-entries.test.ts:385`](../../apps/web-ui/src/lib/agent-log-entries.test.ts#L405), [`agent-log-entries.test.ts:457`](../../apps/web-ui/src/lib/agent-log-entries.test.ts#L477), [`agent-log-entries.test.ts:480`](../../apps/web-ui/src/lib/agent-log-entries.test.ts#L500), [`agent-log-entries.test.ts:504`](../../apps/web-ui/src/lib/agent-log-entries.test.ts#L524), [`agent-log-entries.test.ts:690`](../../apps/web-ui/src/lib/agent-log-entries.test.ts#L710), [`agent-log-entries.test.ts:773`](../../apps/web-ui/src/lib/agent-log-entries.test.ts#L793), [`agent-log-entries.test.ts:779`](../../apps/web-ui/src/lib/agent-log-entries.test.ts#L799), [`agent-log-entries.test.ts:785`](../../apps/web-ui/src/lib/agent-log-entries.test.ts#L805), [`agent-log-entries.test.ts:804`](../../apps/web-ui/src/lib/agent-log-entries.test.ts#L824), [`agent-log-entries.test.ts:816`](../../apps/web-ui/src/lib/agent-log-entries.test.ts#L836), [`agent-log-entries.test.ts:828`](../../apps/web-ui/src/lib/agent-log-entries.test.ts#L848), [`agent-log-entries.test.ts:836`](../../apps/web-ui/src/lib/agent-log-entries.test.ts#L856))
...
A declared-artifact delivery — the subsystem raising a recipe-declared `output.watch` file as a `kind:"file"` event once the agent exits (e.g. `pr.description`) — becomes a file entry carrying the event name, path and content; a declared artifact the agent never produced carries its `reason` instead of content, and a `kind:"file"` line with no event name stays raw, since nothing could route it. ([validated by `agent-log-entries.test.ts:754`](apps/web-ui/src/lib/agent-log-entries.test.ts#L774), [`agent-log-entries.test.ts:766`](apps/web-ui/src/lib/agent-log-entries.test.ts#L786), [`agent-log-entries.test.ts:778`](apps/web-ui/src/lib/agent-log-entries.test.ts#L798))
...
A gemini `init` event becomes a session-init entry exposing the model and the pretty-printed payload, with no Claude Code version claimed. ([validated by `agent-log-entries.test.ts:633`](apps/web-ui/src/lib/agent-log-entries.test.ts#L655))

A gemini `message` with string content classifies by role — user-text for the injected prompt, assistant-text otherwise. ([validated by `agent-log-entries.test.ts:643`](apps/web-ui/src/lib/agent-log-entries.test.ts#L665))

Assistant prose arrives ONLY as `delta: true` chunks — gemini-cli emits no final complete message — so adjacent chunks merge into one assistant-text entry, and the fold is adjacent-only: a tool call between chunks starts a new paragraph rather than gluing across it. `mergedDelta` is the one home for the rule, shared by the blob parser and the run page's timed conversation, which keeps the merged utterance on its FIRST chunk's timestamp. ([validated by `agent-log-entries.test.ts:682`](apps/web-ui/src/lib/agent-log-entries.test.ts#L704), [`agent-log-entries.test.ts:697`](apps/web-ui/src/lib/agent-log-entries.test.ts#L719), [`agent-log-entries.test.ts:733`](apps/web-ui/src/lib/agent-log-entries.test.ts#L755), [`turn-transcript-presenter.test.ts:363`](apps/web-ui/src/app/assembly-runs/[id]/turn-transcript-presenter.test.ts#L341))

A top-level gemini `tool_use` summarizes as `→ <tool>: <most relevant arg>` from `tool_name` and `parameters`, through the same summary rule the claude blocks use. ([validated by `agent-log-entries.test.ts:652`](apps/web-ui/src/lib/agent-log-entries.test.ts#L674))

A top-level gemini `tool_result` yields its `output` — or its `error.message` when there is no output — with `status: "error"` mapped to the error flag; `status` is the CLI's verdict on the dispatch, so a failed command under `status: "success"` stays non-error, exactly as the stream reports it. ([validated by `agent-log-entries.test.ts:662`](apps/web-ui/src/lib/agent-log-entries.test.ts#L684), [`agent-log-entries.test.ts:672`](apps/web-ui/src/lib/agent-log-entries.test.ts#L694))

Gemini's standalone `error` event — a shape claude does not have; claude carries errors inside its result line — becomes an agent-error entry styled by severity: an `error` renders with the error style, a `warning` dimmed. ([validated by `agent-log-entries.test.ts:711`](apps/web-ui/src/lib/agent-log-entries.test.ts#L733), [`LogEntriesView.test.tsx:304`](apps/web-ui/src/components/LogEntriesView.test.tsx#L306), [`LogEntriesView.test.tsx:316`](apps/web-ui/src/components/LogEntriesView.test.tsx#L318))

A gemini `result` keyed by `status` folds into the existing result entry: `status: "error"` sets the error flag and the entry text falls back to `error.message`, since gemini's terminal line carries no `result` string. ([validated by `agent-log-entries.test.ts:721`](apps/web-ui/src/lib/agent-log-entries.test.ts#L743), [`agent-log-entries.test.ts:727`](apps/web-ui/src/lib/agent-log-entries.test.ts#L749))
...
A turn whose content blocks are all kinds the parser knows, but which carry nothing to draw — a thinking block redacted to an empty string is the common one — renders as nothing at all; the raw envelope is kept for the turn whose shape the parser cannot read, so a viewer never dumps JSON for a line it understood. ([validated by [`agent-log-entries.test.ts:270`](apps/web-ui/src/lib/agent-log-entries.test.ts#L270), [`agent-log-entries.test.ts:284`](apps/web-ui/src/lib/agent-log-entries.test.ts#L284), [`agent-log-entries.test.ts:295`](apps/web-ui/src/lib/agent-log-entries.test.ts#L295)) ([validated by [`agent-log-entries.test.ts:270`](apps/web-ui/src/lib/agent-log-entries.test.ts#L270), [`agent-log-entries.test.ts:284`](apps/web-ui/src/lib/agent-log-entries.test.ts#L284), [`agent-log-entries.test.ts:295`](apps/web-ui/src/lib/agent-log-entries.test.ts#L295))

formatTokens abbreviates counts of a thousand or more with a `k` suffix, formatDuration renders the two largest time units, and clip flattens whitespace and truncates with an ellipsis. ([validated by `agent-log-entries.test.ts:263`](apps/web-ui/src/lib/agent-log-entries.test.ts#L284), [validated by `agent-log-entries.test.ts:271`](apps/web-ui/src/lib/agent-log-entries.test.ts#L312), [validated by `agent-log-entries.test.ts:279`](apps/web-ui/src/lib/agent-log-entries.test.ts#L320))
...
The session-init entry renders as a collapsed details block whose summary names the model and Claude Code version, omitting the version parenthetical when the entry carries none. ([validated by `LogEntriesView.test.tsx:68`](apps/web-ui/src/components/LogEntriesView.test.tsx#L76), [`LogEntriesView.test.tsx:90`](apps/web-ui/src/components/LogEntriesView.test.tsx#L90))
