...
The run page's log viewers — the per-node assembly-line pod-log panels and the full transcript panel — parse an NDJSON stream line by line and render a human-readable transcript instead of the raw JSON blob, with a line that cannot be parsed as JSON shown raw, and a Raw/Formatted toggle (formatted by default) preserving access to the underlying stream (the pod's verbatim bytes for the pod-log panels; the stored turn envelopes for the transcript panel); the parser is grounded in real production lines captured from a live review node (`agent-log-entries.fixtures.ts`). ([validated by `agent-log-entries.test.ts:237`](apps/web-ui/src/lib/agent-log-entries.test.ts#L245))
...
A hook renders as one dim line carrying its verdict — `✓ (exit 0)`, `✗ (exit 2)`, or `running…` while it is still going; a finished hook that reported no exit code is judged on its outcome alone, so `success` reads as `✓` rather than being marked failed by the absent code — with any output kept behind the summary and a non-zero exit styled as an error. ([validated by [`LogEntriesView.test.tsx:209`](apps/web-ui/src/components/LogEntriesView.test.tsx#L209), [`LogEntriesView.test.tsx:217`](apps/web-ui/src/components/LogEntriesView.test.tsx#L217), [`LogEntriesView.test.tsx:225`](apps/web-ui/src/components/LogEntriesView.test.tsx#L225), [`LogEntriesView.test.tsx:233`](apps/web-ui/src/components/LogEntriesView.test.tsx#L233), [`LogEntriesView.test.tsx:241`](apps/web-ui/src/components/LogEntriesView.test.tsx#L262), [`LogEntriesView.test.tsx:241`](apps/web-ui/src/components/LogEntriesView.test.tsx#L241))
