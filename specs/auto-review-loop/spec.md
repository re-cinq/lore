...
- A code-review-recheck line opts out of the branch-overlap guard, so a push landing while a review or reply line still holds the PR branch is not silently dropped as `lease_held` (the verdict update always runs). ([validated by does not re-run the inherited node](apps/floor/src/work/assembly-run/advance-line.test.ts#L915))
