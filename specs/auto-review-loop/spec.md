...
- A code-review-recheck line opts out of the branch-overlap guard, so a push landing while a review or reply line still holds the PR branch is not silently dropped as `lease_held` (the verdict update always runs). ([validated by does not re-run the inherited node](apps/floor/src/work/assembly-run/advance-line.test.ts#L870))
...
- *(added 2026-09-24)* The deep review reads the change as its user before it reads
  ...
  ([validated by](libs/shared/src/outbound/project/agents/agent-defaults-content.test.ts#L324))
- Lint, types, formatting and tests are CI's verdict, read through
  ...
  install. ([validated by](libs/shared/src/outbound/project/agents/agent-defaults-content.test.ts#L334))
- The diff is read once, in place, never dumped to a file and read back.
  ([validated by](libs/shared/src/outbound/project/agents/agent-defaults-content.test.ts#L344))
- Context is queried with the PR title, the spec sections it touches and the surface
  ...
  overview. ([validated by](libs/shared/src/outbound/project/agents/agent-defaults-content.test.ts#L350))
- `changes_requested` is for a defect in the changed code or a mismatch between what
  ...
  never blocks. ([validated by](libs/shared/src/outbound/project/agents/agent-defaults-content.test.ts#L359))
