...
bullet under a `## Ready tasks` heading. ([validated by `lore_ready_tasks renders one bullet per ready task`](apps/mcp-server/src/mcp/tools/pipeline-tools.test.ts#L502))

An empty ready set renders the "No ready tasks" message rather than an empty
list. ([validated by `lore_ready_tasks reports an empty ready set`](apps/mcp-server/src/mcp/tools/pipeline-tools.test.ts#L523))
