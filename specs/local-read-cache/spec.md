...
- **Wrapper:** `withReadCache(policy, doProxy, opts)` in `libs/server-core/src/outbound/proxy.ts` —
  ...
  wrapped in a HIT/STALE marker. ([validated by `bypasses the cache entirely when no policy is given`](libs/server-core/src/outbound/proxy.test.ts#L273), [validated by `stores a successful proxied read and serves it as a fresh hit next time`](libs/server-core/src/outbound/proxy.test.ts#L285), [validated by `does not store when cacheIf rejects the body`](libs/server-core/src/outbound/proxy.test.ts#L303), [validated by `serves a labeled stale copy when the backend is unreachable and a cached entry exists`](libs/server-core/src/outbound/proxy.test.ts#L322), [validated by `returns the unreachable result unchanged when no stale copy exists`](libs/server-core/src/outbound/proxy.test.ts#L342), [validated by `passes a denied result straight through without serving any cached copy`](libs/server-core/src/outbound/proxy.test.ts#L356), [validated by `omits the HIT/STALE label when opts.label is false`](libs/server-core/src/outbound/proxy.test.ts#L376), [validated by `omits the STALE label on a stale-copy serve when opts.label is false`](libs/server-core/src/outbound/proxy.test.ts#L387))
...
Only `unreachable` may fall back to a stale cached copy. ([validated by `returns not_configured when LORE_API_URL is unset`](libs/server-core/src/outbound/proxy.test.ts#L34), [`proxy.test.ts:42`](libs/server-core/src/outbound/proxy.test.ts#L42), [`proxy.test.ts:75`](libs/server-core/src/outbound/proxy.test.ts#L75), [`proxy.test.ts:90`](libs/server-core/src/outbound/proxy.test.ts#L90), [`proxy.test.ts:105`](libs/server-core/src/outbound/proxy.test.ts#L105), [validated by `folds the server error body into the detail on a non-retriable 4xx`](libs/server-core/src/outbound/proxy.test.ts#L120))

On a 2xx response the proxy returns `ok` with the upstream body serialized to a string, and forms every upstream call as a bearer-authenticated request to the configured `LORE_API_URL` endpoint. ([validated by `returns ok with the serialized body on 200`](libs/server-core/src/outbound/proxy.test.ts#L50), [validated by `forwards the bearer token and endpoint on a POST`](libs/server-core/src/outbound/proxy.test.ts#L61))

A retriable status (408/429/5xx) retries with backoff and succeeds on a later attempt, or exhausts the retry budget and reports the last detail; a thrown fetch error (timeout or otherwise) is likewise retried, with the timeout named distinctly from a generic error message. ([validated by `retries a retriable 503 and succeeds on the next attempt`](libs/server-core/src/outbound/proxy.test.ts#L159), [validated by `exhausts every retry on a persistently retriable status and reports the last detail`](libs/server-core/src/outbound/proxy.test.ts#L178), [validated by `treats a thrown timeout error as retriable and reports the timeout message`](libs/server-core/src/outbound/proxy.test.ts#L199), [validated by `uses a thrown Error's own message when it is not a timeout`](libs/server-core/src/outbound/proxy.test.ts#L218), [validated by `stringifies a non-Error thrown value when it carries no message`](libs/server-core/src/outbound/proxy.test.ts#L234))
...
11a. **AC11a** A non-retriable 4xx carries its status and raw body back to the
    ...
    outage instead of being reported as "unreachable, retry". ([validated by `carries the status and raw body of a non-retriable 4xx`](libs/server-core/src/outbound/proxy.test.ts#L137))
