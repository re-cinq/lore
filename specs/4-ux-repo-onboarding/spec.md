...
- FR-6.6: The Spend page (`/spend`) is Lore-computed-first (no admin key
  ...
  key is configured, so the page is complete without one, and that card names
  where its figures came from — a live read through the Floor or the last
  nightly sync — omitting the label when no source is given. Because Anthropic's cost report
