...
3. Lore enrols the repo: the `lore`/`lore:*` dispatch labels and the
   priority taxonomy, the GitHub webhook, the ingest callback (repo
   variable `LORE_INGEST_URL` + secret `LORE_INGEST_TOKEN`), and the
   verbatim scaffolding committed on the task branch —
   `.github/workflows/lore-ingest.yml`, `.github/workflows/lore-trace-impact.yml`,
   `.claude/settings.json` and `.github/ISSUE_TEMPLATE/*.yml`.
4. The onboarding ticket — the task's GitHub Issue on the target repo —
   names every repo-specific file still owed with the prompt it is written
   from (`AGENTS.md`, the PR template, `pr-description-check.yml`,
   `.specify/spec.md`, the starter ADRs, `.lore/test-commands.yml` and
   `lore-tests.yml` when absent), and the `onboard` assembly line implements
   it: author → validate → push (opens THE pull request) → review.
5. User sees the PR link in the UI.
6. Repo owner reviews and merges the PR.
7. The merge pushes the scaffolding to the default branch, so the repo's
   own `lore-ingest.yml` and `lore-tests.yml` run on the merge commit and
   its content is ingested — no follow-up task.
8. Repo appears in the Lore dashboard.

**Acceptance Criteria:**
- One-click onboarding from the UI.
- ONE pull request per onboarding: opened by the line's push node via the
  GitHub App (lore-agent bot); the dashboard offers no fix-workflow PR
  while the onboarding PR is unmerged, and no initial-ingestion task follows
  the merge.
- PR contains all required files with sensible defaults.
- After merge, repo content is ingested by the merged workflows' own run.
...
The system MUST onboard new repos by creating ONE pull request *(amended
2026-09-17)*: the Floor enrols the repo and commits the verbatim scaffolding
on the task branch, then hands the onboarding ticket to the `onboard`
assembly line, whose push node opens the PR. The previous shape — an
in-process handler drafting each file with one LLM call from a pre-fetched
top-level tree, opening the PR itself, plus a dashboard fix-workflow PR and a
post-merge ingestion task — could leave three PRs open for one onboarding. ([validated by hands the ticket to the onboard assembly line on the scaffolded branch, off the repo's default branch, after the scaffold landed](apps/floor/src/work/task/worker.onboard.test.ts#L201), [validated by opens no pull request itself and calls no model: the line's push node opens the one PR](apps/floor/src/work/task/worker.onboard.test.ts#L220))
...
- FR-2.2: Lore creates the task branch off the repo's real default branch
  before committing to it, and only when it does not exist yet. ([validated by creates the branch off the default branch and commits the ingest workflow onto it even when the repo already has a .github directory](apps/floor/src/work/task/worker.onboard.test.ts#L122))
- FR-2.3: The Floor brings the deterministic scaffolding on that branch to
  today's requirements itself, by one rule for a first onboarding and an
  update alike: a LORE-owned file — the ingest and spec-impact workflows, the
  three `lore-*.yml` issue templates — is committed whenever the branch's copy
  differs from the canonical content, and a REPO-owned one
  (`.claude/settings.json`, `ISSUE_TEMPLATE/config.yml`) only when absent, so a
  repo's own settings are never overwritten. A file already at its canonical
  content is never committed: GitHub's contents API makes a commit even for
  identical bytes, and an up-to-date repo would get an empty pull request.
  Paths are read exactly, so a repo with a `.github/` directory still receives
  its issue templates (#1201). An LLM never retypes these. ([validated by creates the branch off the default branch and commits the ingest workflow onto it even when the repo already has a .github directory](apps/floor/src/work/task/worker.onboard.test.ts#L122), [validated by commits the issue templates and .claude/settings.json when the branch lacks them, leaving a config.yml the repo already has](apps/floor/src/work/task/worker.onboard.test.ts#L138), [validated by refreshes a stale ingest workflow and a drifted lore issue template, and commits nothing for a file already at its canonical content](apps/floor/src/work/task/worker.onboard.test.ts#L157), [validated by owns the three lore issue templates as lore files and leaves .claude/settings.json and config.yml to the repo](libs/shared/src/work/onboard-content.test.ts#L147))
...
- FR-2.4b: *(amended 2026-09-17)* The fix-ingest and fix-trace-impact
  actions are offered only for repos whose onboarding PR has merged: a repo
  still being onboarded reads as "missing" because its scaffolding sits on
...
- FR-2.7: When enrolment leaves gaps — a file that could not be committed,
  ...
  before its PR exists; a commit rejected for the missing Workflows App
  permission (classified by the shared failure detector across both GitHub
  ...
  task with no ticket still audits and dispatches. ([validated by comments the files that could not be committed on the ticket, naming the missing Workflows App permission](apps/floor/src/work/task/worker.onboard.test.ts#L227), [validated by keeps the permission hint out when a workflow commit fails for another reason](apps/floor/src/work/task/worker.onboard.test.ts#L242), [validated by records failed onboarding files in the audit log as onboard_files_failed](apps/floor/src/work/task/worker.onboard.test.ts#L317), [validated by still audits and dispatches an enrolment gap when the task has no ticket to comment on](apps/floor/src/work/task/worker.onboard.test.ts#L307))
- FR-2.8: The ingest callback config (repo variable `LORE_INGEST_URL`,
  ...
  instead of being written as an empty variable, and a rejected secret
  write is reported with its error. ([validated by configures the ingest variable and secret before dispatching the line](apps/floor/src/work/task/worker.onboard.test.ts#L262), [validated by reports an unconfigured ingest URL and token on the ticket instead of writing an empty variable](apps/floor/src/work/task/worker.onboard.test.ts#L278), [validated by reports a rejected ingest-secret write on the ticket](apps/floor/src/work/task/worker.onboard.test.ts#L292))
- FR-2.9: An onboard task ALWAYS walks the `onboard` assembly line, dark
  mode or not — only the line's push node opens the PR, so a single-Agent
  dispatch would never produce one; every other task type keeps its rule
  (dark mode or a feature-lifecycle type walks the line, otherwise a single
  Agent). The push node records the opened PR as the repo's onboarding PR,
  which the onboard guard blocks on and the merge-check station flips to
  merged; a feature-planning line's PR records nothing there. ([validated by walks the onboard line for an onboard task with dark mode off](apps/floor/src/work/task/dispatch-agent-cr.test.ts#L8), [validated by dispatches an implementation task as a single agent with dark mode off](apps/floor/src/work/task/dispatch-agent-cr.test.ts#L18), [validated by walks the implementation line for an implementation task with dark mode on](apps/floor/src/work/task/dispatch-agent-cr.test.ts#L28), [validated by walks the feature-planning line for a feature-lifecycle task whatever dark mode says](apps/floor/src/work/task/dispatch-agent-cr.test.ts#L38), [validated by records an onboard line's PR as the repo's onboarding PR](apps/floor/src/work/assembly-run/spec-pr.test.ts#L202), [validated by records nothing on the repo row for a feature-planning line's PR](apps/floor/src/work/assembly-run/spec-pr.test.ts#L218), [validated by decides the record by the onboard blueprint name alone](apps/floor/src/work/assembly-run/spec-pr.test.ts#L234))
- FR-2.10: The onboarding TICKET is the whole spec the line's agent gets:
  it names the repo, lists the deterministic scaffolding already on the
  branch so it is not rewritten, owes each repo-specific file with the
  prompt it is written from — the starter ADRs only for a repo with no
  `adrs/` or `docs/`, the test-command manifest and `lore-tests.yml` only
  when absent — and states the rules: existing files stay untouched, no
  `CLAUDE.md`, no code changes, and Lore opens the pull request. The API
  route sends that body as the task description, which becomes the Issue. ([validated by names re-cinq/app as the repository being onboarded](libs/shared/src/work/onboard-content.test.ts#L90), [validated by owes every LLM-drafted path with its prompt](libs/shared/src/work/onboard-content.test.ts#L94), [validated by owes the starter ADRs only for a repo with no adrs/ or docs/ directory](libs/shared/src/work/onboard-content.test.ts#L102), [validated by owes the test-command manifest and lore-tests.yml with their own instructions, only when absent](libs/shared/src/work/onboard-content.test.ts#L113), [validated by says the deterministic scaffolding is already on the branch and must not be rewritten](libs/shared/src/work/onboard-content.test.ts#L124), [validated by forbids CLAUDE.md, existing-file rewrites and code changes, and leaves the pull request to Lore](libs/shared/src/work/onboard-content.test.ts#L134), [validated by sends the onboarding ticket body instead of the bare repo name](apps/lore-api/src/work/repo/repo-onboard.test.ts#L133))
- FR-2.11: *(added 2026-09-17)* Lore does not monitor a repo's setup; its
  owner triggers it by hand. The enrollment box carries ONE button, and only
  one: a missing file's check says what is missing but offers no action of
  its own, because a button per missing file read as one PR per file when
  every one of them queued the same run. That button is a standing trigger
  whose label follows the repo's state: **Open enrolment PR** while the repo
  has no onboarding PR, **Update Lore setup** once that PR has merged —
  standing because a setup that is complete but out of date fails no check,
  so no per-check action would offer it — and nothing while the PR is open,
  where the guard refuses a second run and the row's "review & merge" link is
  the action. The API's and the MCP tool's `reonboard` reach the same run,
  and the TICKET is chosen by the repo's state, never by that flag: the repo
  page always sends it, so a first enrolment raised from the box still gets
  the onboarding ticket. That run is the
  `onboard` assembly line fed the UPDATE ticket: the Floor refreshes Lore's
  own files under FR-2.3, and the agent adds every file a first onboarding
  owes that is missing and realigns only the two workflows written from a Lore
  instruction (`lore-tests.yml`, `pr-description-check.yml`), leaving
  everything else the repository has edited alone. It opens ONE pull request,
  or none: when the Floor had nothing to refresh and the agent nothing to add,
  the line COMPLETES saying the setup is already current, where any other
  line whose push delivered nothing still errors. ([validated by shows exactly one button for a repo missing two files: Open enrolment PR, never one per file](apps/web-ui/src/components/EnrollmentSection.test.tsx#L40), [validated by carries no per-file PR action on a missing github file: the box's one trigger covers every missing file](apps/web-ui/src/lib/enrollment.test.ts#L202), [validated by offers Open enrolment PR in the box for a repo with no onboarding PR, and runs the handler on click](apps/web-ui/src/components/EnrollmentSection.test.tsx#L65), [validated by offers Update Lore setup once the onboarding PR has merged, even with every check passing](apps/web-ui/src/components/EnrollmentSection.test.tsx#L79), [validated by offers neither trigger while the onboarding PR is open, nor without a handler](apps/web-ui/src/components/EnrollmentSection.test.tsx#L99), [validated by reads Open enrolment PR for a repo with no onboarding PR yet](apps/web-ui/src/lib/enrollment.test.ts#L461), [validated by reads Update Lore setup once the onboarding PR has merged](apps/web-ui/src/lib/enrollment.test.ts#L469), [validated by offers nothing while the onboarding PR is still open, where review & merge is the action](apps/web-ui/src/lib/enrollment.test.ts#L473), [validated by sends the update ticket, not the onboarding one, when o/r's onboarding PR has already merged](apps/lore-api/src/work/repo/repo-onboard.test.ts#L232), [validated by sends the onboarding ticket when reonboard is requested for o/r but its onboarding PR never merged](apps/lore-api/src/work/repo/repo-onboard.test.ts#L252), [validated by asks to update re-cinq/app's Lore setup rather than onboard it](libs/shared/src/work/onboard-content.test.ts#L167), [validated by owes every file a first onboarding owes, each only when missing](libs/shared/src/work/onboard-content.test.ts#L174), [validated by realigns only lore-tests.yml and pr-description-check.yml, the workflows written from a Lore instruction](libs/shared/src/work/onboard-content.test.ts#L185), [validated by calls an already-current setup a success, not a failure](libs/shared/src/work/onboard-content.test.ts#L193), [validated by commits nothing at all and still dispatches the line when every deterministic file is current](apps/floor/src/work/task/worker.onboard.test.ts#L182), [validated by completes an onboard line whose push delivered nothing, saying the setup is already current](apps/floor/src/work/assembly-run/spec-pr.test.ts#L243), [validated by errors a feature-planning line whose push delivered nothing, naming its branch](apps/floor/src/work/assembly-run/spec-pr.test.ts#L256))
...
- FR-3.3: Overview tab shows recent tasks (PR + pipeline links,
  ...
  repo's Dark Factory mode. ([validated by `RepoOverviewView.test.tsx:94`](apps/web-ui/src/app/repos/[owner]/[repo]/RepoOverviewView.test.tsx#L90), [`RepoOverviewView.test.tsx:106`](apps/web-ui/src/app/repos/[owner]/[repo]/RepoOverviewView.test.tsx#L105), [`RepoOverviewView.test.tsx:147`](apps/web-ui/src/app/repos/[owner]/[repo]/RepoOverviewView.test.tsx#L146), [`RepoOverviewView.test.tsx:180`](apps/web-ui/src/app/repos/[owner]/[repo]/RepoOverviewView.test.tsx#L179), [`RepoOverviewView.test.tsx:192`](apps/web-ui/src/app/repos/[owner]/[repo]/RepoOverviewView.test.tsx#L191), [`RepoOverviewView.test.tsx:202`](apps/web-ui/src/app/repos/[owner]/[repo]/RepoOverviewView.test.tsx#L201), [`RepoOverviewView.test.tsx:231`](apps/web-ui/src/app/repos/[owner]/[repo]/RepoOverviewView.test.tsx#L230), [`EnrollmentSection.test.tsx:32`](apps/web-ui/src/components/EnrollmentSection.test.tsx#L32), [`EnrollmentSection.test.tsx:48`](apps/web-ui/src/components/EnrollmentSection.test.tsx#L48), [`EnrollmentSection.test.tsx:56`](apps/web-ui/src/components/EnrollmentSection.test.tsx#L120), [`EnrollmentSection.test.tsx:83`](apps/web-ui/src/components/EnrollmentSection.test.tsx#L147), [`ReonboardButton.test.tsx:7`](apps/web-ui/src/components/ReonboardButton.test.tsx#L7), [`ReonboardButton.test.tsx:21`](apps/web-ui/src/components/ReonboardButton.test.tsx#L17))
- FR-3.3a: Overview renders the repo README (or omits it when absent),
  ...
  and HTML base URLs. ([validated by `ReadmeBox.test.tsx:20`](apps/web-ui/src/app/repos/[owner]/[repo]/ReadmeBox.test.tsx#L18), [`ReadmeBox.test.tsx:30`](apps/web-ui/src/app/repos/[owner]/[repo]/ReadmeBox.test.tsx#L30), [`ReadmeBox.test.tsx:42`](apps/web-ui/src/app/repos/[owner]/[repo]/ReadmeBox.test.tsx#L42), [`ReadmeBox.test.tsx:54`](apps/web-ui/src/app/repos/[owner]/[repo]/ReadmeBox.test.tsx#L54), [`ReadmeBox.test.tsx:72`](apps/web-ui/src/app/repos/[owner]/[repo]/ReadmeBox.test.tsx#L72), [`ReadmeBox.test.tsx:92`](apps/web-ui/src/app/repos/[owner]/[repo]/ReadmeBox.test.tsx#L92), [`ReadmeBox.test.tsx:114`](apps/web-ui/src/app/repos/[owner]/[repo]/ReadmeBox.test.tsx#L114), [`ReadmeBox.test.tsx:126`](apps/web-ui/src/app/repos/[owner]/[repo]/ReadmeBox.test.tsx#L126), [`ReadmeBox.test.tsx:139`](apps/web-ui/src/app/repos/[owner]/[repo]/ReadmeBox.test.tsx#L139), [`ReadmeBox.test.tsx:150`](apps/web-ui/src/app/repos/[owner]/[repo]/ReadmeBox.test.tsx#L150), [`ReadmeBox.test.tsx:162`](apps/web-ui/src/app/repos/[owner]/[repo]/ReadmeBox.test.tsx#L162), [`ReadmeBox.test.tsx:171`](apps/web-ui/src/app/repos/[owner]/[repo]/ReadmeBox.test.tsx#L171), [`ReadmeBox.test.tsx:249`](apps/web-ui/src/app/repos/[owner]/[repo]/ReadmeBox.test.tsx#L249), [`readme-markdown.test.ts:7`](apps/web-ui/src/app/repos/[owner]/[repo]/readme-markdown.test.ts#L7), [`readme-markdown.test.ts:13`](apps/web-ui/src/app/repos/[owner]/[repo]/readme-markdown.test.ts#L13), [`readme-markdown.test.ts:19`](apps/web-ui/src/app/repos/[owner]/[repo]/readme-markdown.test.ts#L19), [`readme-markdown.test.ts:23`](apps/web-ui/src/app/repos/[owner]/[repo]/readme-markdown.test.ts#L23), [`readme-markdown.test.ts:27`](apps/web-ui/src/app/repos/[owner]/[repo]/readme-markdown.test.ts#L27), [`readme-markdown.test.ts:31`](apps/web-ui/src/app/repos/[owner]/[repo]/readme-markdown.test.ts#L31), [`readme-markdown.test.ts:37`](apps/web-ui/src/app/repos/[owner]/[repo]/readme-markdown.test.ts#L37), [`readme-markdown.test.ts:43`](apps/web-ui/src/app/repos/[owner]/[repo]/readme-markdown.test.ts#L43), [`readme-markdown.test.ts:51`](apps/web-ui/src/app/repos/[owner]/[repo]/readme-markdown.test.ts#L51), [`readme-markdown.test.ts:55`](apps/web-ui/src/app/repos/[owner]/[repo]/readme-markdown.test.ts#L55), [`readme-markdown.test.ts:59`](apps/web-ui/src/app/repos/[owner]/[repo]/readme-markdown.test.ts#L59), [`RepoOverviewView.test.tsx:69`](apps/web-ui/src/app/repos/[owner]/[repo]/RepoOverviewView.test.tsx#L68), [`RepoOverviewView.test.tsx:85`](apps/web-ui/src/app/repos/[owner]/[repo]/RepoOverviewView.test.tsx#L84))
- FR-3.3c: The README markdown is sanitized after raw-HTML parsing: an
  ...
  ([validated by `ReadmeBox.test.tsx:188`](apps/web-ui/src/app/repos/[owner]/[repo]/ReadmeBox.test.tsx#L182), [`ReadmeBox.test.tsx:194`](apps/web-ui/src/app/repos/[owner]/[repo]/ReadmeBox.test.tsx#L194), [`ReadmeBox.test.tsx:207`](apps/web-ui/src/app/repos/[owner]/[repo]/ReadmeBox.test.tsx#L207), [`ReadmeBox.test.tsx:218`](apps/web-ui/src/app/repos/[owner]/[repo]/ReadmeBox.test.tsx#L218), [`ReadmeBox.test.tsx:229`](apps/web-ui/src/app/repos/[owner]/[repo]/ReadmeBox.test.tsx#L229), [`ReadmeBox.test.tsx:238`](apps/web-ui/src/app/repos/[owner]/[repo]/ReadmeBox.test.tsx#L238), [`ReadmeBox.test.tsx:258`](apps/web-ui/src/app/repos/[owner]/[repo]/ReadmeBox.test.tsx#L258), [`readme-markdown.test.ts:65`](apps/web-ui/src/app/repos/[owner]/[repo]/readme-markdown.test.ts#L65), [`readme-markdown.test.ts:69`](apps/web-ui/src/app/repos/[owner]/[repo]/readme-markdown.test.ts#L69))
- FR-3.3b: PR status pills render the PR state text (nothing when the
  ...
  resolve after unmount. ([validated by `PRStatusBadge.test.tsx:7`](apps/web-ui/src/app/tasks/PRStatusBadge.test.tsx#L7), [`PRStatusBadge.test.tsx:14`](apps/web-ui/src/app/tasks/PRStatusBadge.test.tsx#L14), [`PRStatusBadge.test.tsx:20`](apps/web-ui/src/app/tasks/PRStatusBadge.test.tsx#L20), [`PRStatusBadge.test.tsx:47`](apps/web-ui/src/app/tasks/PRStatusBadge.test.tsx#L47), [validated by `maps the %s status to its pill color`](apps/web-ui/src/app/tasks/PRStatusBadge.test.tsx#L38), [`PRStatusBadgePanel.test.tsx:40`](apps/web-ui/src/app/tasks/PRStatusBadgePanel.test.tsx#L40), [`PRStatusBadgePanel.test.tsx:48`](apps/web-ui/src/app/tasks/PRStatusBadgePanel.test.tsx#L48), [`PRStatusBadgePanel.test.tsx:62`](apps/web-ui/src/app/tasks/PRStatusBadgePanel.test.tsx#L62), [`PRStatusBadgePanel.test.tsx:71`](apps/web-ui/src/app/tasks/PRStatusBadgePanel.test.tsx#L71), [`PRStatusBadgePanel.test.tsx:79`](apps/web-ui/src/app/tasks/PRStatusBadgePanel.test.tsx#L79), [`PRStatusBadgePanel.test.tsx:90`](apps/web-ui/src/app/tasks/PRStatusBadgePanel.test.tsx#L90), [`PRStatusBadgePanel.test.tsx:110`](apps/web-ui/src/app/tasks/PRStatusBadgePanel.test.tsx#L110))
- FR-3.4: Tasks (Assembly Lines) tab shows pipeline runs filtered to
  ...
  and a not-found state with a back link. ([validated by `RepoTasksView.test.tsx:54`](apps/web-ui/src/app/repos/[owner]/[repo]/tasks/RepoTasksView.test.tsx#L57), [`RepoTasksView.test.tsx:35`](apps/web-ui/src/app/repos/[owner]/[repo]/tasks/RepoTasksView.test.tsx#L37), [`RepoTasksView.test.tsx:50`](apps/web-ui/src/app/repos/[owner]/[repo]/tasks/RepoTasksView.test.tsx#L52), [`JobRunView.test.tsx:40`](apps/web-ui/src/app/job-runs/[id]/JobRunView.test.tsx#L40), [`JobRunView.test.tsx:52`](apps/web-ui/src/app/job-runs/[id]/JobRunView.test.tsx#L52), [`JobRunView.test.tsx:70`](apps/web-ui/src/app/job-runs/[id]/JobRunView.test.tsx#L70), [`JobRunView.test.tsx:78`](apps/web-ui/src/app/job-runs/[id]/JobRunView.test.tsx#L78), [`JobRunView.test.tsx:89`](apps/web-ui/src/app/job-runs/[id]/JobRunView.test.tsx#L89), [`JobRunView.test.tsx:97`](apps/web-ui/src/app/job-runs/[id]/JobRunView.test.tsx#L97), [`JobRunView.test.tsx:106`](apps/web-ui/src/app/job-runs/[id]/JobRunView.test.tsx#L106))
...
- FR-3.7: Agents tab shows agent definitions scoped to this repo. ([validated by `AgentList.test.tsx:21`](apps/web-ui/src/app/repos/[owner]/[repo]/agents/AgentList.test.tsx#L52))
...
- FR-3.10: Assembled tab previews assembled context — a template
  ...
  cleared on the next successful assemble. ([validated by `AssembledContextView.test.tsx:84`](apps/web-ui/src/app/repos/[owner]/[repo]/assembled/AssembledContextView.test.tsx#L84), [`AssembledContextView.test.tsx:94`](apps/web-ui/src/app/repos/[owner]/[repo]/assembled/AssembledContextView.test.tsx#L94), [`AssembledContextView.test.tsx:102`](apps/web-ui/src/app/repos/[owner]/[repo]/assembled/AssembledContextView.test.tsx#L102), [`AssembledContextView.test.tsx:121`](apps/web-ui/src/app/repos/[owner]/[repo]/assembled/AssembledContextView.test.tsx#L121), [`AssembledContextView.test.tsx:147`](apps/web-ui/src/app/repos/[owner]/[repo]/assembled/AssembledContextView.test.tsx#L147), [`AssembledContextView.test.tsx:165`](apps/web-ui/src/app/repos/[owner]/[repo]/assembled/AssembledContextView.test.tsx#L165), [`AssembledContextView.test.tsx:191`](apps/web-ui/src/app/repos/[owner]/[repo]/assembled/AssembledContextView.test.tsx#L191), [`AssembledContextView.test.tsx:201`](apps/web-ui/src/app/repos/[owner]/[repo]/assembled/AssembledContextView.test.tsx#L201), [`AssembledContextPanel.test.tsx:79`](apps/web-ui/src/app/repos/[owner]/[repo]/assembled/AssembledContextPanel.test.tsx#L79), [`AssembledContextPanel.test.tsx:86`](apps/web-ui/src/app/repos/[owner]/[repo]/assembled/AssembledContextPanel.test.tsx#L86), [`AssembledContextPanel.test.tsx:105`](apps/web-ui/src/app/repos/[owner]/[repo]/assembled/AssembledContextPanel.test.tsx#L105), [`AssembledContextPanel.test.tsx:122`](apps/web-ui/src/app/repos/[owner]/[repo]/assembled/AssembledContextPanel.test.tsx#L122), [`AssembledContextPanel.test.tsx:135`](apps/web-ui/src/app/repos/[owner]/[repo]/assembled/AssembledContextPanel.test.tsx#L135), [`AssembledContextPanel.test.tsx:151`](apps/web-ui/src/app/repos/[owner]/[repo]/assembled/AssembledContextPanel.test.tsx#L151), [`lore-api-config.test.ts:20`](apps/web-ui/src/lib/lore-api-config.test.ts#L20), [`lore-api-config.test.ts:27`](apps/web-ui/src/lib/lore-api-config.test.ts#L27), [`lore-api-config.test.ts:33`](apps/web-ui/src/lib/lore-api-config.test.ts#L33))
- FR-3.12: Events tab renders a row per repo event (name + status) or
  ...
  and paging one row past the page size from the given offset; the
  infinite-scroll pager shows a sentinel row while a further page exists
  and renders nothing once no page and no appended events remain. ([validated by `EventsView.test.tsx:18`](apps/web-ui/src/app/repos/[owner]/[repo]/events/EventsView.test.tsx#L18), [`EventsView.test.tsx:46`](apps/web-ui/src/app/repos/[owner]/[repo]/events/EventsView.test.tsx#L46), [`events-data.test.ts:20`](apps/web-ui/src/app/repos/[owner]/[repo]/events/events-data.test.ts#L20), [`events-data.test.ts:32`](apps/web-ui/src/app/repos/[owner]/[repo]/events/events-data.test.ts#L32), [`events-data.test.ts:44`](apps/web-ui/src/app/repos/[owner]/[repo]/events/events-data.test.ts#L44), [`events-data.test.ts:55`](apps/web-ui/src/app/repos/[owner]/[repo]/events/events-data.test.ts#L55), [`renders a pager sentinel row when a further page exists`](apps/web-ui/src/app/repos/[owner]/[repo]/events/InfiniteEvents.test.tsx#L17), [`renders nothing when there is no further page and no events loaded yet`](apps/web-ui/src/app/repos/[owner]/[repo]/events/InfiniteEvents.test.tsx#L34))
...
- FR-3.14: The app shell renders a collapsible sidebar (every nav link
  ...
  email / avatar fallbacks and sign-out). ([validated by `SidebarNav.test.tsx:62`](apps/web-ui/src/app/SidebarNav.test.tsx#L62), [`SidebarNav.test.tsx:75`](apps/web-ui/src/app/SidebarNav.test.tsx#L75), [`SidebarNav.test.tsx:87`](apps/web-ui/src/app/SidebarNav.test.tsx#L87), [`SidebarNav.test.tsx:99`](apps/web-ui/src/app/SidebarNav.test.tsx#L99), [`SidebarNav.test.tsx:110`](apps/web-ui/src/app/SidebarNav.test.tsx#L110), [`SidebarNav.test.tsx:127`](apps/web-ui/src/app/SidebarNav.test.tsx#L127), [`SidebarNav.test.tsx:136`](apps/web-ui/src/app/SidebarNav.test.tsx#L136), [`SidebarNav.test.tsx:153`](apps/web-ui/src/app/SidebarNav.test.tsx#L153), [`SidebarNav.test.tsx:166`](apps/web-ui/src/app/SidebarNav.test.tsx#L166), [`SidebarNav.test.tsx:173`](apps/web-ui/src/app/SidebarNav.test.tsx#L173), [`SidebarNav.test.tsx:189`](apps/web-ui/src/app/SidebarNav.test.tsx#L189), [`SidebarNav.test.tsx:203`](apps/web-ui/src/app/SidebarNav.test.tsx#L203), [`SidebarNav.test.tsx:212`](apps/web-ui/src/app/SidebarNav.test.tsx#L212), [`SidebarNav.test.tsx:224`](apps/web-ui/src/app/SidebarNav.test.tsx#L224), [`SidebarNav.test.tsx:239`](apps/web-ui/src/app/SidebarNav.test.tsx#L239), [`SidebarNav.test.tsx:255`](apps/web-ui/src/app/SidebarNav.test.tsx#L255), [`SidebarNav.test.tsx:263`](apps/web-ui/src/app/SidebarNav.test.tsx#L263), [`SidebarNav.test.tsx:276`](apps/web-ui/src/app/SidebarNav.test.tsx#L276), [`SidebarNav.test.tsx:287`](apps/web-ui/src/app/SidebarNav.test.tsx#L287), [`SidebarNav.test.tsx:293`](apps/web-ui/src/app/SidebarNav.test.tsx#L293), [`SidebarNav.test.tsx:318`](apps/web-ui/src/app/SidebarNav.test.tsx#L318), [`AppShell.test.tsx:40`](apps/web-ui/src/app/AppShell.test.tsx#L40), [`AppShell.test.tsx:49`](apps/web-ui/src/app/AppShell.test.tsx#L49), [`AppShell.test.tsx:56`](apps/web-ui/src/app/AppShell.test.tsx#L56), [`AppShell.test.tsx:62`](apps/web-ui/src/app/AppShell.test.tsx#L62), [`AppShell.test.tsx:67`](apps/web-ui/src/app/AppShell.test.tsx#L67), [`AppShell.test.tsx:72`](apps/web-ui/src/app/AppShell.test.tsx#L72), [`AppShell.test.tsx:81`](apps/web-ui/src/app/AppShell.test.tsx#L81), [`AppShell.test.tsx:89`](apps/web-ui/src/app/AppShell.test.tsx#L89), [`AppShell.test.tsx:97`](apps/web-ui/src/app/AppShell.test.tsx#L97), [`AppShell.test.tsx:106`](apps/web-ui/src/app/AppShell.test.tsx#L106), [`AppShell.test.tsx:114`](apps/web-ui/src/app/AppShell.test.tsx#L114), [`AppShell.test.tsx:122`](apps/web-ui/src/app/AppShell.test.tsx#L122), [`AppShell.test.tsx:130`](apps/web-ui/src/app/AppShell.test.tsx#L130), [`AppShell.test.tsx:142`](apps/web-ui/src/app/AppShell.test.tsx#L142), [`AppShell.test.tsx:159`](apps/web-ui/src/app/AppShell.test.tsx#L159), [`AppShell.test.tsx:173`](apps/web-ui/src/app/AppShell.test.tsx#L173), [`UserMenu.test.tsx:32`](apps/web-ui/src/app/UserMenu.test.tsx#L32), [`UserMenu.test.tsx:39`](apps/web-ui/src/app/UserMenu.test.tsx#L39), [`UserMenu.test.tsx:46`](apps/web-ui/src/app/UserMenu.test.tsx#L46), [`UserMenu.test.tsx:53`](apps/web-ui/src/app/UserMenu.test.tsx#L53), [`UserMenu.test.tsx:61`](apps/web-ui/src/app/UserMenu.test.tsx#L61), [`UserMenu.test.tsx:68`](apps/web-ui/src/app/UserMenu.test.tsx#L68), [`UserMenu.test.tsx:74`](apps/web-ui/src/app/UserMenu.test.tsx#L74), [`UserMenu.test.tsx:80`](apps/web-ui/src/app/UserMenu.test.tsx#L80), [`UserMenu.test.tsx:88`](apps/web-ui/src/app/UserMenu.test.tsx#L88), [`UserMenu.test.tsx:99`](apps/web-ui/src/app/UserMenu.test.tsx#L99), [`UserMenu.test.tsx:105`](apps/web-ui/src/app/UserMenu.test.tsx#L105), [`UserMenu.test.tsx:111`](apps/web-ui/src/app/UserMenu.test.tsx#L111), [`UserMenu.test.tsx:122`](apps/web-ui/src/app/UserMenu.test.tsx#L122), [`UserMenu.test.tsx:130`](apps/web-ui/src/app/UserMenu.test.tsx#L130), [`UserMenu.test.tsx:137`](apps/web-ui/src/app/UserMenu.test.tsx#L137), [`UserMenu.test.tsx:145`](apps/web-ui/src/app/UserMenu.test.tsx#L145))
- FR-3.15: A top-level org Settings page exists — it renders the
  ...
  #1057 (issue #1049) in favor of the per-repo Assembly Lines tab. ([validated by `SettingsView.test.tsx:43`](apps/web-ui/src/app/settings/SettingsView.test.tsx#L44), [`SettingsView.test.tsx:63`](apps/web-ui/src/app/settings/SettingsView.test.tsx#L67), [`SettingsView.test.tsx:76`](apps/web-ui/src/app/settings/SettingsView.test.tsx#L80), [`SettingsView.test.tsx:89`](apps/web-ui/src/app/settings/SettingsView.test.tsx#L93), [`SettingsView.test.tsx:101`](apps/web-ui/src/app/settings/SettingsView.test.tsx#L105), [`SettingsView.test.tsx:110`](apps/web-ui/src/app/settings/SettingsView.test.tsx#L114), [`SettingsView.test.tsx:140`](apps/web-ui/src/app/settings/SettingsView.test.tsx#L144), [`SettingsView.test.tsx:162`](apps/web-ui/src/app/settings/SettingsView.test.tsx#L166), [`SettingsView.test.tsx:174`](apps/web-ui/src/app/settings/SettingsView.test.tsx#L178))
- FR-3.16: Every doc card list (specs, ADRs, the global browsers)
  ...
  search/sort controls whose card lists narrow and reorder accordingly. ([validated by `doc-filter:23`](apps/web-ui/src/lib/doc-filter.test.ts#L23), [`doc-filter:30`](apps/web-ui/src/lib/doc-filter.test.ts#L30), [`doc-filter:34`](apps/web-ui/src/lib/doc-filter.test.ts#L34), [`doc-filter:40`](apps/web-ui/src/lib/doc-filter.test.ts#L40), [`doc-filter:46`](apps/web-ui/src/lib/doc-filter.test.ts#L49), [`doc-filter:55`](apps/web-ui/src/lib/doc-filter.test.ts#L60), [`doc-filter:63`](apps/web-ui/src/lib/doc-filter.test.ts#L68), [`doc-filter:67`](apps/web-ui/src/lib/doc-filter.test.ts#L72), [`doc-filter:83`](apps/web-ui/src/lib/doc-filter.test.ts#L88), [`DocListControls:7`](apps/web-ui/src/components/DocListControls.test.tsx#L7), [`DocListControls:18`](apps/web-ui/src/components/DocListControls.test.tsx#L18), [`DocListControls:36`](apps/web-ui/src/components/DocListControls.test.tsx#L36), [`SpecListView:96`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecListView.test.tsx#L94), [`SpecListView:125`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecListView.test.tsx#L123))
...
- FR-3.18: The ADR detail page strips the leading YAML frontmatter from
  ...
  for an unrecognized status, and rendering nothing for empty meta. ([validated by `frontmatter:19`](apps/web-ui/src/lib/frontmatter.test.ts#L19), [`frontmatter:31`](apps/web-ui/src/lib/frontmatter.test.ts#L31), [`frontmatter:39`](apps/web-ui/src/lib/frontmatter.test.ts#L39), [`frontmatter:45`](apps/web-ui/src/lib/frontmatter.test.ts#L45), [`frontmatter:51`](apps/web-ui/src/lib/frontmatter.test.ts#L51), [`frontmatter:57`](apps/web-ui/src/lib/frontmatter.test.ts#L57), [`frontmatter:63`](apps/web-ui/src/lib/frontmatter.test.ts#L63), [`AdrMetaView:16`](apps/web-ui/src/app/repos/[owner]/[repo]/adrs/[...path]/AdrMetaView.test.tsx#L16), [`AdrMetaView:25`](apps/web-ui/src/app/repos/[owner]/[repo]/adrs/[...path]/AdrMetaView.test.tsx#L25), [`AdrMetaView:46`](apps/web-ui/src/app/repos/[owner]/[repo]/adrs/[...path]/AdrMetaView.test.tsx#L46), [`AdrMetaView:54`](apps/web-ui/src/app/repos/[owner]/[repo]/adrs/[...path]/AdrMetaView.test.tsx#L54))
...
The onboarding PR carries deterministic files committed verbatim by the Floor plus repo-specific files the `onboard` line's agent authors from the ticket's fixed prompts, reading the repository itself, reviewed by the owner in the PR. ([validated by creates the branch off the default branch and commits the ingest workflow onto it even when the repo already has a .github directory](apps/floor/src/work/task/worker.onboard.test.ts#L122), [validated by owes every LLM-drafted path with its prompt](libs/shared/src/work/onboard-content.test.ts#L94))
...
- FR-5.5: The ticket owes `AGENTS.md`, the PR template, the pr-description-check
  ...
  canonical sections. ([validated by LLM-drafts AGENTS.md from a non-empty prompt](libs/shared/src/work/onboard-content.test.ts#L42), [validated by PR-template prompt names the five canonical sections](libs/shared/src/work/onboard-content.test.ts#L49), [validated by LLM-drafts the pr-description-check workflow and the .specify spec](libs/shared/src/work/onboard-content.test.ts#L65))
- FR-5.6: The onboarding PR scaffolds no `CLAUDE.md` (requested in the onboarding
  ...
  triggering ride the ingest and spec-impact workflows above. ([validated by scaffolds no CLAUDE.md, requested in the onboarding issue instead](libs/shared/src/work/onboard-content.test.ts#L76), [validated by scaffolds no spec-agent.yml](libs/shared/src/work/onboard-content.test.ts#L80))
...
- FR-6.1: The Audit page (`/audit`) renders one row per audit-log entry
  ...
  preserve the active filters in their hrefs. ([validated by `AuditView.test.tsx:31`](apps/web-ui/src/app/audit/AuditView.test.tsx#L31), [`AuditView.test.tsx:65`](apps/web-ui/src/app/audit/AuditView.test.tsx#L65), [`AuditView.test.tsx:92`](apps/web-ui/src/app/audit/AuditView.test.tsx#L92), [`AuditView.test.tsx:107`](apps/web-ui/src/app/audit/AuditView.test.tsx#L107), [`AuditView.test.tsx:128`](apps/web-ui/src/app/audit/AuditView.test.tsx#L128), [`AuditView.test.tsx:145`](apps/web-ui/src/app/audit/AuditView.test.tsx#L145), [`AuditView.test.tsx:167`](apps/web-ui/src/app/audit/AuditView.test.tsx#L167), [`AuditView.test.tsx:189`](apps/web-ui/src/app/audit/AuditView.test.tsx#L189), [`AuditView.test.tsx:210`](apps/web-ui/src/app/audit/AuditView.test.tsx#L210), [`AuditView.test.tsx:229`](apps/web-ui/src/app/audit/AuditView.test.tsx#L229), [`AuditView.test.tsx:261`](apps/web-ui/src/app/audit/AuditView.test.tsx#L261))
...
- FR-6.6: The Spend page (`/spend`) is Lore-computed-first (no admin key
  required) and interval-scoped: every figure renders from the selected
  window's `/api/spend-window` fetch — there is no second, month-to-date
  data source. It renders the title and every section heading, headlines the
...
- FR-6.7: The knowledge-graph force layout (`lib/graph-layout`) seeds
  ...
  ring so trees don't overlap, and counts edge crossings. ([validated by `graph-layout.test.ts:20`](apps/web-ui/src/lib/graph-layout.test.ts#L20), [`graph-layout.test.ts:37`](apps/web-ui/src/lib/graph-layout.test.ts#L37), [`graph-layout.test.ts:58`](apps/web-ui/src/lib/graph-layout.test.ts#L58), [`graph-layout.test.ts:64`](apps/web-ui/src/lib/graph-layout.test.ts#L64), [`graph-layout.test.ts:83`](apps/web-ui/src/lib/graph-layout.test.ts#L83), [`graph-layout.test.ts:87`](apps/web-ui/src/lib/graph-layout.test.ts#L87), [`graph-layout.test.ts:98`](apps/web-ui/src/lib/graph-layout.test.ts#L98), [`graph-layout.test.ts:109`](apps/web-ui/src/lib/graph-layout.test.ts#L109), [`graph-layout.test.ts:119`](apps/web-ui/src/lib/graph-layout.test.ts#L119), [`graph-layout.test.ts:126`](apps/web-ui/src/lib/graph-layout.test.ts#L126), [`graph-layout.test.ts:133`](apps/web-ui/src/lib/graph-layout.test.ts#L133), [`graph-layout.test.ts:139`](apps/web-ui/src/lib/graph-layout.test.ts#L139), [`graph-layout.test.ts:145`](apps/web-ui/src/lib/graph-layout.test.ts#L145), [`graph-layout.test.ts:151`](apps/web-ui/src/lib/graph-layout.test.ts#L151), [`graph-layout.test.ts:162`](apps/web-ui/src/lib/graph-layout.test.ts#L162), [`graph-layout.test.ts:172`](apps/web-ui/src/lib/graph-layout.test.ts#L172), [`graph-layout.test.ts:182`](apps/web-ui/src/lib/graph-layout.test.ts#L182), [`graph-layout.test.ts:192`](apps/web-ui/src/lib/graph-layout.test.ts#L192), [`graph-layout.test.ts:202`](apps/web-ui/src/lib/graph-layout.test.ts#L202), [`graph-layout.test.ts:219`](apps/web-ui/src/lib/graph-layout.test.ts#L219), [`graph-layout.test.ts:223`](apps/web-ui/src/lib/graph-layout.test.ts#L223), [`graph-layout.test.ts:227`](apps/web-ui/src/lib/graph-layout.test.ts#L227), [`graph-layout.test.ts:235`](apps/web-ui/src/lib/graph-layout.test.ts#L235), [`graph-layout.test.ts:239`](apps/web-ui/src/lib/graph-layout.test.ts#L239), [`graph-layout.test.ts:257`](apps/web-ui/src/lib/graph-layout.test.ts#L257), [`graph-layout.test.ts:271`](apps/web-ui/src/lib/graph-layout.test.ts#L271), [`graph-layout.test.ts:285`](apps/web-ui/src/lib/graph-layout.test.ts#L285), [`graph-layout.test.ts:294`](apps/web-ui/src/lib/graph-layout.test.ts#L294), [`graph-layout.test.ts:320`](apps/web-ui/src/lib/graph-layout.test.ts#L320), [`graph-layout.test.ts:332`](apps/web-ui/src/lib/graph-layout.test.ts#L332), [`graph-layout.test.ts:346`](apps/web-ui/src/lib/graph-layout.test.ts#L346), [`graph-layout.test.ts:352`](apps/web-ui/src/lib/graph-layout.test.ts#L352), [`graph-layout.test.ts:365`](apps/web-ui/src/lib/graph-layout.test.ts#L365), [`graph-layout.test.ts:377`](apps/web-ui/src/lib/graph-layout.test.ts#L377), [`graph-layout.test.ts:396`](apps/web-ui/src/lib/graph-layout.test.ts#L396), [`graph-layout.test.ts:408`](apps/web-ui/src/lib/graph-layout.test.ts#L408))
...
- FR-6.9: The edge-bundling helpers (`lib/edge-bundling`) build a
  ...
  common ancestor, ordering the control ids source → LCA → target. ([validated by `edge-bundling.test.ts:16`](apps/web-ui/src/lib/edge-bundling.test.ts#L16), [`edge-bundling.test.ts:25`](apps/web-ui/src/lib/edge-bundling.test.ts#L25), [`edge-bundling.test.ts:34`](apps/web-ui/src/lib/edge-bundling.test.ts#L34), [`edge-bundling.test.ts:46`](apps/web-ui/src/lib/edge-bundling.test.ts#L46), [`edge-bundling.test.ts:55`](apps/web-ui/src/lib/edge-bundling.test.ts#L55), [`edge-bundling.test.ts:59`](apps/web-ui/src/lib/edge-bundling.test.ts#L59), [`edge-bundling.test.ts:77`](apps/web-ui/src/lib/edge-bundling.test.ts#L77), [`edge-bundling.test.ts:87`](apps/web-ui/src/lib/edge-bundling.test.ts#L87), [`edge-bundling.test.ts:95`](apps/web-ui/src/lib/edge-bundling.test.ts#L95))
...
- FR-7.3: `NavLink`/`NavLabel` apply the active class and `aria-current`
  ...
  while the link's navigation is pending. ([validated by `NavLink.test.tsx:16`](apps/web-ui/src/components/NavLink.test.tsx#L16), [`NavLink.test.tsx:22`](apps/web-ui/src/components/NavLink.test.tsx#L22), [`NavLink.test.tsx:30`](apps/web-ui/src/components/NavLink.test.tsx#L30), [`NavLink.test.tsx:39`](apps/web-ui/src/components/NavLink.test.tsx#L39), [`NavLink.test.tsx:47`](apps/web-ui/src/components/NavLink.test.tsx#L47))
...
- FR-7.9: When `GITHUB_ALLOWED_ORG` is set, GitHub sign-in checks the
  authenticated user's org memberships and denies sign-in when the
...
- FR-8.16: The settings page shows the GitHub section with the accounts
  Lore is connected to. ([validated by renders the GitHub section with the connected re-cinq organization](apps/web-ui/src/app/settings/SettingsView.test.tsx#L190))
