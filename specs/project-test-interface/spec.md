...
2. `tests.list` output is parsed into descriptors `{id, name, file, startLine, endLine, suite?, spec?, passed?}`; each seeds a `TestChunk` with the line range and `test_name`; the runner-native `id` is stored verbatim as the selector. The parser carries the `suite` chain outermost→innermost, drops a `suite` array holding a non-string element, and omits optional fields a descriptor does not declare. ([validated by `parses a descriptor carrying every field`](libs/shared/src/work/test-report.test.ts#L5), [validated by carries the suite chain outermost to innermost](libs/shared/src/work/test-report.test.ts#L31), [validated by omits a suite array holding a non-string element](libs/shared/src/work/test-report.test.ts#L41), [validated by omits optional fields a descriptor does not declare](libs/shared/src/work/test-report.test.ts#L49), [validated by `throws when the required %s is missing`](libs/shared/src/work/test-report.test.ts#L65))

2a. A descriptor's `suite` chain seeds one idempotent `TestSuite` per element (`xid = repo|file_path|suite_chain`), nested via `TestSuite.parent` outermost→innermost; the `TestChunk` links to the innermost via `TestChunk.suite`; a suite shared by many tests is created once. When a suite name resolves to a spec `path`/`path#ordinal` anchor, `TestSuite.spec` (`VALIDATES_SPEC`) is set; descriptors with no `suite` still seed the bare `TestChunk` (suites are optional). ([validated by `ingest-test-report.test.ts:611`](libs/shared/src/work/spec-trace/ingest-test-report.test.ts#L611), [validated by `ingest-test-report:627`](libs/shared/src/work/spec-trace/ingest-test-report.test.ts#L577))

3. When a descriptor carries `spec`, exactly one `VALIDATED_BY` edge (`evidence = generated-provenance`) is created to the single `Statement` or `AcceptanceCriterion` at that `path#ordinal` anchor. ([validated by `ingest-test-report.test.ts:269`](libs/shared/src/work/spec-trace/ingest-test-report.test.ts#L269))

4. `tests.run <id>` output `{passed, covered[]}` upserts one `Coverage` node (`xid = repo|test_file|test_name`) + `COVERS` edges to each overlapping `CodeChunk`; re-running for the same `(id, commit)` is idempotent. ([validated by `parses passed + a list of covered chunks`](libs/shared/src/work/test-report.test.ts#L80), [validated by `throws when a covered chunk is missing its line bounds`](libs/shared/src/work/test-report.test.ts#L96), [validated by `ingest-test-report:162`](libs/shared/src/work/spec-trace/ingest-test-report.test.ts#L121), [validated by `ingest-test-report:590`](libs/shared/src/work/spec-trace/ingest-test-report.test.ts#L540))

5. The binary reports each test's `passed` (json `passed`, or the run command's exit code for lcov/cobertura); a failing **validating** test then sets `violated=true` + `violation_reason` on its `Statement`/`AcceptanceCriterion` and opens/labels a `spec-violated` issue during graph projection (owned by [`spec-traceability-graph`](../spec-traceability-graph/spec.md)) — distinct from `drifted`, raised only after the flaky guard confirms. ([validated by `TestBuildReportParsesLcovRunOutput`](apps/lore-code-trace/report_test.go#L51), [validated by `ingest-test-report:392`](libs/shared/src/work/spec-trace/ingest-test-report.test.ts#L351), [validated by `ingest-test-report:434`](libs/shared/src/work/spec-trace/ingest-test-report.test.ts#L393), [validated by `ingest-test-report:469`](libs/shared/src/work/spec-trace/ingest-test-report.test.ts#L428))
