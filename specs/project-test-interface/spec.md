...
| `apps/floor/src/events/listeners/ci-tests.ts` | NEW: the `POST /api/webhook/ci-tests` ingress → `internal.ingest.spec_trace` (kind `test-report`) → graph fan-out (seeds `TestChunk` + nested `TestSuite` chain). |
...
1. The manifest (`.lore/test-commands.yml` / `lore.repos.settings.test_commands`) is parsed one entry at a time: `run` is the only required field, so an honest whole-suite entry (no per-test `list`, no `{selector}` in `run`, no `coverage_format`) is kept as a run-whole entry; `list` and `coverage_format` are optional and an unknown `coverage_format` is ignored rather than dropping the runnable entry; an entry with no usable `run` is skipped instead of throwing the whole manifest away, so one non-conforming entry never takes a valid sibling down with it; a polyglot list with per-`cwd` entries is supported; settings win over the file; an absent manifest resolves to null so the caller falls back (pattern detection + bulk upload) with no error. ([validated by `normalizes a minimal valid manifest into a one-element list with defaults`](libs/shared/src/domain/test-command-manifest.test.ts#L11), [validated by `drops an entry with no run command while keeping its valid siblings`](libs/shared/src/domain/test-command-manifest.test.ts#L29), [validated by `accepts a run command without a {selector} placeholder as run-whole`](libs/shared/src/domain/test-command-manifest.test.ts#L43), [validated by `keeps the entry with coverage_format undefined when the value is unknown`](libs/shared/src/domain/test-command-manifest.test.ts#L53), [validated by `keeps a valid vitest entry alongside an honest whole-suite entry`](libs/shared/src/domain/test-command-manifest.test.ts#L64), [validated by `normalizes a polyglot array into one entry per manifest`](libs/shared/src/domain/test-command-manifest.test.ts#L84), [validated by `preserves a provided cwd and path_prefix_strip`](libs/shared/src/domain/test-command-manifest.test.ts#L104), [validated by `prefers settings over the file when both are present`](libs/shared/src/domain/test-command-manifest.test.ts#L136), [validated by `falls back to the file when settings are absent`](libs/shared/src/domain/test-command-manifest.test.ts#L142), [validated by `returns null when neither settings nor file declare a manifest`](libs/shared/src/domain/test-command-manifest.test.ts#L132), [validated by `rejects listTests when the manifest entry has no list command`](libs/shared/src/outbound/project/test-runner/test-runner-exec.test.ts#L93), [validated by `reports the entry runs whole when it declares no list command`](apps/mcp-server/src/work/spec-trace/spec-trace-tools.test.ts#L203))

2. `tests.list` output is parsed into descriptors `{id, name, file, startLine, endLine, suite?, spec?, passed?}`; each seeds a `TestChunk` with the line range and `test_name`; the runner-native `id` is stored verbatim as the selector. The parser carries the `suite` chain outermost→innermost, drops a `suite` array holding a non-string element, and omits optional fields a descriptor does not declare. ([validated by `parses a descriptor carrying every field`](libs/shared/src/domain/test-report.test.ts#L5), [validated by carries the suite chain outermost to innermost](libs/shared/src/domain/test-report.test.ts#L31), [validated by omits a suite array holding a non-string element](libs/shared/src/domain/test-report.test.ts#L41), [validated by omits optional fields a descriptor does not declare](libs/shared/src/domain/test-report.test.ts#L49), [validated by `throws when the required %s is missing`](libs/shared/src/domain/test-report.test.ts#L65))

2a. A descriptor's `suite` chain seeds one idempotent `TestSuite` per element (`xid = repo|file_path|suite_chain`), nested via `TestSuite.parent` outermost→innermost; the `TestChunk` links to the innermost via `TestChunk.suite`; a suite shared by many tests is created once. When a suite name resolves to a spec `path`/`path#ordinal` anchor, `TestSuite.spec` (`VALIDATES_SPEC`) is set; descriptors with no `suite` still seed the bare `TestChunk` (suites are optional). ([validated by `ingest-test-report.test.ts:611`](libs/shared/src/work/spec-trace/ingest-test-report.test.ts#L614), [validated by `ingest-test-report:627`](libs/shared/src/work/spec-trace/ingest-test-report.test.ts#L580))

3. When a descriptor carries `spec`, exactly one `VALIDATED_BY` edge (`evidence = generated-provenance`) is created to the single `Statement` or `AcceptanceCriterion` at that `path#ordinal` anchor. ([validated by `ingest-test-report.test.ts:269`](libs/shared/src/work/spec-trace/ingest-test-report.test.ts#L272))

4. `tests.run <id>` output `{passed, covered[]}` upserts one `Coverage` node (`xid = repo|test_file|test_name`) + `COVERS` edges to each overlapping `CodeChunk`; re-running for the same `(id, commit)` is idempotent. A result posted with `covered: null` records no `COVERS` edge rather than failing the whole report. ([validated by `writes the TestChunk and no COVERS edge when a test-report result's covered is null`](libs/shared/src/work/spec-trace/ingest-spec-trace.test.ts#L98), [validated by `parses passed + a list of covered chunks`](libs/shared/src/domain/test-report.test.ts#L80), [validated by `throws when a covered chunk is missing its line bounds`](libs/shared/src/domain/test-report.test.ts#L96), [validated by `ingest-test-report:162`](libs/shared/src/work/spec-trace/ingest-test-report.test.ts#L106), [validated by `ingest-test-report:590`](libs/shared/src/work/spec-trace/ingest-test-report.test.ts#L543))

5. The binary reports each test's `passed` (json `passed`, or the run command's exit code for lcov/cobertura); a failing **validating** test then sets `violated=true` + `violation_reason` on its `Statement`/`AcceptanceCriterion` and opens/labels a `spec-violated` issue during graph projection (owned by [`spec-traceability-graph`](../spec-traceability-graph/spec.md)) — distinct from `drifted`, raised only after the flaky guard confirms. ([validated by `TestBuildReportParsesLcovRunOutput`](apps/lore-code-trace/report_test.go#L53), [validated by `ingest-test-report:392`](libs/shared/src/work/spec-trace/ingest-test-report.test.ts#L343), [validated by `ingest-test-report:434`](libs/shared/src/work/spec-trace/ingest-test-report.test.ts#L385), [validated by `ingest-test-report:469`](libs/shared/src/work/spec-trace/ingest-test-report.test.ts#L420))

6. The `lore-code-trace` binary parses coverage in CI: json `{passed, covered[]}` inline, and LCOV (incl. `TN:`) / Cobertura normalized to canonical `{file, startLine, endLine}` ranges with contiguous lines collapsed — so the server ingests canonical chunks only (the bulk `/coverage` endpoint + its server-side parsers were removed in the cutover). A run that reports no coverage is posted as `covered: []`, never `null`. ([validated by `TestBuildReportPostsEmptyCoveredArrayWhenRunReportsNoCoverage`](apps/lore-code-trace/report_test.go#L81), [validated by `TestParseLcovCoverageKeepsHitLinesAndCollapsesRanges`](apps/lore-code-trace/coverage_test.go#L8), [validated by `TestParseCoberturaCoverageKeepsHitLines`](apps/lore-code-trace/coverage_test.go#L20), [validated by `TestCollapseRangesSortsAndMergesContiguous`](apps/lore-code-trace/coverage_test.go#L34))
...
7a. The `lore-code-trace` binary is served for CI download by the lore-api `/dist/<os>-<arch>` route: `distArtifactFromUrl` resolves an allowed os-arch artifact name (or `checksums.txt`), strips a query string, and rejects an unknown artifact, a non-dist url, or a path-traversal attempt; the handler serves a baked binary as `application/octet-stream` with its byte length, and 404s an artifact missing from the image or an unknown name without touching the filesystem. ([validated by `dist.test.ts:10`](apps/lore-api/src/transport/routes/dist/dist.test.ts#L10), [`dist.test.ts:14`](apps/lore-api/src/transport/routes/dist/dist.test.ts#L14), [`dist.test.ts:18`](apps/lore-api/src/transport/routes/dist/dist.test.ts#L18), [`dist.test.ts:22`](apps/lore-api/src/transport/routes/dist/dist.test.ts#L22), [`dist.test.ts:26`](apps/lore-api/src/transport/routes/dist/dist.test.ts#L26), [`dist.test.ts:30`](apps/lore-api/src/transport/routes/dist/dist.test.ts#L30), [`dist.test.ts:47`](apps/lore-api/src/transport/routes/dist/dist.test.ts#L47), [`dist.test.ts:59`](apps/lore-api/src/transport/routes/dist/dist.test.ts#L59), [`dist.test.ts:68`](apps/lore-api/src/transport/routes/dist/dist.test.ts#L68))
8. The whole path is zero-LLM and deterministic; a per-invocation timeout skips the offending test (logged) without blocking ingest or sibling units. ([validated by `rejects when the command outlives the timeout`](apps/mcp-server/src/work/spec-trace/spec-trace-tools.test.ts#L51), [validated by `rejects when the command outlives the timeout`](apps/mcp-server/src/work/spec-trace/spec-trace-tools.test.ts#L176), [validated by `TestBuildReportSkipsFileWhenRunCommandFails`](apps/lore-code-trace/report_test.go#L184))

9. Project commands execute **only** in a trusted sandbox (local dev, the repo's CI, or the claude-runner Job pod); the long-lived shared MCP/agent services never execute them. ([validated by `test-suite.test.ts:25`](libs/shared/src/outbound/project/test-runner/test-suite.test.ts#L25))

10. MCP tools `lore_list_tests` / `lore_run_test` / `query_trace` run the commands in the caller's sandbox and update the graph through the MCP server (proxied to the backend like memory writes); a call with no trusted local sandbox returns a "run in CI / locally" error instead of executing on the cluster; a failing `lore_run_test` on a validating test sets `violated` identically to the CI path. ([validated by `spec-trace-tools.test.ts:65`](apps/mcp-server/src/work/spec-trace/spec-trace-tools.test.ts#L65))

11. The onboarding TICKET owes a per-toolchain `lore-tests.yml` (one per detected toolchain, subdir-scoped for monorepos) that sets up the toolchain, downloads the `lore-code-trace` binary, and runs it `--post` on push/PR (which POSTs to the Floor `ci-tests` hook), authored by the `onboard` line's agent from `LORE_TESTS_INSTRUCTION` only when the file is absent; the Floor commits the ingest workflow itself, even when the repo already carries a `.github` directory (no coarse directory-existence skip), before the line runs. ([validated by owes the test-command manifest and lore-tests.yml with their own instructions, only when absent](libs/shared/src/work/onboard-content.test.ts#L106), [validated by creates the branch off the default branch and commits the ingest workflow onto it even when the repo already has a .github directory](apps/floor/src/work/task/worker.onboard.test.ts#L124))

12. The **test-interface check** (`decideTestInterfaceCheck`) decides what a repo still owes: when no manifest is declared (neither `.lore/test-commands.yml` nor `lore.repos.settings.test_commands`), a suggested `.lore/test-commands.yml` (+ `lore-tests.yml`) — which the onboarding ticket owes as files the `onboard` line's agent authors only when absent; the manifest and the workflow are independent checks — when the manifest is declared but `.github/workflows/lore-tests.yml` is absent, it scaffolds only the workflow file; "configured" requires both the manifest and the workflow to be present; declining the scaffold leaves the repo in documented fallback mode with no error. ([validated by `scaffolds both files when no manifest is declared`](libs/shared/src/domain/test-command-manifest.test.ts#L150), [validated by `reports configured when the .lore/test-commands.yml file is declared`](libs/shared/src/domain/test-command-manifest.test.ts#L162), [validated by `reports configured when settings declare test_commands and the workflow file is present`](libs/shared/src/domain/test-command-manifest.test.ts#L172), [validated by `returns false when neither a file nor settings declare a manifest`](libs/shared/src/domain/test-command-manifest.test.ts#L188), [validated by `scaffolds only the workflow file when the manifest is declared but lore-tests.yml is absent`](libs/shared/src/domain/test-command-manifest.test.ts#L205))

13. The web UI surfaces a single, language-agnostic **setup prompt** (copy-to-clipboard, on the repo specs/coverage page + onboarding result, and as the `/lore-test-commands` skill) that names no language; running it with Claude in the repo produces a `.lore/test-commands.yml` + any wrapper scripts whose `list`/`run` output conforms to [`contracts/test-commands.md`](./contracts/test-commands.md). The prompt text is stored once and shared by the UI surface and the skill. ([validated by `renders the setup prompt text into the DOM`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/TestCommandsSetup.test.tsx#L12), [validated by `names no concrete language or test runner`](libs/shared/src/lib/test-command-setup-prompt.test.ts#L10), [validated by `copies the full setup prompt to the clipboard on Copy click`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/TestCommandsSetup.test.tsx#L28), [validated by `renders "Set up test commands" as the collapsible card summary`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/TestCommandsSetup.test.tsx#L19), [validated by `is a non-empty string`](libs/shared/src/lib/test-command-setup-prompt.test.ts#L5), [validated by `exports a byte-identical constant to the shared source`](apps/web-ui/src/lib/test-command-setup-prompt.test.ts#L19), [validated by `carries the canonical TEST_COMMAND_SETUP_PROMPT verbatim`](libs/shared/src/lib/lore-test-commands-skill.test.ts#L11))
