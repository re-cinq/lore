...
1. The manifest (`.lore/test-commands.yml` / `lore.repos.settings.test_commands`) is parsed one entry at a time: `run` is the only required field, so an honest whole-suite entry (no per-test `list`, no `{selector}` in `run`, no `coverage_format`) is kept as a run-whole entry; `list` and `coverage_format` are optional and an unknown `coverage_format` is ignored rather than dropping the runnable entry; an entry with no usable `run` is skipped instead of throwing the whole manifest away, so one non-conforming entry never takes a valid sibling down with it; a polyglot list with per-`cwd` entries is supported; settings win over the file; an absent manifest resolves to null so the caller falls back (pattern detection + bulk upload) with no error. ([validated by `normalizes a minimal valid manifest into a one-element list with defaults`](libs/shared/src/test-command-manifest.test.ts#L11), [validated by `drops an entry with no run command while keeping its valid siblings`](libs/shared/src/test-command-manifest.test.ts#L29), [validated by `accepts a run command without a {selector} placeholder as run-whole`](libs/shared/src/test-command-manifest.test.ts#L43), [validated by `keeps the entry with coverage_format undefined when the value is unknown`](libs/shared/src/test-command-manifest.test.ts#L53), [validated by `keeps a valid vitest entry alongside an honest whole-suite entry`](libs/shared/src/test-command-manifest.test.ts#L64), [validated by `normalizes a polyglot array into one entry per manifest`](libs/shared/src/test-command-manifest.test.ts#L84), [validated by `preserves a provided cwd and path_prefix_strip`](libs/shared/src/test-command-manifest.test.ts#L104), [validated by `prefers settings over the file when both are present`](libs/shared/src/test-command-manifest.test.ts#L136), [validated by `falls back to the file when settings are absent`](libs/shared/src/test-command-manifest.test.ts#L142), [validated by `returns null when neither settings nor file declare a manifest`](libs/shared/src/test-command-manifest.test.ts#L132), [validated by `rejects listTests when the manifest entry has no list command`](libs/shared/src/project/test-runner/test-runner-exec.test.ts#L93), [validated by `reports the entry runs whole when it declares no list command`](apps/mcp-server/src/work/spec-trace/spec-trace-tools.test.ts#L203))

2. `tests.list` output is parsed into descriptors `{id, name, file, startLine, endLine, suite?, spec?, passed?}`; each seeds a `TestChunk` with the line range and `test_name`; the runner-native `id` is stored verbatim as the selector. The parser carries the `suite` chain outermost→innermost, drops a `suite` array holding a non-string element, and omits optional fields a descriptor does not declare. ([validated by `parses a descriptor carrying every field`](libs/shared/src/test-report.test.ts#L5), [validated by carries the suite chain outermost to innermost](libs/shared/src/test-report.test.ts#L31), [validated by omits a suite array holding a non-string element](libs/shared/src/test-report.test.ts#L41), [validated by omits optional fields a descriptor does not declare](libs/shared/src/test-report.test.ts#L49), [validated by `throws when the required %s is missing`](libs/shared/src/test-report.test.ts#L65))

2a. A descriptor's `suite` chain seeds one idempotent `TestSuite` per element (`xid = repo|file_path|suite_chain`), nested via `TestSuite.parent` outermost→innermost; the `TestChunk` links to the innermost via `TestChunk.suite`; a suite shared by many tests is created once. When a suite name resolves to a spec `path`/`path#ordinal` anchor, `TestSuite.spec` (`VALIDATES_SPEC`) is set; descriptors with no `suite` still seed the bare `TestChunk` (suites are optional). ([validated by `ingest-test-report.test.ts:611`](libs/shared/src/spec-trace/ingest-test-report.test.ts#L611), [validated by `ingest-test-report:627`](libs/shared/src/spec-trace/ingest-test-report.test.ts#L577))

3. When a descriptor carries `spec`, exactly one `VALIDATED_BY` edge (`evidence = generated-provenance`) is created to the single `Statement` or `AcceptanceCriterion` at that `path#ordinal` anchor. ([validated by `ingest-test-report.test.ts:269`](libs/shared/src/spec-trace/ingest-test-report.test.ts#L269))

4. `tests.run <id>` output `{passed, covered[]}` upserts one `Coverage` node (`xid = repo|test_file|test_name`) + `COVERS` edges to each overlapping `CodeChunk`; re-running for the same `(id, commit)` is idempotent. ([validated by `parses passed + a list of covered chunks`](libs/shared/src/test-report.test.ts#L80), [validated by `throws when a covered chunk is missing its line bounds`](libs/shared/src/test-report.test.ts#L96), [validated by `ingest-test-report:162`](libs/shared/src/spec-trace/ingest-test-report.test.ts#L121), [validated by `ingest-test-report:590`](libs/shared/src/spec-trace/ingest-test-report.test.ts#L540))

5. The binary reports each test's `passed` (json `passed`, or the run command's exit code for lcov/cobertura); a failing **validating** test then sets `violated=true` + `violation_reason` on its `Statement`/`AcceptanceCriterion` and opens/labels a `spec-violated` issue during graph projection (owned by [`spec-traceability-graph`](../spec-traceability-graph/spec.md)) — distinct from `drifted`, raised only after the flaky guard confirms. ([validated by `TestBuildReportParsesLcovRunOutput`](apps/lore-code-trace/report_test.go#L51), [validated by `ingest-test-report:392`](libs/shared/src/spec-trace/ingest-test-report.test.ts#L351), [validated by `ingest-test-report:434`](libs/shared/src/spec-trace/ingest-test-report.test.ts#L393), [validated by `ingest-test-report:469`](libs/shared/src/spec-trace/ingest-test-report.test.ts#L428))
...
9. Project commands execute **only** in a trusted sandbox (local dev, the repo's CI, or the claude-runner Job pod); the long-lived shared MCP/agent services never execute them. ([validated by `test-suite.test.ts:25`](libs/shared/src/project/test-runner/test-suite.test.ts#L25))
...
12. The `onboard` task runs a **test-interface check**: when no manifest is declared (neither `.lore/test-commands.yml` nor `lore.repos.settings.test_commands`), it detects the toolchain and scaffolds a suggested `.lore/test-commands.yml` (+ `lore-tests.yml`) in the onboarding PR; when a manifest already exists it reports "configured" and scaffolds nothing (idempotent); declining the scaffold leaves the repo in documented fallback mode with no error. ([validated by `scaffolds both files when no manifest is declared`](libs/shared/src/test-command-manifest.test.ts#L150), [validated by `reports configured when the .lore/test-commands.yml file is declared`](libs/shared/src/test-command-manifest.test.ts#L162), [validated by `reports configured when settings declare test_commands without a file`](libs/shared/src/test-command-manifest.test.ts#L171), [validated by `returns false when neither a file nor settings declare a manifest`](libs/shared/src/test-command-manifest.test.ts#L186))

13. The web UI surfaces a single, language-agnostic **setup prompt** (copy-to-clipboard, on the repo specs/coverage page + onboarding result, and as the `/lore-test-commands` skill) that names no language; running it with Claude in the repo produces a `.lore/test-commands.yml` + any wrapper scripts whose `list`/`run` output conforms to [`contracts/test-commands.md`](./contracts/test-commands.md). The prompt text is stored once and shared by the UI surface and the skill. ([validated by `renders the setup prompt text into the DOM`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/TestCommandsSetup.test.tsx#L12), [validated by `names no concrete language or test runner`](libs/shared/src/test-command-setup-prompt.test.ts#L10), [validated by `copies the full setup prompt to the clipboard on Copy click`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/TestCommandsSetup.test.tsx#L28), [validated by `renders "Set up test commands" as the collapsible card summary`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/TestCommandsSetup.test.tsx#L19), [validated by `is a non-empty string`](libs/shared/src/test-command-setup-prompt.test.ts#L5), [validated by `exports a byte-identical constant to the shared source`](apps/web-ui/src/lib/test-command-setup-prompt.test.ts#L19), [validated by `carries the canonical TEST_COMMAND_SETUP_PROMPT verbatim`](libs/shared/src/lore-test-commands-skill.test.ts#L11))
...
14. `substituteSelector` builds the concrete `tests.run` invocation by replacing every `{selector}` placeholder in the manifest's `run` command with the runner-native id, leaving a `{selector}` that occurs inside the id itself intact. ([validated by `replaces every {selector} placeholder with the runner-native id`](libs/shared/src/test-command-manifest.test.ts#L192))
