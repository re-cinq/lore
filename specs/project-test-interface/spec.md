...
3. When a descriptor carries `spec`, exactly one `VALIDATED_BY` edge (`evidence = generated-provenance`) is created to the single `Statement` or `AcceptanceCriterion` at that `path#ordinal` anchor. ([validated by `ingest-test-report.test.ts:323`](libs/shared/src/spec-trace/ingest-test-report.test.ts#L323))

4. `tests.run <id>` output `{passed, covered[]}` upserts one `Coverage` node (`xid = repo|test_file|test_name`) + `COVERS` edges to each overlapping `CodeChunk`; re-running for the same `(id, commit)` is idempotent. ([validated by `parses passed + a list of covered chunks`](libs/shared/src/test-report.test.ts#L80), [validated by `throws when a covered chunk is missing its line bounds`](libs/shared/src/test-report.test.ts#L96), [validated by `ingest-test-report:162`](libs/shared/src/spec-trace/ingest-test-report.test.ts#L162), [validated by `ingest-test-report:590`](libs/shared/src/spec-trace/ingest-test-report.test.ts#L590))

5. The binary reports each test's `passed` (json `passed`, or the run command's exit code for lcov/cobertura); a failing **validating** test then sets `violated=true` + `violation_reason` on its `Statement`/`AcceptanceCriterion` and opens/labels a `spec-violated` issue during graph projection (owned by [`spec-traceability-graph`](../spec-traceability-graph/spec.md)) — distinct from `drifted`, raised only after the flaky guard confirms. ([validated by `TestBuildReportParsesLcovRunOutput`](apps/lore-code-trace/report_test.go#L51), [validated by `ingest-test-report:392`](libs/shared/src/spec-trace/ingest-test-report.test.ts#L392), [validated by `ingest-test-report:434`](libs/shared/src/spec-trace/ingest-test-report.test.ts#L434), [validated by `ingest-test-report:469`](libs/shared/src/spec-trace/ingest-test-report.test.ts#L469))
...
9. Project commands execute **only** in a trusted sandbox (local dev, the repo's CI, or the claude-runner Job pod); the long-lived shared MCP/agent services never execute them. ([validated by `test-suite.test.ts:31`](libs/shared/src/project/test-runner/test-suite.test.ts#L31))
