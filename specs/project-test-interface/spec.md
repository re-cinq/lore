...
11. The `onboard` task emits a per-toolchain `lore-tests.yml` (one per detected toolchain, subdir-scoped for monorepos) that sets up the toolchain, downloads the `lore-code-trace` binary, and runs it `--post` on push/PR (which POSTs to the Floor `ci-tests` hook). The onboard handler commits the generated CI workflow even when the repo already carries a `.github` directory (no coarse directory-existence skip), then opens the onboarding PR off the created branch. ([validated by commits the ingest workflow even when the repo already has a .github directory](apps/floor/src/work/task/worker.onboard.test.ts#L113), [validated by opens the PR after creating the branch and committing the workflow](apps/floor/src/work/task/worker.onboard.test.ts#L130))

12. The `onboard` task runs a **test-interface check**: when no manifest is declared (neither `.lore/test-commands.yml` nor `lore.repos.settings.test_commands`), it detects the toolchain and scaffolds a suggested `.lore/test-commands.yml` (+ `lore-tests.yml`) in the onboarding PR; the manifest and the workflow are independent checks — when the manifest is declared but `.github/workflows/lore-tests.yml` is absent, it scaffolds only the workflow file; "configured" requires both the manifest and the workflow to be present; declining the scaffold leaves the repo in documented fallback mode with no error. ([validated by `scaffolds both files when no manifest is declared`](libs/shared/src/domain/test-command-manifest.test.ts#L150), [validated by `reports configured when the .lore/test-commands.yml file is declared`](libs/shared/src/domain/test-command-manifest.test.ts#L162), [validated by `reports configured when settings declare test_commands without a file`](libs/shared/src/domain/test-command-manifest.test.ts#L171), [validated by `returns false when neither a file nor settings declare a manifest`](libs/shared/src/domain/test-command-manifest.test.ts#L186), [validated by `scaffolds only the workflow file when the manifest is declared but lore-tests.yml is absent`](libs/shared/src/domain/test-command-manifest.test.ts#L203))
