...
Run Failure Visibility makes a feature's detail page tell the truth about a planning line that died: the run's own failure is carried to the page and rendered as a failure, and a refusal from lore-api is shown where the author is reading instead of crashing the route. Today a line that fails between nodes reads as a line that is still waiting for the author, and the button that state offers answers with an HTTP 500.
