...
- **FR11** *(added 2026-08-14)* The web UI consumes this contract rather than the database, and `re-lint/no-sql-in-web-ui` enforces it: any string or template literal in the files `eslint.config.mjs` scopes it to (`apps/web-ui/src/**`) that reads as SQL — `SELECT`…`FROM` or `UPDATE`…`SET` across newlines and through `${interpolation}`, `INSERT INTO`, `DELETE FROM`, or DDL — is reported, pointing the author at a lore-api route plus the generated client. Detection requires the keywords UPPERCASE, the casing every query in this repo already uses, because a case-insensitive match flags ordinary UI copy ("Select a repo from the list") and a rule that cries wolf gets disabled rather than obeyed. It runs at `error`. It shipped at `warn` because 143 queries across 51 files predated the contract and a warning marked each as debt without red-lighting a repo that could not be fixed in one change; every one of those queries has since moved behind an lore-api route, `apps/web-ui/src/lib/db.ts` is deleted, and `pg` is no longer a web-ui dependency — so a new query would have to reintroduce the driver to run at all. The rule is what says so at review time rather than at deploy time. ([validated by any file the rule is scoped to is checked, with no built-in path gate](https://github.com/re-cinq/re-lint/blob/v1.0.0/src/rules/no-sql-in-web-ui.test.mjs#L42), [validated by prose whose keywords are not SQL-cased](https://github.com/re-cinq/re-lint/blob/v1.0.0/src/rules/no-sql-in-web-ui.test.mjs#L12), [`no-sql-in-web-ui.test.mjs:17`](https://github.com/re-cinq/re-lint/blob/v1.0.0/src/rules/no-sql-in-web-ui.test.mjs#L17), [`no-sql-in-web-ui.test.mjs:22`](https://github.com/re-cinq/re-lint/blob/v1.0.0/src/rules/no-sql-in-web-ui.test.mjs#L22), [`no-sql-in-web-ui.test.mjs:36`](https://github.com/re-cinq/re-lint/blob/v1.0.0/src/rules/no-sql-in-web-ui.test.mjs#L36), [`no-sql-in-web-ui.test.mjs:48`](https://github.com/re-cinq/re-lint/blob/v1.0.0/src/rules/no-sql-in-web-ui.test.mjs#L48), [`no-sql-in-web-ui.test.mjs:54`](https://github.com/re-cinq/re-lint/blob/v1.0.0/src/rules/no-sql-in-web-ui.test.mjs#L54), [`no-sql-in-web-ui.test.mjs:59`](https://github.com/re-cinq/re-lint/blob/v1.0.0/src/rules/no-sql-in-web-ui.test.mjs#L59), [`no-sql-in-web-ui.test.mjs:64`](https://github.com/re-cinq/re-lint/blob/v1.0.0/src/rules/no-sql-in-web-ui.test.mjs#L64), [`no-sql-in-web-ui.test.mjs:69`](https://github.com/re-cinq/re-lint/blob/v1.0.0/src/rules/no-sql-in-web-ui.test.mjs#L69), [`no-sql-in-web-ui.test.mjs:75`](https://github.com/re-cinq/re-lint/blob/v1.0.0/src/rules/no-sql-in-web-ui.test.mjs#L75), [`no-sql-in-web-ui.test.mjs:81`](https://github.com/re-cinq/re-lint/blob/v1.0.0/src/rules/no-sql-in-web-ui.test.mjs#L81); implemented by [`no-sql-in-web-ui.mjs:1`](https://github.com/re-cinq/re-lint/blob/v1.0.0/src/rules/no-sql-in-web-ui.mjs#L1))
