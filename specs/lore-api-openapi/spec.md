...
- Every route whose method can carry a body (`POST`/`PUT`/`DELETE`/`*`) either
  ...
  the doc cannot silently rot. (Design fork 5.) ([validated by `coverage.test.ts:22`](apps/lore-api/src/openapi/coverage.test.ts#L22), [validated by `build-document.test.ts:41`](apps/lore-api/src/openapi/build-document.test.ts#L41))
...
- The output is a structurally valid OpenAPI 3.1 document (`openapi: "3.1.0"`,
  ...
  every `requestBody` is an `application/json` schema. ([validated by `coverage.test.ts:65`](apps/lore-api/src/openapi/coverage.test.ts#L119), [validated by `coverage.test.ts:78`](apps/lore-api/src/openapi/coverage.test.ts#L132), [validated by `coverage.test.ts:91`](apps/lore-api/src/openapi/coverage.test.ts#L145))
...
- **FR7** Non-API operational paths (`/healthz`, `/dist/*`) are excluded from the
  document and the exclusion is logged, not silent. ([validated by `build-document.test.ts:45`](apps/lore-api/src/openapi/build-document.test.ts#L45), [`coverage.test.ts:46`](apps/lore-api/src/openapi/coverage.test.ts#L96))
...
- **SC-2** Every `/api/*` route registered in `routeList` appears exactly once in
  ...
  hapi optional path param to `required: true` with an "optional" note. ([validated by `coverage.test.ts:38`](apps/lore-api/src/openapi/coverage.test.ts#L38), [`build-document.test.ts:111`](apps/lore-api/src/openapi/build-document.test.ts#L111), [validated by `build-document.test.ts:123`](apps/lore-api/src/openapi/build-document.test.ts#L123))
...
- **SC-4** A body-bearing route with neither a `zodValidate` schema nor a sidecar
  ...
  test). ([validated by `coverage.test.ts:26`](apps/lore-api/src/openapi/coverage.test.ts#L26))
...
- **FR12** *(added 2026-08-20)* A path that multiplexes verbs declares ONE CONTRACT PER VERB. The generator stamps a route's contract onto every method that route serves, so `/api/tokens` and the dark-factory settings path — one `method: "*"` route each — could only be declared as the union of both answers, leaving a generated client to narrow "the list or the write acknowledgement" on a verb that only ever returns one of them. Each is split into concrete per-verb routes with its own contract; the wildcard route that remains at each path exists ONLY to answer 405, because hapi meets an unmatched verb on a matched path with a 404 and "you may not DELETE this" is the better answer. A 405 fallback declares nothing and is absent from the wildcard-verb sidecar, so it contributes no operation and cannot quietly re-add the union. `POST /api/memory` is NOT split: its action rides in the body, so every caller posts to one path and moving them is an expand/contract across a separately-shipped image — its five answers are declared as a named union instead of the open document they were. ([validated by `coverage.test.ts:46`](apps/lore-api/src/openapi/coverage.test.ts#L46), [`coverage.test.ts:86`](apps/lore-api/src/openapi/coverage.test.ts#L86); implemented by [`tokens.ts:98`](apps/lore-api/src/api/routes/tokens/tokens.ts#L98), [`dark-factory.ts:73`](apps/lore-api/src/api/routes/dark-factory/dark-factory.ts#L73))

- **FR13** *(added 2026-08-20)* A DECLARED response contract is HELD TO the answer, not just written down. `zodResponse` is documentation-only by design — validating a response at runtime lets a doc comment 500 a working endpoint — so nothing in the serving path can notice a declaration that has gone wrong. An open document could not BE wrong; a named union can be wrong silently, and a generated client narrows over it. So the multiplexed memory surface carries a contract test that drives all five actions for real and parses every answer through the published schema. It runs against BOTH backends — the file fallback under a throwaway `$HOME`, and a migrated Postgres in the integration job — because one endpoint served by two backends is a lie half the time if only one of them satisfies the contract. Writing it is what surfaced the three places the two disagreed: the pool path's one-version read omitted the `key` it was asked for, and the fallback's search named no `source` while its listing answered whole entries where the pool answered a projection. The fallback was moved onto the pool path's answer rather than the union widened to cover both. ([validated by `memory-contract.test.ts:66`](apps/lore-api/src/api/routes/memory/memory-contract.test.ts#L66), [`memory-contract.test.ts:77`](apps/lore-api/src/api/routes/memory/memory-contract.test.ts#L77), [`memory-contract.test.ts:88`](apps/lore-api/src/api/routes/memory/memory-contract.test.ts#L88), [`memory-contract.test.ts:99`](apps/lore-api/src/api/routes/memory/memory-contract.test.ts#L99), [`memory-contract.test.ts:106`](apps/lore-api/src/api/routes/memory/memory-contract.test.ts#L106), [`memory-contract.test.ts:117`](apps/lore-api/src/api/routes/memory/memory-contract.test.ts#L117), [`memory-contract.test.ts:126`](apps/lore-api/src/api/routes/memory/memory-contract.test.ts#L126), [`memory-contract.test.ts:144`](apps/lore-api/src/api/routes/memory/memory-contract.test.ts#L144), [`memory-contract.test.ts:84`](apps/lore-api/src/integration-tests/memory-contract.test.ts#L84), [`memory-contract.test.ts:101`](apps/lore-api/src/integration-tests/memory-contract.test.ts#L101), [`memory-contract.test.ts:108`](apps/lore-api/src/integration-tests/memory-contract.test.ts#L108), [`memory-contract.test.ts:115`](apps/lore-api/src/integration-tests/memory-contract.test.ts#L115), [`memory-contract.test.ts:122`](apps/lore-api/src/integration-tests/memory-contract.test.ts#L122), [`memory-contract.test.ts:128`](apps/lore-api/src/integration-tests/memory-contract.test.ts#L128), [`memory-contract.test.ts:135`](apps/lore-api/src/integration-tests/memory-contract.test.ts#L135))
- **FR14** *(added 2026-08-20)* Every `method: "*"` route DECLARES whether it serves verbs or refuses them. `WILDCARD_METHODS` names the verbs a wildcard means to serve and `METHOD_NOT_ALLOWED_FALLBACKS` names the paths whose wildcard exists only to answer 405; a wildcard in neither table fails the coverage guard. The path-count assertion cannot see this case: a wildcard that means to serve a real verb on a path concrete verbs ALREADY document adds no new path key, so the operation goes missing from the document in silence. On a new path the same omission surfaces as a missing path — this closes the half that does not. ([validated by `coverage.test.ts:68`](apps/lore-api/src/openapi/coverage.test.ts#L68); implemented by [`build-document.ts:145`](apps/lore-api/src/openapi/build-document.ts#L145))
