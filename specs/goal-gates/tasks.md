...
- [x] T015 [P] PR check — failing test in `apps/floor/src/jobs/assembly-line/pr-check.test.ts` that a PR-linked line closing `goal_gate_unmet` publishes a failing `lore/<definition>` check; extend only if the mapping is not already generic
- [x] T016 [P] Escalation diagnostic — add `goal_gate_unmet` to the reason union in `apps/floor/src/jobs/platform/escalation.ts` with a covering test, if the union does not already admit it
...
- [x] T019 Full verification: `npx eslint .` (0 errors), `npx prettier --check` on touched files, `npm run typecheck:drift`, and the `libs/assembly-lines`, `apps/floor`, `apps/web-ui` suites green
