...
- `goal_gate` on the exit node is rejected at load: the walk never records a visit for its terminal marker, so such a gate could never be satisfied and the line would fail forever. ([validated by `loader.test.ts:835`](libs/assembly-lines/src/loader.test.ts#L835))
...
- Only the gated node's LATEST visit decides: a gate satisfied on iteration 1 whose re-run failed leaves the line `goal_gate_unmet` (the earlier verdict applied to an earlier state of the branch), and a latest clean visit supersedes an earlier failed one. ([validated by `transition.test.ts:363`](libs/assembly-lines/src/transition.test.ts#L363), [`transition.test.ts:377`](libs/assembly-lines/src/transition.test.ts#L377))
