...
- **Silent-500 guard.** The Floor's HTTP server logs every boomified unexpected throw — handler,
  auth scheme, or extension — on the hapi `request` error channel with method, path, and stack under
  an `[http]` tag, so no route 500s anonymously (the #1319 outage was undiagnosable for exactly that
  reason). Deliberate Boom 4xx/503 responses do not hit the channel and are unchanged. ([validated by `agent-logs.test.ts:97`](apps/floor/src/delivery/http/routes/agent-logs.test.ts#L97))
