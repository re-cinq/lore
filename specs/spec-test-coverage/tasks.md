...
- [x] T118 PR opener: for each spec with ≥1 suggestion, the cron uses `platform().createBranch + commitFile + createPR` to open `"Suggested test links for {spec_path}"` PRs labelled `lore-managed` + `spec-coverage-backfill`. Inlined the v2 `judgeLink` / `classifyLLM` / `classifyAllStatements` / `extractAssertions` helpers (deleted from `spec-test-linker.ts` in Phase 5) directly into `spec-coverage-backfill.ts`. Added new pure helper `pickStatementsForBackfill` (6 tests) to filter testable + un-linked statements before the judge runs.
