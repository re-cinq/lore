...
2. Tasks survive rollout restarts via the lease backend.
   ([validated by `lease-backends.test.ts:14`](libs/shared/src/outbound/project/leases/lease-backends.test.ts#L14),
   [`lease-backends.test.ts:142`](libs/shared/src/outbound/project/leases/lease-backends.test.ts#L142))
...
1. The per-repo specs page renders an `<a>` whose `href`'s path passes `isTestFile()` with the green `stmt-tested` class; regular links render unchanged. ([validated by `SpecDetails.test.tsx:27`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L27))
...
7. `parseTestLinksInStatement(text)` extracts every `(...[label](path#Lline)...)` token at the end of a statement; multiple links comma-separated inside one paren are parsed as a list; it returns `[]` when there is no trailing parenthetical or the paren holds no markdown links (or only non-test links), ignores mid-text links with no trailing paren, tolerates a trailing period after the paren, strips a leading slash and recognises the Go `_test.go` convention on the href, yields `line: null` for a link with no `#Lline` anchor, and collapses internal whitespace in the label. ([validated by `spec-link-parser.test.ts:22`](libs/shared/src/domain/spec-link-parser.test.ts#L22), [`no-paren`](libs/shared/src/domain/spec-link-parser.test.ts#L10), [`no-links`](libs/shared/src/domain/spec-link-parser.test.ts#L16), [`multi-comma`](libs/shared/src/domain/spec-link-parser.test.ts#L36), [`ignores-nontest`](libs/shared/src/domain/spec-link-parser.test.ts#L55), [`only-nontest`](libs/shared/src/domain/spec-link-parser.test.ts#L65), [`no-anchor`](libs/shared/src/domain/spec-link-parser.test.ts#L73), [`strip-slash`](libs/shared/src/domain/spec-link-parser.test.ts#L83), [`go-path`](libs/shared/src/domain/spec-link-parser.test.ts#L91), [`midtext-ignored`](libs/shared/src/domain/spec-link-parser.test.ts#L101), [`trailing-period`](libs/shared/src/domain/spec-link-parser.test.ts#L109), [`collapse-ws-shared`](libs/shared/src/domain/spec-link-parser.test.ts#L117))

8. The cron `spec_coverage_validate` runs on every successful `/api/ingest` for a repo, parses each spec's links, and resolves each against `{schema}.chunks` AST metadata — passing on a covering chunk (or a file-level null line resolved by any chunk of the file, or a line link into a file whose chunks all lack line ranges: pre-v2-chunker output is unverifiable, not broken), flagging `file-missing`, `line-out-of-range` (judged only against the ranged chunks when ranged and range-less coexist; a line past the file's LAST ranged line whose chunks were ingested before the spec carrying the link is index lag — tests are appended at file end, so the anchor outruns the not-yet-reindexed chunks — and passes until the chunks catch up, while the same past-end line on fresher-than-spec chunks, a line in a mid-file gap, or a miss with either ingest stamp unknown still flags), canonicalizing each href before matching (a `../`-relative link resolves against the spec's own directory the way GitHub renders it, a `./` prefix is stripped, and a broken link is reported under its resolved repo-root-relative path), or a non-trailing link (only a genuine test-file link buried mid-prose counts — mid-prose source references, intra-doc anchors, absolute URLs, placeholder paths like `path/to/test.ts` or `<owner>`-style template segments, links quoted in inline code spans, and prose brackets that would fuse with the trailing parenthetical's first link are all legitimate spec prose, not rot), grouping the broken-link report by spec, capping the rendered report body under GitHub's 65,536-char issue limit by eliding whole trailing bullets behind an `…and N more broken link(s) truncated` line (never cutting mid-bullet; a spec heading whose bullets were all elided is skipped rather than rendered empty; small reports render uncapped and the total counts survive at the top), opening a PR comment listing broken links when any exist, and treating a `spec-link-rot`-labelled open issue as the already-filed fallback. ([validated by `spec-coverage-validate-trigger.test.ts:18`](apps/lore-api/src/transport/routes/spec-coverage-validate-trigger.test.ts#L18), [`spec-coverage-validate.test.ts:92`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L92), [`resolve-covers`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L29), [`file-missing`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L37), [`line-out-of-range`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L45), [`null-line-file`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L54), [`null-line-missing`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L62), [`any-chunk-covers`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L70), [`rangeless-passes`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L245), [`ranged-still-judges`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L253), [`all-resolve`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L108), [`no-test-links`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L118), [`non-trailing`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L124), [`nt-anchor`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L294), [`nt-url`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L301), [`nt-placeholder`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L308), [`nt-template`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L315), [`nt-source-ref`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L322), [`nt-code-span`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L329), [`nt-bracket-fusion`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L336), [`nt-genuine`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L343), [`report-grouped`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L140), [`empty-report`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L171), [`report-cap`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L175), [`report-uncapped`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L206), [`empty-heading-skipped`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L264), [`issue-open`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L227), [`issue-none`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L233), [`issue-empty`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L239), [`lag-pass`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L365), [`lag-fresh-flags`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L375), [`lag-gap-flags`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L385), [`lag-no-spec-stamp`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L398), [`lag-no-file-stamp`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L406), [`lag-collect`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L416), [`relative-href-resolves`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L432), [`relative-href-missing`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L439), [`dot-prefix-resolves`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L454), [`nt-relative-resolved`](libs/shared/src/work/detect/spec-coverage-validate.test.ts#L465))

9. The cron `spec_coverage_backfill` runs weekly (Mon 11:00 UTC) and on demand, segments + classifies + judges each spec's testable un-linked statements, and **opens a PR per spec** adding the suggested `([validated by ...](path#Lline))` parentheticals; no DB writes. `proposeLinkInsertions` inserts the trailing parenthetical at the matched statement — composing a unified-diff preview, collapsing multiple suggestions for one statement into a single comma-separated paren, omitting the `#Lline` anchor when the line is null, returning content unchanged for no suggestions, and skipping an already-linked or drifted (statement_text not-found) suggestion — while `pickStatementsForBackfill` returns only the testable statements with no existing trailing link, excluding already-linked, untestable, or unclassified ones. ([validated by `spec-coverage-backfill.test.ts:28`](libs/shared/src/work/detect/spec-coverage-backfill.test.ts#L28), [`spec-judge.test.ts:160`](libs/shared/src/domain/spec-judge.test.ts#L160), [`spec-judge.test.ts:237`](libs/shared/src/domain/spec-judge.test.ts#L237), [`diff-preview`](libs/shared/src/work/detect/spec-coverage-backfill.test.ts#L44), [`skip-linked`](libs/shared/src/work/detect/spec-coverage-backfill.test.ts#L56), [`skip-drift`](libs/shared/src/work/detect/spec-coverage-backfill.test.ts#L72), [`collapse-paren`](libs/shared/src/work/detect/spec-coverage-backfill.test.ts#L84), [`null-anchor`](libs/shared/src/work/detect/spec-coverage-backfill.test.ts#L109), [`no-suggestions`](libs/shared/src/work/detect/spec-coverage-backfill.test.ts#L119), [`pick-unlinked`](libs/shared/src/work/detect/spec-coverage-backfill.test.ts#L141), [`pick-excl-linked`](libs/shared/src/work/detect/spec-coverage-backfill.test.ts#L158), [`pick-excl-untestable`](libs/shared/src/work/detect/spec-coverage-backfill.test.ts#L172), [`pick-all-linked`](libs/shared/src/work/detect/spec-coverage-backfill.test.ts#L186), [`pick-none-testable`](libs/shared/src/work/detect/spec-coverage-backfill.test.ts#L199), [`pick-no-classification`](libs/shared/src/work/detect/spec-coverage-backfill.test.ts#L209))
...
13. The backfill judge pipeline is deterministic where it can be: `featureDir` extraction (feature folder under `specs/`, parent dir otherwise, null for a bare filename), slug-token directory affinity, cosine similarity (1 for identical, 0 for orthogonal / empty / length-mismatched / zero-magnitude vectors), pgvector-string parsing, assertion-name referencing (case-insensitive, skips names under three characters), symbol-name normalization, and `hashSpecContent` (stable 64-char sha-256, distinct per content) are pure; `selectCandidates` ranks assertion-overlap then directory affinity then embedding proximity, skips non-test files and chunks with no test name, dedups by test keeping the strongest signal, and caps at `maxCandidates` flagging truncation; `judgeLink` keeps the highest-scoring judgment per test and drops non-matches and sub-threshold scores; the re-run diff returns links and ordinals no longer confirmed this run. The LLM judge returns the model tool call's assertions (propagating the caller's `jobName` for cost accounting) or `[]` when the model yields no assertions field. ([validated by `spec-judge:21`](libs/shared/src/domain/spec-judge.test.ts#L21), [validated by `spec-judge:27`](libs/shared/src/domain/spec-judge.test.ts#L27), [validated by `spec-judge:31`](libs/shared/src/domain/spec-judge.test.ts#L31), [validated by `spec-judge:37`](libs/shared/src/domain/spec-judge.test.ts#L37), [validated by `spec-judge:46`](libs/shared/src/domain/spec-judge.test.ts#L46), [validated by `spec-judge:55`](libs/shared/src/domain/spec-judge.test.ts#L55), [validated by `spec-judge:63`](libs/shared/src/domain/spec-judge.test.ts#L63), [validated by `spec-judge:67`](libs/shared/src/domain/spec-judge.test.ts#L67), [validated by `spec-judge:71`](libs/shared/src/domain/spec-judge.test.ts#L71), [validated by `spec-judge:76`](libs/shared/src/domain/spec-judge.test.ts#L76), [validated by `spec-judge:87`](libs/shared/src/domain/spec-judge.test.ts#L87), [validated by `spec-judge:93`](libs/shared/src/domain/spec-judge.test.ts#L93), [validated by `spec-judge:97`](libs/shared/src/domain/spec-judge.test.ts#L97), [validated by `spec-judge:103`](libs/shared/src/domain/spec-judge.test.ts#L103), [validated by `spec-judge:112`](libs/shared/src/domain/spec-judge.test.ts#L112), [validated by `spec-judge:118`](libs/shared/src/domain/spec-judge.test.ts#L118), [validated by `spec-judge:125`](libs/shared/src/domain/spec-judge.test.ts#L125), [validated by `spec-judge:129`](libs/shared/src/domain/spec-judge.test.ts#L129), [validated by `spec-judge:133`](libs/shared/src/domain/spec-judge.test.ts#L133), [validated by `spec-judge:171`](libs/shared/src/domain/spec-judge.test.ts#L171), [validated by `spec-judge:187`](libs/shared/src/domain/spec-judge.test.ts#L187), [validated by `spec-judge:196`](libs/shared/src/domain/spec-judge.test.ts#L196), [validated by `spec-judge:208`](libs/shared/src/domain/spec-judge.test.ts#L208), [validated by `spec-judge:247`](libs/shared/src/domain/spec-judge.test.ts#L247), [validated by `spec-judge:254`](libs/shared/src/domain/spec-judge.test.ts#L254), [validated by `spec-judge:267`](libs/shared/src/domain/spec-judge.test.ts#L267), [validated by `spec-judge:273`](libs/shared/src/domain/spec-judge.test.ts#L273), [validated by `spec-judge:280`](libs/shared/src/domain/spec-judge.test.ts#L280), [validated by `spec-judge-llm:9`](libs/shared/src/work/spec-judge-llm.test.ts#L9), [validated by `spec-judge-llm:32`](libs/shared/src/work/spec-judge-llm.test.ts#L32), [validated by `spec-judge-llm:42`](libs/shared/src/work/spec-judge-llm.test.ts#L42))

14. Each spec card derives its title from the spec's first H1 (stripping the marker and any "Feature Specification:" prefix), falling back to the feature directory name and then the raw file path when there is no H1; its summary is the first non-heading, non-table prose paragraph — skipping a leading blockquote note or whitespace-only block, collapsing internal whitespace and joining wrapped lines, truncating with an ellipsis past the limit, and empty when the content is only headings, tables, and lists; a spec's chunks are joined in `metadata.chunk_index` order, falling back to ingest order for chunks without one (sorted last), with identical contents de-duplicated. ([validated by `spec-summary:9`](libs/shared/src/work/spec-summary.test.ts#L9), [validated by `spec-summary:18`](libs/shared/src/work/spec-summary.test.ts#L18), [validated by `spec-summary:27`](libs/shared/src/work/spec-summary.test.ts#L27), [validated by `spec-summary:33`](libs/shared/src/work/spec-summary.test.ts#L33), [validated by `spec-summary:39`](libs/shared/src/work/spec-summary.test.ts#L39), [validated by `spec-summary:46`](libs/shared/src/work/spec-summary.test.ts#L46), [validated by `spec-summary:52`](libs/shared/src/work/spec-summary.test.ts#L52), [validated by `spec-summary:60`](libs/shared/src/work/spec-summary.test.ts#L60), [validated by `spec-summary:64`](libs/shared/src/work/spec-summary.test.ts#L64), [validated by `spec-summary:73`](libs/shared/src/work/spec-summary.test.ts#L73), [validated by `spec-summary:82`](libs/shared/src/work/spec-summary.test.ts#L82), [validated by `spec-summary:91`](libs/shared/src/work/spec-summary.test.ts#L91), [validated by `spec-summary:101`](libs/shared/src/work/spec-summary.test.ts#L101), [validated by `spec-summary:110`](libs/shared/src/work/spec-summary.test.ts#L110))

15. `segmentStatements()` splits prose paragraphs into sentences and each list item into its own ordinal-stable statement — joining wrapped continuation lines, ending a continuation at a heading, table row, or code fence, dropping bare list markers, guarding against splitting on single-initial (`J. B.`) and `e.g.`/`i.e.` abbreviations or a lowercase next character, splitting after punctuation-only leading sentences, excluding headings/fenced code/tables, tracking the enclosing heading per statement, keeping a trailing or own-line markdown-link parenthetical attached to its statement, and returning an empty array for content with no statements. ([validated by `ss-sentences`](libs/shared/src/domain/spec-segment.test.ts#L10), [`ss-continuations`](libs/shared/src/domain/spec-segment.test.ts#L37), [`ss-excludes`](libs/shared/src/domain/spec-segment.test.ts#L46), [`ss-abbrev`](libs/shared/src/domain/spec-segment.test.ts#L77), [`ss-initials`](libs/shared/src/domain/spec-segment.test.ts#L87), [`ss-lowercase`](libs/shared/src/domain/spec-segment.test.ts#L96), [`ss-heading`](libs/shared/src/domain/spec-segment.test.ts#L102), [`ss-ordinals`](libs/shared/src/domain/spec-segment.test.ts#L124), [`ss-trailing-link`](libs/shared/src/domain/spec-segment.test.ts#L134), [`ss-ownline-link`](libs/shared/src/domain/spec-segment.test.ts#L143), [`ss-empty`](libs/shared/src/domain/spec-segment.test.ts#L156))

16. `buildIntroOrdinals()` marks statements under the document's first heading, or with no enclosing heading at all, as intro; `classifyByHeuristic()` returns those intro ordinals untestable as `intro`, marks narrative doc sections (alternatives / research / personas / out-of-scope), the standard narrative headings (Problem Statement → background, Vision, Clarifications, Open Questions, Limitations, Rationale), and `Decision:`-prefixed or bare `See ADR-NNN` cross-reference statements untestable, and leaves everything else — unrecognised headings, a statement with no enclosing heading, a real functional requirement — testable. ([validated by `cbh-intro-first`](libs/shared/src/domain/spec-segment.test.ts#L163), [`cbh-no-heading`](libs/shared/src/domain/spec-segment.test.ts#L181), [`cbh-intro`](libs/shared/src/domain/spec-segment.test.ts#L200), [`cbh-background`](libs/shared/src/domain/spec-segment.test.ts#L211), [`cbh-vision`](libs/shared/src/domain/spec-segment.test.ts#L220), [`cbh-clarification`](libs/shared/src/domain/spec-segment.test.ts#L226), [`cbh-open-question`](libs/shared/src/domain/spec-segment.test.ts#L232), [`cbh-limitation`](libs/shared/src/domain/spec-segment.test.ts#L238), [`cbh-rationale`](libs/shared/src/domain/spec-segment.test.ts#L246), [`cbh-unrecognised`](libs/shared/src/domain/spec-segment.test.ts#L252), [`cbh-decision`](libs/shared/src/domain/spec-segment.test.ts#L270), [`cbh-xref`](libs/shared/src/domain/spec-segment.test.ts#L285), [`cbh-narrative`](libs/shared/src/domain/spec-segment.test.ts#L297), [`cbh-functional`](libs/shared/src/domain/spec-segment.test.ts#L312))

17. `parseCodeLinksInStatement()` extracts the trailing parenthetical as a non-test code reference, excluding markdown doc links so an ADR ref is not a code link; `linksForStatements()` pairs each segmented statement with its parsed test links; `findMisplacedCoverageLinks()` flags a test-file coverage link buried in a non-trailing parenthetical while ignoring one correctly in the trailing paren, a non-trailing prose doc link, an intra-doc anchor, an absolute URL, a placeholder or `<owner>`-style template path, a mid-prose source-file reference, a link quoted inside an inline code span, and a prose bracket that would otherwise fuse with the trailing parenthetical's first link. ([validated by `pcl-single`](libs/shared/src/domain/spec-link-parser.test.ts#L127), [`pcl-excl-doc`](libs/shared/src/domain/spec-link-parser.test.ts#L137), [`pcl-other-lang`](libs/shared/src/domain/spec-link-parser.test.ts#L145), [`lfs-pairs`](libs/shared/src/domain/spec-link-parser.test.ts#L157), [`fmcl-buried`](libs/shared/src/domain/spec-link-parser.test.ts#L185), [`fmcl-trailing-ok`](libs/shared/src/domain/spec-link-parser.test.ts#L195), [`fmcl-prose-doc`](libs/shared/src/domain/spec-link-parser.test.ts#L203), [`fmcl-anchor`](libs/shared/src/domain/spec-link-parser.test.ts#L211), [`fmcl-url`](libs/shared/src/domain/spec-link-parser.test.ts#L219), [`fmcl-placeholder`](libs/shared/src/domain/spec-link-parser.test.ts#L227), [`fmcl-source-ref`](libs/shared/src/domain/spec-link-parser.test.ts#L235), [`fmcl-code-span`](libs/shared/src/domain/spec-link-parser.test.ts#L243), [`fmcl-bracket-fusion`](libs/shared/src/domain/spec-link-parser.test.ts#L251))

18. `parseSpecStatus()` reads the `| Status |` table row (case-insensitive label, bold or plain, ignoring a non-table line that merely contains the word Status), keeping the leading word as label and bucketing it into `shipped` (Shipped / Complete / Implemented), `draft`, `in-progress` (In Progress / In review), `retired` (Retired / Removed / superseded), or `rejected` — dropping any trailing suffix and returning null for a missing row or unrecognised value; `matchesSpecStatusFilter()` matches everything (including unparsed specs) on `all` and only the selected bucket otherwise. ([validated by `spec-status`](apps/web-ui/src/lib/spec-status.test.ts), [`spec-status-parity-spec`](apps/web-ui/src/lib/spec-status.parity.test.ts#L33), [`spec-status-parity-adr`](apps/web-ui/src/lib/spec-status.parity.test.ts#L39), [`spec-status-parity-null`](apps/web-ui/src/lib/spec-status.parity.test.ts#L45))

19. The per-repo `SpecListView` counts folder statuses into chips that filter the cards when clicked, and the global doc browsers (`/specs` and `/adrs`, one shared `GlobalDocsView`) group docs by repo — linking each path to its per-repo detail page, spec or adr by `kind` — count statuses into chips that filter the list (the adr kind swapping in the frontmatter legend), narrow to a typed search text, and show the page's empty-state and no-match hints. ([validated by `SpecListView:50`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecListView.test.tsx#L48), [`SpecListView:55`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecListView.test.tsx#L53), [`GlobalDocsView:7`](apps/web-ui/src/components/GlobalDocsView.test.tsx#L7), [`GlobalDocsView:28`](apps/web-ui/src/components/GlobalDocsView.test.tsx#L28), [`GlobalDocsView:39`](apps/web-ui/src/components/GlobalDocsView.test.tsx#L39), [`GlobalDocsView:68`](apps/web-ui/src/components/GlobalDocsView.test.tsx#L68), [`GlobalDocsView:89`](apps/web-ui/src/components/GlobalDocsView.test.tsx#L89), [`GlobalDocsView:109`](apps/web-ui/src/components/GlobalDocsView.test.tsx#L109), [`GlobalDocsView:128`](apps/web-ui/src/components/GlobalDocsView.test.tsx#L128))

20. `SpecCard` renders its title and a Details link pointing at the given `detailsHref`, hides the coverage note when `testable` is 0, renders one file chip per file instead of a Details link when `files` is given, and renders the status pill label when `status` is given; `SpecDocument` renders one framed `data-doc-section` card per markdown section. ([validated by `SpecCard:7`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecCard.test.tsx#L7), [`SpecCard:36`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecCard.test.tsx#L36), [`SpecCard:47`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecCard.test.tsx#L47), [`SpecCard:67`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecCard.test.tsx#L67), [`SpecDocument:8`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/[...path]/SpecDocument.test.tsx#L8))

21. `resolveHref()` rewrites a repo-relative path (stripping a leading `./`) to a `github.com/<owner>/<repo>/blob/<branch>/<path>` URL marked external on the given branch, leaves an absolute https URL external, and leaves an in-page `#anchor`, a relative path when the repo is not `owner/name`, and an empty href unchanged and not external; `SpecDetails` renders an inline test link as that absolute GitHub URL opening in a new tab (`target=_blank`, `rel=noopener noreferrer`), rewrites a non-test ADR author link the same way without the test-link cue, and leaves links relative (no new tab) when the repo is not `owner/name`. ([validated by `resolveHref:347`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L347), [`resolveHref:356`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L356), [`resolveHref:362`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L362), [`resolveHref:368`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L368), [`resolveHref:377`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L377), [`resolveHref:384`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L384), [`resolveHref:391`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L391), [`SpecDetails:117`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L117), [`SpecDetails:148`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L148), [`SpecDetails:171`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L171), [`SpecDetails:213`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L213))

22. `SpecDetails` marks a drifted statement's `<mark>` with `data-drifted="true"` and surfaces a drift notice in its hover popover, keeps the statement highlight stable across re-renders, wraps statements spanning inline bold, inline code, or a code span containing literal link/emphasis syntax with `stmt-tested`, and renders none of the legacy DB-driven `tests[]` list, `TestLink` prop, or list-only/legacy badges. ([validated by `SpecDetails:46`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L46), [`SpecDetails:234`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L234), [`SpecDetails:253`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L253), [`SpecDetails:265`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L265), [`SpecDetails:285`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L285), [`SpecDetails:305`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L305), [`SpecDetails:326`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L326), [`SpecDetails:445`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L445), [`SpecDetails:464`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L464))
23. Enforcement support in `@re-cinq/lore-shared`: `segmentStatements()` stamps each statement's 1-based source start line (the paragraph line for a sentence, the marker line for a list item, shared across sentences split from one paragraph) so the `require-statement-links` rule can report at the offending line; `parseDocStatus()` normalizes a spec's `| Status |` table row or an ADR's frontmatter `status:` (read only inside the `---` block) into one of five buckets — `draft`, `in-progress` (ADR `proposed`), `shipped` (folding Shipped / Implemented / Complete / Accepted / Done / Live and ADR `accepted`, stripping bold markers and trailing prose), `retired` (Retired / Removed / ADR `superseded` — shipped then terminated), `rejected` (never accepted), or `null` for an absent/unrecognised value; and `statusTier()` maps those to the report gate — `rejected` and `retired` skip, every other status (shipped / draft / in-progress / unknown) warns. ([validated by `ss-line`](libs/shared/src/domain/spec-segment.test.ts#L327), [`spec-status`](libs/shared/src/domain/spec-status.test.ts))


24. `GlobalDocsView` is a client component rendered by the `/specs` and `/adrs` server components, so those pages pass it only serializable props — a function prop fails the render at runtime, with the message redacted in production builds. ([validated by `specs boundary`](apps/web-ui/src/app/global-docs-boundary.test.tsx#L40), [`adrs boundary`](apps/web-ui/src/app/global-docs-boundary.test.tsx#L46))

25. The global and per-repo doc list endpoints return each document's lifecycle pill (`docStatusPill`) alongside the list, so a list page resolves statuses in its one API call instead of one source fetch per document — a fan-out that put a single `/specs` render at 114 requests against the API's shared 200/min `default` bucket. ([validated by `docStatusPill`](libs/shared/src/domain/spec-status.test.ts#L319))

26. `SpecDetails` sanitizes the spec markdown after raw-HTML parsing and before statement highlighting: an injected `<script>` is stripped while statement `<mark>` wrapping still applies, an `onerror` attribute is removed from raw `<img>` HTML, a `javascript:` href is stripped from a raw anchor, and an injected `<svg onload>` is dropped. ([validated by `SpecDetails.test.tsx:400`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L400), [`SpecDetails.test.tsx:418`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L418), [`SpecDetails.test.tsx:428`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L428), [`SpecDetails.test.tsx:437`](apps/web-ui/src/app/repos/[owner]/[repo]/specs/SpecDetails.test.tsx#L437))
