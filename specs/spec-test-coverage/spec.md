...
Enforcement is tiered by lifecycle status, consistently for specs and ADRs,
which `parseDocStatus` folds into the same four buckets. `statusTier` maps them:
**rejected** specs (and `superseded` ADRs) — the rule does not run; **shipped**
specs (and `accepted` ADRs) — **error**; **draft** / **in-progress** (ADR
`proposed`) or an absent status — **warn**. Because ESLint severity is static
per rule, the one core is registered as two rules — a `warn` variant firing on
the warn tier and an `error` variant on the error tier — so a doc's tier matches
at most one (rejected matches neither), and the two never double-report.
Statement start lines for the report location come from the `line` field on
`segmentStatements`.
...
| Enforcement | **ESLint `lore/require-statement-links`** — testable statement with no link, tiered by status: `rejected` (ADR `superseded`) skipped, `shipped` (ADR `accepted`) → `error`, `draft` / `in-progress` (ADR `proposed`) → `warn` | Deterministic, always-on CI gate; the statement-side mirror of the test-side `lore/require-spec-link` |
...
8. The cron `spec_coverage_validate` runs on every successful `/api/ingest` for a repo, parses each spec's links, and resolves each against `{schema}.chunks` AST metadata — passing on a covering chunk (or a file-level null line resolved by any chunk of the file), flagging `file-missing`, `line-out-of-range`, or a non-trailing link, grouping the broken-link report by spec, opening a PR comment listing broken links when any exist, and treating a `spec-link-rot`-labelled open issue as the already-filed fallback. ([validated by `spec-coverage-validate-trigger.test.ts:18`](apps/lore-api/src/api/routes/spec-coverage-validate-trigger.test.ts#L18), [`spec-coverage-validate.test.ts:92`](libs/shared/src/detect/spec-coverage-validate.test.ts#L92), [`resolve-covers`](libs/shared/src/detect/spec-coverage-validate.test.ts#L29), [`file-missing`](libs/shared/src/detect/spec-coverage-validate.test.ts#L37), [`line-out-of-range`](libs/shared/src/detect/spec-coverage-validate.test.ts#L45), [`null-line-file`](libs/shared/src/detect/spec-coverage-validate.test.ts#L54), [`null-line-missing`](libs/shared/src/detect/spec-coverage-validate.test.ts#L62), [`any-chunk-covers`](libs/shared/src/detect/spec-coverage-validate.test.ts#L70), [`all-resolve`](libs/shared/src/detect/spec-coverage-validate.test.ts#L108), [`no-test-links`](libs/shared/src/detect/spec-coverage-validate.test.ts#L118), [`non-trailing`](libs/shared/src/detect/spec-coverage-validate.test.ts#L124), [`report-grouped`](libs/shared/src/detect/spec-coverage-validate.test.ts#L140), [`empty-report`](libs/shared/src/detect/spec-coverage-validate.test.ts#L171), [`issue-open`](libs/shared/src/detect/spec-coverage-validate.test.ts#L177), [`issue-none`](libs/shared/src/detect/spec-coverage-validate.test.ts#L183), [`issue-empty`](libs/shared/src/detect/spec-coverage-validate.test.ts#L189))
...
23. Enforcement support in `@re-cinq/lore-shared`: `segmentStatements()` stamps each statement's 1-based source start line (the paragraph line for a sentence, the marker line for a list item, shared across sentences split from one paragraph) so the `require-statement-links` rule can report at the offending line; `parseDocStatus()` normalizes a spec's `| Status |` table row or an ADR's frontmatter `status:` (read only inside the `---` block) into one of four buckets — `draft`, `in-progress` (ADR `proposed`), `shipped` (folding Shipped / Implemented / Complete / Accepted / Done / Live and ADR `accepted`, stripping bold markers and trailing prose), `rejected` (ADR `superseded`), or `null` for an absent/unrecognised value; and `statusTier()` maps those to the enforcement tier — `rejected` skips, `shipped` errors, and `draft` / `in-progress` / unknown warn. ([validated by `ss-line`](libs/shared/src/spec-segment.test.ts#L330), [`ds-shipped`](libs/shared/src/spec-status.test.ts#L8), [`ds-folds`](libs/shared/src/spec-status.test.ts#L14), [`ds-strips`](libs/shared/src/spec-status.test.ts#L26), [`ds-draft`](libs/shared/src/spec-status.test.ts#L34), [`ds-inprogress`](libs/shared/src/spec-status.test.ts#L40), [`ds-rejected`](libs/shared/src/spec-status.test.ts#L46), [`ds-null`](libs/shared/src/spec-status.test.ts#L52), [`ds-adr-accepted`](libs/shared/src/spec-status.test.ts#L70), [`ds-adr-proposed`](libs/shared/src/spec-status.test.ts#L76), [`ds-adr-superseded`](libs/shared/src/spec-status.test.ts#L82), [`ds-adr-frontmatter`](libs/shared/src/spec-status.test.ts#L88), [`tier-skip`](libs/shared/src/spec-status.test.ts#L102), [`tier-error`](libs/shared/src/spec-status.test.ts#L106), [`tier-warn`](libs/shared/src/spec-status.test.ts#L110))
