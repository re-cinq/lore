...
- Only one run per repo is in flight at a time. Serialisation reuses the existing unique index on `pipeline.assembly_runs (repo, subject_key)` for open rows: an implementation-loop task's run declares the constant `backlog` subject (the repo is the index's other half, so the key carries no repo — exactly as `reviewSubject` carries none), and `PgAssemblyRuns.start` returns the id of the in-flight run instead of creating a second one. The mint-to-run gap is covered by the picked issue's active task. ([validated by backlog subject on the run](../../apps/floor/src/jobs/assembly-run/assembly-run-station-backend.test.ts#L92), [validated by constant backlog key](../../libs/shared/src/project/assembly-runs/subject-keys.test.ts#L58), [validated by open run skips the repo](../../apps/floor/src/jobs/backlog/implementation-loop.test.ts#L72), [validated by active task skips the pick](../../apps/floor/src/jobs/backlog/implementation-loop.test.ts#L80))
