...
- The driver runs on a cron tick as a safety net and is also re-armed from the run's terminal hook — `finishLine`'s winning finisher emits a repo-scoped tick event — so there is no scheduling gap between one ticket finishing and the next starting. ([validated by completed run re-arms](../../apps/floor/src/jobs/backlog/loop-run-closed.test.ts#L65), [validated by repo-scoped tick](../../apps/floor/src/jobs/backlog/implementation-loop.test.ts#L96))
