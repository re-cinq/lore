...
list (RepoContextView / ContextView)
  → ContextFilters  (data-driven chips + GET search form)
  → ContextCard     (badge, linked path, metadata header, clamped preview)
       → ChunkBody  (preview)
...
- **Search** — a GET `<form>` (`?q=` + `?type=`) keeps both views
  server-rendered; the backend filters on `search_tsv @@
  websearch_to_tsquery('english', $q)` and ranks by `ts_rank`. The chip set is a
  `SELECT DISTINCT content_type`, unaffected by the active filter.
...
3. The content-type filter renders one chip per type actually present in the data — never a hardcoded list — in a canonical order. ([validated by `ContextFilters.test.tsx:7`](web-ui/src/app/repos/[owner]/[repo]/context/ContextFilters.test.tsx#L7), [`content-types.test.ts:27`](web-ui/src/lib/content-types.test.ts#L27), [`ContextView.test.tsx:36`](web-ui/src/app/context/ContextView.test.tsx#L36))
4. The keyword search box preserves the active content-type filter via a hidden field, so searching within a filtered view stays filtered. ([validated by `ContextFilters.test.tsx:35`](web-ui/src/app/repos/[owner]/[repo]/context/ContextFilters.test.tsx#L35))
...
6. The global cross-repo view labels each chunk with its repo, and the global detail page groups a file's chunks by repo with a "view in repo →" link. ([validated by `ContextView.test.tsx:18`](web-ui/src/app/context/ContextView.test.tsx#L18), [`ContextFileView.test.tsx:48`](web-ui/src/app/repos/[owner]/[repo]/context/ContextFileView.test.tsx#L48))
7. A list card shows a height-clamped rich preview with a metadata header instead of a raw `<pre>` dump. ([validated by `ChunkBody.test.tsx:67`](web-ui/src/app/repos/[owner]/[repo]/context/ChunkBody.test.tsx#L67), [`ContextCard.test.tsx:35`](web-ui/src/app/repos/[owner]/[repo]/context/ContextCard.test.tsx#L35))
