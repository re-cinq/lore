...
- [x] T008 Delete the 10 migrated `registerJob` lines + their imports from `index.ts`; leave the 6 in-process registrations intact

## Phase 4 — Helm CronJobs

- [x] T009 Extract the Deployment env block into `agent-helm/templates/_env.tpl` (`define "lore-agent.env"`); update `deployment.yaml` to consume it (no rendered diff)
- [x] T010 Add `agent-helm/templates/cronjob.yaml`: range over `.Values.cronJobs`, one `CronJob` each — `command: ["node","dist/job-runner.js","{{ .job }}"]`, shared env/SA, `concurrencyPolicy: Forbid`, `startingDeadlineSeconds`, history limits, `restartPolicy: Never` + `backoffLimit`, `activeDeadlineSeconds`, and a `resources` block merging per-job overrides over the default
- [x] T011 Add to `agent-helm/values.yaml`: `cronJobDefaults` (resources requests+limits, activeDeadlineSeconds, startingDeadlineSeconds, backoffLimit, history limits) + the `cronJobs` list (10 entries: name, schedule, job, optional resources/deadline overrides — bump memory/deadline for `context_reindex`, `eval_runner`, `autoresearch`)

## Phase 5 — Verify

- [x] T012 `agent` typecheck + `vitest run` green after the reorg and runner
- [x] T013 `helm template` the agent chart: 10 CronJobs render with correct schedules, identical env to the Deployment, the documented policies, and non-empty `resources.{requests,limits}` + `activeDeadlineSeconds` on every one (heavy jobs higher than cleanups); confirm AC 1–8 incl. 1a/1b/5a
