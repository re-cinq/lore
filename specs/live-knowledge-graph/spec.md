...
- FR-4.4: Optional `repo` scope parameter. The live-graph read binds the queried
  repo and maps each row to a `GraphEdge`. ([validated by `queries the live graph bound to the repo and maps to GraphEdge`](libs/shared/src/outbound/project/knowledge/knowledge-pg.test.ts#L29))
