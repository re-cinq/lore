...
7. `driftCheckFile()` flags a `Statement` `drifted=true` with `drift_reason="code-content-changed"` when a linked **or covered** chunk's `content_hash` changes — including the case where the implementation changed but its test did not — and still catches link rot (`file-missing`/`line-out-of-range`).
