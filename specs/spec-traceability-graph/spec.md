...
38. `runIngestGraph` prunes whole subtrees for graph docs whose files left the repo tree (a moved or deleted spec/ADR no longer haunts the graph or the web-UI). After the projection loop, docs in scope for the run but absent from the tree selection are deleted: a Spec with its Statements, Sections, AcceptanceCriteria, TraceLinks, Blocks and the `Repo.specs` edge — while a sibling spec survives intact — and an ADR with its Blocks, the `Repo.adrs` edge, and incoming `decided_by`/`supersedes`/TraceLink refs. Link-target chunks are GC'd only when nothing else owns them (a TestChunk still validated by another spec's Statement or AcceptanceCriterion, or carrying coverage, survives; a solely-owned one is deleted) and the owning Feature only when its last spec goes. The prune runs even when every current file hash-skipped (a moved file's OLD path never re-projects, so freshness gives no signal), re-runs are no-ops, and it is guarded: an empty tree selection prunes nothing (a bad or partial tree read must never wipe the graph), an all-failed run prunes nothing, a glob-chunked run prunes only inside its own directory (candidates pass the same selection filter that produced the tree files), and a prune failure is per-candidate isolated with the deleted count reported as `pruned` in the summary — while a failed doc-list read reports `pruned` as absent (didn't run), never a misleading `pruned 0` that reads as a clean reconcile. ([validated by `prune-removed-docs:20`](libs/shared/src/spec-trace/prune-removed-docs.test.ts#L20), [validated by `prune-removed-docs:30`](libs/shared/src/spec-trace/prune-removed-docs.test.ts#L30), [validated by `prune-removed-docs:36`](libs/shared/src/spec-trace/prune-removed-docs.test.ts#L36), [validated by `prune-removed-docs:46`](libs/shared/src/spec-trace/prune-removed-docs.test.ts#L46), [validated by `prune-removed-docs:167`](libs/shared/src/spec-trace/prune-removed-docs.test.ts#L167), [validated by `prune-removed-docs:203`](libs/shared/src/spec-trace/prune-removed-docs.test.ts#L203), [validated by `prune-removed-docs:240`](libs/shared/src/spec-trace/prune-removed-docs.test.ts#L240), [validated by `prune-removed-docs:268`](libs/shared/src/spec-trace/prune-removed-docs.test.ts#L268), [validated by `prune-removed-docs:282`](libs/shared/src/spec-trace/prune-removed-docs.test.ts#L282), [validated by `prune-removed-docs:315`](libs/shared/src/spec-trace/prune-removed-docs.test.ts#L315), [validated by `prune-removed-docs:570`](libs/shared/src/spec-trace/prune-removed-docs.test.ts#L570), [validated by `ingest-graph-task:250`](libs/shared/src/spec-trace/ingest-graph-task.test.ts#L250), [validated by `ingest-graph-task:271`](libs/shared/src/spec-trace/ingest-graph-task.test.ts#L271), [validated by `ingest-graph-task:288`](libs/shared/src/spec-trace/ingest-graph-task.test.ts#L288), [validated by `ingest-graph-task:308`](libs/shared/src/spec-trace/ingest-graph-task.test.ts#L308), [validated by `ingest-graph-task:327`](libs/shared/src/spec-trace/ingest-graph-task.test.ts#L327), [validated by `ingest-graph-task:347`](libs/shared/src/spec-trace/ingest-graph-task.test.ts#L347); implemented by [`prune-removed-docs.ts:179`](libs/shared/src/spec-trace/prune-removed-docs.ts#L179), [`ingest-graph-task.ts:367`](libs/shared/src/spec-trace/ingest-graph-task.ts#L367))
...
40. Subtree deletion is crash-resumable: chunk GC, Feature GC, and the Block sweep run BEFORE the doc node is removed — discounting the doomed subtree's own ownership via exclusion sets — and the doc node with its children and `Repo.specs`/`Repo.adrs` edge goes last in one atomic mutation, so a prune interrupted at any earlier point (or on the final delete itself) leaves the doc listed as a prune candidate and a later run completes the cleanup instead of stranding orphan Blocks, chunks, or Features forever. A TraceLink targeting a pruned ADR is also detached from an owning AcceptanceCriterion's `trace_links` edge, symmetric to the Statement back-edge handling. ([validated by `prune-removed-docs:381`](libs/shared/src/spec-trace/prune-removed-docs.test.ts#L381), [validated by `prune-removed-docs:424`](libs/shared/src/spec-trace/prune-removed-docs.test.ts#L424), [validated by `prune-removed-docs:463`](libs/shared/src/spec-trace/prune-removed-docs.test.ts#L463), [validated by `prune-removed-docs:505`](libs/shared/src/spec-trace/prune-removed-docs.test.ts#L505); implemented by [`prune-removed-docs.ts:179`](libs/shared/src/spec-trace/prune-removed-docs.ts#L179), [`prune-removed-docs.ts:280`](libs/shared/src/spec-trace/prune-removed-docs.ts#L280))
