...
`present_paths` is optional. Omitted, the server reads the repo's tree over the GitHub API and reconciles against that, so a caller with no checkout can sweep — which is what makes the sweep automatic rather than a script somebody remembers to run. A tree that reads empty is treated as a failed read and deletes nothing, answering 502: an empty tree is never an empty repo. ([validated by `reads the tree itself when the body omits present_paths, so a scheduler with no checkout can reconcile`](apps/lore-api/src/transport/routes/repos/chunks-prune.test.ts#L79), [`answers 502 rather than deleting anything when the tree cannot be read`](apps/lore-api/src/transport/routes/repos/chunks-prune.test.ts#L103), [`prunes re-cinq/lore against the tree it read, with no paths from the caller`](apps/lore-api/src/work/chunks/reconcile-orphans.test.ts#L21), [`deletes nothing when the tree reads empty, because that is a failed read and not an empty repo`](apps/lore-api/src/work/chunks/reconcile-orphans.test.ts#L44))
