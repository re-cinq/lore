...
- **Method + path**: `POST /api/repos/:owner/:repo/coverage`
  ...
  Implemented by the [route registration](../../../mcp-server/src/api/routes/index.ts#L72)
  dispatching to the [`handleCoverageRoute` handler](../../../mcp-server/src/api/routes/coverage.ts#L38).
...
chunks, and file = distinct file count. ([validated by `returns 200 with node, edge, and file counts derived from the body`](../../../mcp-server/src/api/routes/coverage-route.test.ts#L17))

An LCOV `payload` is parsed and normalized into per-file groups with collapsed
ranges before counting. ([validated by `returns 200 with counts from a normalized lcov payload`](../../../mcp-server/src/api/routes/coverage-route.test.ts#L42))

Two `TN` tests on the same file produce two distinct coverage nodes.
([validated by `counts per-test nodes for an lcov payload with two TN tests on the same file`](../../../mcp-server/src/api/routes/coverage-route.test.ts#L60))

A Cobertura `payload` is parsed into one file-keyed group with collapsed
ranges. ([validated by `returns 200 with counts from a normalized cobertura payload`](../../../mcp-server/src/api/routes/coverage-route.test.ts#L79))

The spec-trace trigger fires with the normalized groups (not the raw payload)
when the agent env is configured. ([validated by `fires the spec-trace trigger with the normalized coverage groups when the agent env is configured`](../../../mcp-server/src/api/routes/coverage-route.test.ts#L98))

An unsupported `format` is rejected with 400. ([validated by `returns 400 for an unsupported format`](../../../mcp-server/src/api/routes/coverage-route.test.ts#L129))

A missing `commit` is rejected with 400. ([validated by `returns 400 when commit is missing`](../../../mcp-server/src/api/routes/coverage-route.test.ts#L142))

LCOV `DA` rows with zero hits are filtered and a single covered line yields a
length-1 range. ([validated by `returns a length-1 range for one covered line`](../../../mcp-server/src/api/routes/coverage.test.ts#L5))

Contiguous covered lines collapse into one range. ([validated by `collapses three contiguous covered lines into one range`](../../../mcp-server/src/api/routes/coverage.test.ts#L12))

Records sharing a `TN` merge into one group spanning both files' chunks.
([validated by `merges two records sharing a TN into one group concatenating both files' chunks`](../../../mcp-server/src/api/routes/coverage.test.ts#L41))

A Cobertura class block collapses contiguous covered lines per class.
([validated by `collapses contiguous covered lines per class into ranges`](../../../mcp-server/src/api/routes/coverage.test.ts#L67))
