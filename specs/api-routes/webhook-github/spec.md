...
length-mismatched signature is rejected without throwing. ([validated by `returns true for a matching sha256 signature`](../../../apps/mcp-server/src/api/routes/webhook-signature.test.ts#L11), [`returns false when the signature is for a different body`](../../../apps/mcp-server/src/api/routes/webhook-signature.test.ts#L15), [`returns false on a length mismatch without throwing`](../../../apps/mcp-server/src/api/routes/webhook-signature.test.ts#L19))

An unset secret returns 503; a missing signature header returns 401 `missing signature`; an invalid signature returns 401 `invalid signature`. ([validated by `returns 503 when the webhook secret is unset`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L57), [`returns 401 when the signature header is missing`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L62), [`returns 401 on an invalid signature`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L67))

A merged spec PR parses tasks.md and syncs spec-tasks; a non-spec branch, an already-synced spec, and a missing tasks.md each skip with the matching reason; a null pool returns 503. ([validated by `syncs spec-tasks on a merged spec PR`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L87), [`skips a non-spec branch`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L110), [`skips when spec-tasks already synced`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L125), [`skips when tasks.md is missing (no GitHub token)`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L131), [`returns 503 when pool is null on a spec merge`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L106))

A `synchronize` pull_request triggers the review reactor; an unhandled action skips. ([validated by `triggers the review reactor on synchronize`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L145), [`skips an unhandled pull_request action`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L153))

A submitted review triggers both the review reactor and auto-merge; a non-submitted review skips; missing repo/pr returns 400. ([validated by `triggers reactor and auto-merge on a submitted review`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L175), [`skips a non-submitted review`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L184), [`returns 400 when repo/pr missing on submitted review`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L180))

A completed `check_run` / `check_suite` fans out auto-merge to every PR; a non-completed check and an empty PR list each skip. ([validated by `fans out auto-merge for check_run`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L195), [`fans out auto-merge for check_suite`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L199), [`skips a non-completed check`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L203), [`skips when there are no pull_requests`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L207))

A created PR comment triggers the review reactor; an edited comment and a non-PR comment skip. ([validated by `triggers reactor on a PR comment`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L230), [`skips an edited (non-created) PR comment`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L242), [`skips a non-PR comment`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L238))

A `lore`-labeled issue creates a task (type from issue labels) and labels the issue; a mismatched label, a duplicate, missing fields, a null pool, and a createTask failure each return their documented status. ([validated by `picks the implementation task type from issue labels`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L319), [`skips when the label does not match dispatch_label`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L276), [`skips a duplicate task and comments on the issue`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L330), [`returns 400 on missing fields`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L272), [`returns 503 when pool is null after the label matches`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L315), [`returns 500 when createTask throws`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L363))

Invalid JSON on any dispatched event returns 400. ([validated by `returns 400 on invalid JSON`](../../../apps/mcp-server/src/api/routes/webhook-github.test.ts#L168))
