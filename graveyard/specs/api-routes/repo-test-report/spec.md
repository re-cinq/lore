...
correct chunk/validated/coverage/edge counts with `violated = 0`. ([validated by `connects Statement to File via validated_by to TestChunk to coverage to covers for a per-it descriptor`](libs/shared/src/work/spec-trace/ingest-test-report.test.ts#L463))

Only a failing spec-anchored test increments `violated`; a failing
non-anchored test does not. ([validated by `marks the spec Statement violated with a reason naming failing test renders when its result passed is false`](libs/shared/src/work/spec-trace/ingest-test-report.test.ts#L317))
