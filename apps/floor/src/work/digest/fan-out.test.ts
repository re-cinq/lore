...
describe("createDailyDigestTickHandler", () => {
  ...
  it("carries every repo's window, sections and grouping in the run's args, with ref set to the host repo's default branch", async () => {
    ...
    expect({
      ...args,
      digest_repos: decodeDigestRepos(String(args.digest_repos)),
    }).toEqual({
      job_run_id: "jr-1",
      channel: "C1",
      ...
      digest_repos: [
        {
          repo: "re-cinq/lore",
          since: "2026-09-24T07:05:00.000Z",
          sections: ["roadmap"],
          group_by: "area",
        },
      ],
  ...
  it("opens the window at the repo's last finished post", async () => {
    const { assemblyRuns, posts, handler } = harness([
      repoRow("re-cinq/lore", "C1"),
    ]);
    const yesterday = new InMemoryDigestPosts(
      () => new Date("2026-09-24T07:10:00Z"),
    );

    await yesterday.claim("old", [
    ...
    expect(
      decodeDigestRepos(String(assemblyRuns.rows[0].args.digest_repos))[0]
        .since,
    ).toBe("2026-09-24T07:10:00.000Z");
  });
  ...
  it("skips a channel whose run for today is in flight", async () => {
    ...
    expect({ runs: assemblyRuns.rows.length, started }).toEqual({
      runs: 1,
      started: [],
    });
  });
