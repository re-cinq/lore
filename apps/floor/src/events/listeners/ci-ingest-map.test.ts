...
describe("mapCiIngest", () => {
      ...
      events: [
        {
          ...
          params: {
            repo: "re-cinq/lore",
            kind: "adrs",
            payload: { commit: "deadbeef", force: undefined },
          },
        },
        {
          eventName: "internal.ingest.spec_trace",
  ...
  it("rejects a non-doc kind with a 400 naming the unsupported kind", () => {
    ...
    expect(result).toEqual({
      ...
      status: 400,
      error:
        "unsupported kind(s): tests — only specs/adrs project here; test projection is CI-only (POST /test-report + /coverage)",
    });
  });
