...
async function processCluster(cluster: Cluster): Promise<"pr" | "task" | "skip"> {
  ...
  if (best.delta >= IMPROVEMENT_THRESHOLD) {
    // Only count a PR when one was actually opened — a missing target repo or a
    // failed open must not inflate the run summary.
    const opened = await openResearchPR(cluster, best, candidates);
    return opened ? "pr" : "skip";
  } else {
...
async function evaluateCandidate(
  ...
  // mkdtemp outside the try: if it throws there is nothing to clean up, and the
  // finally can then unconditionally remove the dir even when the eval throws
  // (the expected failure path — previously the rm sat before the throw and leaked).
