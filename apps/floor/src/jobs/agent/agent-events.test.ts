...
const result = (extra: Record<string, unknown>) => ({ type: "result", subtype: "success", ...extra });
...
describe("parseAgentEvents", () => {
  ...
  it("falls back to a flat model field, then to unknown", () => {
    const flat = line(src, result({ model: "claude-haiku-4-5", usage: { input_tokens: 1 } }));
  ...
  it("skips non-object envelopes, string events, blank and unparseable lines", () => {
    const ndjson = ["", "  ", "5", "{not json", line(src, "raw stdout text")].join("\n");
    expect(parseAgentEvents(ndjson)).toEqual([]);
  });
