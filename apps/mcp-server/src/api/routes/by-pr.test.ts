...
describe("GET /api/tasks/by-pr/:owner/:repo/:n", () => {
  ...
  it("returns 503 when pool is null", async () => {
    const res = makeRes();
    await handleApiRoute(makeReq({ url: "/api/tasks/by-pr/o/r/5", headers: AUTH }), res, null);
  ...
  it("resolves via the DB fast path", async () => {
    ...
    expect(res.json).toEqual({ task_id: "t1", trailer_source: "db" });
  });
  it("resolves from the PR body when the DB misses", async () => {
    ...
    const res = makeRes();
    await handleApiRoute(makeReq({ url: "/api/tasks/by-pr/o/r/5", headers: AUTH }), res, pool as any);
    expect(res.json).toEqual({ task_id: "abc-123", trailer_source: "pr_body" });
  });
  it("resolves from the final commit when the body has no trailer", async () => {
    ...
    const res = makeRes();
    await handleApiRoute(makeReq({ url: "/api/tasks/by-pr/o/r/5", headers: AUTH }), res, pool as any);
    expect(res.json).toEqual({ task_id: "xyz", trailer_source: "final_commit" });
  });
  it("returns 404 when no trailer is found anywhere", async () => {
    ...
    const res = makeRes();
    await handleApiRoute(makeReq({ url: "/api/tasks/by-pr/o/r/5", headers: AUTH }), res, pool as any);
    expect(res.json).toEqual({ error: "no_trailer_found" });
  });
  it("returns 404 when the PR is not found", async () => {
    ...
    const res = makeRes();
    await handleApiRoute(makeReq({ url: "/api/tasks/by-pr/o/r/5", headers: AUTH }), res, pool as any);
    expect(res.json).toEqual({ error: "pr_not_found" });
  });
