...
vi.mock("../webhook/webhook-ensure.js", () => ({ ensureFloorWebhook: vi.fn() }));
vi.mock("../pipeline/pipeline.js", () => ({ createTask: vi.fn() }));

import { ensureFloorWebhook } from "../webhook/webhook-ensure.js";
import { createTask } from "../pipeline/pipeline.js";
...
describe("onboardRepo", () => {
  it("ensures the Floor webhook for the onboarded repo and returns its outcome", async () => {
    vi.mocked(ensureFloorWebhook).mockResolvedValue({ ok: true, hookId: 7, created: true });
  ...
  it("still completes onboarding when the webhook ensure is skipped", async () => {
    ...
    expect(result.webhook).toEqual({ ok: false, reason: "app_no_webhook_permission" });
  });
});
