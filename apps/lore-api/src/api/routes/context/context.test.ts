...
describe("GET /api/context", () => {
  ...
  it("passes debug=1 through and returns the trace in the envelope", async () => {
    ...
    expect(res.result).toEqual({ text: "ctx", sections: [], trace: [{ section: "repo", included: true }] });
  });
  ...
  it("forwards max_tokens, agent_id, and cross_repo through to assembleContext", async () => {
    ...
    await get(makePool(), "/api/context?query=hi&repo=o/r&max_tokens=16000&agent_id=a-1&cross_repo=true");
