...
import { enforceFeatureInput, parseSectionAnswers, ValidationError } from "@re-cinq/lore-shared/feature-planning/feature-input.js";
import { roundInFlight, canFinalize, latestReadyGap } from "@re-cinq/lore-shared/project/features/features-port.js";
...
async function run(h: ResponseToolkit, fn: () => Promise<ResponseObject>): Promise<ResponseObject> {
  try {
    ...
    if (err instanceof ValidationError) return h.response({ error: err.message }).code(400);
 ...
 */
async function kickPlanning(repoFullName: string, featureId: string, iteration: number, description: string): Promise<string> {
  const task = await createTask(description, "feature-planning", repoFullName, "ui", { feature_id: featureId, iteration }, "immediate");
...
export function featuresRoutes(): ServerRoute[] {
  return [
    ...
    // POST .../features — create a draft + kick planning round 1.
    {
      ...
      handler: (request, h) =>
        run(h, async () => {
          ...
          const taskId = await kickPlanning(repo, feature.id, row.iteration, prompt);
    ...
    // GET .../features/:id — feature + iterations.
    {
      ...
      handler: (request, h) =>
        run(h, async () => {
          const feature = await (await projectFor(repoOf(request.params))).features.get(request.params.id);
    ...
    // DELETE .../features/:id — remove the feature + its iterations (CASCADE).
    {
      ...
      handler: (request, h) =>
        run(h, async () => {
          ...
          if (!deleted) return h.response({ error: "feature not found" }).code(404);
    ...
    // POST .../features/:id/iterations — submit a refinement round.
    {
      ...
      handler: (request, h) =>
        run(h, async () => {
          ...
          const taskId = await kickPlanning(repo, id, row.iteration, description);
          await features.attachIterationTask(id, row.iteration, taskId);
          return h.response({ task_id: taskId, iteration: row.iteration }).code(202);
        }),
    },
      ...
      handler: (request, h) =>
        run(h, async () => {
          ...
          if (!Number.isInteger(iteration) || iteration < 0) {
            return h.response({ error: "iteration must be a non-negative integer" }).code(400);
          }
          ...
          if (isPlanningPhase(feature.status)) {
            await features.transitionStatus(id, decideFeatureStatus(planningResult), { draft_spec_md: planningResult.draft_spec_markdown });
          }
          return h.response({ ok: true });
        }),
    },
      ...
      handler: (request, h) =>
        run(h, async () => {
          ...
          if (!canFinalize(feature.status)) {
            return h.response({ error: `cannot finalize a feature in '${feature.status}' state` }).code(409);
          }
    ...
    // POST .../features/:id/split — create a child draft from a split suggestion.
    {
      ...
      handler: (request, h) =>
        run(h, async () => {
          ...
          if (!parent) return h.response({ error: "feature not found" }).code(404);
          if (!latestReadyGap(parent.iterations)?.split_suggestion) {
            return h.response({ error: "parent feature has no split suggestion to split from" }).code(409);
          }
