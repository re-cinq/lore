...
function clientQueries(pool: ReturnType<typeof makePool>, over: Record<string, unknown> = {}) {
  const map: Record<string, unknown> = { "FOR UPDATE": { rows: [{ settings: {} }] }, ...over };
  pool.__client.query.mockImplementation((sql: string) => {
    for (const [frag, result] of Object.entries(map)) {
      if (sql.includes(frag)) return result instanceof Error ? Promise.reject(result) : Promise.resolve(result);
    }
    return Promise.resolve({});
  });
}

const get = (pool: unknown) => buildServer(() => pool as any).inject({ method: "GET", url: URL_BASE, headers: AUTH });
...
describe("routes — dark-factory settings", () => {
  ...
  it("returns 405 for unsupported methods", async () => {
    const res = await buildServer(() => makePool() as any).inject({ method: "DELETE", url: URL_BASE, headers: AUTH });
  ...
  describe("GET", () => {
    ...
    it("returns the resolved dark_factory settings", async () => {
      ...
      const res = await get(makePool());
      expect(res.result).toEqual({ resolved: true, partial: { enabled: true } });
    });
  ...
  describe("PUT", () => {
    function put(body: unknown, headers: Record<string, string> = {}, pool = makePool()) {
      ...
      return buildServer(() => pool as any).inject({ method: "PUT", url: URL_BASE, headers: { ...AUTH, ...headers }, payload });
    }
    ...
    it("returns 400 with issues when schema validation fails", async () => {
      ...
      expect(res.result).toEqual({ error: "invalid_settings", issues: [{ path: "enabled" }] });
    });
    ...
    it("returns 400 with the message when a plain error is thrown", async () => {
      ...
      const res = await put({ enabled: "nope" });
      expect(res.result).toEqual({ error: "invalid_settings", issues: "bad shape" });
    });
    ...
    it("applies an admin-tier change and writes the audit log", async () => {
      ...
      expect(res.result).toMatchObject({ ok: true, applied: { review: "auto" }, ceremony: { tier: "admin" } });
    });
    ...
    it("merges the nested auto_merge object", async () => {
      ...
      expect((res.result as { applied: { auto_merge: unknown } }).applied.auto_merge).toEqual({ paths: ["x"], min_trust: "full" });
    });
    ...
    it("merges auto_merge when there is no prior auto_merge and null settings", async () => {
      ...
      const res = await put({ auto_merge: { min_trust: "full" } }, {}, pool);
      expect((res.result as { applied: { auto_merge: unknown } }).applied.auto_merge).toEqual({ min_trust: "full" });
    });
    ...
    it("two-key gates a per-task-type execution.image change", async () => {
      vi.mocked(twoKeyFieldsTouched).mockReturnValue(["task_overrides.implementation.execution.image"]);
      const res = await put({ task_overrides: { implementation: { execution: { image: "golang:1.23" } } } });
      expect(res.statusCode).toBe(403);
    ...
    it("applies a two-key change after CODEOWNERS approval", async () => {
      ...
      expect((res.result as { ceremony: unknown }).ceremony).toEqual({ tier: "two_key", pr_ref: "#5", approver: "alice", pr_url: "https://gh/5" });
    });
