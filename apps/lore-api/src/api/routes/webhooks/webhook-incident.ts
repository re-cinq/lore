 ...
 */
export function verifyPagerDutySignature(secret: string, header: string | undefined, body: string): boolean {
 ...
 */
export function parseIncident(body: string, now: number): { error: string } | { repo: string; entry: IncidentEntry } {
  let payload: unknown;
  try {
  ...
  const envelope = root.incident && typeof root.incident === "object" ? root.incident : root;
  ...
  if (typeof repo !== "string" || !REPO_NAME.test(repo)) return { error: "repo must be in owner/name form" };
  ...
  const candidate = {
    ...
    date: typeof incident.date === "string" ? incident.date : new Date(now).toISOString(),
    resolved: Boolean(incident.resolved ?? incident.status === "resolved"),
    url: typeof (incident.url ?? incident.html_url) === "string" ? (incident.url ?? incident.html_url) : null,
  };
  ...
  return { repo, entry: { ...parsed.data, date: new Date(clampedMs).toISOString() } };
}
...
export function incidentWebhookRoute(getPool: () => Pool | null): ServerRoute {
  return {
    ...
    handler: async (request, h) => {
      ...
      if (!secret && !token) return h.response({ error: "incident webhook not configured" }).code(503);

      const body = rawBody(request);
      const sigHeader = request.headers["x-pagerduty-signature"];
      const signature = Array.isArray(sigHeader) ? sigHeader[0] : sigHeader;
      const signatureOk = !!secret && verifyPagerDutySignature(secret, signature, body);
      const presented = presentedToken(request);
      const tokenOk = !!token && !!presented && safeEqual(presented, token);
      if (!signatureOk && !tokenOk) return h.response({ error: "unauthorized" }).code(401);
      ...
      try {
        ...
        return h.response({ error: err instanceof Error ? err.message : "internal error" }).code(500);
      }
    },
  };
}
