  ...
  return {
    ...
    handler: async (request, h) => {
      ...
      try {
        ...
        if (rows.length > 0) return h.response({ task_id: rows[0].id, trailer_source: "db" });
      } catch (err) {
      ...
      try {
        ...
        const pr = await octokit.rest.pulls.get({ owner, repo: repoName, pull_number: prNumber });

        const fromBody = pr.data.body?.match(LORE_TASK_TRAILER_RE);
        if (fromBody) return h.response({ task_id: fromBody[1], trailer_source: "pr_body" });

        // Final commit on the PR head branch.
        const commit = await octokit.rest.git.getCommit({ owner, repo: repoName, commit_sha: pr.data.head.sha });
        const trailers = parseTrailers(commit.data.message);
        if (trailers?.taskId) return h.response({ task_id: trailers.taskId, trailer_source: "final_commit" });

        return h.response({ error: "no_trailer_found" }).code(404);
      } catch (err) {
        if ((err as { status?: number }).status === 404) return h.response({ error: "pr_not_found" }).code(404);
