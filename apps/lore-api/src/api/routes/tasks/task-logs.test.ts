...
describe("/api/task-logs", () => {
  ...
  describe("POST", () => {
    const post = (body: Record<string, unknown>) =>
      inject({ method: "POST", url: "/api/task-logs", payload: JSON.stringify(body) });
  ...
  describe("GET", () => {
    it("returns 400 when task_id is missing", async () => {
      const res = await inject({ method: "GET", url: "/api/task-logs?repo=o/r" });
      expect(res.statusCode).toBe(400);
    });
    it("returns 503 when repo is omitted and no pool resolves it", async () => {
      const res = await inject({ method: "GET", url: "/api/task-logs?task_id=t" });
    ...
    it("resolves repo from task_id when repo is omitted", async () => {
      ...
      const res = await inject({ method: "GET", url: "/api/task-logs?task_id=t" }, pool);
    ...
    it("returns a slice from offset when the file exists", async () => {
      ...
      const res = await inject({ method: "GET", url: "/api/task-logs?task_id=t&repo=o/r&offset=6" });
