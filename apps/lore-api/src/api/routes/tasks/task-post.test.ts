...
describe("POST /api/task", () => {
  ...
  function post(body: unknown, pool: unknown = makePool()) {
    ...
    return buildServer(() => pool as any).inject({ method: "POST", url: "/api/task", headers: AUTH, payload });
  }
  ...
  it("sets immediate priority", async () => {
    ...
    expect(res.result).toEqual({ ok: true, task_id: "t1", priority: "immediate" });
  });
  ...
  it("updates status with pr_url and error", async () => {
    ...
    expect(res.result).toEqual({ ok: true, task_id: "t1", status: "pr-created" });
  });
