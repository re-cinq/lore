...
describe("migration 0015 — agent_definitions table", () => {
  it("creates lore.agent_definitions with the org/project partial unique indexes", () => {
    ...
    expect(sql).toMatch(
      /agent_definitions_org_name[\s\S]*?WHERE project_id IS NULL/,
    );
    expect(sql).toMatch(
      /agent_definitions_proj_name[\s\S]*?WHERE project_id IS NOT NULL/,
    );
  });
  ...
  it("backfills existing settings.task_overrides into per-project rows (object-guarded)", () => {
    ...
    expect(sql).toMatch(
      /jsonb_typeof\(r\.settings->'task_overrides'\) = 'object'/,
    );
    expect(sql).toMatch(
  ...
  it("grants lore_ui read only when the role exists (cluster may not have it)", () => {
    expect(sql).toMatch(
      /IF EXISTS \(SELECT 1 FROM pg_roles WHERE rolname = 'lore_ui'\)/,
    );
