...
import { WILDCARD_METHODS, BODYLESS_WRITES, domainBody } from "./domain-routes.js";
...
const FREEFORM_BODY: JsonSchema = { type: "object", additionalProperties: true };

const isExcludedPath = (path: string): boolean => path === "/healthz" || path.startsWith("/dist/");

/** hapi route options are always object-form in this codebase. */
const optionsOf = (route: ServerRoute): RouteOptions =>
  (typeof route.options === "object" && route.options ? route.options : {}) as RouteOptions;

const isPublic = (route: ServerRoute): boolean => optionsOf(route).auth === false;

function scopeOf(route: ServerRoute): string | undefined {
  const plugins = optionsOf(route).plugins as Record<string, { scope?: string } | undefined> | undefined;
...
function routeSchema(route: ServerRoute): ZodType | undefined {
  const validate = optionsOf(route).validate as { payload?: unknown } | undefined;
...
export function toRequestSchema(schema: ZodType): JsonSchema {
  const out = zodToJsonSchema(schema, { $refStrategy: "none", target: "jsonSchema7" }) as JsonSchema;
  delete out.$schema;
  return out;
}
...
function operationId(method: string, normPath: string): string {
  const slug = normPath.replace(/^\/+/, "").replace(/[/{}]/g, "_").replace(/_+/g, "_").replace(/_$/, "");
  return `${method.toLowerCase()}_${slug}`;
}

function responsesFor(isPublicOp: boolean, hasBody: boolean): Record<string, JsonSchema> {
  const responses: Record<string, JsonSchema> = {
    "200": { description: "Successful response (2xx; the response body is not described — see info.description)" },
  };
  ...
  if (hasBody) responses["413"] = { $ref: "#/components/responses/PayloadTooLarge" };
...
function resolveBody(route: ServerRoute, method: string, key: string, coverage: Coverage): JsonSchema | undefined {
...
function buildOperation(route: ServerRoute, method: string, normPath: string, coverage: Coverage): Operation {
  ...
  if (hasOptional) op.description = "A trailing path parameter is optional; omit it for the collection form.";
  if (hasBody) {
    const key = `${method} ${route.path}`;
    if (selfHandlesBody(route)) {
      coverage.selfHandled.push(key);
      op.description = "Request body is verified and parsed by the handler (HMAC/form-encoded), not JSON.";
...
export function generateOpenApi(routes: ServerRoute[], opts: GenerateOptions = {}): { document: OpenApiDocument; coverage: Coverage } {
  const paths: Record<string, Record<string, Operation>> = {};
  const coverage: Coverage = { covered: [], lifted: [], freeform: [], selfHandled: [], bodyless: [], uncovered: [], excluded: [] };
...
export function buildOpenApiDocument(routes: ServerRoute[], opts?: GenerateOptions): OpenApiDocument {
...
function errorResponses(): Record<string, JsonSchema> {
  const body = { content: { "application/json": { schema: { $ref: "#/components/schemas/Error" } } } };
  return {
    ...
    PayloadTooLarge: { description: "Request body exceeds the size cap", ...body },
    RateLimited: { description: "Rate limit exceeded (Retry-After: 60)", ...body },
    ServiceUnavailable: { description: "Database or a dependency is unavailable", ...body },
  };
}
