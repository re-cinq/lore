...
const MEMORY_MATCH_SQL = `SELECT key, substring(value, 1, 300) as value, agent_id,
...
const FACT_MATCH_SQL = `SELECT COALESCE(m.key, e.source || ':' || COALESCE(e.ref, e.id::text)) as key,
