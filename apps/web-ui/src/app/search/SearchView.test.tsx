...
describe('SearchView', () => {
  it('renders the repo filter options with the active repo preselected', () => {
    ...
    expect(screen.getByRole('option', { name: 're-cinq/lore' })).toBeInTheDocument();
    expect(screen.getByRole('option', { name: 're-cinq/atlas' })).toBeInTheDocument();
    expect((screen.getByRole('combobox') as HTMLSelectElement).value).toEqual('re-cinq/atlas');
  });

  it('preserves the typed query in the search input', () => {
    render(<SearchView q="env var" repos={REPOS} results={[result({})]} />);
    expect((screen.getByPlaceholderText(/Search across all agent memories/) as HTMLInputElement).value).toEqual('env var');
  });
  ...
  it('appends the active repo to the result-count line', () => {
    render(<SearchView q="env" repo="re-cinq/lore" repos={REPOS} results={[result({})]} />);
  ...
  it('renders a memory result with truncated agent id, fixed score and a read badge', () => {
    render(
      <SearchView
        q="env"
        repos={REPOS}
        results={[result({ key: 'deployment-gotchas-2026', agent_id: 'abcdef0123456789', score: 0.4287, source: 'memory', repo: null })]}
      />,
    );
    expect(screen.getByText('deployment-gotchas-2026')).toBeInTheDocument();
    expect(screen.getByText(/agent: abcdef01\.\.\. · score: 0\.429/)).toBeInTheDocument();
    expect(screen.getByText('Always set the env var before deploy')).toBeInTheDocument();
    expect(screen.getByText('memory')).toHaveClass('op-badge', 'op-read');
  });

  it('renders a fact result with the op-search badge', () => {
    render(<SearchView q="env" repos={REPOS} results={[result({ source: 'fact' })]} />);
    expect(screen.getByText('fact')).toHaveClass('op-badge', 'op-search');
  });

  it('renders an episode result with the op-read badge', () => {
    render(<SearchView q="env" repos={REPOS} results={[result({ source: 'episode' })]} />);
  ...
  it('renders a chunk result with the op-write badge plus the repo meta and repo badge', () => {
    render(
      <SearchView
        q="env"
        repos={REPOS}
        results={[result({ key: 'src/index.ts', source: 'chunk', repo: 're-cinq/lore', agent_id: 'ingestion-svc' })]}
      />,
    );
    const card = screen.getByText('src/index.ts').closest('.search-result') as HTMLElement;
  ...
  it('omits the repo meta and repo badge when the result has no repo', () => {
    render(<SearchView q="env" repos={REPOS} results={[result({ repo: null })]} />);
    const card = screen.getByText('deployment-gotchas-2026').closest('.search-result') as HTMLElement;
    ...
    render(
      <SearchView
        ...
        results={[
          ...
          result({ key: 'chunk-three', source: 'chunk', repo: 're-cinq/atlas' }),
        ]}
      />,
    );
