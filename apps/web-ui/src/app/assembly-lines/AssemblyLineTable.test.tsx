...
describe('AssemblyLineTable', () => {
  ...
  it('renders one stage dot per member with a dropdown link to each member', () => {
    ...
    const { container } = render(<AssemblyLineTable runs={group(review, impl)} />);
  ...
  it('renders a Run Now POST form when the lead task is pending', () => {
    render(<AssemblyLineTable runs={group(taskRow({ id: 'p1', status: 'pending' }))} />);
  ...
  it('renders an Open PR link only when the run has a PR url', () => {
    render(<AssemblyLineTable runs={group(taskRow({ pr_url: 'https://gh/pr/9', pr_number: 9 }))} />);
  ...
  it('renders the PR badge as plain "PR" and no status pill when pr_number is absent', () => {
    const { container } = render(<AssemblyLineTable runs={group(taskRow({ pr_url: 'https://gh/pr/x', pr_number: null }))} />);
  ...
  it('omits the repo link when the run has no target repo', () => {
    render(<AssemblyLineTable runs={group(taskRow({ target_repo: '', pr_url: null }))} />);
    expect(screen.queryByRole('link', { name: 're-cinq/lore' })).not.toBeInTheDocument();
  });

  it('shows uppercase initials for the creator', () => {
    render(<AssemblyLineTable runs={group(taskRow({ created_by: 'review-agent' }))} />);
  ...
  it('renders a Cost column summing member costs when showCost is set', () => {
    const impl = taskRow({ id: 'impl', target_branch: 'lore/z', cost_usd: 0.1, created_at: '2026-06-01T10:00:00.000Z' });
    const review = taskRow({ id: 'review', target_branch: 'lore/z', parent_task_id: 'impl', cost_usd: 0.2, created_at: '2026-06-01T11:00:00.000Z' });
