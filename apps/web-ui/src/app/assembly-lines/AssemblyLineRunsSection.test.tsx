...
describe('AssemblyLineRunsSection', () => {
  ...
  it('renders one row per run with definition, repo, node count, and outcome', () => {
    render(
      <AssemblyLineRunsSection
        runs={[run(), run({ id: '22222222-2222-4333-8444-555555555555', status: 'running', outcome: null, nodeCount: 2, durationSeconds: null })]}
      />,
    );
  ...
  it('shows the failure reason when a run failed', () => {
    render(
      <AssemblyLineRunsSection
        runs={[run({ status: 'failed', outcome: 'error', reason: 'iteration_max exceeded' })]}
      />,
    );
