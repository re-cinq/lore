 ...
 */
export default function AssemblyLineTable({ runs, showCost = false }: AssemblyLineTableProps) {
  const columns = showCost ? 6 : 5;
  return (
    <table className={styles.table}>
      <thead>
        <tr>
          <th>Status</th><th>Assembly Line</th><th>Created by</th>
          {showCost && <th>Cost</th>}
          <th>Stages</th><th>Actions</th>
        </tr>
      </thead>
      <tbody>
        {runs.map(run => <AssemblyLineRow key={run.runKey} run={run} showCost={showCost} />)}
        {runs.length === 0 && <tr><td colSpan={columns} className={`meta ${styles.emptyCell}`}>No assembly lines</td></tr>}
      </tbody>
    </table>
  );
}

function AssemblyLineRow({ run, showCost }: { run: AssemblyLine; showCost: boolean }) {
