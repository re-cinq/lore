...
describe("FailurePanel", () => {
  ...
  it("renders null when metadata itself is undefined (optional-chain guard)", () => {
    const { container } = render(
      <FailurePanel
        metadata={undefined as unknown as { error?: string }}
        repo={repo}
      />,
    );
  ...
  it("renders the Failure heading and a top-level error paragraph", () => {
    ...
    expect(
      screen.getByRole("heading", { name: /Failure/ }),
    ).toBeInTheDocument();
  ...
  it("maps a known category code to its human label badge", () => {
    render(
      <FailurePanel
        metadata={{ error: "x", category: "anthropic-credit" }}
        repo={repo}
      />,
    );
  ...
  it("falls back to the raw category string for an unmapped code", () => {
    render(
      <FailurePanel
        metadata={{ error: "x", category: "some-novel-code" }}
        repo={repo}
      />,
    );
  ...
  it('renders the remediation hint with the "How to fix" label', () => {
    render(
      <FailurePanel
        metadata={{ error: "x", hint: "try topping up credits" }}
        repo={repo}
      />,
    );
  ...
  it("linkifies a file path inside the top-level error via the real Linkified", () => {
    render(
      <FailurePanel
        metadata={{ error: "see src/a.ts for context" }}
        repo={repo}
      />,
    );
  ...
  it("renders details rows even when there is no top-level error", () => {
    render(
      <FailurePanel
        metadata={{
          details: [{ step: "only-step", error: "detail-only failure" }],
        }}
        repo={repo}
      />,
    );
  ...
  it("renders the unknown category label for the reserved unknown code", () => {
    render(
      <FailurePanel
        metadata={{ error: "x", category: "unknown" }}
        repo={repo}
      />,
    );
