...
const stats: Stats = { entity_count: 12, active_edge_count: 34, invalidated_edge_count: 5 };
...
describe('GraphView', () => {
  it('renders the three stat cards from the stats view-model', () => {
    render(<GraphView showInvalid={false} stats={stats} entityTypes={[]} entities={[]} edges={[]} />);
  ...
  it('renders an entity row with name, type badge, repo, edge count and explore link', () => {
    ...
    expect(explore.getAttribute('href')).toEqual('/graph?entity=lore-agent&type=service');
  });
  ...
  it('omits the type query param from the explore link when no type filter is active', () => {
    ...
    expect(screen.getByText('explore').getAttribute('href')).toEqual('/graph?entity=lore-agent');
  });

  it('shows the entities empty state when there are no entities', () => {
    render(<GraphView showInvalid={false} stats={stats} entityTypes={[]} entities={[]} edges={[]} />);
    expect(screen.getByText('No entities yet. Write episodes to populate the graph.')).toBeInTheDocument();
  });

  it('does not render the relationships section when no entity is selected', () => {
    render(<GraphView showInvalid={false} stats={stats} entityTypes={[]} entities={[entityRow({})]} edges={[]} />);
  ...
  it('renders the relationships section with a Show invalidated toggle when an entity is selected', () => {
    ...
    expect(screen.getByText('Relationships for "lore-agent"')).toBeInTheDocument();
    const toggle = screen.getByText('Show invalidated edges');
    expect(toggle.getAttribute('href')).toEqual('/graph?entity=lore-agent&show_invalid=1');
  ...
  it('shows the relationships empty state when a selected entity has no edges', () => {
    ...
    expect(screen.getByText('No relationships found for this entity.')).toBeInTheDocument();
  });
});
