...
describe('TasksView', () => {
  ...
  it('shows both task empty state and activity empty state when nothing is populated', () => {
    const action = vi.fn();
    render(<TasksView tasks={[]} recentActivity={[]} createTask={action} />);
    expect(screen.getByText('No tasks found. Create one above.')).toBeInTheDocument();
    expect(screen.getByText('No recent agent activity recorded.')).toBeInTheDocument();
  ...
  it('renders one card per task with the badge-open class on open status', () => {
    ...
    render(
      <TasksView
        tasks={[task({ id: 'a', content: 'Open task', metadata: { status: 'open' } })]}
        recentActivity={[]}
        createTask={action}
      />,
    );
  ...
  it('renders the activity table with one row per audit entry', () => {
    ...
    render(
      <TasksView
        tasks={[]}
        recentActivity={[
          entry({ agent_id: 'agent-1', operation: 'read_memory', memory_key: 'k1' }),
          entry({ agent_id: 'agent-2', operation: 'delete_memory', memory_key: 'k2' }),
        ]}
        createTask={action}
      />,
    );
