...
describe("PRStatusPanel", () => {
  ...
  it("refetches when the task id prop changes", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(1, "/api/tasks/first/pr-status", expect.objectContaining({ signal: expect.anything() }));
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(2, "/api/tasks/second/pr-status", expect.objectContaining({ signal: expect.anything() }));
  });
