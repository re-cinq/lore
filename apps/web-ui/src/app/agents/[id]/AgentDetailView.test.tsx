...
const fullMemory: MemoryRow = {
  ...
  has_facts: true,
  versions: [
    { version: 3, value: 'always set the env var', created_at: '2026-06-01T10:00:00.000Z' },
    { version: 2, value: 'maybe set the env var', created_at: '2026-05-30T10:00:00.000Z' },
    { version: 1, value: 'env var unknown', created_at: '2026-05-29T10:00:00.000Z' },
  ],
  facts: [
    { fact_text: 'The agent runs on GKE', created_at: '2026-06-01T10:00:00.000Z' },
...
const minimalMemory: MemoryRow = {
  ...
  has_facts: false,
  versions: [
    { version: 1, value: 'transient observation', created_at: '2026-06-02T12:00:00.000Z' },
  ],
  facts: [],
};
...
describe('AgentDetailView', () => {
  it('renders truncated agent id and memory count', () => {
    render(<AgentDetailView agentId="abcdefghijklmnopqrstuvwxyz" memoryCount={2} memories={[fullMemory, minimalMemory]} />);
    expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent('Agent: abcdefghijkl...');
  ...
  it('renders key, version meta, facts badge and TTL badge for a full memory', () => {
    render(<AgentDetailView agentId="agent-x" memoryCount={1} memories={[fullMemory]} />);
  ...
  it('omits version history, facts badge, TTL badge and facts section for a minimal memory', () => {
    render(<AgentDetailView agentId="agent-x" memoryCount={1} memories={[minimalMemory]} />);
  ...
  it('renders empty state with zero memories and no cards', () => {
    render(<AgentDetailView agentId="agent-empty" memoryCount={0} memories={[]} />);
