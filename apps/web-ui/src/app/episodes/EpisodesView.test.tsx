...
describe('EpisodesView', () => {
  it('renders a row per episode with truncated agent id, source badge, ref and fact count', () => {
    render(
      <EpisodesView
        ...
        pageSize={30}
        episodes={[
          row({ id: 'ep-1', agent_id: 'abcdef0123456789', source: 'manual', ref: 'pr-42', fact_count: 3 }),
          row({ id: 'ep-2', agent_id: '99887766aabbccdd', source: 'session', ref: 'sess-7', fact_count: 9, content_preview: 'Another preview' }),
        ]}
      />,
    );
  ...
  it('renders the source filter options', () => {
    ...
    expect(screen.getByRole('option', { name: 'pr-review' })).toBeInTheDocument();
    expect((screen.getByRole('combobox') as HTMLSelectElement).value).toEqual('session');
  });

  it('shows the empty-state row when there are no episodes', () => {
    render(<EpisodesView offset={0} totalCount={0} sources={SOURCES} pageSize={30} episodes={[]} />);
