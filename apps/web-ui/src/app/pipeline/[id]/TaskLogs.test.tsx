...
type Payload = { logs: string | null; status: string; totalSize: number; error?: string };

function jsonResponse(body: Payload, init: { ok?: boolean; status?: number } = {}) {
  return {
    ok: init.ok ?? true,
...
function offsetCalls(fetchMock: ReturnType<typeof vi.fn>): string[] {
  return fetchMock.mock.calls.map((c) => c[0] as string).filter((u) => u.includes('offset='));
}

describe('TaskLogs', () => {
  it('renders the empty placeholder before any logs resolve', async () => {
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse({ logs: null, status: 'queued', totalSize: 0 }));
    vi.stubGlobal('fetch', fetchMock);

    render(<TaskLogs taskId="t1" initialStatus="queued" />);
    expect(screen.getByText('Logs will appear when the agent starts.')).toBeInTheDocument();
  ...
  it('requests the bare logs URL (no offset) on the initial full fetch', async () => {
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse({ logs: 'line-1\n', status: 'succeeded', totalSize: 7 }));
    vi.stubGlobal('fetch', fetchMock);

    render(<TaskLogs taskId="abc" initialStatus="succeeded" />);
    await settle();

    expect(fetchMock).toHaveBeenCalledWith('/api/pipeline/abc/logs');
  ...
  it('renders fetched logs in the terminal block after the first resolved fetch', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      jsonResponse({ logs: 'hello world output', status: 'succeeded', totalSize: 18 }),
    );
    vi.stubGlobal('fetch', fetchMock);
    ...
    expect(screen.queryByText('Logs will appear when the agent starts.')).not.toBeInTheDocument();
  });

  it('shows the Completed badge for succeeded status', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(jsonResponse({ logs: 'x', status: 'succeeded', totalSize: 1 })));
  ...
  it('shows the Completed badge for pr-created status', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(jsonResponse({ logs: 'x', status: 'pr-created', totalSize: 1 })));
  ...
  it('shows the Completed badge for merged status', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(jsonResponse({ logs: 'x', status: 'merged', totalSize: 1 })));
  ...
  it('shows the In Review badge for review status', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(jsonResponse({ logs: 'x', status: 'review', totalSize: 1 })));
  ...
  it('shows the Failed badge for failed status', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(jsonResponse({ logs: 'x', status: 'failed', totalSize: 1 })));
  ...
  it('shows the Failed badge for cancelled status', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(jsonResponse({ logs: 'x', status: 'cancelled', totalSize: 1 })));
  ...
  it('renders the pulse indicator and KB polling note while running', async () => {
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse({ logs: 'x', status: 'running', totalSize: 2048 }));
    vi.stubGlobal('fetch', fetchMock);
  ...
  it('shows the bare polling note when running with zero bytes received', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(jsonResponse({ logs: null, status: 'running', totalSize: 0 })));
  ...
  it('appends new content using the offset URL once totalSize is known and status is running', async () => {
    ...
    const fetchMock = vi.fn().mockImplementation((url: string) => {
      ...
      if (!m) {
        return Promise.resolve(jsonResponse({ logs: FULL.slice(0, 11), status: 'running', totalSize: 11 }));
      }
      const off = Number(m[1]);
      return Promise.resolve(jsonResponse({ logs: FULL.slice(off), status: 'running', totalSize: FULL.length }));
    });
    vi.stubGlobal('fetch', fetchMock);
    ...
    expect(offsetCalls(fetchMock)).toContain('/api/pipeline/job9/logs?offset=11');
  ...
  it('does not append when an offset poll returns an empty logs string', async () => {
    const fetchMock = vi.fn().mockImplementation((url: string) => {
      if (url.includes('offset=')) {
        return Promise.resolve(jsonResponse({ logs: '', status: 'running', totalSize: 10 }));
      }
      return Promise.resolve(jsonResponse({ logs: 'only-chunk', status: 'running', totalSize: 10 }));
    });
    vi.stubGlobal('fetch', fetchMock);
  ...
  it('appends onto a still-null log buffer when the first fetch reports size but no body', async () => {
    ...
    const fetchMock = vi.fn().mockImplementation((url: string) => {
      ...
      if (!m) {
        ...
        return Promise.resolve(jsonResponse({ logs: null, status: 'running', totalSize: HEAD }));
      }
      const off = Number(m[1]);
      // Tail beyond the offset converges to empty once the whole body is delivered.
      const tail = off >= HEAD ? BODY.slice(off - HEAD) : BODY;
      return Promise.resolve(jsonResponse({ logs: tail, status: 'running', totalSize: TOTAL }));
    });
    vi.stubGlobal('fetch', fetchMock);
  ...
  it('replaces logs on the full (non-offset) fetch path', async () => {
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse({ logs: 'replaced-entirely', status: 'succeeded', totalSize: 17 }));
    vi.stubGlobal('fetch', fetchMock);
  ...
  it('renders the access-denied message and stops polling on a 403 response', async () => {
    vi.useFakeTimers();
    const fetchMock = vi.fn().mockResolvedValue(
      jsonResponse({ logs: null, status: 'running', totalSize: 0 }, { ok: false, status: 403 }),
    );
    vi.stubGlobal('fetch', fetchMock);
    ...
    expect(screen.getByText('Access denied — you do not have access to this repository.')).toBeInTheDocument();
    expect(screen.queryByText('Logs will appear when the agent starts.')).not.toBeInTheDocument();
  ...
  it('renders the sign-in message on a 401 response', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(
      jsonResponse({ logs: null, status: 'running', totalSize: 0 }, { ok: false, status: 401 }),
    ));
    render(<TaskLogs taskId="t1" initialStatus="running" />);
    await settle();

    expect(screen.getByText('Failed to load logs: You must be signed in to view logs.')).toBeInTheDocument();
  });

  it('renders an HTTP error message on a non-ok, non-auth response', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(
      jsonResponse({ logs: null, status: 'failed', totalSize: 0 }, { ok: false, status: 500 }),
    ));
    render(<TaskLogs taskId="t1" initialStatus="failed" />);
    await settle();

    expect(screen.getByText('Failed to load logs: HTTP 500')).toBeInTheDocument();
  });
  ...
  it('renders the rejection message when fetch itself throws', async () => {
    ...
    expect(screen.getByText('Failed to load logs: network down')).toBeInTheDocument();
  ...
  it('clears the error once a later fetch succeeds', async () => {
    ...
    const fetchMock = vi.fn().mockImplementation((url: string) => {
      if (failFirst) {
        failFirst = false;
        return Promise.resolve(jsonResponse({ logs: null, status: 'running', totalSize: 0 }, { ok: false, status: 500 }));
      }
      // Converging buffer so the re-fired offset poll returns an empty tail (no double-append).
      const m = url.match(/offset=(\d+)/);
      const off = m ? Number(m[1]) : 0;
      return Promise.resolve(jsonResponse({ logs: RECOVERED.slice(off), status: 'running', totalSize: RECOVERED.length }));
    });
    vi.stubGlobal('fetch', fetchMock);
  ...
  it('keeps polling on the running interval and stops fetching after unmount', async () => {
    vi.useFakeTimers();
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse({ logs: 'tick', status: 'running', totalSize: 4 }));
    vi.stubGlobal('fetch', fetchMock);

    const { unmount } = render(<TaskLogs taskId="t1" initialStatus="running" />);
    await settle();
  ...
  it('does not start an interval when the status is terminal', async () => {
    vi.useFakeTimers();
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse({ logs: 'final', status: 'succeeded', totalSize: 5 }));
    vi.stubGlobal('fetch', fetchMock);
  ...
  it('always renders the Agent Output heading', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(jsonResponse({ logs: null, status: 'queued', totalSize: 0 })));
    render(<TaskLogs taskId="t1" initialStatus="queued" />);
  ...
  it('transitions from running to a done badge when the server reports completion', async () => {
    ...
    const fetchMock = vi.fn().mockImplementation(() => {
      ...
      if (phase <= 2) {
        return Promise.resolve(jsonResponse({ logs: 'working...', status: 'running', totalSize: 10 }));
      }
      return Promise.resolve(jsonResponse({ logs: 'working...done', status: 'pr-created', totalSize: 14 }));
    });
    vi.stubGlobal('fetch', fetchMock);
