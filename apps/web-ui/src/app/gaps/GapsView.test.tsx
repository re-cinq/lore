...
const search = (
  over: Partial<ZeroResultSearchRow> = {},
): ZeroResultSearchRow => ({
...
describe("GapsView", () => {
  it("renders the global-view note linking to Repositories and the draft-PR GitHub link", () => {
    ...
    expect(
      screen.getByRole("link", {
        name: "View context-gap-draft PRs on GitHub →",
      }),
    ).toHaveAttribute(
      "href",
      "https://github.com/re-cinq/lore/pulls?q=label:context-gap-draft",
    );
  });
  ...
  it("shows the empty findings message when there are no gap memories", () => {
    ...
    expect(
      screen.getByText("No findings from the gap detection agent yet."),
    ).toBeInTheDocument();
  });
  ...
  it("renders a table row per zero-result search with its query and serialized metadata", () => {
    ...
    expect(
      screen.getByText(JSON.stringify({ result_count: "0" })),
    ).toBeInTheDocument();
  });
  ...
  it("shows the empty zero-result message when there are no recorded searches", () => {
    ...
    expect(
      screen.getByText("No zero-result searches recorded."),
    ).toBeInTheDocument();
  });
});
