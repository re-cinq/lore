...
/**
 * Creates the spec-traceability graph ingestion tasks for a repo — one
 * `ingest-<kind>` pipeline task per kind, grouped under one task_group_id, with
 * an in-flight dedupe. web-ui is not an npm workspace member, so this inserts
 * directly via the db pool (mirrors mcp-server's createIngestGraphTasks).
 */
