...
describe('ContextFileView', () => {
  it('renders a Not Found state when no chunks exist', () => {
    render(<ContextFileView filePath="docs/missing.md" contextLink="/context" groups={[]} />);
  ...
  it('renders breadcrumb, basename title and the chunk body for a single group', () => {
    render(
      <ContextFileView
        ...
        groups={[{ repo: 'o/r', chunks: [chunk({ content: 'the spec body' })] }]}
      />,
    );
    ...
    expect(screen.getByRole('heading', { level: 1, name: 'spec.md' })).toBeInTheDocument();
  ...
  it('renders a repo header and view-in-repo link per group in the global view', () => {
    render(
      <ContextFileView
        ...
        contextLink="/context"
        groups={[
          { repo: 'o/a', repoHref: '/repos/o/a/context/CLAUDE.md', chunks: [chunk({ id: 'a' })] },
          { repo: 'o/b', repoHref: '/repos/o/b/context/CLAUDE.md', chunks: [chunk({ id: 'b' })] },
        ]}
      />,
    );
    expect(screen.getByText('repo: o/a')).toBeInTheDocument();
    expect(screen.getByText('repo: o/b')).toBeInTheDocument();
    expect(screen.getAllByRole('link', { name: 'view in repo →' })).toHaveLength(2);
  });
  ...
  it('renders a separator between multiple chunks in a group', () => {
    const { container } = render(
      <ContextFileView
        filePath="a.md"
        contextLink="/context"
        groups={[{ repo: 'o/r', chunks: [chunk({ id: 'a' }), chunk({ id: 'b' })] }]}
      />,
    );
