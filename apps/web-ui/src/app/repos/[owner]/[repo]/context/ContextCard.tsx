  ...
  return (
    <div className={styles.card}>
      <div className={styles.head}>
        <span className={badgeClassForType(chunk.content_type)}>{chunk.content_type}</span>
        <Link href={detailHref} className={styles.path}>
          {chunk.file_path}
        </Link>
        {repoLabel && <span className={styles.repo}>{repoLabel}</span>}
        <span className={styles.date}>{new Date(chunk.ingested_at).toLocaleDateString()}</span>
      </div>
