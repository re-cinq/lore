...
describe('ContextCard', () => {
  it('links the file path to the detail route', () => {
    ...
    expect(link).toHaveAttribute('href', '/repos/o/r/context/specs%2Fa%2Fspec.md');
  });

  it('renders the type badge with its color class', () => {
    render(<ContextCard chunk={chunk({ content_type: 'code' })} detailHref="/x" repo="o/r" />);
  ...
  it('shows the derived metadata header when metadata is present', () => {
    render(
      <ContextCard
        chunk={chunk({
          content_type: 'code',
          metadata: { symbol_type: 'function', symbol_name: 'foo', start_line: 1, end_line: 9 },
        })}
        detailHref="/x"
        repo="o/r"
      />,
  ...
  it('shows the repo label only when one is passed', () => {
    const { rerender } = render(<ContextCard chunk={chunk()} detailHref="/x" repo="o/r" />);
    expect(screen.queryByText('o/r')).toBeNull();
    rerender(<ContextCard chunk={chunk()} detailHref="/x" repo="o/r" repoLabel="o/r" />);
