...
describe("RepoContextView", () => {
  ...
  it("links each chunk file path to its detail route", () => {
    ...
    expect(
      screen.getByRole("link", { name: "specs/a/spec.md" }),
    ).toHaveAttribute("href", "/repos/o/r/context/specs%2Fa%2Fspec.md");
  });
  ...
  it("shows the search-scoped empty state with the query", () => {
    ...
    expect(
      screen.getByText("No context matches “widgets”."),
    ).toBeInTheDocument();
  });
  ...
  it("shows the type-scoped empty state when a type filter yields nothing", () => {
    ...
    expect(
      screen.getByText("No spec context ingested yet."),
    ).toBeInTheDocument();
  });
  ...
  it("shows the fresh-repo empty state when there are no chunks, type or query", () => {
    ...
    expect(
      screen.getByText(
        "No context ingested yet. Context will appear after the nightly ingestion runs.",
      ),
    ).toBeInTheDocument();
  });
  ...
  it("renders the help popover trigger and lead description", () => {
    render(
      <RepoContextView owner="o" repo="r" types={["doc"]} chunks={[chunk()]} />,
    );
    expect(
      screen.getByRole("button", { name: "How context is used" }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
