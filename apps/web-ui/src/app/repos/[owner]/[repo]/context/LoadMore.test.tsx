...
describe('LoadMore', () => {
  ...
  it('fetches the next page and appends a linked card', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole('link', { name: 'docs/next.md' })).toBeInTheDocument(),
    );
  ...
  it('forwards the active query and type to the API', async () => {
    (fetch as ReturnType<typeof vi.fn>).mockResolvedValueOnce({ ok: true, json: async () => page() });
    render(<LoadMore owner="o" repo="r" q="hello world" type="doc" initialOffset={50} hasMore />);
  ...
  it('keeps paging from an advancing offset', async () => {
    (fetch as ReturnType<typeof vi.fn>)
      .mockResolvedValueOnce({ ok: true, json: async () => page({ hasMore: true }) })
      .mockResolvedValueOnce({ ok: true, json: async () => page({ hasMore: false }) });
    render(<LoadMore owner="o" repo="r" initialOffset={50} hasMore />);
    ...
    expect(fetch).toHaveBeenNthCalledWith(1, '/api/repos/o/r/context?offset=50');
    expect(fetch).toHaveBeenNthCalledWith(2, '/api/repos/o/r/context?offset=100');
  });
});
