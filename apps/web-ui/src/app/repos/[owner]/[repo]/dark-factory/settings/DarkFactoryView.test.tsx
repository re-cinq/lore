...
describe("DarkFactoryView", () => {
  it("prefills the dark-factory fields from resolved defaults (opt-out posture)", () => {
    ...
    expect(
      container.querySelector('select[name="df_create_issue"]'),
    ).toHaveValue("always");
    expect(container.querySelector('select[name="df_review"]')).toHaveValue(
      "always",
    );
    expect(
      container.querySelector('select[name="df_am_min_trust"]'),
    ).toHaveValue("docs");
    expect(
      (
