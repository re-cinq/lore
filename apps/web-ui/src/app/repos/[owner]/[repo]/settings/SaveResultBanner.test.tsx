...
describe("SaveResultBanner", () => {
  ...
  it("confirms applied privileged changes on ok", () => {
    render(
      <SaveResultBanner
        state={{
          saved: true,
          privileged: { status: "ok", applied: {}, ceremony: {} },
        }}
      />,
    );
      ...
      <SaveResultBanner
        state={{
          ...
          privileged: {
            ...
            fieldPaths: ["enabled", "execution.image"],
            detail: "",
          },
        }}
      />,
  ...
  it("warns when the gated API is unconfigured", () => {
    render(
      <SaveResultBanner
        state={{ saved: true, privileged: { status: "unconfigured" } }}
      />,
    );
  ...
  it("shows the message on a generic error", () => {
    render(
      <SaveResultBanner
        state={{
          saved: true,
          privileged: { status: "error", message: "boom" },
        }}
      />,
    );
