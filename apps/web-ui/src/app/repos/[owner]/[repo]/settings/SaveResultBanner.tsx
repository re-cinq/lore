...
export default function SaveResultBanner({ state }: { state: SaveState }) {
  ...
  return (
    <div className={styles.banner} role="status">
      ...
      {p?.status === 'ok' && (
        <p className={styles.savedOk}>Privileged changes applied (two-key ceremony recorded).</p>
      )}
      {p?.status === 'two_key_required' && (
        <div className={styles.warn} role="alert">
          ...
          <ul>{p.fieldPaths.map((f) => <li key={f}><code>{f}</code></li>)}</ul>
          <p>
            Open a PR labeled <code>dark-factory-approval</code> approved by a CODEOWNER of
            this repo&apos;s <code>CLAUDE.md</code>, enter it as <code>owner/repo#N</code> in
            the Approval PR field above, and save again. General settings were saved.
          </p>
        </div>
      )}
      ...
      {p?.status === 'unconfigured' && (
        <p className={styles.warn} role="alert">
          The privileged settings API is not configured, so dark-factory and execution-image
          changes were not applied. General settings were saved.
        </p>
      )}
      {p?.status === 'error' && (
        <p className={styles.warn} role="alert">Could not apply privileged changes: {p.message}</p>
      )}
    </div>
  );
}
