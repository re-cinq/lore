...
export default function SettingsView({
  ...
  allRepos, knownTaskTypes, saveAction,
}: SettingsViewProps) {
  ...
  return (
    <div>
      ...
      <p className={`meta ${styles.lede}`}>
        Per-repo configuration: general routing, trust, dark-factory autonomy, and per-task-type overrides.
      </p>
      ...
      <form action={formAction} className={`task-form ${styles.form}`}>
        ...
        <h3 className={styles.section}>Per-task-type Overrides</h3>
        <span className={`meta ${styles.hint}`}>
          Override model, timeout, prompt suffix, or execution image per task type. Execution-image
          changes are security-gated.
        </span>
        {knownTaskTypes.map((type) => {
          const ov = overrides[type] ?? {};
          return (
            <fieldset key={type} className={styles.override}>
              <legend>{type}</legend>
              <label>Model</label>
              <input name={`to_${type}_model`} defaultValue={ov.model ?? ''} placeholder="(inherit)" />
              <label>Timeout (minutes)</label>
              <input name={`to_${type}_timeout`} type="number" min={1} max={1440} defaultValue={ov.timeout_minutes ?? ''} placeholder="(inherit)" />
              <label>System prompt suffix</label>
              <textarea name={`to_${type}_suffix`} rows={2} defaultValue={ov.system_prompt_suffix ?? ''} />
              <label>Execution image (gated)</label>
              <input name={`to_${type}_image`} defaultValue={ov.execution?.image ?? ''} placeholder="(inherit)" />
            </fieldset>
          );
        })}
