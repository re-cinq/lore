...
beforeEach(() => {
  ...
  fetchMock = vi.fn(async () => new Response(JSON.stringify({ meta: { id: "p1" }, documentName: "plan:re-cinq/lore:p1" })));
...
describe("createPlanAction", () => {
  it("asks for a title and creates nothing when the title is blank", async () => {
    const result = await createPlanAction("re-cinq/lore", null, form({ title: " ", type: "feature" }));
  ...
  it("creates gedaiu's feature plan, seeds its intent, and opens it", async () => {
    await expect(
      createPlanAction(
        "re-cinq/lore",
        null,
        form({ title: "Faster checkout", type: "feature", description: "Checkout is slow." }),
      ),
    ...
    expect(calls()).toEqual([
      ...
      },
      {
        ...
        body: { actor: "gedaiu", ops: [{ op: "set-section-text", slot: "intent", paragraphs: ["Checkout is slow."] }] },
      },
    ]);
  });
  ...
  it("leaves the template's intent alone when the description is empty", async () => {
    await expect(
      createPlanAction("re-cinq/lore", null, form({ title: "Faster checkout", type: "feature", description: "" })),
    ).rejects.toThrow(new Error("redirect /repos/re-cinq/lore/plans/p1"));
    expect(calls().map((call) => call.url)).toEqual(["http://api:3000/api/plans"]);
  });
});
