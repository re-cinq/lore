...
describe('RepoTasksView', () => {
  it('renders the Assembly Lines heading, intro copy and New Task link', () => {
    ...
    expect(screen.getByText(/Assembly lines targeting this repo/)).toBeInTheDocument();
    expect(screen.getByRole('link', { name: '+ New Task' })).toHaveAttribute('href', '/repos/re-cinq/lore/tasks/create');
  });
  ...
  it('renders a run with its stage mini-graph and a summed cost column', () => {
    const impl = taskRow({ id: 'task-abcd1234', target_branch: 'lore/x', cost_usd: 0.3, created_at: '2026-06-01T10:00:00.000Z' });
    const review = taskRow({ id: 'review', task_type: 'review', parent_task_id: 'task-abcd1234', target_branch: 'lore/x', cost_usd: 0.2, created_at: '2026-06-01T11:00:00.000Z' });
    render(<RepoTasksView owner="re-cinq" repo="lore" runs={group(review, impl)} />);
