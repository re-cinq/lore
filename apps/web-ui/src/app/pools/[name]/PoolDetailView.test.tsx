...
describe('PoolDetailView', () => {
  it('renders not found state with breadcrumb and message when not found', () => {
    render(<PoolDetailView poolName="ghost-pool" found={false} createdBy="" createdAt="" entries={[]} />);
    expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent('Pool Not Found');
    expect(screen.getByText('No pool named "ghost-pool" exists.')).toBeInTheDocument();
  ...
  it('renders pool name heading, breadcrumb and truncated creator with plural entry count', () => {
    ...
    expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent('team-pool');
    expect(screen.getByRole('link', { name: 'Pools' })).toHaveAttribute('href', '/pools');
    expect(screen.getByText(/Created by creator-id-very-long-value on/)).toBeInTheDocument();
  ...
  it('renders short value verbatim and truncates value over 200 chars with ellipsis', () => {
    ...
    expect(within(table).getByText('always set the env var')).toBeInTheDocument();
  ...
  it('renders key, readable agent id with title and version prefix', () => {
    ...
    expect(within(table).getByText('agent-abcdef-1234567890')).toBeInTheDocument();
    expect(within(table).getByText('agent-abcdef-1234567890')).toHaveAttribute('title', 'agent-abcdef-1234567890');
