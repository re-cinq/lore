...
describe('PoolDetailView', () => {
  ...
  it('renders pool name heading, breadcrumb and truncated creator with plural entry count', () => {
    ...
    expect(screen.getByRole('link', { name: 'Pools' })).toHaveAttribute('href', '/pools');
    expect(screen.getByText(/Created by creator-id-v\.\.\. on/)).toBeInTheDocument();
  ...
  it('renders key, truncated agent id with title, version prefix and short untruncated agent id', () => {
    ...
    expect(within(table).getByText('agent-ab...')).toHaveAttribute('title', 'agent-abcdef-1234567890');
