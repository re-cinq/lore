...
describe('PoolsView', () => {
  it('renders heading and column headers', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Shared Memory Pools' })).toBeInTheDocument();
  ...
  it('renders entry and agent counts', () => {
    render(<PoolsView pools={[makePool({ entry_count: 42, agent_count: 5 })]} />);
    expect(screen.getByText('42')).toBeInTheDocument();
    expect(screen.getByText('5')).toBeInTheDocument();
  });

  it('truncates created_by to first 8 chars with full value in title', () => {
    render(<PoolsView pools={[makePool({ created_by: 'agent-abcdef0123456789' })]} />);
