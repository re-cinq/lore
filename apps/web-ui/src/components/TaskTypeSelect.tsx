...
const DESCRIPTIONS: Record<string, string> = {
  'feature-request': 'PM intent in plain language becomes a spec, plan, and tasks for review.',
...
export function TaskTypeSelect({ options }: { options: { value: string; label: string }[] }) {
  const [selected, setSelected] = useState(options[0]?.value ?? '');
  const description = DESCRIPTIONS[selected];
  return (
    <>
      <select name="task_type" id="task_type" value={selected} onChange={(e) => setSelected(e.target.value)}>
        {options.map((o) => (
