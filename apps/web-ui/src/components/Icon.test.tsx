...
describe('Icon icon-family mapping', () => {
  it.each(ALL_NAMES)('renders the lucide glyph for the elegant family: %s', (name) => {
    ...
    expect(svgOf(name, 'elegant').getAttribute('class')).toContain('iconify--lucide');
  });

  it.each(ALL_NAMES)('renders the pixelarticons glyph for the retro family: %s', (name) => {
    const expected = ICONS.retro[name];
    expect(expected.startsWith('pixelarticons:')).toBe(true);
    expect(svgOf(name, 'retro').getAttribute('class')).toContain('iconify--pixelarticons');
  });

  it('swaps the rendered collection when the family changes for the same name', () => {
    expect(svgOf('check', 'elegant').getAttribute('class')).toContain('iconify--lucide');
    expect(svgOf('check', 'retro').getAttribute('class')).toContain('iconify--pixelarticons');
  });
});
