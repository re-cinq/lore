...
describe('FixIngestButton', () => {
  it('renders nothing when no repos are misaligned', () => {
    const { container } = render(<FixIngestButton repos={[]} action={vi.fn()} />);
  ...
  it('shows the misaligned count and runs the action on click', async () => {
    ...
    render(<FixIngestButton repos={['re-cinq/a', 're-cinq/b']} action={action} />);

    const button = screen.getByRole('button', { name: '⚠ Fix ingest workflow (2)' });
    fireEvent.click(button);

    await waitFor(() => expect(action).toHaveBeenCalledWith(['re-cinq/a', 're-cinq/b']));
  ...
  it('uses the singular PR label when one repo is fixed', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '⚠ Fix ingest workflow (1)' }));
