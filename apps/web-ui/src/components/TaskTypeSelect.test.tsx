...
describe('TaskTypeSelect', () => {
  it('describes the first option by default', () => {
    ...
    expect(screen.getByText('Open-ended task with full Lore context.')).toBeInTheDocument();
  });

  it('updates the description when the selection changes', () => {
    render(<TaskTypeSelect options={OPTIONS} />);
    fireEvent.change(screen.getByRole('combobox'), { target: { value: 'runbook' } });
    expect(screen.getByText('Generates an incident runbook.')).toBeInTheDocument();
  });
