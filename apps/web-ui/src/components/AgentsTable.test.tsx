...
describe('AgentsTable rendering', () => {
  ...
  it('renders the base column headers without a Why column when no reason is present', () => {
    ...
    for (const header of ['Agent', 'Type', 'Created by', 'Tasks', 'Cost', 'Memories', 'Last Active']) {
      expect(within(table).getByRole('columnheader', { name: header })).toBeInTheDocument();
    }
    expect(within(table).queryByRole('columnheader', { name: 'Why' })).not.toBeInTheDocument();
  });
    ...
    render(
      <AgentsTable
        agents={[
          ...
          local({ agent_id: 'a50f9f29-two', reason: 'no type here', reason_type: null }),
        ]}
      />,
    );
    const table = screen.getByRole('table');
    expect(within(table).getByRole('columnheader', { name: 'Why' })).toBeInTheDocument();
...
describe('AgentsTable local-vs-task visibility', () => {
  it('shows only local agents by default and hides task agents behind the toggle', () => {
    ...
    expect(screen.getByRole('link', { name: 'a50f9f29-local' })).toBeInTheDocument();
    expect(screen.queryByRole('link', { name: 'lore-agent-abc123' })).not.toBeInTheDocument();
  });

  it('labels the toggle with the hidden task-agent count', () => {
    render(<AgentsTable agents={[local(), task(), task({ agent_id: 'lore-agent-def456' })]} />);
    expect(
      screen.getByRole('button', { name: 'Show task agents (audit) — 2 hidden' }),
    ).toBeInTheDocument();
  });
  ...
  it('reveals task agents and flips the label when the toggle is clicked', () => {
    ...
    expect(screen.getByRole('link', { name: 'lore-agent-abc123' })).toBeInTheDocument();
  ...
  it('renders no toggle when there are no task agents', () => {
    render(<AgentsTable agents={[local()]} />);
    expect(screen.queryByRole('button', { name: /task agents/ })).not.toBeInTheDocument();
  });
});

describe('AgentsTable rows', () => {
  it('renders the kind badge, encoded link, counts and cost per row', () => {
    render(<AgentsTable agents={[local({ agent_id: 'agent x/1', memory_count: 99 })]} />);
  ...
  it('falls back to unknown creator and an em dash when last_active is null', () => {
    render(<AgentsTable agents={[local({ created_by: null, last_active: null })]} />);
