...
export function chunkHeader(contentType: string, metadata?: ChunkMeta | null): string {
  if (!metadata) return '';
  if (contentType === 'code') {
    const symbol = [metadata.symbol_type, metadata.symbol_name].filter(Boolean).join(' ');
    const lines =
