...
describe('parseSettingsForm', () => {
  ...
  it('dispatch_default_type trimmed when present, omitted when blank', () => {
    expect(parseSettingsForm(form({ dispatch_default_type: ' implementation ' })).dispatch_default_type)
      .toBe('implementation');
    expect('dispatch_default_type' in parseSettingsForm(form({ dispatch_default_type: '  ' }))).toBe(false);
  });
...
describe('parsePrivilegedChanges', () => {
  ...
  it('empty patch when nothing differs from current', () => {
    const current = { dark_factory: { enabled: false, execution: { image: 'ghcr.io/re-cinq/lore-claude-runner:latest' } } };
    const fd = form({
      df_enabled: 'no',
  ...
  it('flags dark_factory.enabled when toggled on', () => {
    ...
    expect(parsePrivilegedChanges(fd, { dark_factory: { enabled: false } }, TYPES))
  ...
  it('emits changed auto_merge sub-fields nested under auto_merge', () => {
    ...
    expect(parsePrivilegedChanges(fd, current, TYPES))
      .toEqual({ dark_factory: { auto_merge: { min_trust: 'full', require_green_ci: false } } });
  });

  it('splits auto_merge paths on newlines and commas', () => {
    const fd = form({ df_am_paths: 'specs/**\nCLAUDE.md, docs/**' });
    expect(parsePrivilegedChanges(fd, { dark_factory: {} }, TYPES))
      .toEqual({ dark_factory: { auto_merge: { paths: ['specs/**', 'CLAUDE.md', 'docs/**'] } } });
  });
  ...
  it('diffs the notify channel list', () => {
    ...
    expect(parsePrivilegedChanges(fd, current, TYPES))
      .toEqual({ dark_factory: { notify: ['escalation', 'watched'] } });
  });
  ...
  it('emits task_overrides.<type>.execution.image when a per-type image changes', () => {
    ...
    expect(parsePrivilegedChanges(fd, {}, TYPES))
      .toEqual({ task_overrides: { implementation: { execution: { image: 'golang:1.23' } } } });
  });

  it('emits non-privileged per-type fields (model/timeout/suffix) as changes too', () => {
    const fd = form({ to_review_model: 'claude-opus-4-8', to_review_timeout: '45' });
    expect(parsePrivilegedChanges(fd, {}, TYPES))
      .toEqual({ task_overrides: { review: { model: 'claude-opus-4-8', timeout_minutes: 45 } } });
  });

  it('does not emit a per-type entry when nothing in that row changed', () => {
    const current = { task_overrides: { implementation: { model: 'claude-opus-4-8' } } };
