...
describe("isNavActive", () => {
  it("matches the root link only on the exact root path", () => {
    ...
    expect(isNavActive("/assembly-runs", "/", "/")).toBe(false);
  });

  it("matches a non-root link on its exact path", () => {
    expect(isNavActive("/assembly-runs", "/assembly-runs", "/")).toBe(true);
  });

  it("matches a non-root link on its sub-routes", () => {
    expect(isNavActive("/assembly-runs/123", "/assembly-runs", "/")).toBe(true);
  });
  ...
  it("treats a repo base as the exact-only root for its tab group", () => {
    ...
    expect(isNavActive(`${base}/specs/a%2Fb.md`, `${base}/specs`, base)).toBe(
      true,
    );
  });
});
