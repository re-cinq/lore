...
describe('privilegedRequestBody', () => {
  ...
  it('omits task_overrides when absent', () => {
    expect(privilegedRequestBody({ dark_factory: { enabled: false } })).toEqual({ enabled: false });
  });
});
...
describe('putPrivilegedSettings', () => {
  ...
  it('returns unconfigured when env is missing', async () => {
    delete process.env.LORE_API_URL;
    expect(await putPrivilegedSettings('o/r', { dark_factory: { enabled: true } }))
      .toEqual({ status: 'unconfigured' });
  });

  it('returns ok with applied + ceremony on 200', async () => {
    mockFetch(200, { ok: true, applied: { enabled: true }, ceremony: { tier: 'admin' } });
    expect(await putPrivilegedSettings('o/r', { dark_factory: { create_issue: 'never' } }))
      .toEqual({ status: 'ok', applied: { enabled: true }, ceremony: { tier: 'admin' } });
  });
  ...
  it('sends the approval-PR header when provided', async () => {
    ...
    await putPrivilegedSettings('o/r', { dark_factory: { enabled: true } }, 'o/r#5');
  ...
  it('maps 403 two_key_required to a two_key_required result', async () => {
    mockFetch(403, { error: 'two_key_required', field_paths: ['execution.image'], detail: 'need PR' });
    expect(await putPrivilegedSettings('o/r', { dark_factory: { execution: { image: 'x' } } }))
      .toEqual({ status: 'two_key_required', fieldPaths: ['execution.image'], detail: 'need PR' });
  });
  ...
  it('maps 403 codeowners_check_failed to a codeowners_failed result', async () => {
    ...
    expect(await putPrivilegedSettings('o/r', { dark_factory: { enabled: true } }, 'o/r#5'))
