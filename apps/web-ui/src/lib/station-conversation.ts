...
function toolSummary(block: { name?: string; input?: Record<string, unknown> }): string {
  ...
  const arg = [input.command, input.file_path, input.pattern, input.path, input.description].find(
    (v): v is string => typeof v === 'string' && v.length > 0,
  );
  return arg ? `→ ${block.name}: ${clip(arg, 100)}` : `→ ${block.name ?? 'tool'}`;
}
...
function renderEvent(event: StreamEvent): string | null {
  ...
  if (event.type === 'assistant') {
    ...
    for (const block of content) {
      if (block.type === 'text' && block.text?.trim()) parts.push(clip(block.text, 300));
      else if (block.type === 'thinking' && block.thinking?.trim()) parts.push(`thinking: ${clip(block.thinking, 240)}`);
...
export function formatStationConversation(rawLog: string, maxEvents = 30): string {
  const out: string[] = [];
