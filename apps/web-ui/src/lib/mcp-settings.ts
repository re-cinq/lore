 ...
 * is a sibling key.
 */
export function privilegedRequestBody(patch: PrivilegedPatch): Record<string, unknown> {
  return {
    ...(patch.dark_factory ?? {}),
...
export async function putPrivilegedSettings(
  ...
  if (res.status === 403 && data.error === 'two_key_required') {
    return { status: 'two_key_required', fieldPaths: data.field_paths ?? [], detail: data.detail ?? '' };
  }
  if (res.status === 403 && data.error === 'codeowners_check_failed') {
    return { status: 'codeowners_failed', code: data.code ?? 'unknown', detail: data.detail ?? '' };
  }
  return { status: 'error', message: data.error || data.detail || `HTTP ${res.status}` };
}
