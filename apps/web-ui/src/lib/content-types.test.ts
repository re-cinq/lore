...
import { badgeClassForType, labelForType, orderTypes, contextHref } from './content-types';
...
describe('orderTypes', () => {
  it('orders known types by the canonical order regardless of input order', () => {
    expect(orderTypes(['code', 'adr', 'doc', 'spec'])).toEqual(['doc', 'spec', 'adr', 'code']);
  });

  it('sorts unknown types after known ones, alphabetically', () => {
    expect(orderTypes(['zeta', 'code', 'alpha'])).toEqual(['code', 'alpha', 'zeta']);
  });
});
