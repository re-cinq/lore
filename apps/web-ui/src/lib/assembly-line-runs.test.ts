...
import { toAssemblyLineRun, type AssemblyLineRunRow } from './assembly-line-runs';
...
describe('toAssemblyLineRun', () => {
  ...
  it('maps a running row: zero nodes yet, no duration', () => {
    const run = toAssemblyLineRun(
      row({ status: 'running', outcome: null, finished_at: null, node_count: '0' }),
    );
    expect(run).toMatchObject({ status: 'running', outcome: null, nodeCount: 0, durationSeconds: null });
  });

  it('maps a queued row that never started', () => {
    const run = toAssemblyLineRun(
      row({ status: 'queued', outcome: null, started_at: null, finished_at: null, node_count: '0' }),
    );
