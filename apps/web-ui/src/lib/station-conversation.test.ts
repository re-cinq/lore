...
describe("formatStationConversation", () => {
  it("keeps runner markers and renders thinking, tool use, results and text", () => {
    const raw = [
      ...
      JSON.stringify({
        ...
        message: {
          content: [
            {
              type: "thinking",
              thinking: "Let me analyze the request and write result.json",
            },
          ],
        },
      }),
          ...
          content: [
            {
              type: "tool_use",
              name: "Bash",
              input: { command: "jq empty result.json" },
            },
          ],
        },
      }),
      JSON.stringify({
        type: "user",
        message: { content: [{ type: "tool_result", content: "VALID JSON" }] },
      }),
      JSON.stringify({
        type: "assistant",
        message: {
          content: [{ type: "text", text: "result.json is written." }],
        },
      }),
      JSON.stringify({
        type: "system",
        ...
        content: [
          {
            type: "tool_use",
            name: "Write",
            input: { file_path: "/workspace/repo/result.json", content: "{}" },
          },
        ],
      },
    });
