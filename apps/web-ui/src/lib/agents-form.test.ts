...
describe("saveResultToState", () => {
  ...
  it("maps two_key_required to a twoKey flag", () => {
    expect(
      saveResultToState({ status: "two_key_required", detail: "x" }),
    ).toEqual({ twoKey: true });
  });
  it("maps unconfigured + codeowners + error to messages", () => {
    expect(saveResultToState({ status: "unconfigured" }).error).toMatch(
      /LORE_API_URL/,
    );
    expect(
      saveResultToState({
    ...
    expect(saveResultToState({ status: "error", message: "boom" }).error).toBe(
      "boom",
    );
  });
});
