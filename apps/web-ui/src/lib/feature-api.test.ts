...
describe("feature-api", () => {
  ...
  it("returns unconfigured when env is missing", async () => {
    delete process.env.LORE_API_URL;
    expect(await createFeature("o/r", "T", "P")).toEqual({
      status: "unconfigured",
    });
  });
  ...
  it("create posts to /features and returns ok with data", async () => {
    ...
    ).toEqual({ title: "My feature", prompt: "do it" });
  ...
  it("refine posts user_answers to the iterations path", async () => {
    ...
    expect(spy.mock.calls[0][0]).toBe(
      "https://lore-api.test/api/repos/o/r/features/f1/iterations",
    );
    expect(
      JSON.parse((spy.mock.calls[0][1] as RequestInit).body as string),
    ).toEqual({ user_answers: { free_form: "x" } });
  });
  ...
  it("finalize posts to the finalize path", async () => {
    ...
    expect(spy.mock.calls[0][0]).toBe(
      "https://lore-api.test/api/repos/o/r/features/f1/finalize",
    );
  });
  ...
  it("split posts title + prompt to the split path", async () => {
    ...
    expect(spy.mock.calls[0][0]).toBe(
      "https://lore-api.test/api/repos/o/r/features/parent/split",
    );
    expect(
      JSON.parse((spy.mock.calls[0][1] as RequestInit).body as string),
    ).toEqual({ title: "Part A", prompt: "carve A" });
  });
  ...
  it("maps a non-ok response with an error body to an error result", async () => {
    ...
    expect(await createFeature("o/r", "", "")).toEqual({
      status: "error",
