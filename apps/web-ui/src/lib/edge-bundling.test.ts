...
import { buildContainmentForest, ancestorChain, bundleControlIds } from './edge-bundling';

const CONTAINMENT = new Set(['in_feature', 'in_spec', 'in_section', 'has_statement']);

describe('buildContainmentForest', () => {
  it('maps a Spec child to its Feature parent via in_feature', () => {
    const parent = buildContainmentForest([{ source: 'feat', target: 'spec', kind: 'in_feature' }], CONTAINMENT);

    expect(parent.get('spec')).toBe('feat');
  });

  it('maps a Statement to its Spec via in_spec', () => {
    const parent = buildContainmentForest([{ source: 'spec', target: 'stmt', kind: 'in_spec' }], CONTAINMENT);

    expect(parent.get('stmt')).toBe('spec');
  });

  it('ignores cross-cutting kinds', () => {
    const parent = buildContainmentForest([{ source: 'stmt', target: 'test', kind: 'validated_by' }], CONTAINMENT);
...
describe('bundleControlIds', () => {
  ...
  it('routes an edge between two specs through their lowest common ancestor', () => {
    expect(bundleControlIds(parent, 'stmtA', 'stmtB')).toEqual(['stmtA', 'specA', 'feat', 'specB', 'stmtB']);
  });

  it('orders control ids source then LCA then target', () => {
    expect(bundleControlIds(parent, 'stmtA', 'feat')).toEqual(['stmtA', 'specA', 'feat']);
  });

  it('returns just the endpoints when there is no common ancestor', () => {
    expect(bundleControlIds(parent, 'stmtA', 'orphan')).toEqual(['stmtA', 'orphan']);
  });
});
