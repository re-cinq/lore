...
describe.skipIf(!reachable)("driftCheckFile (live Dgraph)", () => {
  ...
  it("drifts a statement connected only through the coverage chain when the covered code changes", async () => {
    const repo = `drift/${randomUUID()}`;
    createdRepo = repo;
    const statementXid = `${repo}|specs/foo/spec.md|7`;
    createdStatementXid = statementXid;
    ...
    const seededCoverage = await dgraphClient.newTxn().mutate({
      setJson: {
        uid: "_:cov",
    ...
    const covUid = seededCoverage.data.uids.cov;

    const seededTestChunk = await dgraphClient.newTxn().mutate({
      setJson: {
        uid: "_:tc",
    ...
    const tcUid = seededTestChunk.data.uids.tc;
    ...
    expect(statementData.stmt?.[0]?.["Statement.drift_reason"]).toBe("code-content-changed (render)");
  });

  it("marks the covering Coverage node stale when the covered code changes", async () => {
    const repo = `drift/${randomUUID()}`;
    createdRepo = repo;
    const statementXid = `${repo}|specs/foo/spec.md|7`;
    createdStatementXid = statementXid;
    ...
    const coverageXid = `${repo}|t.test.ts|renders`;
    const seededCoverage = await dgraphClient.newTxn().mutate({
      setJson: {
        uid: "_:cov",
        "dgraph.type": "Coverage",
        "Coverage.xid": coverageXid,
        "Coverage.repo": repo,
        "Coverage.covers": [{ uid: ccUid }],
      },
      commitNow: true,
    ...
    const tcUid = (await dgraphClient.newTxn().queryWithVars(
      `query tc($xid: string) { tc(func: eq(TestChunk.xid, $xid)) { uid } }`,
      { $xid: `${repo}|t1` },
    )).data as { tc?: { uid: string }[] };
    ...
    await dgraphClient.newTxn().mutate({
      setJson: {
        ...
        "Statement.ordinal": 7,
        "Statement.text": "The widget renders a click.",
        "Statement.validated_by": [{ uid: tcUid.tc?.[0]?.uid }],
      },
      commitNow: true,
    });
    ...
    const coverageData = (await readGraph(
      `query q($xid: string) {
    ...
    expect(coverageData.cov?.[0]?.["Coverage.stale"]).toBe(true);
  });
