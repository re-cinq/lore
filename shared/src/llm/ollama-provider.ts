...
export class OllamaProvider implements LlmProvider {
  ...
  async complete(req: LlmCompleteRequest): Promise<LlmCompletion> {
    const text = await this.generate(this.combine(req.systemPrompt, req.prompt));
    return { text, model: this.opts.model, inputTokens: 0, outputTokens: 0, durationMs: 0, ...ZERO_CACHE };
  }
  ...
  async completeWithTool<T>(req: LlmToolRequest): Promise<LlmToolResult<T>> {
    ...
    const text = await this.generate(this.combine(req.systemPrompt ? `${req.systemPrompt}\n${instruction}` : instruction, req.prompt));
    return { data: JSON.parse(text) as T, model: this.opts.model, inputTokens: 0, outputTokens: 0, durationMs: 0, ...ZERO_CACHE };
  }
  ...
  private async generate(prompt: string): Promise<string> {
    const baseUrl = this.opts.baseUrl ?? process.env.LORE_OLLAMA_URL ?? "http://localhost:11434";
