...
async function querySchema(dql: string): Promise<{ schema: Array<Record<string, unknown>> }> {
...
describe.skipIf(!reachable)("setup-spec-trace-schema applier (live Dgraph)", () => {
  ...
  it("declares the Statement xid upsert index and the Statement embedding HNSW vector index", async () => {
    ...
    expect(byPred["Statement.embedding"]).toMatchObject({ type: "float32vector" });
    expect(String((byPred["Statement.embedding"].tokenizer as string[])?.[0])).toMatch(/hnsw/);
    expect(byPred["Statement.xid"]).toMatchObject({ index: true, upsert: true });
  });

  it("declares every traceability node type's xid as a hash upsert index", async () => {
    const xids = [
      "Repo.xid", "Spec.xid", "Section.xid", "Statement.xid", "AcceptanceCriterion.xid",
      "CodeChunk.xid", "TestChunk.xid", "TestSuite.xid", "Coverage.xid", "ADR.xid",
    ];
    const { schema } = await querySchema(`schema(pred: [${xids.join(", ")}]) {index upsert tokenizer}`);
    const byPred = Object.fromEntries(schema.map((s) => [s.predicate, s]));
    for (const pred of xids) {
      expect(byPred[pred], `${pred} should be declared`).toMatchObject({ index: true, upsert: true });
  ...
  it("declares every embedding predicate as a float32vector HNSW index", async () => {
    const embeds = [
      "Statement.embedding", "AcceptanceCriterion.embedding",
      "CodeChunk.embedding", "TestChunk.embedding", "ADR.embedding",
    ];
    const { schema } = await querySchema(`schema(pred: [${embeds.join(", ")}]) {type tokenizer}`);
    const byPred = Object.fromEntries(schema.map((s) => [s.predicate, s]));
    for (const pred of embeds) {
      expect(byPred[pred], `${pred} should be a vector`).toMatchObject({ type: "float32vector" });
      expect(String((byPred[pred].tokenizer as string[])?.[0])).toMatch(/hnsw/);
    }
  });

  it("declares the relationship edge predicates as uid lists", async () => {
    const edges = ["Repo.specs", "Spec.sections", "Section.statements", "Statement.validated_by", "Coverage.covers"];
    const { schema } = await querySchema(`schema(pred: [${edges.join(", ")}]) {type list}`);
    const byPred = Object.fromEntries(schema.map((s) => [s.predicate, s]));
    for (const pred of edges) {
      expect(byPred[pred], `${pred} should be a uid list`).toMatchObject({ type: "uid", list: true });
    }
  });
  ...
  it("coexists with the memory schema — Memory.xid stays intact on the shared cluster", async () => {
    const { schema } = await querySchema("schema(pred: [Memory.xid]) {index upsert}");
    const byPred = Object.fromEntries(schema.map((s) => [s.predicate, s]));
