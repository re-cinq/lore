...
describe.skipIf(!reachable)("backfillMemoryToDgraph (live Postgres + Dgraph)", () => {
  ...
  async function deleteDgraphNodeByPredicate(predicate: string, xid: string): Promise<void> {
    ...
    try {
      ...
      const uids: string[] = ((res.data as { node?: { uid: string }[] }).node ?? []).map(
        (node) => node.uid,
      );
      if (uids.length) {
        await txn.mutate({
