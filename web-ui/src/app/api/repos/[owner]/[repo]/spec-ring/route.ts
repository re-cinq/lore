...
import { fetchSpecRing } from '@/lib/spec-graph';
...
export async function GET(req: Request, { params }: { params: Promise<{ owner: string; repo: string }> }) {
  ...
  if (!specPath) {
    return NextResponse.json({ error: 'required: spec query param' }, { status: 400 });
  }
  try {
    const ring = await fetchSpecRing(`${owner}/${repo}`, specPath);
