...
describe('OnboardView', () => {
  it('renders the heading and intro copy', () => {
    ...
    expect(screen.getByRole('heading', { level: 1, name: 'Add Repository' })).toBeInTheDocument();
    expect(
      screen.getByText(/Onboard a repository to Lore. This will create a PR on the target repo/),
    ).toBeInTheDocument();
  });
  ...
  it('lists already-onboarded repos joined by comma when present', () => {
    render(
      <OnboardView
        onboarded={[{ full_name: 're-cinq/lore' }, { full_name: 're-cinq/other' }]}
  ...
  it('omits the already-onboarded hint when the list is empty', () => {
    ...
    expect(screen.getByText(/The GitHub App must have access to this repo\./)).toBeInTheDocument();
  });

  it('wires the onboard form with the full_name input and submit button', () => {
    const { container } = render(<OnboardView onboarded={[]} onboardRepoAction={action} />);
    expect(screen.getByRole('button', { name: 'Onboard Repository' })).toBeInTheDocument();
