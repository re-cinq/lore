...
describe('HomeView', () => {
  it('renders one card per repo with the full name', () => {
    ...
    expect(screen.getByRole('heading', { level: 3, name: /re-cinq\/lore/ })).toBeInTheDocument();
    expect(screen.getByRole('heading', { level: 3, name: /re-cinq\/atlas/ })).toBeInTheDocument();
  });

  it('links each card to the repo overview page', () => {
    renderHome([repo({ owner: 're-cinq', name: 'lore' })]);
    const card = screen.getByRole('heading', { level: 3, name: /re-cinq\/lore/ }).closest('a');
    expect(card).toHaveAttribute('href', '/repos/re-cinq/lore');
  });

  it('shows the empty state when there are no repos', () => {
    renderHome([]);
    expect(screen.getByText('No repositories onboarded yet.')).toBeInTheDocument();
    expect(screen.getByRole('link', { name: 'Add your first repo' })).toHaveAttribute('href', '/onboard');
    expect(screen.queryByRole('heading', { level: 3 })).not.toBeInTheDocument();
  });

  it('always renders the + Add Repo link to onboard', () => {
    renderHome([]);
    expect(screen.getByRole('button', { name: '+ Add Repo' }).closest('a')).toHaveAttribute('href', '/onboard');
  });
  ...
  it('renders the missing-workflow badge for a repo with missing ingest status', () => {
    renderHome([repo({ full_name: 're-cinq/lore' })], { 're-cinq/lore': 'missing' });
  ...
  it('renders the stale-workflow badge for a repo with stale ingest status', () => {
    renderHome([repo({ full_name: 're-cinq/lore' })], { 're-cinq/lore': 'stale' });
  ...
  it('renders no workflow badge for aligned or unknown ingest status', () => {
    renderHome([repo({ full_name: 're-cinq/lore' })], { 're-cinq/lore': 'aligned' });
  ...
  it('shows the last-ingested date when last_ingested_at is present', () => {
    const card = repo({ full_name: 're-cinq/lore', last_ingested_at: '2026-05-01T12:00:00Z' });
    renderHome([card]);
  ...
  it('shows awaiting-ingestion when never ingested but PR merged', () => {
    ...
    expect(screen.getByText('Onboarded, awaiting ingestion')).toBeInTheDocument();
  });
  ...
  it('uses the fresh freshness indicator for a recent ingest', () => {
    renderHome([repo({ full_name: 're-cinq/lore', last_ingested_at: new Date().toISOString() })]);
  ...
  it('uses the stale freshness indicator for an ingest within a week', () => {
    ...
    renderHome([repo({ full_name: 're-cinq/lore', last_ingested_at: threeDaysAgo })]);
    expect(screen.getByTitle('Stale (< 7d)')).toHaveStyle({ backgroundColor: 'var(--warning)' });
  });

  it('uses the outdated freshness indicator for an ingest older than a week', () => {
    const tenDaysAgo = new Date(Date.now() - 10 * 24 * 60 * 60 * 1000).toISOString();
    renderHome([repo({ full_name: 're-cinq/lore', last_ingested_at: tenDaysAgo })]);
    expect(screen.getByTitle('Outdated (> 7d)')).toHaveStyle({ backgroundColor: 'var(--danger)' });
  });
  ...
  it('uses the never-ingested freshness indicator when last_ingested_at is null', () => {
    ...
    expect(screen.getByTitle('Never ingested')).toHaveStyle({ backgroundColor: 'var(--text-muted)' });
  });

  it('renders the Fix-ingest button wired to the injected action when repos are misaligned', () => {
    renderHome([repo({ full_name: 're-cinq/lore' })], { 're-cinq/lore': 'missing' }, ['re-cinq/lore']);
    expect(screen.getByRole('button', { name: '⚠ Fix ingest workflow (1)' })).toBeInTheDocument();
  });

  it('hides the Fix-ingest button when no repos are misaligned', () => {
    renderHome([repo({ full_name: 're-cinq/lore' })], { 're-cinq/lore': 'aligned' }, []);
    expect(screen.queryByRole('button', { name: /Fix ingest workflow/ })).not.toBeInTheDocument();
  });

  it('renders a fully-populated card with every badge present', () => {
    renderHome(
      [repo({ full_name: 're-cinq/lore', team: 'platform', task_count: 5, active_agents: 2 })],
      { 're-cinq/lore': 'stale' },
    );
    const card = screen.getByRole('heading', { level: 3, name: /re-cinq\/lore/ }).closest('a')!;
