...
function typeQuery(value: string) {
  fireEvent.change(screen.getByPlaceholderText(/Describe the task/), { target: { value } });
}
...
describe('AssembledContextPanel', () => {
  ...
  it('fetches the context-preview endpoint with encoded query and selected template', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Template'), { target: { value: 'review' } });
    submit();
    await flush();
  ...
  it('renders an HTTP error when the response is not ok', async () => {
    ...
    expect(screen.getByText('Context unavailable: HTTP 403')).toBeInTheDocument();
  });
  ...
  it('renders the rejection message when fetch throws', async () => {
    ...
    expect(screen.getByText('Context unavailable: network down')).toBeInTheDocument();
  });
