...
describe('ChunkBody', () => {
  ...
  it('highlights a code chunk and shows a symbol/line header linking to the GitHub range', () => {
    const { container } = render(
      <ChunkBody
        ...
        repo="re-cinq/lore"
        metadata={{ symbol_type: 'function', symbol_name: 'foo', start_line: 10, end_line: 42 }}
      />,
    );
  ...
  it('shows the section title and a plain blob link (no line range) for a doc chunk', () => {
    ...
    expect(
      container.querySelector('a[href="https://github.com/re-cinq/lore/blob/main/docs/a.md"]'),
    ).not.toBeNull();
  });
  ...
  it('leaves a markdown link relative (no new tab) when the repo is unknown', () => {
    ...
    const link = container.querySelector<HTMLAnchorElement>('a[href="specs/x.md"]');
    expect(link).not.toBeNull();
