...
describe('ReadmeBox', () => {
  it('renders the markdown heading as an h1 element', () => {
    const { container } = render(
      <ReadmeBox markdown={twoBlocks} rawBaseUrl={rawBaseUrl} htmlUrl={htmlUrl} />,
    );
    expect(container.querySelector('h1')?.textContent).toBe('Heading');
  });

  it('renders all blocks and no expand button when content is not collapsible', () => {
    render(<ReadmeBox markdown={twoBlocks} rawBaseUrl={rawBaseUrl} htmlUrl={htmlUrl} />);
  ...
  it('renders a single block as full markdown without a button', () => {
    render(
      <ReadmeBox markdown="just one block" rawBaseUrl={rawBaseUrl} htmlUrl={htmlUrl} />,
    );
  ...
  it('shows a Read more button and hides later blocks when collapsible and collapsed', () => {
    render(<ReadmeBox markdown={threeBlocks} rawBaseUrl={rawBaseUrl} htmlUrl={htmlUrl} />);
    ...
    expect(screen.queryByText('Second paragraph body.')).not.toBeInTheDocument();
