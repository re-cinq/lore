...
const populated: SpendViewProps = {
  ...
  orgAvailable: true,
  orgByModel: [
    { model: 'claude-opus-4', cost_usd: 900.25, input_tokens: 1000000, output_tokens: 50000 },
...
describe('SpendView', () => {
  it('renders the page title and all section headings', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Claude API Spend', level: 1 })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'Month to Date', level: 2 })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'Billed Cost by Model (MTD)', level: 2 })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'Daily Billed Cost (This Month)', level: 2 })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'Lore-Computed Cost by Repo (MTD)', level: 2 })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'Lore-Computed Cost by Task Type (MTD)', level: 2 })).toBeInTheDocument();
  });
  ...
  it('shows billed cost, computed cost, and token totals when org data is available', () => {
    ...
    expect(screen.getByText(`as of ${new Date('2026-06-04T10:00:00.000Z').toLocaleString()}`)).toBeInTheDocument();
    ...
    expect(screen.queryByText('Org-wide billed cost unavailable.')).not.toBeInTheDocument();
  });
  ...
  it('renders billed-cost-by-model rows including the (non-token) fallback label', () => {
    ...
    expect(within(table).queryByText('No billed data yet')).not.toBeInTheDocument();
  ...
  it('renders daily billed cost rows with localized dates', () => {
    ...
    expect(within(table).getByText(new Date('2026-06-04').toLocaleDateString())).toBeInTheDocument();
    expect(within(table).getByText(new Date('2026-06-03').toLocaleDateString())).toBeInTheDocument();
  ...
  it('shows dashes and the warning card when org billed data is unavailable', () => {
    ...
    expect(screen.getByText('Org-wide billed cost unavailable.')).toBeInTheDocument();
