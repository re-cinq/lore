...
const taskSummary: TaskSummary = { total: 1234, succeeded: 1000, failed: 34, active: 200 };

const latencyStats: LatencyStats[] = [
  // p95 over 200ms → the ">200ms" delete badge
  { tool: 'search_memory', call_count: 5000, p50_ms: 12.4, p95_ms: 350.9, p99_ms: 800.2 },
  // p95 at/under 200ms → the "OK" write badge
  { tool: 'assemble_context', call_count: 99, p50_ms: 8.1, p95_ms: 150.5, p99_ms: 190.0 },
];

const usageByTaskType: UsageByTaskType[] = [
  { task_type: 'implementation', task_count: 42, total_input_tokens: 1500000, total_output_tokens: 250000 },
  { task_type: 'review', task_count: 7, total_input_tokens: 300, total_output_tokens: 90 },
];
...
const dailyUsage: DailyUsage[] = [
  { day: '2026-06-03', calls: 4200, input_tokens: 9000000, output_tokens: 1200000 },
...
const jobRuns: JobRun[] = [
  ...
  { id: 'job-secs', job_name: 'auto-merge', started_at: '2026-06-03T10:00:00.000Z', completed_at: '2026-06-03T10:00:42.000Z', status: 'completed', result_summary: 'merged 3 PRs', error: null, log_path: 'gs://logs/job-secs' },
  // completed over a minute → "Nm", has error (overrides result), no log_path → em-dash
  { id: 'job-mins', job_name: 'lease-reaper', started_at: '2026-06-03T11:00:00.000Z', completed_at: '2026-06-03T11:05:00.000Z', status: 'failed', result_summary: 'ignored', error: 'boom', log_path: null },
  // not completed → duration em-dash, no result_summary → em-dash
  { id: 'job-running', job_name: 'baseline', started_at: '2026-06-03T12:00:00.000Z', completed_at: null, status: 'running', result_summary: null, error: null, log_path: null },
];
...
describe('AnalyticsView', () => {
  it('renders all six section headings', () => {
    ...
    ['Task Summary', 'Retrieval Performance (Last 7 Days)', 'Usage by Task Type', 'Tasks by Repo', 'Daily Usage (Last 14 Days)', 'Recent Job Runs'].forEach(name => {
  ...
  it('renders the empty-latency message when there are no latency stats', () => {
    const { container } = render(<AnalyticsView {...fullProps} latencyStats={[]} />);
  ...
  it('renders the empty-repo message when there are no repo rows', () => {
    const { container } = render(<AnalyticsView {...fullProps} usageByRepo={[]} />);
  ...
  it('renders daily-usage rows with formatted dates and token counts', () => {
    ...
    const day = within(table).getByText(new Date('2026-06-03').toLocaleDateString()).closest('tr')!;
  ...
  it('renders a minutes duration and shows the error in place of the result when a job failed', () => {
    ...
    expect(within(row).getByText('failed')).toHaveClass('op-badge', 'op-failed');
  });
