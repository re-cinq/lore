...
describe('PRStatusBadge', () => {
  ...
  it.each(knownStatuses)('maps the %s status to its pill color', async (status, color) => {
  ...
  it('falls back to the muted color for an unknown status', async () => {
    ...
    expect(pill.style.getPropertyValue('--pill-color')).toBe('var(--text-muted)');
  });
  ...
  it('re-fetches and updates when the taskId prop changes', async () => {
    const byTask: Record<string, string> = {
      '/api/pipeline/first/pr-status': 'open',
      '/api/pipeline/second/pr-status': 'merged',
    };
    const fetchMock = stubFetch(url => jsonResponse({ computed_status: byTask[url] }));
