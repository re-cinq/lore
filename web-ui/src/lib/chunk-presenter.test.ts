...
describe('chunkHeader', () => {
  it('builds a code header from symbol type, name and line range', () => {
    expect(
      chunkHeader('code', { symbol_type: 'function', symbol_name: 'foo', start_line: 10, end_line: 42 }),
    ).toEqual('function foo · L10–42');
  });

  it('omits the line range when only the symbol is present', () => {
    expect(chunkHeader('code', { symbol_type: 'class', symbol_name: 'Foo' })).toEqual('class Foo');
  });
  ...
  it('returns the section title for a doc chunk', () => {
    expect(chunkHeader('doc', { section_title: 'Architecture' })).toEqual('Architecture');
  });
