...
# A deployer already holds every GitHub credential this needs, in GCP Secret
# Manager: ghcr as a dockerconfigjson blob, the App as an id/installation/PEM
# triple. Minting fresh PATs to restate them is busywork and one more secret to
# leak. Read them from GSM instead — but only what the agent pods need, and only
# with a yes: pulling live production credentials onto a laptop is the
# developer's call, not the script's default.
#
# This used to parse infra/terraform/secrets.tfvars. That file no longer holds
...
# The stored value is raw JSON: {"auths":{"ghcr.io":{"auth":"<b64 user:token>"}}}.
# Pull the auth field and decode the pair. Pure coreutils, no python dependency.
ghcr_pair_from_gsm() {
  local auth
  auth="$(gsm lore-ghcr-pull-secret \
...
if command -v gcloud >/dev/null 2>&1 && needs_github_creds && gsm lore-github-app-id >/dev/null; then
  log "Your gcloud login can read the platform's GitHub credentials from Secret"
  log "Manager — the ghcr pull pair and the GitHub App, all the agent pods need."
  read -r -p "       Import them into .env.local? [Y/n]: " answer
  if [ "${answer:-Y}" != "n" ] && [ "${answer:-Y}" != "N" ]; then
    if pair="$(ghcr_pair_from_gsm)" && [ -n "$pair" ]; then
      ...
      log "Could not read lore-ghcr-pull-secret — falling back to prompts"
    fi

    app_id="$(gsm lore-github-app-id)"
    app_install="$(gsm lore-github-app-installation-id)"
    app_key="$(gsm lore-github-app-private-key)"
