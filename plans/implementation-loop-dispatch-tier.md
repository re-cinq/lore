...
2. Nothing checks, before minting a row, that any active unpaused agent offers the node's tags — [advance.ts:290](apps/floor/src/jobs/assembly-run/advance.ts#L290) gates only on `llmGate`, and [:344](apps/floor/src/jobs/assembly-run/advance.ts#L344) resolves the tags without asking who can serve them.
