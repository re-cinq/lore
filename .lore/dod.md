...
Facets (the red-green-refactor steps you expect, smallest first):
  - [DONE] Add `- from: dod, to: open-pr, on: no_new_test` to `libs/assembly-lines/src/assembly-lines/implementation-loop.yaml` AND add `"no_new_test"` to the `EdgeCondition` enum in `libs/assembly-lines/src/loader.ts` (schema validates the `on:` field; without the enum entry the YAML fails to parse)
  - Add `no_new_test` outcome text to the `acceptance-dod` recipe in `scripts/task-types.yaml` so the agent knows when to return it
