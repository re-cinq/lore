...
Facets (red-green-refactor steps, smallest first):
  - [x] Add a try/catch inside `runStationHandler` that catches any error thrown by `station()`, logs it internally, and re-throws a bare `Boom.internal()` (no message), so sensitive details stay in server logs rather than the wire response.
  - [x] The `finally` block already releases the in-flight latch — the catch must sit between the await and the finally so the latch is still freed on all paths.
