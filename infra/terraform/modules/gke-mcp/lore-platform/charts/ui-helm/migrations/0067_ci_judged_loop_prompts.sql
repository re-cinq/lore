-- CI judges the branch, not the pod (specs/implementation-loop FR15). Every
-- delivering recipe loses the "run the repository's OWN format/fix step"
-- instruction: lint and formatting are what CI already runs on the pull
-- request, and since red CI now routes to another round rather than blocking
-- the ticket, running them in a pod pays for a machine to answer a question
-- the build answers anyway. The four implementation-loop recipes additionally
-- stop re-running the suite: `tdd-round` runs only the acceptance tests and
-- the one test it writes, `pr-ready` re-runs nothing, and `fix-ci` is handed
-- the failed check names instead of reproducing the whole build.
--
-- Same rollout shape as 0057/0061/0062/0063/0064: the seeded org rows outrank
-- the yaml, only rows still carrying the prior text are rewritten, and only
-- rewritten rows emit a catalog event for the cluster-agents to re-apply.
...
Spec: {description}$lore_mig$,
 ...
 WHERE name = 'implementation'
   AND prompt LIKE '%OWN format/fix step%'
...
and runs all of it on every push, on hardware sized for it. What the
...
- If a `CI reported failures` section is appended below, it is the verdict
  on the previous push. A red check in code you did not touch is a
  regression this branch caused: fix it FIRST, before your round. Do not
  re-run the whole build to find it; the section names the checks.
...
- You cannot name a next facet, or the bar will not go green: say exactly
  ...
  is. Report this too if you changed nothing at all.
...
The `CI reported failures` section below names the checks that failed.
CI is the judge of this branch, and that section is its verdict — start
from it, not from a full local build. Map each check name to the job that
publishes it in `.github/workflows/`, find the command that job runs, and
run ONLY that. Reproducing the whole build to rediscover what you have
already been told costs the ticket a pod and tells you nothing new.
