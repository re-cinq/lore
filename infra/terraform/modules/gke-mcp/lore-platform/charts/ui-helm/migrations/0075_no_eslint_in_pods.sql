-- CI already ran the linter; the pod must not (specs/implementation-loop FR15,
-- amended 2026-09-13). 0068 restored a SCOPED `npx eslint --fix <files>` to the
-- delivery contract on the argument "seconds in the pod against a full round
-- on CI". In this repository that command peaks near 800 MB even for two files
-- (the type-aware config, not the spec-link rules), so against the 1Gi pod it
-- OOM-killed runs 112256d9 (repair-build, `eslint .`) and 62c1ca5e
-- (ready-for-review, two files) on 2026-09-12. Every delivering recipe now runs
-- the formatter over the changed files and nothing else from CI's checklist;
-- a lint finding returns as a red build naming the file and line (#2039).
--
-- Same rollout shape as 0067/0068/0071: the seeded org rows outrank the yaml,
-- only rows still carrying the prior bullet are rewritten, and only rewritten
-- rows emit a catalog event for the cluster-agents to re-apply. Five rows;
-- tdd-round and fix-ci carry NULL and resolve from the yaml in the image.
...
Spec: {description}$lore_mig$,
 ...
 WHERE name = 'implementation'
   AND prompt LIKE '%npx eslint --fix <files>%'
