-- A branch that cannot merge gets no CI at all (specs/implementation-loop FR14).
--
-- GitHub runs no workflow on a conflicted pull request. Run c5671c93 resumed a
-- nine-day-old branch for issue 1624, pushed two rounds onto it, and got zero
-- check runs: main had moved 186 commits, the PR was CONFLICTING, and `await-ci`
-- waited 22 minutes on a build that was never going to start. The wait was
-- right to wait -- no checks plus a CI-running repo is "not started" -- so the
-- fix belongs where the branch is written, not where it is judged.
--
-- Every delivering recipe now brings its branch up to date with the base before
-- it stops, and resolves the conflict there. Same conditional shape as
-- 0067/0068/0069.
...
Spec: {description}$lore_mig$,
 ...
 WHERE name = 'implementation'
   ...
   AND prompt NOT LIKE '%bring the branch up to date with its base%'
