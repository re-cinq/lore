-- The definition-of-done artifact, written to be read (specs/implementation-loop FR6).
--
-- `.lore/dod.md` was specified as consecutive `Key: value` lines. Single
-- newlines collapse in rendered markdown, so the ticket claim, the strategy and
-- the rationale ran together as one paragraph; acceptance tests were one long
-- `::`-joined line each; and rounds tracked progress with an invented `[DONE]`
-- prefix that renders as literal text. The shape is now proper markdown: the
-- claim as a blockquote, headed sections, and real task-list checkboxes that
-- render as checkboxes and show a round's progress at a glance.
--
-- TWO recipes: `acceptance-dod` writes the artifact and `pr-ready` reads it.
-- `tdd-round` also reads and ticks it, but its row carries a NULL prompt and
-- resolves from the yaml in the image, so rewriting it here would do nothing.
...
Ticket: {description}$lore_mig$,
 ...
 WHERE name = 'acceptance-dod'
   ...
   AND prompt NOT LIKE '%Done when these pass%'
