-- Passing tests do not mean the code compiles (specs/implementation-loop FR14).
--
-- PR #1914 ran five rounds with its whole acceptance suite green on a branch
-- that did not build: a test referenced a type it never imported. vitest
-- transpiles each file without checking types, so nothing the round ran could
-- see it, and only CI did -- one round and one CI cycle later, every time.
--
-- 0068 restored lint and formatting to the pod on the same argument (seconds in
-- the pod against a full round on CI) but left the typechecker out. This adds
-- it, scoped to the packages a round touched. Same conditional shape as
-- 0067/0068/0069/0070; five rows, since tdd-round and fix-ci carry NULL and
-- resolve from the yaml in the image.
...
Spec: {description}$lore_mig$,
 ...
 WHERE name = 'implementation'
   ...
   AND prompt NOT LIKE '%TYPECHECK the packages you touched%'
