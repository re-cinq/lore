-- Every recipe that writes tests re-anchors the spec links its tests moved
-- (specs/implementation-loop FR14, 2026-09-13). Three tickets parked in one evening
-- (#1737, #1899, and #1716's neighbours) with line-shifted `#Lnn` anchors as the
-- last red check: inserting one test shifts every link below it, and no step in
-- the pipeline re-anchored them. `scripts/spec-links/reanchor.mjs` now does,
-- deterministically, as the last step of `npm run format`; the recipes tell the
-- pod to run it after the formatter instead of re-verifying anchors by hand.
--
-- Same rollout shape as 0075-0077: only rows still carrying the prior text are
-- rewritten, and only rewritten rows emit a catalog event. Three rows carry a
-- TRACEABILITY block on a non-NULL prompt; tdd-round's row is NULL and resolves
-- from the yaml.
...
$lore_mig$,
 ...
 WHERE name = 'implementation-tdd'
   AND prompt LIKE '%shifts the lines below it%'
