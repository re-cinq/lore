-- A ticket the definition-of-done step finds already resolved is closed, not
-- parked (specs/implementation-loop FR8, 2026-09-14).
--
-- Run e0b9e349 was queued for #1948 fourteen minutes after #2064 had merged
-- its fix. The step found the branch identical to main, had no honest red
-- test to write, and the only endings its contract offered were "pushed
-- tests" and "cannot be expressed" -- so the run ended iteration_max and the
-- issue got `lore:blocked`: a failure report for work that was done. The
-- contract gains a third ending, `Lore-Dod-Resolved`, which the Floor lifts
-- into the row like `Lore-Dod-Blocked` and answers by closing the issue and
-- the draft pull request with the step's reason. This tells the step so.
-- Same conditional shape as 0067-0073: one row, since only `acceptance-dod`
-- carries the sentence, rewritten only while it lacks the new ending.
...
Ticket: {description}$lore_mig$,
 ...
 WHERE name = 'acceptance-dod'
   ...
   AND prompt NOT LIKE '%Lore-Dod-Resolved%'
