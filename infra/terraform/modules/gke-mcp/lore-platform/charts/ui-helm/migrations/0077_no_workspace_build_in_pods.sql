-- A pod never runs a workspace build (specs/implementation-loop, delivery
-- contract, 2026-09-13). Tests now resolve every workspace package from
-- source (tools/vitest/workspace-source.ts), so no test needs dist; the
-- build is the 950 MB step that OOM-killed run abac6ee9 while it rebuilt
-- libs/shared for a Floor test that imported the package it had just edited.
--
-- Same rollout shape as 0075/0076: only rows still carrying the prior text
-- are rewritten, and only rewritten rows emit a catalog event. Five rows;
-- tdd-round and fix-ci carry NULL and resolve from the yaml.
...
Spec: {description}$lore_mig$,
 ...
 WHERE name = 'implementation'
   AND prompt LIKE '%Read your imports and signatures twice instead.%'
   AND prompt NOT LIKE '%NEVER run a workspace build%'
