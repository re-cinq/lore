-- Meaningful implementation-loop PR titles (#1756). The pr-ready recipe now
-- names the title a human reads in a PR list — one imperative line under 70
-- characters describing what the branch DID — and reports it as
-- `Lore-Pr-Title`, which the Floor writes at the draft-to-ready flip. Before
-- this, every backlog pull request was called `lore: <branch>`. Same rollout
-- shape as 0057/0061/0062/0063: the seeded org rows outrank the yaml, only
...
- Every acceptance test is green, `.lore/pr-body.md` is written in the
  ...
  fixes are committed and pushed. Set `"Lore-Pr-Title"` to the title
  you wrote above, and
...
$lore_mig$,
       updated_at = now()
 WHERE name = 'pr-ready'
   AND prompt LIKE '%do NOT commit `.lore/pr-body.md`%'
   AND prompt NOT LIKE '%Lore-Pr-Title%'
