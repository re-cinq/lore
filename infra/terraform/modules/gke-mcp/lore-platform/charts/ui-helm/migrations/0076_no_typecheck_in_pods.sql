-- CI's build step is the compiler; the pod must not typecheck
-- (specs/implementation-loop, delivery contract, reversed 2026-09-13). 0071
-- added a scoped `npx tsc --noEmit -p <pkg>` because vitest transpiles without
-- checking types. In this repository that command peaks near 950 MB on
-- libs/shared, so against the 1Gi pod it OOM-killed run 2a291314's dod on
-- 2026-09-13, the day after the linter did the same to two runs (0075). A type
-- error returns as a red build whose failed step and log tail name the file
-- and line (#2039); the next round fixes it with an editor.
--
-- Same rollout shape as 0067/0068/0071/0075: only rows still carrying the
-- prior bullet are rewritten, and only rewritten rows emit a catalog event.
-- Five rows; tdd-round and fix-ci carry NULL and resolve from the yaml.
...
Spec: {description}$lore_mig$,
 ...
 WHERE name = 'implementation'
   AND prompt LIKE '%Then TYPECHECK the packages you touched%'
