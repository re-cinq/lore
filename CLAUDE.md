...
- `agent/src/supervisor/runner-cli.ts` — Job pod CLI entry point invoked by `entrypoint.sh` when `LORE_DARK_FACTORY_WORKFLOW` is set. Loads workflows from `/app/dist/workflows/`, drives the supervisor inside the pod's working tree, exits with a documented matrix (0/2/3/4/5/6/7/8/9) the watcher reads to decide retry vs needs-human-help
