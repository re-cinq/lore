...
- `lore.repos.settings.dark_factory` block: `enabled`, `create_issue`, `auto_merge.{paths,min_trust,require_*}`, `review`, `notify`. Schema in `apps/lore-api/src/features/dark-factory/dark-factory-settings.ts`; defaults in `resolveSettings()`. The resolver lives in `@re-cinq/lore-shared` (`libs/shared/src/dark-factory-settings.ts`, kept dependency-free because web-ui reaches it by relative path); the SHAPE is `libs/shared/src/models/dark-factory-settings.ts`, which asserts at compile time that its schema infers exactly those types.
