...
- `specs/assembly-line-run-viz/spec.md` — live assembly-line run observability (Shipped): projects every claude stream-json line POSTed to `POST /api/agent-events` into `pipeline.agent_run_events` (write-time truncation + `file_paths` extraction, correlated to `station_runs` via `source.agent` == `agent_cr_name`, 14-day prune), and since 2026-09-09 (FR7) streams ONE multiplexed SSE connection per run from lore-api at `GET /api/assembly-runs/{id}/stream` — `agent_event` (cursored by row id), `node_status`, `run_status`, `task_event`, `ci_check` (snapshots, re-sent on every connect) — fanned out by Postgres `NOTIFY` triggers (migration 0070). The page (`RunLiveShell` → `RunVisualizationPanel`) renders the DAG with a persistent inspector beside it (auto-selects running → failed → last finished; model badge per agent node from the resolved catalog), the transcript as a terminal session with the task's transitions folded in, a Definition of Done card (implementation-loop FR16) and per-file PR diffs (FR8); the 10 s `router.refresh()`, the Timeline card, the replay scrubber and the Event Timeline card are retired. Adding an unlinked statement REQUIRES flipping its `| Status |` row to `In Progress`, or `eslint .` goes red repo-wide
- `adrs/` — architecture decision records (MADR format)
- `adrs/ADR-037-sse-run-observability.md` — SSE (not WebSocket, not polling) for run observability; the stack's first streaming transport. Amended 2026-09-09: the stream is served by lore-api (`apps/lore-api/src/work/run-stream/`: `run-notify-hub.ts` holds one `LISTEN lore_run_stream` client outside the pool, `run-stream-session.ts` snapshots + replays + re-reads what a notification names), the Floor's in-process bus is deleted, and the `replicaCount: 1` coupling is gone for this feature. Only `agent_event` frames carry an SSE `id:`; the other families are snapshots. Durability lives in the table, not the bus. The raw stream is NOT discarded — since #1148 it lives in `pipeline.agent_run_turns` (`specs/turn-level-transcript-store`, readable at `GET /api/agent-turns/{assemblyRunId}`); the earlier fire-and-forget GCS archive is retired
