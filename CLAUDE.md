...
- `apps/floor/src/jobs/merge/merge-check.ts` — `handleMergedTask` also runs the **spec-status-upkeep FR1** hook (`specs/spec-status-upkeep/`): when a merged `spec-task` leaves no sibling with work outstanding in its `task_group_id` (via `taskQueue().countOutstandingInGroup` — `merged`/`completed`/`retried` are settled per `SETTLED_GROUP_STATUSES`, `failed`/`cancelled` still hold the group open; `retryTask` passes the group + priority to the replacement so a retry can't strand it or demote it), the pure `decideSpecStatusFlip()` gate resolves the owning feature and `openSpecStatusFlipPr` (`@re-cinq/lore-shared`) opens a deterministic one-line `lore-managed`+`spec-status-upkeep` PR flipping the spec's `| Status |` row to `Shipped` (mirrors the spec-coverage-backfill PR plumbing; `rewriteSpecStatusRow` is the idempotent pure rewriter), then transitions `lore.features` → `implemented`. No LLM. Human-review PRs (no auto-merge wiring). **FR2** is the weekly `status-staleness` detect line (`libs/shared/src/detect/status-staleness.ts`, `cron.status_staleness.tick` Mondays 12:00 UTC → one line per repo with specs): for each spec still bucketing `draft`/`in-progress`, it scores deterministic evidence (resolving `([validated by ...])` links, the feature's `spec-task` rows all merged, backticked paths present in code chunks) and files ONE aggregated `stale-spec-status` issue per repo, deduped against an open one. Task evidence goes through `tasks.specTasksForSlug(slug)` (`context_bundle->>'spec_slug'`, the only spec link a spec-task row carries) — `driftTasksForSpec`'s `spec_path` key belongs to spec-drift's gap-fill tasks and matches no spec-task ever written. It files an issue rather than a flip PR because a station pod has no `project.repo.read`. The canonical status vocabulary (five buckets, `Retired` vs `Rejected`, shared as single source + the web-ui parity test) is ADR-037
